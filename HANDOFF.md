...
1. **ADR-0030 Model-Based Universal Ingestion is COMPLETE + MERGED — ingestion is now FORMAT-AGNOSTIC; the ONLY remaining step is the operator-present LIVE run.** main @ `8361f82` (2026-06-27 S99 close): ADR-0030 (accepted, amends ADR-0001/0003/0013/0029) → spec (5 tasks) → build-plan (4 waves) → 5 recipes → 4-wave build → Tier-2 reviews → Tier-3 6-agent `/review-pr` (3 LEGITIMATE → fixed → all blind-verified RESOLVED) → rebase merge (PR #266). Any-format upload (PDF / arbitrary CSV-JSON / a photo) → the SAME no-train lane chat/de-id use → `extract_readings` → operator confirms every value → lands via the UNCHANGED `store.append`. Suite 1846/7 + 2 env; frozen-engine numstat=0; core-cap audit exit 0; crown-jewel PII 0-leak HELD by execution. **REFRAME: ingestion now accepts whatever format the operator's real data arrives in; the operator-present LIVE run (real key + real data + spend, ADR-0030 OQ-1) is the ONLY remaining step to exercise it.**
2. **Before / as part of the operator-present LIVE run: the standing live-run gates + the NEW ADR-0030 follow-ups.** Standing: `6hts` (de-id value-scan residuals) + `stsq` (SEC-3 live-dispatch 0-leak) + the live `author` leg (`_ClaudeNoTrainBackend.author` still `NotImplementedError`, ADR-0015-T3). NEW from the Tier-3 review: **`by30` (F4, P2): harden POST /upload against cross-site denial-of-wallet** — multipart is a CORS-simple request + the live client is now wired, so a cross-site page could drive metered extraction; bounded (loopback-only, nothing LANDS via the CSRF-gated confirm path, attacker's own content — NOT a PII leak), operator-DECISION (a same-origin gate is defense-in-depth needing sign-off). **`nuct` (F5, P3): extract a shared `_bounded_call` helper** (the SEC-01 fail-closed scaffolding is now 3 copies). Safety follow-ups `s7qz`/`sc97`/`4oha` open.
3. **Standing mechanics (all HELD) + 2 NEW PF lessons.** Full pipeline build (PF-S36-01, ran in full); FULL gated `/review-pr` for multi-file/security code (PF-S96-01 — APPLIED on #264 + #266, caught F1); 3-tier review with profile-less blind-triage + EXECUTED blind-verify (each finding RED-reproduced on scratch copies); EXTEND-NOT-REBUILD numstat=0; full role profiles inlined VERBATIM on EVERY role dispatch; review/verify agents READ-ONLY + no system mutation (PF-S98-01) + READ-ONLY git only (PF-S97-01, scratch-copy mutations only); `OBJC_DISABLE_INITIALIZE_FORK_SAFETY=YES` on EVERY pytest (PF-S98-02); **NEW PF-S99-01 (`2xu4`): a new production WRITE path reachable from the operator-entry factory MUST have a test driving the real factory default (None-resolved) path — not only injected-tmp-root tests; the confirm-land production path was dead (store_root=None → TypeError) and only Tier-3 caught it**; **NEW PF-S99-02 (`2l69`): do NOT present the operator a false binary — verify both options are real + on-topic before offering a choice**; `/merge` via REST when GraphQL exhausted (full-40-char SHA guard); `git add` SEPARATE from `git commit`; single trunk; stage close docs by EXPLICIT path; never stop mid-loop (PF-S92-01); re-run close gate on final main + cite SHA (PF-S74-01).

## Current State (volatile)

- **S99 (2026-06-27): landed the operator-approved SPA front-end (PR #264/#265), then ran the ENTIRE build pipeline (create-adr → spec → build-plan → task-plan → execute-plan → review-pr → merge) autonomously to make ingestion FORMAT-AGNOSTIC (ADR-0030).** Triggered by the operator finding the system could not ingest a DNA report PDF ("that's a real flaw … each section [must] parse whatever format comes in").
- **The front-end land:** PR #264 (faithful SPA — top-nav 3-column "Chat with Team" workspace + Profile + in-app `/settings/key` save to keychain `a-plus-maxing-api-key`) through the full 6-agent `/review-pr` (9 LEGITIMATE fixed + 9/9 blind-verified) → `main`; PR #265 (ADR-0013 `[AMENDED]` /settings/key record) → `main` (both merged 2026-06-27 S99; the merge SHAs are in the PF-log S99 skill-trace table).
- **ADR-0030 (Model-Based Universal Ingestion):** the red-team correctly flagged it as a crown-jewel boundary EXTENSION (the first model step on the ingestion axis) → amended ADR-0001/0003/0013/0029; operator informed sign-off by accepting the ADR (PF-S63-02 satisfied). Built any-format upload → no-train `extract_readings` → operator-confirm → unchanged `store.append`; mock/fixture-tested, 0 live spend.
- **The Tier-3 6-agent review caught the showstopper the mock-tested per-wave build missed:** F1 — the production confirm-landing was DEAD (`__main__` builds with store_root=None → `land_confirmed(root=None)` → `Path(None)` TypeError → degraded to `{landed:[]}`); found independently by Bug-Hunter + Contracts, both reproduced by execution. Fixed (resolve None→DEFAULT_ROOT) + F2 (SPA false-success/data-loss) + F3 (tautological negative-control) + F6 (non-dict guard test); all blind-verified RESOLVED. F4 (CSRF DECISION) + F5 (refactor) beaded.
- **Beads:** new `2xu4` (PF-S99-01), `2l69` (PF-S99-02), `by30` (F4 CSRF), `nuct` (F5 refactor). Carry: `6hts`/`stsq` (live-run gates), `s7qz`/`sc97`/`4oha` (safety), `9x5r`/`6s8v` (S98 PFs), `02m1` (daemon config), `myts`/`fbyr`/`zsp5`.
- **main @ `8361f82` (as of 2026-06-27 S99 close, PF-S74-01). Vault perpetually daemon-dirty (stage by explicit path).**

**Historical (kept for reference):** `vault/sessions/session-98.md`.

## What Is Next (volatile)

### After S99 — the operator-present LIVE run is the remaining step to a usable real plan over real (any-format) data

The build (intake + plan engine + now format-agnostic ingestion) is COMPLETE + mock-proven. To produce the operator's ACTUAL plan:
1. **Operator runs it live:** restart the loopback server against the merged main (`python -m scripts.serve` → 127.0.0.1:8765 — the operator's prior server still runs the pre-merge main, so restart to pick up ADR-0030), open the SPA at GET /, save the no-train key via the Profile screen (→ keychain `a-plus-maxing-api-key`) or export it, then upload documents of ANY format on Upload Documents + confirm the extracted readings, and talk through goals/training/nutrition in the chat. The agent NEVER touches the keychain.
2. **Before REAL PII / for the FULL real path:** land `6hts` (de-id value-scan residuals) + observe `stsq` (SEC-3 live-dispatch 0-leak); wire the live `author` (`_ClaudeNoTrainBackend.author`, still `NotImplementedError` — ADR-0015-T3); decide F4 (`by30`, the /upload same-origin gate).
3. **Then `/generate-plan`** over the operator's de-identified real data → a usable plan + doctor-visit queue + the dashboard made live (Dashboard/Plan render honest awaiting-states — the per-surface live-wiring is the follow-on).

**Deferred / NOT blocking:** `nuct` (F5 refactor); `zsp5`; the safety follow-ups; `02m1` (daemon config); the local-model migration (North Star, post-V1). The LM-01 scoped-drift-audit window opens 2026-06-29 (see Landmark check).

## Landmark window check (close step 8.7)

S99 close (2026-06-27): re-read `vault/meta/landmarks.md`. The ADR-0030 build is on mock clients + synthetic fixtures (no real operator data, no live API, no compound/biomarker page edits). LM-01 (First MD visit) **2026-07-13**: today (2026-06-27) is BEFORE the 14-day scoped-drift-audit window open (**2026-06-29**, 2 days out) → no trigger today. **NOTE for the next session on/after 2026-06-29: run the scoped drift audit on the LM-01 relevant_scopes** (operator-profile January-2026 section + `risk_tier: medium+`/experimental compounds + baseline-panel biomarkers + the doctor-handout queue); the 7-day MD-handoff window opens 2026-07-06. LM-02/03/04 not triggered. No landmark ACTIONS due this session.
