...
- **Review-agent roster rotation (S10 scope expansion candidate, user-directed).** Cycle health-specialist-architect into the `/review-pr` agent roster and cycle out one of the standard 6 code-focused agents. Rationale: medical-LLM design work has been generating PRs (doc-only + design-doc cycles); having an architect-domain reviewer with the role's full context loaded by default would catch issues like the S9 architectural-layer-mismatch earlier (Phase 1 instead of Phase 3 triage). Open questions for S10: (1) which existing code agent gets cycled out — candidates by relevance to current work: Security (least used; S9 review skipped it as not applicable) or Bug Hunter (least used in doc-only PRs); (2) is rotation per-PR (selected via flag) or permanent in `~/.claude/commands/review-pr.md` Phase 1 dispatch table; (3) does this require an architect-reviewer profile distinct from health-specialist-architect (which is project-local), or can the project-local profile be invoked cross-repo. Note: the rotation lives in `~/.claude/commands/review-pr.md` (global), not in this project — coordinate with skills_library if the change is permanent.
