...
## Scope Contract — Session 78 (2026-06-19)

Goal: Wire the closed loop's **measure leg + adjust read-back** (`design/vision.md`: plan → act → measure → adjust) — a production caller for `record_plan_tracking` (the operator logs what they actually did against a plan) + a plan-vs-actual `resolve_plan_progress` join (the "feeds back into the next plan" read side), closing the `record_plan_tracking`-has-no-production-caller gap (the exact PF-S63-02 component-built-but-unwired class). The domain-specific **progression algorithm** (how a specialist *changes the prescription* from the progress) is explicitly DEFERRED — specialist-reasoning that needs a real dispatch + design, not invented solo. Operator delegated S78 autonomously ("open S78 and proceed"); this is the autonomous-buildable half (the SBAR render + the progression algorithm both need design/operator input).

Acceptance criteria:
- [ ] AC1 (measure production caller): `record_tracking(domain, tracking, on_date, root)` in a new `scripts/plan/track.py` (mirroring `generate_plan.py`) — validates the domain is tracked, checks a plan EXISTS (the honest no-plan-to-track boundary — a domain with no recorded plan has nothing to have been done), records via the REUSED `record_plan_tracking`, returns a result record. **Mutation-proven RED.**
- [ ] AC2 (adjust read-back): `resolve_plan_progress(domain, on_date, root)` — joins the recorded plan + its tracking snapshot into a plan-vs-actual view (`{plan, tracking, has_plan, has_tracking, specialist, plan_date}`); absence states honest (no plan → no progress; plan-but-no-tracking → plan-only). **Mutation-proven RED.**
- [ ] AC3 (STORE-SURFACE BATTERY — the marquee gate): the new write (the measure caller) + read join per `docs/checklists/store-adversarial-tests.md` — cross-stream isolation (progress/tracking for domain X never returns domain Y's plan/tracking — the S41 `read_panel` class), same-snapshot dedupe/idempotent (via the existing record), mutation-style (→ RED, reverted).
- [ ] AC4 (closed-loop E2E): record a plan (the existing `generate_plan` production path) → `record_tracking` against it → `resolve_plan_progress` shows plan-vs-actual — end-to-end through the production path on SYNTHETIC operator data. No specialist dispatch (tracking is operator-OBSERVED, not specialist-authored — a manufactured dispatch would be a fake; the production-path data flow is the real integration).
- [ ] AC5 (backward-compat + integration mandate): `track.py` is a new caller; `plan_schema`/`orchestrate`/`generate_plan` UNCHANGED; all S70–S77 tests stay green; `--self-test` + core-capability gate green.
- [ ] AC6 (review + close): plan-integrity + QA (Tier-2; **the store-surface battery cited in the QA dispatch**; the design docs LISTED in WILL-touch — MF-1) → `/review-pr` (Tier-3, 6-agent) → fix legitimate → `/merge`; full close, **re-run the close gate on the final post-merge `main`** + cite the `main` SHA (PF-S74-01). **At Tier-1 RUN the interaction-probe with contract-grounding (PF-S77-01)** — ground every field `track.py` reads off `record_plan_tracking`/`read_plan`/`resolve_tracking` against their ACTUAL return shapes.

Files I WILL touch: `scripts/plan/track.py` (NEW — `record_tracking` + `resolve_plan_progress`); `tests/plan/test_track.py` (NEW — the measure/read-back tests + the store-surface battery + the closed-loop E2E); `vault/design/plan-generation-pipeline-v1.md` + `docs/plan-generation/author-dispatch-process.md` (measure leg + read-back WIRED freshness); HANDOFF/PF-log/harvest/beads/session-note at close.
Files I will NOT touch: `scripts/store/plan_schema.py` (REUSE `record_plan_tracking`/`read_plan`/`read_plan_tracking` — call only, the streams unchanged); `scripts/plan/{orchestrate,adjudicate,router,generate_plan,assemble}.py`; `scripts/store/store.py`/`keying.py` core; the deployed `.claude/agents/*`; the wiki / `vault/library/`; `main` directly.
NOT doing: the domain-specific PROGRESSION/de-load ALGORITHM (the specialist-reasoning adjust — needs a real personal-trainer/nutritionist dispatch + design, its own session, NOT invented solo); the SBAR handout render (design-led, deferred); a new store stream (the measure leg writes the EXISTING `plan-track::` via `record_plan_tracking`); changing the tracking schema/validators; a carry-forward "active plan window" model (same-date plan-existence per the existing date-specific resolution).
Invariants at risk: INV-BRANCH-NOT-MAIN (built in `../aplus-s78measure` worktree off `main`; idle trunk parked on `parking/s78-measure`; `.venv` provisioned); **store-surface battery (the new read join + the measure write — the marquee gate this session, at Tier-1 + the Tier-2 QA dispatch)**; INV-CORE-CAPABILITY (extends the wired path to the closed loop — stays green + `--self-test`); INV-ROLE-INLINING (review agents FULL profiles; NO clinical dispatch — tracking is operator-observed, so no real-agent E2E is part of the real flow); **PF-S77-01 contract-grounding at Tier-1** (ground every field `track.py` reads off the existing returns); INV-CLOSE-AUDIT / INV-PF-ATTESTATION / INV-SKILL-TRACE / INV-HARVEST-CAPTURE at close. Core-capability-first gate (PF-S63-02): SATISFIED — extends the wired plan path to the closed loop's measure→read-back (the vision's "closes the loop") + wires the `record_plan_tracking` no-production-caller gap (the exact PF-S63-02 class). **PF-S74-01 discipline: re-run the full close on the final post-merge `main` + cite the `main` SHA.**

### S78 Scope Contract Evaluation (2026-06-19, volatile)

- **AC1 (measure production caller) — PASS.** `scripts/plan/track.py` `record_tracking` is the production caller for `record_plan_tracking` (the no-production-caller PF-S63-02 gap) — validates the domain is tracked, records via the REUSED `record_plan_tracking` only when a plan is dated `on_date` (the same-date no-plan-to-track boundary — REFINED post-Tier-2 from `state==NO_PLAN` to `state is None`, fixing the NO_PLAN_TODAY seam). Mutation-proven RED.
- **AC2 (adjust read-back) — PASS.** `resolve_plan_progress` joins the plan + tracking into the plan-vs-actual view; absence states honest (`has_plan` True iff a plan is dated `on_date` ⟺ `plan` not None — the post-Tier-3 fix removed the `has_plan=True/plan=None` contradiction; `plan_date` aligned across both surfaces, CONTRACTS-TRACK-1). Mutation-proven RED.
- **AC3 (STORE-SURFACE BATTERY) — PASS.** Cross-stream isolation + dedupe + mutation on the new write/read surface; `track.py` adds no new store key (reuses `plan_schema`'s `plan-track::`/`plan::` keying — the category-4 keying mutation IS the cross-stream + dedupe tests, which go RED under domain-collapse / constant-source; documented post-Tier-3, HIST). Tier-3 historical-context confirmed no S41-class store-keying regression.
- **AC4 (closed-loop E2E) — PASS.** `generate_plan` (production path) → `record_tracking` → `resolve_plan_progress` shows plan-vs-actual, synthetic operator. No specialist dispatch (tracking is operator-OBSERVED — a manufactured dispatch would be a fake; the production-path data flow is the real integration).
- **AC5 (backward-compat + integration mandate) — PASS.** `plan_schema`/`orchestrate`/`generate_plan` UNCHANGED (Tier-2 + Tier-3 contracts confirmed); all S70–S77 tests stay green; suite 1083/3, floor 15/0, `--self-test` + core-capability green.
- **AC6 (review + close) — PASS.** plan-integrity (contract-grounding CLEAN — no S77-class field-off-wrong-return; caught the NO_PLAN_TODAY seam) + QA (store-battery PASS + coverage) Tier-2 → `/review-pr` 6-agent Tier-3 (security + bug-hunter CLEAN; test-framing + contract-consistency findings fixed — self-contained gate test, plan_date agreement, docstring, coverage; blind-verified all RESOLVED executed) → `/merge` PR #176; full close, close gate re-run on final `main` + the `main` SHA cited (PF-S74-01). The PF-S77-01 contract-grounding RUN at Tier-1.

### Drift checks (S78 close)

- **Task drift:** all 6 ACs PASS. The measure leg + adjust read-back were built; the progression ALGORITHM (specialist-reasoning) + the SBAR render (design-led) were explicitly DEFERRED at open — flagged, not silent. The gate semantics were REFINED to same-date mid-review (Tier-2, aligning the code with the scope contract's own same-date exclusion) — flagged (PF-S78-01). No silent drift.
- **Architecture drift:** no invariant degraded; `plan_schema`/`orchestrate`/`generate_plan`/`adjudicate`/`router` UNCHANGED. The new `track.py` write/read surface carries the store-surface battery; no S41-class regression. INV-CORE-CAPABILITY green; INV-BRANCH-NOT-MAIN held (built in `../aplus-s78measure` off `main`, idle trunk parked `parking/s78-measure`, `.venv` provisioned); INV-ROLE-INLINING held (the 6 review agents + plan-integrity + QA full-profile; no clinical dispatch — tracking is operator-observed). Toward the design (the closed loop's measure→read-back landed).
- **Vision drift:** none, toward-vision. The system now records what actually happened against a plan + reads it back as plan-vs-actual — the vision's "closes the loop — plan → act → measure → adjust." `design/vision.md` first sentence unchanged.

### PF attestation

S78 close (2026-06-19): **One new PF promoted — PF-S78-01** (the no-plan gate branched on only 1 of `read_plan`'s 3 state values, drifting more permissive than the scope contract's same-date semantics; Tier-2 caught it — recurrence 5 of self-review-under-probes-the-new-code-×-existing-contract; gate-caught before merge, low blast radius, fixed to same-date + mutation-proven; structural value-domain-enumeration + code-vs-contract discipline beaded `cjgg`). The disciplines that HELD: the `uerm` contract-grounding for field EXISTENCE held (plan-integrity confirmed no field-off-wrong-return defect of the prior session's class — the recurrence was on the field's VALUE DOMAIN, a sharpening not a repeat); the store-surface battery held (no S41-class regression; `track.py` adds no new key); the production modules genuinely UNCHANGED; the closed-loop E2E verified through the real production path; the design-led/specialist surfaces correctly DEFERRED, not invented solo. Tier-3 found NO safety/security defect (security + bug-hunter clean). Full attestation + the per-PR skill-trace table (PR #176) + the disclosure ledger in `memory/process-failures.md` Session 78.

## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-77 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; …; S76 archived S75; S77 archived S76; S78 archived S77) to keep this handoff lean. The current (S78) scope contract is above; the archive holds the prior-session archaeology.
...
1. **Contract-grounding at Tier-1 means the field's full VALUE DOMAIN — and check the code against your own scope contract (PF-S78-01, recurrence 5).** FIVE times now (S74 routing-key types; S75 multi-concern holds; S76 a list-field defeating the scan cap; S77 reading a field off an outcome that didn't carry it; S78 branching on 1 of `read_plan`'s 3 `state` values, letting NO_PLAN_TODAY fall through → more permissive than my OWN scope contract's same-date exclusion) a gate caught a defect in my new code's interaction with an EXISTING contract that my Tier-1 missed. The `uerm` discipline (ground that a read field EXISTS) HELD this session — but grounding existence is not grounding the VALUE DOMAIN. **For any new code that reads + BRANCHES on a field off an existing return: enumerate the field's full value domain (every enum state) and decide each; AND check the branching against the scope contract's own stated semantics + NOT-doing exclusions — before Tier-2/3** (bead `cjgg`, pairs with `uerm`/`pwpr`/`sip9`; consider mechanizing — the family keeps recurring).
2. **The disciplines that HELD and must keep holding.** (a) **Store-surface battery (mandate):** any new/changed `scripts/store/` read/write surface carries cross-stream / dedupe / mutation, mutation-proven RED, at Tier-1 + the Tier-2 QA dispatch — S78's `track.py` did (reuses `plan_schema` keying, no new key, no S41-class regression). (b) **Contract-grounding field EXISTENCE (`uerm`):** plan-integrity confirmed no field-off-wrong-return defect this session. (c) **Defer design-led + specialist-reasoning surfaces, never invent solo:** the SBAR render (design-led) + the progression algorithm (specialist-reasoning) were correctly deferred. (d) **Close on the FINAL post-merge `main` (`cvf1`):** re-run pytest + `close-audit --session N` on `main` AFTER merge+pull; the attestation CITES the post-merge `main` SHA.
3. **Standing mechanics + carry-overs.** **Worktree discipline:** separate `git worktree`s; park the idle trunk OFF `main`; Read worktree files at the worktree-absolute path; re-verify `git branch --show-current` before every commit; restore `main` at session end; **provision the worktree `.venv`**; **commit fixes PROMPTLY (`git checkout -- <file>` reverts an uncommitted fix to HEAD).** **Paste FULL role profiles verbatim — abbreviating one is denied by `enforce-role-inlining`.** **Design-led visual artifacts (the SBAR handout render, dashboards) are NEVER originated solo — Pencil + operator sign-off + the design agents (`feedback_design_via_agent_team`).** GraphQL EXHAUSTED → REST for PR-create/merge (`gh api`, full-40-char SHA guard). `git add` SEPARATE from `git commit` (PII hook denies the combined call). bd repo-ID mismatch in a worktree: `bd sync --flush-only` does NOT write `issues.jsonl` — surgically edit `issues.jsonl` (scrub new-bead owner → the `your-github-email@example.com` placeholder) + commit it in the worktree. Two parallel wiki sessions push to `origin/main` — rebase-sync at merge, never touch `vault/library/`. **Close-step-8 freshness un-gated until `dfg1` — RUN step 8 explicitly each close.** Carry-overs: `cjgg` (value-domain + code-vs-contract grounding, PF-S78-01) / `uerm` / `pwpr` / `sip9` / `cvf1` / `dfg1`/`df1x` / `aem1` / `axm7` / `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`. Pencil: ALWAYS pass `filePath`.

## Current State (volatile)

- **S78 (2026-06-19) wired the closed loop's MEASURE leg + ADJUST read-back — the vision's plan → act → measure → adjust.** `scripts/plan/track.py` (NEW): `record_tracking` is the production caller for `plan_schema.record_plan_tracking` (the no-production-caller PF-S63-02 gap) — records one operator-OBSERVED tracking snapshot against a domain's plan, with the honest same-date no-plan-to-track boundary (record only when a plan is dated `on_date`); `resolve_plan_progress` joins the plan + tracking into the plan-vs-actual view the deferred re-plan + dashboard read (`has_plan` ⟺ a plan dated `on_date`; `plan_date` aligned across both surfaces). Writes the EXISTING `plan-track::` via `record_plan_tracking` (no new store key) + reads `plan::`/`plan-track::`; the store-surface battery (cross-stream / dedupe / mutation) carried, no S41-class regression. `plan_schema`/`orchestrate`/`generate_plan`/`adjudicate`/`router` UNCHANGED. Tracking is operator-observed (no agent dispatch). The progression ALGORITHM (specialist-reasoning) + the SBAR render (design-led) are deferred (not invented solo).
- **PR #176 → `main` (rebase merge; the S78 close re-run on the post-merge `main`, SHA cited in the close report).** Tier-2 plan-integrity (contract-grounding CLEAN — no S77-class field-off-wrong-return; caught the NO_PLAN_TODAY semantic seam) + QA (store-surface battery PASS + coverage) → `/review-pr` 6-agent Tier-3 (security + bug-hunter CLEAN — NO safety/security defect; test-framing + contract-consistency findings — a mis-framed mutation test made self-contained, plan_date cross-surface agreement, docstring accuracy, coverage — all fixed, mutation-proven RED, **blind-verified all RESOLVED executed**) → `/merge`. Verified by the closed-loop E2E through the real `generate_plan` production path. Single-PR close.
- **PF-S78-01 promoted** (the no-plan gate branched on 1 of `read_plan`'s 3 state values, drifting more permissive than the scope contract's same-date semantics; Tier-2 caught it; recurrence 5 of self-review-under-probes-the-new-code-×-existing-contract; gate-caught before merge, low blast radius; structural value-domain-enumeration + code-vs-contract discipline beaded `cjgg`).
- **pytest 1083/3 (worktree, native `.venv`).** Core-capability gate green; negative-test floor 15/0.
- **Beads:** `71s4` (measure leg + adjust read-back WIRED; the SBAR handout RENDER + the progression algorithm + the deeper adjust remain); `cjgg` (PF-S78-01) created; carry-overs `uerm`/`pwpr`/`sip9`/`cvf1`/`dfg1`/`df1x`/`aem1`/`axm7`/`llna`/`0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`.
- **Active landmarks:** LM-01 (First MD visit) **2026-07-13** — 14-day scoped-drift-audit window opens **2026-06-29** (next session on/after that date runs the LM-01 scoped drift audit). LM-02 Whoop baseline date-TBD.

**Historical (kept for reference):** `vault/sessions/session-78.md`.

## What Is Next (volatile)

### S79 — the SBAR handout render (DESIGN-LED) or the adjust progression algorithm

The plan-generation pipeline is built end-to-end through the closed loop's measure→read-back (plan → reconcile → adjudicate → queue → measure → plan-vs-actual). Candidate next surfaces (operator chooses):

1. **The SBAR handout RENDER** — the one-page MD handout (design §9.4: Situation / Background med-supplement table + flagged-interactions / Assessment / Recommendation) rendered from the `dvq::queue` stream + the operator BPMH. A **DESIGN-LED VISUAL artifact** — per the design memory NEVER originated solo: **Pencil mockup + operator sign-off on the visual design FIRST, then the design agents + the authoritative schema** (`feedback_design_via_agent_team`, `feedback_dashboard_pencil_mockups`, `feedback_build_from_authoritative_design_artifact`; PF-S68-01/S49-01 family). Honor the render precondition (an entry may lack the liaison's GRADE/watchlist extras → degrade to the tier-only ordering). Highest LM-01 relevance.
2. **The adjust leg's PROGRESSION ALGORITHM** — how a specialist (personal-trainer / nutritionist) changes the prescription from the `resolve_plan_progress` plan-vs-actual view (progress → de-load / advance). This is **specialist-reasoning** — it needs a real specialist dispatch (full profile inlined) + design, NOT invented solo. It builds on the wired measure leg + read-back. A code build with a real-agent E2E (unlike the measure leg, which was operator-observed).

**Discipline:** scope contract at open (LIST the design spec + any doc in WILL-touch — MF-1); **worktree + idle trunk off `main` + provision the `.venv`**; **for any new/changed store surface, satisfy the store-surface battery**; **at Tier-1, contract-ground the FULL VALUE DOMAIN of every field new code branches on + check the branching against the scope contract's exclusions (PF-S78-01, `cjgg`) — plus the field-existence (`uerm`), multi-concern (`sip9`), boundary-size×infra (`pwpr`) probes**; **for a visual artifact, Pencil + operator sign-off FIRST (never solo); for specialist-reasoning, a real specialist dispatch (full profile)**; **write close artifacts in-worktree BEFORE merge**; **commit fixes promptly**; **paste FULL role profiles verbatim**; `/review-pr` → `/merge`; **re-run the close gate on the final post-merge `main` + cite the `main` SHA (PF-S74-01)**.

**Deferred (NOT blocking):** `cjgg` (value-domain + code-vs-contract grounding — PF-S78-01) + `uerm` (field-existence grounding) + `pwpr` (boundary-size×infra probe) + `sip9` (multi-concern probe); `dfg1` (the doc-freshness gate — PF-S73-01 structural fix) + `df1x` (the doc-freshness backlog); `cvf1` (the mechanical post-merge-`main` verify); `aem1` (the parenthetical-name interaction-match precision gap); the malformed-`ae_profile` fail-loud upgrade; `axm7` (extend the core-capability gate to the orchestrator); the standalone full-plan render screen + the dashboard reskin (operator's worktrees); the `/generate-plan` slash-command wrapper; the `llna` hook fix; carry-overs `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`; operator-gated July-visit prep (LM-01) — first session on/after **2026-06-29** runs the LM-01 scoped drift audit.

## Landmark window check (close step 8.7)

S78 close (2026-06-19): RE-OPENED `vault/meta/landmarks.md` at close step 8.7 (unchanged, `last_reviewed` 2026-06-19). No landmark edited — S78 was plan-generation code (the closed loop's measure leg + adjust read-back) on PII-free SYNTHETIC fixtures (no real operator data ingested, no Whoop baseline start, no MD-handoff artifact GENERATED). LM-01 (First MD visit) **2026-07-13**: no trigger window open today (the 14-day scoped-drift-audit window opens **2026-06-29**, the 7-day MD-handoff window 2026-07-06). NOTE: the measure→read-back closes the tracking loop the MD handout's "backed by tracked progress data" bar (vision pt 5) needs — ahead-of-window LM-01 prep. LM-02 (Whoop) / LM-03 (23andMe) date-TBD. LM-04 (first HTML artifact) NOT triggered (no committed HTML artifact). No landmark actions due.
