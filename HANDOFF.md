...
1. **The A′ gate-dispatch control-inversion (ADR-0028) is COMPLETE on main; the live `/generate-plan` run is now GENUINELY EXECUTABLE — only the operator-present run remains.** main @ `747ff39` (as of 2026-06-25 S97 close): ADR-0028 (accepted, #255) → spec (#256) → build-plan (#257) → 5 recipes (#258) → the 5-task build + Tier-2 + full Tier-3 `/review-pr` + 4 fixes (#259). The skill now drives the ONE shared driver via the `plan_step.py` step-harness, fulfilling every yielded typed request (AUTHOR/GATE/REAUTHOR/ADJUDICATOR) as a SUBSCRIPTION agent — closing the dry-run gap that made the live run non-executable. Suite 1721/2; core-cap audit exit 0; EXTEND-NOT-REBUILD held (inner engine + `deid_in.py` numstat=0). **Next = the operator-present LIVE run** (`/generate-plan` synthetic-first, then real data): the operator injects `export ANTHROPIC_API_KEY=$(security find-generic-password -s "quant-primary-api" -w)`; the agent NEVER touches the keychain; never commit the key (PUBLIC repo).
...
### Next — the operator-present LIVE end-to-end run (the build is COMPLETE and the run is now EXECUTABLE)

The entire A′ gate-dispatch control-inversion (ADR-0028) is built + merged to `main` `747ff39` (as of 2026-06-25 S97 close). The dry-run gap is closed: the `/generate-plan` skill now drives the ONE shared driver via the `plan_step.py` step-harness, fulfilling every yielded typed request (AUTHOR/GATE/REAUTHOR/ADJUDICATOR) as a SUBSCRIPTION agent — so the live run is genuinely executable, not just "described." Directed next steps:

1. **The operator-present LIVE run** — `/generate-plan` over a PII-free SYNTHETIC summary FIRST (the integration-mandate E2E: de-id IN → real specialist + judge + safety-lens subscription dispatch → composed gate → driver → render; real subscription spend, 0 real PII), THEN real operator data. The operator injects `export ANTHROPIC_API_KEY=$(security find-generic-password -s "quant-primary-api" -w)`; the agent NEVER touches the keychain; never commit the key (PUBLIC repo). `stsq` (SEC-3 live-dispatch-0-raw-PII observation) is the release gate.
2. **Before REAL operator PII (not the synthetic run): land `6hts`** (the de-id value-scan recall residuals — numeric-typed identifiers + two-line postal). The synthetic-first run does NOT need it.
3. **After the live run proves the loop:** `f0gh` (chat elicitation wiring); the conversational intake agent epic (`w3y8`); the local-model migration (North Star, post-V1).

**Operational notes (S97 lessons):** (a) review/verify agents use READ-ONLY git ONLY — never `git checkout`/`reset`/`commit` in the shared checkout (PF-S97-01); (b) run the FULL gated `/review-pr` for any multi-file/security-relevant code PR (PF-S96-01) — it earns its keep (caught BUG-1, a per-wave-checkpoint-invisible integration defect); (c) the per-wave EXECUTED checkpoint verifies a task in isolation; the integrated final review is the layer that catches cross-task defects — never skip it.

**Deferred / NOT blocking:** `zsp5`; the stale plan-gen docs reconcile (`78be`); `02m1` (daemon config).
