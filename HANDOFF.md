...
- **Follow-on (operator-requested, same session):** Create-Profile **draft autosave** — the wizard persists every entry to `localStorage` as the operator fills it and restores it on load while the profile is incomplete, so a refresh/closed tab never loses typed work (the wizard steps are not store-prefilled; nothing reaches the store until the final Save & continue; the draft clears once complete). Client-only, on-device.
