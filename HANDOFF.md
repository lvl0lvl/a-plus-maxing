...
- **Beads:** `i1t`/`azf`/`04uk` CLOSED. `i2yw`/`y0h0` reopened with recorded residuals (projection-readout deferred; card dates/ranges/deltas unbuilt). `juc` annotated (mechanism landed; raw-lab-values residual). 1 review-deferral bead filed (the `_escape` double-quote note); F2 folded into existing `5q5`.
