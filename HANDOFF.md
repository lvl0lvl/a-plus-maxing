...
## Scope Contract — Session 75 (2026-06-19)

Goal: Wire the **author-conflict adjudication (`cfaj`)** — the Phase-4 follow-on that routes author-declared cross-domain conflicts (`reconcile`'s `report["conflicts"]`, currently detect-only) through the SAME medical-liaison terminal gate (`scripts/plan/adjudicate.py`) the S74 additive-AE axis uses. A cross-domain conflict HOLDS the declaring (`from`) domain by default (the safe no-conflict-shipped state, consistent with the additive-AE supplement hold), and a real medical-liaison adjudication RELEASES it via a content-valid override or HOLDS it (non-overridable / invalid). Operator delegated S75 autonomously ("open S75 and work through it autonomously using rigor and review"); slice chosen for being the cleanest gate-reuse with no new substrate surface (the `rxbp` Rx/BPMH axis earns its own session for the PII-boundary work).

Acceptance criteria:
- [ ] AC1: `reconcile` HOLDS the declaring (`from`) domain when it carries a plan and declares a cross-domain conflict — `holds[from_domain] = CONFLICT_HELD` — UNLESS that domain is already held by a higher-precedence behavior (RED-S/LEA short-circuit, energy bounce, additive-AE). Detect+report is preserved in `report["conflicts"]` (unchanged). The hold is the safe default (set in `reconcile`, like additive-AE). Mutation-proven RED.
- [ ] AC2: `generate_plans` routes each CONFLICT_HELD domain's conflicts to the liaison gate — builds a `safety_finding` (deterministic `finding_id` + a `caution` reproducing the conflict) via a `_conflict_safety_finding` helper, calls `adjudicate(sf, adjudicator(sf))`, and RELEASES the hold on a content-valid override (records via the existing `record_plan`); a non-overridable / invalid / absent adjudication leaves it HELD. Reuses `adjudicate` + the override-record schema + the critical-non-overridable gate UNCHANGED. No new store stream (the adjudication rides the returned result). Mutation-proven RED.
- [ ] AC3: verified by a real `medical-liaison` dispatch (full profile inlined per INV-ROLE-INLINING) adjudicating a genuine author-declared cross-domain conflict E2E through `generate_plans` — a content-valid override clears + records the held domain, an invalid/refused one holds. Captured as a `liaison-conflict-*.example.json` envelope. Not a unit stub.
- [ ] AC4: backward-compat — with NO adjudicator wired, a conflict still HOLDS the from-domain (the safe default); all S70–S74 tests stay green except the 2 existing conflict tests, which are UPDATED to pin the new hold-and-adjudicate behavior (integration mandate: a behavior change updates its tests). `--self-test` + core-capability gate green.
- [ ] AC5: plan-integrity + QA (Tier-2) → `/review-pr` (Tier-3, 6-agent) → fix legitimate → `/merge`; full close, **re-run the close gate on the final post-merge `main`** + cite the `main` SHA (PF-S74-01 discipline).

Files I WILL touch: `scripts/plan/orchestrate.py` (the conflict hold in `reconcile` + the conflict adjudication in `generate_plans` + `_conflict_safety_finding`); `tests/plan/test_orchestrate.py` (update the 2 conflict tests + new cfaj tests); new `docs/plan-generation/examples/liaison-conflict-*.example.json`; `docs/plan-generation/author-dispatch-process.md` (cfaj WIRED); HANDOFF/PF-log/harvest/beads/session-note at close.
Files I will NOT touch: `scripts/plan/adjudicate.py` (REUSE unchanged — the gate generalizes); `scripts/store/plan_schema.py` (`record_plan` — call only); `assemble.py`/`router.py` core + the PII boundary; the deployed `.claude/agents/*` (inline for dispatch); the wiki research-list / `vault/library/` (parallel sessions own it); `main` directly.
NOT doing: the supplement↔Rx BPMH axis (`rxbp` — needs the PII-boundary read, own session); the doctor-visit-queue/SBAR artifact (a store-write surface); the measure/adjust legs; changing the conflict ENVELOPE contract (authors already declare `reconciliation.conflicts`); a new store stream; auto-resolving conflicts beyond hold/adjudicate.
Invariants at risk: INV-BRANCH-NOT-MAIN (built in `../aplus-s75cfaj` worktree off `main`; idle trunk parked on `parking/s75-cfaj`); INV-CORE-CAPABILITY (extends the wired path — stays green + `--self-test`); INV-ROLE-INLINING (real medical-liaison dispatch); INV-OVERRIDE-RECORD-SCHEMA + INV-CRITICAL-NON-OVERRIDABLE (REUSED, not changed); INV-CLOSE-AUDIT / INV-PF-ATTESTATION / INV-SKILL-TRACE / INV-HARVEST-CAPTURE at close; store-surface battery (no new write surface). Core-capability-first gate (PF-S63-02): SATISFIED — this extends the held-line's terminal gate to the conflict axis. **PF-S74-01 discipline: re-run the full close on the final post-merge `main` + cite the `main` SHA.**

## Scope Contract — Session 74 (2026-06-19)

Goal: Wire the **medical-liaison terminal adjudication gate** (pipeline Phase 4, design `vault/design/plan-generation-pipeline-v1.md`) over the reconciler's held additive-AE finding — the held-line closer. A real medical-liaison dispatch adjudicates the held supplement↔peptide additive-AE finding into either a content-validated **override record** (HIGH/MEDIUM → clears the hold, the supplement records) or a non-overridable **auto-block** (`composite_band==CRITICAL` OR `harm_class∈{H1,H2}` → block stands), validated mechanically against the override-record schema + the critical-non-overridable rule. Operator delegated the two scope judgment calls at session open ("do what makes the platform more robust and do it with rigor"): scoped to the additive-AE axis (the one with a live HOLD) with the gate built to generalize, Rx/BPMH + conflict axes beaded as wired follow-ons (`rxbp`/`cfaj`); both INVs promoted build-then-promote.

Acceptance criteria:
- [ ] AC1: the adjudication input contract — the held additive-AE finding (from `reconcile`) is routed to the liaison as a `safety_finding`; a liaison adjudication envelope is defined (runtime-A output: `finding_id`, `composite_band`, `harm_class`, `verdict`, `severity_final`, `override_record|null`). All S70–S73 tests + `--self-test` stay green (single-domain + reconciler public contracts unchanged).
- [ ] AC2: the override-record validator (INV-OVERRIDE-RECORD-SCHEMA) — content-not-presence: the canonical literal "operator is overriding a safety block" (referenced, never redefined); content-bearing `operator_reason` (length floor + vacuous stop-list) at HIGH; `evidence_provided.rung ≥ evidence_tier_required(band)` (MEDIUM→clear-choice; HIGH→understanding+appreciation+reasoning); a risks-of-proceeding clause; a `contradictions_log_ref`. Any failure → override rejected, block stands. Mutation-proven RED.
- [ ] AC3: the critical-non-overridable gate (INV-CRITICAL-NON-OVERRIDABLE) — `composite_band==CRITICAL` OR `harm_class∈{H1,H2}` → must be `mechanical-auto-block-per-R3`, null/absent override_path; an override presented for such a finding is rejected (non-overridable). Mutation-proven RED.
- [ ] AC4: the terminal outcome wired into `generate_plans` — the held additive-AE supplement routes through the liaison adjudication: valid HIGH/MEDIUM override → hold RELEASED, supplement records via the existing `record_plan` (override record surfaced in the returned result); auto-block or invalid/insufficient override → supplement stays HELD. No new store stream (override record returned, like the reconciliation report; `plan_schema` untouched); store-adversarial surface unchanged.
- [ ] AC5: real medical-liaison E2E — a real deployed `medical-liaison` dispatch (full profile inlined per INV-ROLE-INLINING) adjudicates the S73 held fish-oil↔BPC-157 `bleeding-risk` finding through `generate_plans` E2E; the code validates + applies it; the supplement clears-or-stays-held correctly. Not a unit stub.
- [ ] AC6: INV promotion + review + close — promote INV-OVERRIDE-RECORD-SCHEMA + INV-CRITICAL-NON-OVERRIDABLE via the 4-step change-discipline ritual (operator delegation at session open + change-log rows) with `scripts/audit-medical-liaison-override.sh` + its negative test as the mechanical verification (bead `mdv`), built-then-promoted (the guard goes RED on a bad record before the register row is written); the negative test joins the negative-test floor. plan-integrity + QA (Tier-2) → `/review-pr` (Tier-3, 6-agent) → fix → `/merge`; full close, close artifacts written in the worktree BEFORE merging the build PR (the S73 lesson).

Files I WILL touch: `scripts/plan/orchestrate.py` (wire the held additive-AE supplement → liaison adjudication terminal step); new `scripts/plan/adjudicate.py` (the override-record validator + the critical-non-overridable gate + the `safety_finding`/envelope shape); `tests/plan/test_orchestrate.py` + new `tests/plan/test_adjudicate.py`; new `scripts/audit-medical-liaison-override.sh` + new `scripts/tests/test_audit_medical_liaison_override.sh`; `scripts/tests/run-all-tests.sh` (register the negative test in the floor); `docs/plan-generation/author-dispatch-process.md` (the liaison adjudication envelope + override-record schema); new `docs/plan-generation/examples/liaison-adjudication-*.example.json` (the captured real liaison envelope); `INVARIANTS.md` (register the 2 INVs + change-log rows); `vault/design/plan-generation-pipeline-v1.md` (Build status: Phase 4 wired); HANDOFF/PF-log/harvest/beads/session-note at close.
Files I will NOT touch: `scripts/store/plan_schema.py` (`record_plan` — call only, no schema change; the override record is returned, not persisted into the plan stream); `scripts/plan/assemble.py` + `router.py` core + the PII boundary (call only); `scripts/store/care_team_rollup.py` (the read-model, not the queue surface); the deployed `.claude/agents/*` profiles (inline for dispatch, don't edit); the wiki research-list / `vault/library/` (the two parallel wiki sessions own it); `main` directly.
NOT doing: the doctor-visit-queue collation + SBAR handout artifact (a substrate write surface — follow-on); the supplement↔Rx BPMH reconciliation axis (needs the operator Rx-list read + de-id path — beaded `rxbp`); adjudicating the author-declared `conflicts`/overlaps (detect-only stays — additive-AE is the held axis this slice closes; beaded `cfaj`); persisting the override record as a new store stream (returned in the result, like the reconciliation report); the malformed-`ae_profile` fail-loud upgrade (S73 candidate — separate); the measure leg / adjust leg; a 5th plan domain.
Invariants at risk: INV-BRANCH-NOT-MAIN (built in the dedicated `../aplus-liaison-gate` worktree off `main`; idle main trunk parked on `parking/s74-liaison` per PF-S71-01); INV-CORE-CAPABILITY (the gate extends the wired path — stays green + `--self-test`); INV-ROLE-INLINING (the real medical-liaison dispatch); INV-OVERRIDE-RECORD-SCHEMA + INV-CRITICAL-NON-OVERRIDABLE (NEW — promoted this session via the ritual; mechanical verification live at promotion); INV-CLOSE-AUDIT / INV-PF-ATTESTATION / INV-SKILL-TRACE / INV-HARVEST-CAPTURE at close; store-surface battery (no new write surface — the override record is returned; QA confirms). Core-capability-first gate (PF-S63-02): SATISFIED — this is the held line's terminal gate, the closer to operator-usable.

### S74 Scope Contract Evaluation (2026-06-19, volatile)

- **AC1 (adjudication input contract) — PASS.** `_additive_ae_safety_finding` builds a deterministic `finding_id` + `caution`; the liaison adjudication envelope is defined (`finding_id`, `composite_band`, `harm_class`, `verdict`, `severity_final`, `override_record|null`). All S70–S73 tests + the `--self-test` stay green; the single-domain + reconciler public contracts are UNCHANGED — the `adjudicator` param is additive (keyword-only, default None) and `adjudication` is an additive return key (contracts-reviewer confirmed doc-matches-code, no breaking change).
- **AC2 (override-record validator, INV-OVERRIDE-RECORD-SCHEMA) — PASS.** Content-not-presence: canonical literal, content-bearing `operator_reason` (floor + vacuous stop-list) at HIGH, `evidence_provided.rung ≥ evidence_tier_required`, a risks-of-proceeding clause, a contradictions-log ref. Mutation-proven RED (neuter content-validation → vacuous tests RED), independently re-confirmed by QA + the blind verifier.
- **AC3 (critical-non-overridable gate, INV-CRITICAL-NON-OVERRIDABLE) — PASS.** CRITICAL or `harm_class∈{H1,H2}` → null/absent override path; an override built for such a finding is rejected. Mutation-proven RED (neuter `is_non_overridable` → 11+ RED). The Tier-3 security review hardened it against a casing/whitespace bypass (SEC-1) + crash-on-malformed (BUG-1/BUG-2): the gate now fail-SAFE to block-stands on any malformed routing key.
- **AC4 (terminal outcome wired, no new store stream) — PASS.** `generate_plans` routes the held additive-AE supplement through `adjudicate`: a content-valid override RELEASES the hold (records via the existing `record_plan`); an auto-block / invalid / absent adjudication leaves it HELD. No new store stream (the override record rides the returned `adjudication`; `plan_schema` untouched — QA + contracts confirmed). Core-capability gate green.
- **AC5 (real medical-liaison E2E) — PASS.** Two real deployed `medical-liaison` dispatches (full profile inlined): an informed-refusal request → a content-valid MEDIUM/H3 override → the held supplement CLEARS + records; a vacuous request → REFUSED → block stands. Captured as `liaison-adjudication-{cleared,blocked}.example.json` + codified as regression E2E.
- **AC6 (INV promotion + review + close) — PASS.** Both INVs promoted via the change-discipline ritual (operator delegation at session open + change-log rows), built-then-promoted (the audit + negative test went RED on bad input BEFORE the register rows). `scripts/audit-medical-liaison-override.sh` + its negative test (13/13, in the floor) are the mechanical verification. plan-integrity (INTEGRITY-CLEAN) + QA (2 SHOULD-FIX, fixed) Tier-2 → `/review-pr` 6-agent Tier-3 (6 LEGITIMATE incl. 2 CRITICAL safety findings → all fixed + blind-verified 6/6 RESOLVED) → `/merge`. Close artifacts written in the worktree BEFORE merging the build PR (the S73 lesson applied).

### Drift checks (S74 close)

- **Task drift:** all 6 ACs PASS. The one planned file NOT touched — `scripts/tests/run-all-tests.sh` (the floor auto-discovers `test_*.sh`, so the mdv negative test needs no manual registration) — a simplification, flagged not silent. The deferred Phase-4 axes are beaded (`rxbp` supplement↔Rx BPMH, `cfaj` conflict adjudication) — honest scope. No silent drift.
- **Architecture drift:** no invariant degraded; TWO new invariants ADDED via the ritual (INV-OVERRIDE-RECORD-SCHEMA + INV-CRITICAL-NON-OVERRIDABLE) — they strengthen the safety surface. INV-CORE-CAPABILITY green (the gate extends the wired path; `--self-test` passes). INV-BRANCH-NOT-MAIN held (built on `feature/liaison-gate` in `../aplus-liaison-gate`; idle trunk parked on `parking/s74-liaison`). INV-ROLE-INLINING held (the real medical-liaison + the 6 review agents + QA/plan-integrity all full-profile). Toward the design (Phase 4 wired).
- **Vision drift:** none, toward-vision. The held additive-AE line now CLOSES to operator-usable through the liaison gate — a content-valid informed-refusal override releases the supplement; a non-overridable / invalid adjudication holds it. `design/vision.md` first sentence unchanged.

### PF attestation

S74 close (2026-06-19): **One new PF promoted post-close at operator direction — PF-S74-01** (declared the S74 close "fully gated"/"synced" on the PRE-MERGE worktree audit without re-running the full close gate on the final post-merge `main`; operator-surfaced — "you didn't run the full close again"; recurrence 2 of the close-completeness/operator-surfaced-under-report family; captured 3-layer + the structural close-protocol fix beaded `cvf1`; the full close was then re-run on `main` @ `972b880` — pytest 978/2 + close-audit PASS 0/0 + harvest-gate PASS — confirming the merged state clean). The build itself held its disciplines — both gate invariants mutation-proven RED (independently re-confirmed by QA + the profile-less blind verifier); the production path verified E2E with REAL `medical-liaison` dispatches (the integration mandate), the liaison genuinely discriminating (an informed-refusal cleared, a vacuous request refused); the two INVs were built-then-promoted (the guard went RED on bad input before the register row). The Tier-3 review caught 6 real findings — including 2 SAFETY-CRITICAL (a `harm_class` casing block-release bypass + a crash-instead-of-block-stand) that the Tier-1 self-check + Tier-2 did NOT catch — all gate-caught before merge, nothing shipped: the layered review working as designed (the same mechanism the prior build sessions relied on). The honest observation that did NOT promote: the module's "malformed → block-stands, never release" contract was implemented for record CONTENT but trusted the routing-key TYPES/casing — the review caught the gap; recurrence 1, caught by the correct mechanism, so watch-not-promote, with the lesson logged (a fail-safe contract must cover every input class, not just content). One process hazard surfaced + handled: the blind verifier's mutation-restore used `git checkout` (unsafe on an uncommitted working-tree fix — it reverted to HEAD); the file was reconstructed + I independently re-verified (markers + suite + mutation-proof) before committing, then committed promptly to lock it in. The verify-first discipline held; the freshness discipline held (a stale-prose line caught by the review + fixed in-session). Prior PFs referenced descriptively. Full attestation + the per-PR skill-trace table + the disclosure ledger in `memory/process-failures.md` Session 74.


## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-72 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; …; S72 archived S71; S73 archived S72; S74 archived S73) to keep this handoff lean. The current (S74) scope contract is above; the archive holds the prior-session archaeology.
...
1. **Run the close gate on the FINAL post-merge `main`, not the pre-merge worktree (PF-S74-01, recurrence 2 — operator-surfaced).** At S74 close I ran pytest + close-audit + harvest-gate on the build WORKTREE, merged + pulled, then declared "fully gated"/"synced" WITHOUT re-running the close on the final `main` — the operator caught it ("you didn't run the full close again"). "The close ran" (worktree) ≠ "the final closed artifact is gated" (`main`). **Every close re-runs pytest + `close-audit --session N` on `main` AFTER the merge+pull and the attestation CITES the post-merge `main` SHA** (until the protocol amendment + mechanical post-merge-verify land — bead `cvf1`). Never claim "fully gated"/"synced" before re-verifying the actual merged trunk; log an operator-surfaced catch AS operator-surfaced (the ledger's "0" was wrong twice now — the family the disclosure ledger exists to catch).
2. **Fail-SAFE for EVERY input class, not just content (S74 review lesson).** The Tier-3 review caught the medical-liaison gate shipped (into review) able to RELEASE a non-overridable block on a `harm_class` casing variance ("h1" vs "H1") and to CRASH (instead of block-stand) on a non-dict `severity_final` / unhashable `harm_class` — the gate validated record CONTENT but TRUSTED the routing-key types/casing. A fail-safe gate's "malformed → safe-default" contract must cover EVERY input class (type, casing, hashability), and the Tier-1/Tier-2 self-check must exercise the malformed-input surface — don't lean on Tier-3 to catch a safety-gate bypass. (The held line is now CLOSED to operator-usable: `scripts/plan/adjudicate.py` adjudicates the held additive-AE finding; INV-OVERRIDE-RECORD-SCHEMA + INV-CRITICAL-NON-OVERRIDABLE enforce it. The CLAIM-PHRASING RULE + the fail-SAFE-to-block-stands contract stay load-bearing for every liaison dispatch + gate extension.)
3. **Standing mechanics + carry-overs.** **Worktree discipline (PF-S70-01/S71-01, held S74):** separate `git worktree`s; park the idle main trunk OFF `main`; Read worktree files at the worktree-absolute path; re-verify `git branch --show-current` before every commit; restore `main` at session end. **Commit fixes PROMPTLY — `git checkout -- <file>` reverts an uncommitted working-tree fix to HEAD (the S74 blind-verifier hazard); provision a worktree `.venv` (`python3 -m venv .venv && .venv/bin/python -m pip install -r requirements.txt`) so the full close-audit + pytest run natively in-worktree.** GraphQL EXHAUSTED (0/0) → REST for PR-create/merge (`gh api`, full-40-char SHA guard). `git add` separate from `git commit` (PII hook). bd repo-ID mismatch + `.beads/daemon-error` + vault re-mangle benign — no backfill `bd sync`; commit `.beads/issues.jsonl` in a worktree (bd hooks skip there) or DB-move + `--no-verify` on the trunk (memory `bd-commit-on-main-trunk-workaround`). Two parallel wiki sessions push to `origin/main` — rebase-sync at merge, never touch `vault/library/`. **Close-step-8 freshness still un-gated until `dfg1` lands — RUN step 8 explicitly each close.** Carry-overs: `cvf1` (close-gate-on-final-`main`, PF-S74-01) / `dfg1`/`df1x` (doc-freshness gate + backlog) / `aem1` (ae-matching) / `axm7` (gate-extend) / `rxbp`/`cfaj` (Phase-4 follow-ons) / `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`. Pencil: ALWAYS pass `filePath`.

## Current State (volatile)

- **S74 (2026-06-19) wired the medical-liaison terminal adjudication gate — the Phase-4 closer for the held additive-AE finding.** `scripts/plan/adjudicate.py`: the deployed medical-liaison adjudicates the held supplement↔peptide finding into a content-validated override record (HIGH/MEDIUM → the hold RELEASES, the supplement records via the existing `record_plan`) or a non-overridable auto-block (`CRITICAL` or `harm_class∈{H1,H2}` → block stands). Wired into `generate_plans(..., adjudicator=...)`; no new store stream (the override record rides the returned `adjudication`; `plan_schema` untouched). Two real `medical-liaison` dispatches verified E2E: an informed-refusal → a content-valid MEDIUM/H3 override → the supplement CLEARS + records; a vacuous "just want to try both" → REFUSED → block stands. Envelopes `docs/plan-generation/examples/liaison-adjudication-{cleared,blocked}.example.json`.
- **PR #NNN** — plan-integrity (INTEGRITY-CLEAN) + QA (2 SHOULD-FIX fixed) Tier-2 → `/review-pr` 6-agent Tier-3 (6 LEGITIMATE incl. 2 CRITICAL safety findings — SEC-1 `harm_class` casing block-release bypass + BUG-1 non-dict-`severity_final` crash; all fixed + blind-verified 6/6 RESOLVED) → `/merge` (REST rebase under GraphQL exhaustion). Per-PR skill-trace table in `memory/process-failures.md` Session 74. Single-PR close — close artifacts written in-worktree BEFORE merge (the S73 lesson applied).
- **INV-OVERRIDE-RECORD-SCHEMA + INV-CRITICAL-NON-OVERRIDABLE promoted (bead `mdv`)** via the change-discipline ritual (operator delegation at open + change-log rows), built-then-promoted (the guard went RED on bad input before the register row); `scripts/audit-medical-liaison-override.sh` + its negative test (13/13) in the floor.
- **pytest 977/3 (worktree, native `.venv`).** Core-capability gate green; negative-test floor 15/0.
- **Beads:** `71s4` (the liaison gate is WIRED; the Phase-4 follow-on axes + the measure/adjust legs remain); `mdv` CLOSED (both INVs promoted + the audit built); `rxbp` (supplement↔Rx BPMH axis) + `cfaj` (conflict adjudication) — new Phase-4 follow-ons that REUSE the gate; carry-overs `dfg1`/`df1x`/`aem1`/`axm7`/`llna`/`0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`.
...
**Historical (kept for reference):** `vault/sessions/session-74.md`.

## What Is Next (volatile)

### S75 — the Phase-4 follow-on axes + the measure/adjust legs

The held additive-AE line is CLOSED to operator-usable (the liaison gate adjudicates it). Next, REUSING the S74 gate (the override-record validator + the critical-non-overridable gate + the `safety_finding` envelope), per `vault/design/plan-generation-pipeline-v1.md` Phase 4 — in a dedicated `git worktree`, idle trunk off `main`, with a provisioned worktree `.venv`:

1. **The supplement↔Rx BPMH axis** (`rxbp`) — the liaison's marquee watchlist check (warfarin/levothyroxine/CYP3A4/antithrombotic-stacking). Adds the operator medication-list read THROUGH the `router.summarize` PII de-identification boundary (ADR-0001) — a substrate surface that EARNS its own adversarial PII-safety + E2E verification. Reuses the gate unchanged.
2. **The author-conflict adjudication** (`cfaj`) — routes `report["conflicts"]` (currently detect-only) through the same gate (detect-only → hold-and-adjudicate for the conflict axis).
3. **The doctor-visit-queue collation + SBAR handout artifact** — the medical-liaison's owned queue surface (a substrate write — satisfy `docs/checklists/store-adversarial-tests.md`).
4. Then the **measure** leg (a production caller for `record_plan_tracking` — currently schema-only) + the **adjust** leg (re-plan from tracking) — the rest of the closed loop.

**Discipline:** scope contract at open; **worktree + idle trunk off `main` + provision the worktree `.venv`**; **write close artifacts in-worktree BEFORE merging the build PR** (single-PR close); **commit fixes promptly — `git checkout` reverts an uncommitted working-tree fix**; re-verify `git branch --show-current` before each commit; honor the CLAIM-PHRASING RULE + the fail-SAFE-to-block-stands contract; **verify the production path E2E with a real `medical-liaison` dispatch**; `/review-pr` → `/merge`.

**Deferred (NOT blocking):** `dfg1` (the doc-freshness gate — PF-S73-01 structural fix) + `df1x` (the doc-freshness backlog); `aem1` (the parenthetical-name interaction-match precision gap); the malformed-`ae_profile` fail-loud upgrade (now adjacent to the gate's fail-safe contract); `axm7` (extend the core-capability gate to the orchestrator); the standalone full-plan render screen + the dashboard reskin (operator's worktrees); the `/generate-plan` slash-command wrapper; the `llna` hook fix; carry-overs `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`; operator-gated July-visit prep (LM-01) — first session on/after **2026-06-29** runs the LM-01 scoped drift audit.

## Landmark window check (close step 8.7)

S74 close (2026-06-19): RE-OPENED `vault/meta/landmarks.md` at close step 8.7 (`last_reviewed` bumped to 2026-06-19). No landmark edited — S74 was plan-generation code on PII-free synthetic fixtures (no operator data ingested, no Whoop baseline start, no MD-handoff artifact). LM-01 (First MD visit) **2026-07-13**: no trigger window open today (the 14-day scoped-drift-audit window opens **2026-06-29**, the 7-day MD-handoff window 2026-07-06). LM-02 (Whoop) / LM-03 (23andMe) date-TBD. LM-04 (first HTML artifact) NOT triggered — the dashboard rendered only to a tempdir (synthetic fixtures); no committed HTML artifact this session. No landmark actions due.
