...
- **Pass-3 pilots: 2 of 14 specialists DEPLOYED** (post-S16, integrator session). `medical-liaison` (Role 7 adjudicator; PR #8) + `labs-specialist` (PR #6) merged to `main` (`origin/main` at `0ce76ca` as of 2026-05-29). `peptide-specialist` still BUILDING (3rd pilot). **Gate recalibrated** (PR #7: R13-12 collation-only exempt + R13-3 token→WARN — pilot-surfaced). **Frontmatter convention DECIDED: none** (specialists match the foundation no-frontmatter convention; dispatched via INV-ROLE-INLINING). **`81.8%` authority-impersonation figure adjudicated CORRECT** (a builder flagged it as a JBDistill misattribution; integrator re-verified the Ekram full text — 9/11 = 81.8% — foundation docs unchanged; bead `amc` REJECTED). Two PF-S16 entries logged (worktree-path hazard; rm-rf near-miss). **Pilot checkpoint with Walter due after peptide merges; do NOT auto-start the 11 remaining.**
