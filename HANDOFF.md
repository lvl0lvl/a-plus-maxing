...
## Scope Contract — Session 88 (2026-06-22)

Goal: Lay the architectural foundation for the conversational intake agent — author the ADR (the first model client + the scoped ADR-0001 relaxation + the conversation→de-identified-store extraction contract + the form/chat split + the plan-generation wiring decision), update the vision, and produce a grounded design pass — then review, merge, and close. (AUTONOMOUS per the operator's standing directive 2026-06-22; scope contract self-confirmed, no operator confirmation gate. This is the FOUNDATION wave; the build is S89+ via `/execute-plan`.)

Acceptance criteria:
- [ ] AC1 (ADR): An ADR authored via `/create-adr` (read in full first, PF-S17-01) covering (a) the FIRST model client (no-train API for V1, designed SWAPPABLE at the boundary for the local-model North Star — the API conversations double as the local-model eval corpus); (b) the SCOPED ADR-0001 zero-egress relaxation (raw conversation → no-train API; everything PERSISTED stays de-identified); (c) the conversation→de-identified-store EXTRACTION CONTRACT (how dialogue becomes the SUMMARY_FIELD_SET facts the planner reads — the load-bearing seam); (d) the form/chat split (form = demographics + content-uploads; chat = goals/training/nutrition/supplements/peptides); (e) the explicit decision on whether the same model client also wires core plan-generation (PF-S63-02 / bead `71s4`). Committed under `docs/adr/`.
- [ ] AC2 (vision): `design/vision.md` updated so its first sentence describes the conversational personal-health-agent direction (no longer the rigid-form tracker). Old direction not silently deleted — superseded with a pointer.
- [ ] AC3 (design pass): A design artifact produced via the design/architecture agents (NEVER solo-originated, per `feedback_design_via_agent_team`), grounded against the LIVE planner/capture/store code (`scripts/plan/`, `scripts/serve/capture.py`, `scripts/plan/router.py` SUMMARY_FIELD_SET, `store.py`), covering: the question strategy, the chat transport (a chat endpoint on the existing loopback server), the extraction mechanism, and the PII handling. Committed under `vault/design/` (or `design/`).
- [ ] AC4 (crown-jewel boundary honored in design): the design states the PII boundary EXPLICITLY — only the live conversation reaches the no-train API; every fact written to the store stays de-identified (the extraction writes only de-identified SUMMARY_FIELD_SET-class facts; no raw PII persists).
- [ ] AC5 (review + merge): PR opened against `main`; `/review-pr` run with the independence phases intact (docs/design-appropriate agent subset); legitimate findings fixed or beaded; `/merge` to `main` behind the head-SHA guard.
- [ ] AC6 (close): full S88 close — pytest + 3 drift checks + HANDOFF 6-clause rotation + PF attestation + disclosure ledger + `scripts/close-audit.sh` on the FINAL post-merge `main` + harvest-gate + cite the SHA (PF-S74-01).

Files I WILL touch: a new `docs/adr/ADR-00NN-*.md` (via `/create-adr`); `design/vision.md`; a new conversational-intake design artifact under `vault/design/` (or `design/`); the close docs (HANDOFF / `memory/process-failures.md` / `harvest.jsonl` / beads / `vault/sessions/session-88.md` / `vault/meta/overview.md`); possibly a `docs/prd/` linkage if `/create-adr` requires it.
Files I will NOT touch: the operator's MAIN-checkout vault WIP (`stash@{0}` + the 3 `bpc-157` files — never destructively); ANY `scripts/` production code (the build is S89+); the rigid form's dead Step-1 / orphan-token gaps (SUPERSEDED by the pivot — not patched); `main` directly; the `aplus-wiki-research` worktree / the operator's wiki branch.
NOT doing: building the conversational intake agent — the model client, the chat endpoint, the extraction code (S89+ via `/execute-plan` + 3-tier review); the downstream spec / build-plan / task-plan (S89+, after this ADR); the local-model migration (North Star, post-V1); patching the form.
Invariants at risk: ADR-0001 (zero-egress — DELIBERATELY relaxed by the operator-signed-off pivot; the ADR formalizes the SCOPED relaxation [raw conversation → no-train API; the store stays de-identified] — flagged, NOT silent); INV-CORE-CAPABILITY (PF-S63-02 — this ADR is the path TO the first model client / real plan generation, so it ADVANCES the core; no `scripts/plan/` code changes this session, so the wired path stays green); INV-CLOSE-AUDIT / INV-PF-ATTESTATION / INV-SKILL-TRACE / INV-HARVEST-CAPTURE (close gates); INV-BRANCH-NOT-MAIN; INV-ROLE-INLINING (design-agent dispatches inline full profiles / use registered agent types).

**Core-capability-first gate (PF-S63-02):** does the core capability — a followable health plan, end-to-end — work yet? **NO** (no model client in `scripts/`; plan generation renders only dashboard/report; bead `71s4`). This session's scope is NOT secondary work — the conversational-intake ADR is the FIRST model integration and the direct path to wiring real plan generation (the core). The gate is satisfied: this IS core-capability work, not substrate/polish around an unbuilt core.

**S88 self-confirmation (autonomous):** contract self-confirmed 2026-06-22 per the operator's standing autonomous directive; no operator confirmation gate. Driving continuously ADR → vision → design → review → merge → close; the session close is the wave boundary; the build is the next autonomous session.

### S88 Scope Contract Evaluation (2026-06-22, volatile)

- **AC1 (ADR set 0015–0019) — PASS.** Authored via the full `/create-adr` pipeline (discovery → rubric → DAG → tier-by-tier author/verify/judge all ACCEPT → whole-set red-team → final-fix → final-verify OVERALL PASS); accepted + merged (PR #216). The set is 5 ADRs — **ADR-0019** (the `SUMMARY_FIELD_SET` extension) was ADDED during discovery as the "most robust path" call (without it the pivot recreates the PF-S87-01 gap); flagged for operator review, not silent.
- **AC2 (vision.md) — PASS.** First sentence + PII-boundary principle + trajectory + cross-refs updated to the conversational direction; the rigid-form direction superseded with pointers (not deleted).
- **AC3 (design pass) — PASS.** Architect-authored interface-contract design (`vault/design/conversational-intake-design.md`) + an independent adversarial critique + remediation (the critique caught a Critical the create-adr pipeline missed — the 8j6-gate-doesn't-scan-the-derived-token-path inaccuracy — corrected in the design doc + ADR-0019/0017).
- **AC4 (crown-jewel boundary explicit) — PASS.** The design states the per-path PII boundary truthfully (pass-through 8j6 / derived-token coarseness + per-token proof; only the live conversation egresses; nothing committed).
- **AC5 (review + merge) — PASS.** PR #216; `/review-pr` (3-agent docs subset, 4 Suggestion findings all blind-triaged LEGITIMATE → fixed → blind-verified RESOLVED); rebase-merged → `main` @ `6d1d85f`.
- **AC6 (close) — PASS** (this close; close-audit + harvest-gate green on final post-merge `main`, SHA cited).
- **Scope addition (justified, not silent):** the 16-edge inverse backfill into 6 existing ADRs (0001/0002/0005/0006/0013/0014) — a required bidirectional-consistency consequence of the new set (append-only, no accepted Decision mutated). Beyond the literal "new ADRs" file list; documented here.

### Drift checks (S88 close)

- **Task drift:** None. All 6 ACs PASS. The notable expansions (ADR-0019 added; the 6-ADR backfill; the design-doc remediation) were each flagged/justified, never silent. AC1's "ADR set" became 5 ADRs (count set by discovery, not a fixed number).
- **Architecture drift:** YES — deliberately, exactly as the scope contract flagged. ADR-0016 scopes-relaxes ADR-0001's zero-egress (operator-signed-off); ADR-0015 introduces the first programmatic model client. INV-CORE-CAPABILITY is ADVANCED (ADR-0015/0019 are the path to real plan generation); no `scripts/` code changed, so the wired path stays green (core-capability-audit passes). No invariant silently degraded.
- **Vision drift:** None. The vision was DELIBERATELY updated this session to the conversational direction — system and vision now agree (the S87-flagged staleness is resolved).

### PF attestation

S88 close (2026-06-22): No new PF-class entries this session. The layered review system worked as designed — each independent gate caught what the prior missed, all BEFORE merge, 0 reaching the operator: the adversarial design-review caught a behavioral inaccuracy (the 8j6 gate does not scan the derived-token path) that the create-adr citation/rubric passes did not (a distinct behavioral-guarantee lens, not a single-layer miss); `/review-pr` caught 2 missing inverse edges the create-adr backfill enumeration omitted; verify-first re-greps caught an incomplete author fix. Full attestation + skill-trace table + disclosure ledger in `memory/process-failures.md` Session 88. Prior lessons referenced DESCRIPTIVELY (the verify-first discipline held; the behavioral-guarantee-review lesson; build/review via the gated skills; close on final post-merge main + cite the SHA), never by bare token.

**Historical (kept for reference):** `vault/sessions/session-88.md`.

## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-87 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; …; S85 archived S84; S86 archived S85; S87 archived S86; S88 archived S87) to keep this handoff lean. The current (S88) scope contract is above; the archive holds the prior-session archaeology.
...
1. **The conversational-intake FOUNDATION is merged (ADR-0015–0019 + design); S89 BUILDS it.** The ADR set + the interface-contract design (`vault/design/conversational-intake-design.md`) are accepted on `main` @ `6d1d85f` (2026-06-22 S88 close). The next work is the BUILD via the V1 pipeline: spec → build-plan → task-plan → **`/execute-plan`** + 3-tier review — of the model client + the `/chat` endpoint + the extraction + the demographic-form activation. **Per PF-S87-01 the acceptance MUST verify END-TO-END capability** (a usable plan from the conversation), not just chat plumbing. Crown jewel: only the live conversation egresses (no-train API); everything persisted/committed de-identified; per-path de-id (pass-through 8j6 / derived-token coarseness + a per-token coarseness proof — a spec AC). Full grounding: ADR-0015–0019 + `vault/design/conversational-intake-design.md` + epic `w3y8`.
2. **S89 spec must carry the design-stage requirements the foundation DEFERRED (all in `docs/adr/.pipeline/design-review.md` + the design doc §7).** The per-token coarseness proof for each new ADR-0019 derived token (the 8j6 gate does NOT cover the derived path — de-id is the derivation's coarseness, proven by a per-token output scan); the `identity_config` wiring at the `/chat` + `/upload` capture call sites (H-2 — dropped at `server.py:158` today, so operator-identity PII detection is empty in production); the chat-covers-domain-but-token-not-minted divergence (CONCERN-1); per-turn vs end-of-session extraction cadence (CONCERN-2); the partial-extraction turn receipt (M-3). **Surface the foundation for operator review first** (esp. the autonomously-added ADR-0019 + the ADR-0001 relaxation specifics) — the operator can course-correct before the build commits.
3. **Standing mechanics + the operator's data state.** Build via **`/execute-plan`** (never hand-rolled, PF-S36-01); design via the **design agents** (never solo); full role profiles inlined VERBATIM (the hook denies abbreviation); `/review-pr` off the **THREE-dot diff** with both independence phases INTACT → `/merge` (REST fallback when GraphQL exhausted, full-40-char SHA guard); `git add` SEPARATE from `git commit`; **the `block-commit-main` hook false-blocks a worktree commit when the main checkout sits on `main` → PARK the main checkout on a same-commit branch (WIP carries untouched), commit in the worktree, un-park** (PF-S71-01; used twice this session, WIP verified identical both times); re-run the close gate on final post-merge `main` + cite the SHA (PF-S74-01). **The operator's MAIN checkout is on `main` @ `dfd44e8` (as of 2026-06-22 S88 close), behind origin — S89 syncs at start;** their parallel-wiki vault WIP is in `stash@{0}` + 3 `bpc-157` files to reconcile (their call — never touch destructively). The basic-memory daemon re-adds `permalink:` frontmatter (benign churn).

## Current State (volatile)

- **S88 (2026-06-22): the conversational-intake FOUNDATION is built + merged.** The ADR set (0015–0019) formalizing the S87 pivot, the interface-contract design doc, and the vision update — all accepted on `main` @ `6d1d85f` (2026-06-22 S88 close). Authored autonomously per the operator's standing directive. The BUILD (the model client + `/chat` endpoint + extraction) is S89+.
- **The 5 ADRs (via the full `/create-adr` pipeline, OVERALL PASS):** 0015 swappable no-train model client (the codebase's FIRST programmatic model client; fail-closed on model-call failure; closes the PF-S63-02 core-capability gap); 0016 scoped ADR-0001 egress relaxation (raw live conversation → no-train API; persisted + committed stay de-identified); 0017 conversation→de-identified-store extraction ("model proposes, gate disposes"); 0018 form/chat split (form = demographics + uploads; chat = the rich sections); 0019 `SUMMARY_FIELD_SET` extension (closes the PF-S87-01 gap — chat-sourced nutrition/supplement/peptide/training facts reach the planner). Plus a 16-edge inverse backfill into 6 existing ADRs (append-only).
- **The design pass:** `vault/design/conversational-intake-design.md` — interface contracts (question strategy, chat transport, extraction, PII flow) grounded against the planner/capture/store code, the crown-jewel boundary stated PER-PATH. An independent adversarial critique caught + corrected the 8j6-gate-doesn't-scan-the-derived-token-path inaccuracy (a Critical the create-adr pipeline missed).
- **pytest 1264 passed / 3 skipped on `main` @ `6d1d85f` (2026-06-22 S88 close).** No `scripts/` code changed (the foundation is ADRs + design + vision); the core-capability path stays wired.
- **No new PF.** The layered reviews (create-adr pipeline → adversarial design review → `/review-pr`) each caught what the prior missed, all before merge; 0 reached the operator (disclosure ledger in `memory/process-failures.md` S88).
- **Beads:** epic `w3y8` (the pivot) advanced — the ADR foundation landed; the build is the next phase. The S89 forward-work (the build + the design-review spec requirements — per-token coarseness proof / `identity_config` wiring / CONCERN-1/2 / M-3) is durably captured in the committed design doc §7 + the HANDOFF "What Is Next"; `bd` operations were DEFERRED this close to avoid touching the operator's MAIN-checkout WIP `.beads` (the worktree-`bd` flush lands in the main-trunk copy — S87 rough edge). S89 creates the build beads against the synced main trunk.

**Historical (kept for reference):** `vault/sessions/session-88.md`.

## What Is Next (volatile)

### S89 — build the conversational intake agent (the foundation is merged)

The conversational-intake FOUNDATION (ADR-0015–0019 + the interface-contract design `vault/design/conversational-intake-design.md` + the vision update) is accepted on `main` @ `6d1d85f` (2026-06-22 S88 close). S89 BUILDS it through the V1 pipeline: **spec → build-plan → task-plan → `/execute-plan` + 3-tier review.** Read each gated skill IN FULL before invoking.

**First — surface the foundation for operator review.** S88 authored the ADR set autonomously. Two decisions were AUTONOMOUS "most-robust-path" calls flagged for the operator: **ADR-0019** (the `SUMMARY_FIELD_SET` extension — added during discovery so the chat actually feeds the planner; without it the pivot recreates the PF-S87-01 gap) and **the RT-01 fail-closed model-call contract** in ADR-0015. Also operator-reviewable: the **no-train API provider** (ADR-0015 records a Claude no-train API as the swappable default). Present these so the operator can course-correct BEFORE the build commits to them. The build can run autonomously per the standing directive once the foundation is confirmed (or if the operator lets it run).

**What the build delivers (the accepted ADRs):** the swappable no-train model client (ADR-0015); the `/chat` endpoint on the loopback server (ADR-0013/0018); the conversation→de-identified-store extraction ("model proposes, gate disposes" — ADR-0017); the demographic-form activation so the 4 orphan tokens get working inputs (ADR-0018); the `SUMMARY_FIELD_SET` extension (ADR-0019). **Per PF-S87-01 the acceptance MUST verify END-TO-END capability — a usable plan from the conversation, not just chat plumbing.**

**The spec MUST carry the deferred design-stage requirements** (all in `docs/adr/.pipeline/design-review.md` + design doc §7): the per-token coarseness proof for each new ADR-0019 derived token (the 8j6 gate does NOT cover the derived path); the `identity_config` wiring at the `/chat` + `/upload` capture call sites (H-2 — `server.py:158` drops it today, so operator-identity PII detection is empty); CONCERN-1 (chat-covers-domain-but-token-not-minted divergence); CONCERN-2 (extraction cadence); M-3 (partial-extraction receipt).

**Deferred / NOT blocking:** the S86 Wave-B follow-up beads (mostly moot under the pivot); the operator's `bpc-157` WIP reconcile (their call); the local-model migration (North Star, post-V1); the operator loading real data (once the chat intake exists).

## Landmark window check (close step 8.7)

S88 close (2026-06-22): RE-OPENED `vault/meta/landmarks.md` at close step 8.7 (unchanged this session). No landmark edited — S88 was PII-free foundation work (ADRs + design + vision; no operator data ingested). LM-01 (First MD visit) **2026-07-13**: no trigger today (the 14-day scoped-drift-audit window opens **2026-06-29**, the 7-day MD-handoff window 2026-07-06). **NOTE for S89: the 14-day LM-01 window opens 2026-06-29 — if S89 runs on/after that date, run the scoped drift audit on the LM-01 relevant_scopes.** LM-02 (Apple Health) / LM-03 (23andMe): no real data ingested → not triggered. LM-04 (first HTML artifact): not triggered. No landmark ACTIONS due.
