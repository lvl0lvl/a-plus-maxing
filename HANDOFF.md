...
1. **The core deliverable is STILL UNBUILT — gate it before any secondary work (PF-S63-02 / bead `71s4`).** The system has substrate (store, ingestion incl. WHOOP, PII routing, plan-composition `assemble`, render/dashboard/report) but NO end-to-end plan generation: no model/API client in `scripts/`, `assemble` has no production caller, `generate.run` renders only dashboard/report. S64 adopted the rigor floor that makes the rebuild safe; the rebuild itself (`71s4`) is next. Per the now-adopted core-capability-first discipline: do not start secondary work without a checked answer to "does plan generation work end-to-end yet?" (today: NO).
2. **Heartbeat-hook wiring is deferred — sequence it before the next PR lifecycle (bead `0qf6`).** `enforce-heartbeat-clause` (vendored) DENYs every Task dispatch lacking a liveness clause, with no role-tag gating — it would break `/review-pr`+`/aplus-research`. Wire it ONLY after those flows + a dispatch convention carry the clause (couples with Wave D), and extend `test_settings_hook_paths.sh` to accept the `toolkit/hooks/` prefix then. Until then, PR lifecycles run with the hook OFF.
3. **The re-tightened rigor floor is LIVE but its newest mechanisms are UNTESTED in production — don't treat them as proven.** (a) the Discipline-11 pull cadence has never run a real pull; (b) `enforce-heartbeat-clause` stays UNWIRED (`0qf6`) and is a live trip-wire — wiring it before `/review-pr`+`/aplus-research` dispatches carry the liveness clause would DENY their agents; (c) `po4x` (harvest-gate over-extracts prior-PF tokens cited in prose; skill-trace escape sentence must be line-anchored) bit at THIS close and is worked-around, not fixed. At the next drift-audit boundary, run the pull cadence: diff the root `rigor_version` (1.0.0) vs `${SKILLS_LIBRARY}/frameworks/rigor/VERSION`, read the CHANGELOG delta, pull (re-run `toolkit/tests/run-all-tests.sh` first). Standing mechanics: REST `gh api` full-40-char SHA for PR ops (GraphQL throttles); stage `git add` separately from `git commit`; `.claude/hooks/*` edits need per-session authorization.

## Current State (volatile)

- **S64 (2026-06-15) adopted the Rigor Framework v1.0.0 toolkit (Wave A) and LANDED it — PR #139 `/review-pr` (6-agent → blind triage → blind verify; 18 findings → 14 fixed+verified, incl. a real fail-open F1) + `/merge` to `main`.** Walter: "check if the skills library on this machine has been updated" → it was 72 commits stale on a dead branch; updated `skills_library` to `origin/main`. Then: read the whole library, write an adoption scope contract, proceed with recommended paths, write an implementation log, do the close.
- **What landed (Wave A):** `toolkit/` vendored pristine (14/14 tests); `rigor_version: 1.0.0` pinned in CLAUDE.md; `scripts/close-audit.sh` single fail-closed close gate + `scripts/tests/run-all-tests.sh` aggregator (close step 8.5 repointed, 8.6 harvest gate added); `audit-helpers.sh` gained `skipped()`/FATAL-on-skip; self-improvement loop seeded (`harvest.jsonl` + PF-S63-02 back-filled + bead `71s4`). pytest 833/2 unaffected.
- **Implementation log:** `docs/rigor-adoption-log.md` — the living Discipline-11 deployment ledger (method, per-AC build, 7 issues, verification); maintained through Waves A→D, then frozen as the Loop-B harvest input.
- **Skills library (this machine):** `~/Documents/Projects/skills_library` now on `origin/main` (rigor v1.0.0 + `plan-integrity` role + new skills); `~/.claude/roles` + `~/.claude/library` symlinks serve it. The GLOBAL `~/.claude/skills`+`commands` are still STALE vs the library (Wave D, separate task — not in the repo).
- **Beads:** `71s4` (P1, core plan-generation + core-capability gate); adoption tail beaded — `eyn4` (Wave B), `4hmo` (Wave C), `ckl1` (Wave D), `0qf6` (heartbeat wiring), `d1kc` (provenance `jsonschema`), `po4x` (harvest-gate/skill-trace attestation format).
...
**Historical (kept for reference):** `vault/sessions/session-64.md`.

## What Is Next (volatile)

### Resume checklist — next session (S65) open

Open normally (Session Start Protocol; `branch-completeness-audit.sh` at OPEN; baseline `main` 833/2). **Wave A is MERGED to `main`** (PR #139). Read `docs/rigor-adoption-log.md` (the deployment ledger) before continuing the adoption.

**S65 forward options (operator prioritizes at open, NOT pre-adopted):**

1. **The core deliverable (`71s4`, P1) — start FRESH with its own scope contract. This is the point of everything.** Build the plan-generation path end-to-end: a model/API client (none exists in `scripts/`), a production caller for `assemble` (today only a stubbed-roster test path), a `plan` render target (`generate.run` renders only dashboard/report), and real operator meta-inputs (`goals.md`/`current-state.md`/`operator-profile.md` are placeholders). Open it under the now-adopted core-capability-first discipline — what PF-S63-02 says to do.
2. **Adoption Wave B (`eyn4`):** approaches + disclosure ledgers + `falsification-scan` advisory + the core-capability-first session gate (best done WITH `71s4` — the gate guards exactly that build).
3. **Adoption Wave C (`4hmo`) / Wave D (`ckl1`):** C = `plan-integrity` role + INVARIANTS rows for the new gates (change-discipline ritual); D = global `~/.claude/skills`+`commands` sync + wire `enforce-heartbeat-clause` (`0qf6`, couples with D) + fix `d1kc` (jsonschema) + `po4x` (harvest-gate/skill-trace attestation format).
