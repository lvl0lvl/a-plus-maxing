...
**FIRST: merge the S48 close PR #91** (`fix/s48-close` → `main`) — left OPEN at S48 close; docs-only, strict-descendant of `main`; run the docs 3-agent `/review-pr 91` subset then REST rebase `/merge` (GraphQL throttled). Then `main` carries this close. **Re-assign Fable as the session model at open** (S48 was auto-switched to Opus 4.8 by an incorrect biological-content flag). `main` at `6cb85a2` carries Slice 1 + both S48 close commits' predecessors. V1 build COMPLETE (18/18); Track-1 PII/safety COMPLETE; dashboard DATA LAYER built (Slice 1); the v1 VISUAL DESIGN is SIGNED OFF but UNBUILT (`vault/design/dashboard-v1-design.md`). Open normally (Session Start Protocol + `branch-completeness-audit.sh` at OPEN; baseline **404 passed / 2 skipped**). Commit-sequencing + `--no-verify` + separate-`git push`-tool-call rules per Top-3 #2. Each unit its own short-lived `fix/` branch → FULL `/review-pr` → `/merge` (REST; GraphQL throttled). **Editing `.claude/hooks/*` / `settings.json` needs EXPLICIT per-session authorization** (does not persist). NOTE (Walter, S48): re-assign **Fable** as the session model at open — S48 was auto-switched to Opus 4.8 by an (incorrect) biological-content flag.
