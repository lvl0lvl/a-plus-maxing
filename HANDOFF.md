...
> Confirmed by Walter. On the S60 forward question (the dashboard done-path is complete; the backlog is the correctness/governance tail), Walter redirected from the governance item `d3w` to "work on the correctness tail" → **"C sounds right"** on the `vjsw` adjudication. Verify-first (PF-S6-01) found three of the four flagged P2 tail candidates deferred-by-design (`10h` beyond V1's trust model; `1ww` KEEP-DEFERRED, no concurrent writer; `e3b` mechanism built, pending ADR-0006-T2), leaving `vjsw` (clone-init never inits bd) as the one unconditional, cleanly-buildable correctness item. For `vjsw`, verify-first against live bd 0.49.0 found the bead's `bd-init-step`/`no-db` fork both wrong: bd self-heals via auto-import on normal commands (only `bd sync --flush-only` fails first-command), clone commits are already safe (PR #107 hook flush-skip), bd is dev-tooling, and `init_instance` is a THIN LEAF (ADR-0005-T2). So a third disposition — **Option C: document the contract** — surfaced + operator-confirmed.

Goal: Merge the owed S59 close PR #125, then fix `vjsw` — the clone-init contract never inits bd — by DOCUMENTING that `.beads/` is db-less-by-design dev-tooling (`init_instance` deliberately leaves bd alone; a contributor runs `bd init --from-jsonl`), through a full gated `/review-pr` + `/merge` lifecycle.

Acceptance criteria:
- [ ] AC1: PR #125 (S59 close docs) reviewed (3-agent docs subset, full methodology) + REST rebase-merged; both `/review-pr` and `/merge` invoked fresh via the Skill tool; baseline green after (821/2).
- [ ] AC2: adjudicate bd-init-step vs no-db-decision against ACTUAL bd 0.49.0 behavior (verify-first, not the bead text); the chosen path implemented in `scripts/clone/init_instance.py`.
- [ ] AC3: `docs/clone-init.md` documents the contract; the "zero bd references" gap closed; the decision recorded (docstring + docs + a vault decision note).
- [ ] AC4: a clone-init verification test — RED without the fix, GREEN after; existing tests + the Python suite still green.
- [ ] AC5: full 6-agent `/review-pr` + `/merge` on the vjsw PR; every legitimate finding fixed + blind-verified, 0 suppressed; suite green after merge.
- [ ] AC6: full session close per protocol — all FIVE audits incl. `skill-trace-audit.sh --session 60`, 6-clause rotation, PF attestation with the per-PR table; DOCUMENT_RUBRIC + landmarks RE-OPENED at steps 8/8.7.

Files I WILL touch: `scripts/clone/init_instance.py` (docstring); `docs/clone-init.md`; `tests/clone/test_init_instance.py`; `vault/decisions/2026-06-14-clone-init-bd-contract.md` (new); `.beads/issues.jsonl` via `bd` (close `vjsw`); HANDOFF/vault at close.

Files I will NOT touch: `.claude/hooks/*` + `.claude/settings.json`; the Python product code (`vault/design/templates/*`, `scripts/store/*`, `report.py`); `PALETTE`/`SERIES`/`ACCENTS` + `SUMMARY_FIELD_SET`/`EXCLUDED_RAW_PII`; `main` directly; INVARIANTS.md rows; `.beads/config.yaml` (Option C does not flip no-db); the other tail beads.

NOT doing: the other correctness candidates (`02pe`/`pq7m`/`dqyv`/`1ww`/`10h`/`e3b`); the governance items (`d3w`/`ofn0`/`dt0t`/`imev`/`tdre`); LM-01 visit prep (window opens 2026-06-29); LM-02/LM-04; the library-population track.

Invariants at risk: none structurally — `vjsw` fixes a clone-bootstrap gap and touches no invariant surface; `init_instance` stays a THIN LEAF (ADR-0005-T2, 0 outgoing deps). INV-SKILL-TRACE binds PR #125 + the vjsw PR (per-PR table at close).

### S60 Scope Contract Evaluation (2026-06-14, volatile)

- **AC1 (PR #125 S59-close reviewed + REST rebase-merged; baseline green) — PASS.** Docs 3-agent subset, full methodology; the review caught a REAL residue — the S59 close-correction commit (`8d74f11`) missed `session-59.md`, which still claimed the close "HELD / DOCUMENT_RUBRIC RUN for real" while HANDOFF + the PF log recorded the soft recurrence (all 3 agents converged), plus a half-updated LM-01 date in 3 places; 3 LEGITIMATE fixed + blind-verified 3/3, 0 suppressed (the blind triage classified the 2 `session-59.md` edits NOT_A_BUG on a frozen-snapshot theory — orchestrator-overridden toward honesty, see Flagged); both skills fresh; rebase-merge on-main `43aae47`; baseline 821/2.
- **AC2 (adjudicate bd-init vs no-db) — PASS (CHANGED → Option C).** Verify-first against live bd 0.49.0 found both bead framings wrong (bd auto-heals via auto-import; only `bd sync --flush-only` fails first-command; clone commits already safe via PR #107; bd is dev-tooling; `init_instance` is a THIN LEAF). A third disposition — Option C, document the contract; `init_instance` leaves bd alone — surfaced + operator-confirmed ("C sounds right"). The AC's "implement the chosen path in init_instance" became a docstring-only non-responsibility note (no subprocess). Documented, not silent.
- **AC3 (docs + decision note) — PASS.** `docs/clone-init.md` gained the "project issue tracker (.beads/)" section (db-less-by-design; operator ignores it; contributor runs `bd init --from-jsonl`); the decision recorded in the `init_instance` docstring + `vault/decisions/2026-06-14-clone-init-bd-contract.md`. The "zero bd references" gap closed.
- **AC4 (clone-init verification test; RED→GREEN) — PASS (CHANGED → decision-pin).** Per Option C the test asserts the contract, not "bd operational": `test_init_does_not_initialize_bd` (run() creates no `beads.db` — mutation-proven RED by injecting a db-creating line) + `test_clone_readme_documents_bd_tracker_contract` (RED on pre-fix docs, which had 0 bd refs). Suite 821 → 823 (+2).
- **AC5 (full 6-agent `/review-pr` + `/merge` on #126) — PASS.** Security/Bug-Hunter/Code-Quality/Contracts/Hist-Context all 0 — and 5 of 6 independently VERIFIED the Option-C contract's factual claims against live bd 0.49.0 + the hook source (the `block-pii-commit` flush-skip at `block-pii-commit.sh:257-258` / commit `43b11a1`, `.beads/*.db` gitignored, ADR-0005-T2 preserved, no relitigation). 1 test-coverage finding (string-match brittleness) blind-triaged NOT_A_BUG (the exact-match is the intended pin); 0 LEGITIMATE → 0 fixes; 0 suppressed; rebase-merge on-main `2c49ea8`; suite 823/2.
- **AC6 (full session close per protocol) — PASS** (this close; five mandatory audits incl. `skill-trace-audit.sh --session 60`, 6-clause rotation, PF attestation with the #125/#126 per-PR table; `DOCUMENT_RUBRIC.md` + `landmarks.md` RE-OPENED + run at steps 8/8.7 from the files — the S57/S59 recurring miss did NOT recur).

Flagged additions (none silent): (1) AC2/AC4 CHANGED to Option C (a third disposition beyond the bead's fork) — flagged to Walter with evidence + the rejected options A/B BEFORE building; operator-confirmed. (2) On #125 I OVERRODE the blind triage's NOT_A_BUG on the 2 `session-59.md` edits (it applied a "frozen at-write snapshot" exemption the project doesn't hold — session-59.md was a pending file in the open PR, written this same session; the S57 precedent corrects open close PRs; all 3 review agents flagged it) — overrode toward honesty, not suppression; recorded as an observed item in the PF log.

### Drift checks (S60 close)

- **Task drift:** all 6 ACs PASS; two flagged CHANGES (AC2/AC4 → Option C, operator-confirmed; the #125 triage override toward honesty). The session redirected once at open (governance `d3w` → correctness tail `vjsw`, Walter-directed) — flagged, not silent. No silent drift.
- **Architecture drift:** none degraded; the clone contract STRENGTHENED. `init_instance` stays a THIN LEAF (ADR-0005-T2, 0 outgoing deps — docstring-only change; the existing egress test pins run() subprocess-free); `.beads/config.yaml` untouched (Option B's dev-repo-bd-mode flip rejected); `PALETTE`/`SERIES`/`ACCENTS` + the PII boundary byte-identical (vjsw is clone-bootstrap, not product code). The #126 review independently verified every documented contract claim against the live system. INV-SKILL-TRACE bound #125+#126; INV-TRUNK-COMPLETENESS green at open + close.
- **Vision drift:** none. After S60 the same local-first health tracking + planning system, V1 complete, dashboard done-path complete, now with the clone-init contract complete + honest (the clonable-single-operator V1 story: `.beads/` documented as db-less-by-design dev-tooling the operator ignores). Forward work continues on the correctness/governance tail. Matches `design/vision.md`.

### PF attestation

S60 close (2026-06-14): **No new PF-class entries this session.** **PF-S13-01/PF-S37-01 (the S57/S59 close-step-8/8.7 recurring miss) did NOT recur** — this close RE-OPENED `DOCUMENT_RUBRIC.md` (step 8) + `landmarks.md` (step 8.7) and ran their checklists from the files with cited evidence (the falsification window held; the structural fix is `d3w`, still open). **PF-S6-01 HELD and LOAD-BEARING throughout** — verify-first deferred 3 of 4 tail candidates by reading their live notes (`10h`/`1ww`/`e3b`), then found the `vjsw` bead's bd-init/no-db fork both wrong against live bd 0.49.0, driving Option C. **PF-S40-01 HELD ×2** (blind triage dispatched for #125 + #126, never self-triaged) and LOAD-BEARING: the #125 triage independently classified the findings (I overrode 1 verdict toward honesty, cited); the #126 triage killed the lone finding (NOT_A_BUG). **PF-S26-01 HELD** (every legitimate finding fixed + blind-verified, 0 suppressed: #125 3/3; #126 0 legitimate). **PF-S39-01 HELD** (`/review-pr` + `/merge` invoked fresh via the Skill tool per PR; merge-methodology read fresh once at the #125 merge). **PF-S25-01 HELD** (close on `fix/s60-close` after both merges). **PF-S49-01 N/A** (no visual target). Full attestation with the per-PR invocation table (#125, #126, all YES) + non-promoted observations (incl. the #125 blind-triage override, and that the S59 remediation was incomplete — caught + fixed by the #125 review, the layered mechanism working) in `memory/process-failures.md` (Session 60 section).

## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-59 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; S33 archived S32; …; S57 archived S56; S58 archived S57; S59 archived S58; S60 archived S59) to keep this handoff lean. The current (S60) scope contract is above; the archive holds the prior-session archaeology.
...
1. **Gated-skill invocation discipline is MECHANICAL (INV-SKILL-TRACE):** invoke `/review-pr` AND `/merge` via the Skill tool fresh for EVERY PR; the PF-log `## Session <N>` section must carry the per-PR invocation table (every NO cell needs the word "violation"); `scripts/skill-trace-audit.sh --session <N>` is the fifth mandatory close audit.
2. **The dashboard "done" path is COMPLETE; forward work is the correctness/governance tail.** `vjsw` (clone-init bd contract) FIXED at S60. **Verify-first BEFORE picking** — the flagged P2s `10h` (beyond V1's trust model), `1ww` (KEEP-DEFERRED, no concurrent writer), `e3b` (mechanism built, pending ADR-0006-T2) are deferred-by-design (read their live `bd show` notes, don't re-pick blindly). Cleanly-buildable: `d3w` (mechanize close-step-8 DocRubric — the highest-leverage rigor compounder, catches the PF-S13-01/S37-01 class); `02pe` (plan-track revert correction path — conditioned on "friction real"); `dqyv` (3-consumer promotion); `ofn0` (archive-SHA-check). Plus P3s (`pq7m`/`a94f`/`dt0t`/`rn3v`/`imev`/`tdre`/`20d`/`vvs`). Each a full review/merge lifecycle, verify-first against live code (the S55 stale-bead lesson). `SUMMARY_FIELD_SET`/`EXCLUDED_RAW_PII` locked (Security MEDIUM-2); `PALETTE`/`SERIES`/`ACCENTS` locked (ADR-0009). Standing mechanics: REST `gh api` full-40-char SHA for PR ops (GraphQL throttles ~hourly + low core quota — budget the calls, never poll-loop); stage `git add` separately from `git commit`; `.claude/hooks/*` edits need per-session authorization.
3. **Three active process watches.** (a) **PF-S13-01/S37-01 close-step-8/8.7 (DocRubric + landmark run from memory):** operator-caught at S57 AND S59, did NOT recur at S60 (this close RE-OPENED both docs + ran the checks from the files); the structural fix is `d3w` (mechanize close step 8) — STILL OPEN, the strongest rigor-compounder candidate. (b) **Off-main-archive-SHA (count=2):** the repo rebase-merges, so the close-time `scope-contract-archive.md` entry must cite ON-MAIN SHAs (`git merge-base --is-ancestor`-verified), NOT the pre-rebase `fix/*` tips; HELD at S60 (the S59 contract was archived at its on-main S59-close commit, `git merge-base --is-ancestor`-verified); `ofn0` mechanizes it — build at N=3. (c) **Concurrent-mutation-in-shared-tree (PF-S51-01) — mitigation in force:** Phase-1 `/review-pr` agents are dispatched READ-ONLY; verify `git status` pristine after every review (the #126 panel was all read-only).

## Current State (volatile)

- **S60 (2026-06-14) ran two full PR lifecycles, both merged to `main` (at `2c49ea8` as of 2026-06-14 S60 close); suite 823 passed / 2 skipped (+2 from the vjsw decision-pin + doc-contract tests).** Merged: #125 (S59 close docs — 3-agent subset; 3 LEGITIMATE fixed + blind-verified, 0 suppressed), #126 (the `vjsw` clone-init bd contract — full 6-agent review; 0 legitimate). Both PRs invoked `/review-pr` + `/merge` fresh via the Skill tool (REST rebase-merge under GraphQL throttle).
- **`vjsw` FIXED + closed — the clone-init bd contract.** A fresh clone shipped `.beads/` without a database (`*.db` gitignored) and the clone contract had zero bd references. Verify-first (live bd 0.49.0) found the bead's bd-init/no-db fork both wrong (bd auto-heals on normal commands; only `bd sync --flush-only` fails first-command; clone commits already safe via PR #107; bd is dev-tooling; `init_instance` is a THIN LEAF) → **Option C** (operator-confirmed "C sounds right"): DOCUMENT the contract; `init_instance` deliberately leaves bd alone (docstring-only, no subprocess). `docs/clone-init.md` + the `init_instance` docstring + `vault/decisions/2026-06-14-clone-init-bd-contract.md` now carry it; 2 non-tautological tests (mutation-proven RED).
- **The #125 review was load-bearing (the layered mechanism caught an incomplete remediation):** the S59 close-correction commit had missed `session-59.md`, leaving it claiming the close "HELD" while HANDOFF + the PF log recorded the soft recurrence; 3 agents converged, 3 LEGITIMATE fixed + blind-verified 3/3 (the blind triage's "frozen-snapshot" NOT_A_BUG on 2 of them was orchestrator-overridden toward honesty — see the PF log). The #126 review's 5 agents independently VERIFIED the Option-C contract's claims against live bd + the hook source (0 legitimate; the lone finding blind-triaged NOT_A_BUG).
- **Beads:** closed `vjsw` (merge provenance #126). Still OPEN and next-up: the correctness/governance tail — `d3w` (mechanize close-step-8 DocRubric, the strongest rigor-compounder), `02pe` (plan-track revert correction path), `dqyv` (3-consumer promotion), `ofn0` (archive-SHA-check), plus `pq7m`/`a94f`/`dt0t`/`rn3v`/`imev`/`tdre`/`20d`/`vvs` (P3). Verified deferred-by-design (re-read their notes before re-picking): `10h`/`1ww`/`e3b`.
- **Active landmarks:** LM-01 (First MD visit) **2026-07-13**. No trigger window open yet — the **14-day scoped-drift-audit window opens 2026-06-29**, the 7-day MD-handoff-artifact window 2026-07-06. The first session on/after June 29 must run the LM-01 scoped drift audit (relevant_scopes in `landmarks.md`).

**Historical (kept for reference):** `vault/sessions/session-60.md`.

## What Is Next (volatile)

### Resume checklist — next session (S61) open

**FIRST: merge the S60 close PR** (`fix/s60-close` → `main`; docs 3-agent `/review-pr` subset → REST rebase `/merge`, BOTH invoked fresh via the Skill tool; `skill-trace-audit.sh --session 61` runs at close per INV-SKILL-TRACE). Open normally (Session Start Protocol; `branch-completeness-audit.sh` at OPEN; baseline on `main` **823 passed / 2 skipped**).

**S61 plan (continue the correctness/governance tail — operator to prioritize at open, NOT pre-adopted):**

1. **The correctness/governance tail.** Strong candidates: `d3w` (mechanize close-step-8 DocRubric as `scripts/session-close-audit.sh` — would mechanically end the PF-S13-01/S37-01 close-step-8/8.7 class operator-caught at S57+S59, the highest-leverage rigor compounder; the bead needs user approval before building); `02pe` (plan-track revert A→B→A correction path — but verify-first whether the revert friction is real, may be a defer like `1ww`); `dqyv` (promote `loop_schema._reading`/`_content_tag` public — 3-consumer trigger fired at S57); `ofn0` (mechanize the on-main-SHA check for `scope-contract-archive.md` — off-main-SHA class count=2; build at N=3). Plus P3s (`pq7m`/`a94f`/`dt0t`/`rn3v`/`imev`/`tdre`/`20d`/`vvs`). Each a full review/merge lifecycle; verify-first against the live code — **`10h`/`1ww`/`e3b` are deferred-by-design (re-read their `bd show` notes before re-picking, S60 confirmed)**.
2. **Operator-gated.** The **July-visit prep track (LM-01 / the `c6k` epic)** is date-fixed (visit 2026-07-13; the 14-day scoped-drift-audit window opens 2026-06-29 — the first session on/after that runs the LM-01 audit); LM-02 (Oura wearable — the last dashboard surface) + LM-04 gated on Walter's data; the library-population `/aplus-research` track operator-deferred.

## Landmark window check (close step 8.7)

S60 close (2026-06-14): RE-OPENED `vault/meta/landmarks.md` from the file at close step 8.7 (the S57/S59 from-memory miss did NOT recur). LM-01 (First MD visit) date **2026-07-13**. No `active` landmark's trigger window is open as of today 2026-06-14 — LM-01's 14-day scoped-drift-audit window opens **2026-06-29** (the 7-day MD-handoff-artifact window 2026-07-06); LM-02/LM-03 date-TBD pending Walter; LM-04 not triggered (S60 was a clone-contract docs/test fix, no operator-facing artifact archived to `vault/artifacts/`). No landmark actions due THIS session; the first session on/after 2026-06-29 runs the LM-01 scoped drift audit.
