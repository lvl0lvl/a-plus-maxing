...
- **AC2 (real clearance-deferred author) — PASS.** `personal-trainer` dispatched for real (full profile inlined); returned a **6-rec** clearance-deferred session — `assemble`'s fail-closed HALT filter struck 1 (the "without any overhead pressing" claim-phrasing case → the CLAIM-PHRASING RULE), leaving **5 exercises rendered**, 0 load prescriptions; the real output flowed through the production path.
