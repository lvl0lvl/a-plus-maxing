...
Goal: Resolve the highest-priority open beads — the PII/safety boundary first (before LM-04 can flow real operator data), then the V1-correctness build residuals — via reviewed-and-merged `fix/` PRs.

Acceptance criteria:
- [ ] AC1 (verify-first, no blind action — PF-S6-01): every bead touched this session is either FIXED (with a failing-capable test where it is code) or CLOSED-as-already-resolved with cited evidence; no bead acted on from its HANDOFF description alone.
- [ ] AC2 (priority order honored): **Track 1 — PII/safety** (`8j6` P1 spike pass-through PII hardening · `3lv` register `block-pii-commit.sh` + backstop · `rnm` pin scaffold-value path convention ADR-0005 · `2x1` case-insensitive `pii_scan` · `fga` recursive payload allowlist · `dv3` pre-push/CI `pii_scan` backstop · `10h` HALT-filter category-trust · `cvr` shared git-commit matcher) is fully cleared before any Track-2 work begins.
- [ ] AC3 (full lifecycle per PR): each PR runs the FULL `/review-pr` methodology — 6 agents + dispatched profile-less Phase-3 blind triage + Phase-7 blind verification (PF-S40-01) → `/merge`; every LEGITIMATE finding fixed or beaded, 0 suppressed by severity (PF-S26-01); skills invoked fresh via the Skill tool (PF-S39-01).
- [ ] AC4 (no regression): full `.venv/bin/python -m pytest` stays green (272→growing); `branch-completeness-audit.sh` = 0 at each merge; all hook + shell baseline suites green.
- [ ] AC5 (consumed read-only contracts preserved): a bead modifies only its actual target; the frozen V1 interfaces it does NOT own (`keying.py`, `loop_schema.py` published contract, `render.emit`, `component_set`) stay unmodified — if a bead appears to require changing a consumed contract (e.g. `s38` vs `loop_schema`), HALT + escalate rather than freelance it.
- [ ] AC6 (close after lifecycle): S43 close sequenced AFTER the work PRs merge, on a `fix/s43-close` branch (PF-S25-01); the 4 close audits @ `--session 43` green.

Files I WILL touch: (per the in-scope beads only) `scripts/plan/router.py` + the ADR-0006 router spike (`8j6`/`fga`/`e3b`/`7lt`); `scripts/plan/assemble.py` (`10h`/`7lt`); `scripts/store/store.py` (`4yk`/`1ww`/`u8u`/`1vi`/`s38`); `scripts/generate/render_views.py` (`byj`/`juc`/`r5l`/`bhc`) + `scripts/generate/render.py` (`5q5`); `scripts/guard/pii_scan.py` (`2x1`); `.claude/settings.json` + `.claude/hooks/*.sh` (`3lv`/`cvr`/`dv3`); the ADR-0005 doc (`rnm`); the matching `tests/**`; HANDOFF.md; `vault/sessions/session-43.md` (NEW); `vault/meta/overview.md`; `vault/meta/log.md`; `vault/sessions/scope-contract-archive.md` (archive S42 at close); `.beads/issues.jsonl` via `bd`. Short-lived `fix/s43-*` branches off `main`, one per coherent PR.

Files I will NOT touch: `scripts/store/keying.py` (never redefine); `scripts/store/loop_schema.py` published contract (escalate if a bead seems to need it); `vault/design/templates/component_set.py` + the `render.emit` engine signature; the deployed roster `.claude/agents/*` (Track 5, out of scope); INVARIANTS.md register rows + the audit scripts (Track 3 process-tooling, deferred); `main` directly; real operator-PII values.

NOT doing: Track 3 (process-tooling — `pka`/`vvs`/`d3w`/`mxo`/`rc1`), Track 4 (spec/ADR/recipe doc-consistency — `dke`/`njy`/`eeu`/`e3d`/`4xe`/`bpu`/`434`/`12p`/`kz6`/`0oy`/`ivt`/`z2u`), Track 5 (specialist-roster/wiki — `xg4`/`3v5`/`78p`/`5l9`/`5bd`/`w3n`/`382`/`h1z`/`pmp`), library population; **`20d` (defensive — no explicit approval; stays open)**; redefining any consumed read-only V1 contract (escalate); the AskUserQuestion widget; LM-04 first-artifact generation (no operator data); a `v1-build` wave / `/execute-plan` (build plan fully drained).

Invariants at risk: INV-BRANCH-NOT-MAIN, INV-SCOPE-CONTRACT, INV-PF-ATTESTATION, INV-HO-ROTATION, INV-HO-NO-STALE-HASH, INV-TRUNK-COMPLETENESS, INV-ROLE-INLINING. Governing: the PII/safety beads must strengthen (never weaken) the ADR-0005 PII-free-trunk boundary + the 0-egress / 0-model-bound guarantees; no defensive programming without stated motivation + approval.

## Scope Contract — Session 42 (2026-06-07)

> Confirmed by Walter ("rate limits have been increased. please continue") — go for the next unit (Wave 7), run the same lifecycle as the Walter-authorized Wave 6. **Unit = Phase C, build-plan Wave 7 via `/execute-plan` WAVE mode — the LAST leaf → V1 18/18.** Wave 7 is a single task: `1ih`/**ADR-0007-T2** — biomarker-matrix + projection render-time views (`scripts/generate/render_views.py`), the terminal sink of the V1 DAG. CONSUMES (read-only) the `loop_schema` store-state contract (W6), the `render.emit` engine + `component_set.py` (ADR-0004-T1), the `ADR-0004-T0` cap, `store.read` (ADR-0002-T1), `egress_guard.run` (ADR-0001-T1); publishes nothing. Built via `/execute-plan`, reading SKILL.md + references IN FULL fresh via the Skill tool (PF-S17-01/PF-S39-01); never a hand-rolled per-task dispatch (PF-S36-01). The wave completes when build + the full W7→Done checkpoint passes + the wave PR merges via `/review-pr` (6-agent, FULL methodology incl. Phase-3 blind triage + Phase-7 blind verify as dispatched profile-less agents — PF-S40-01) → `/merge`.

**`v1-build` wave attestation:** Build-plan **Wave 7** (`docs/build-plan/build-plan-v1-full.md`). Prior-wave **W6→W7 Go/No-Go attested PASS (run this session, not recited):** `tests/store/test_loop_schema.py` = 20 passed + NG-6 0-automation floor over `loop_schema.py` = 0; full baseline 247 passed / 2 skipped. `1ih` entry-state cross-spec prereqs verified importable at orientation (`loop_schema` 5-marker state set; `render.emit`; `store.read`; `egress_guard.run`; `component_set`; the ADR-0004-T0 cap spike present; neither target file exists yet) — re-verified by the SE agent at its entry-state phase.

Goal: Build `1ih`/ADR-0007-T2 via `/execute-plan`, gate on the W7→Done checkpoint, run the three-tier review, and merge the wave PR — completing the V1 build (18/18).

Acceptance criteria:
- [ ] AC1 (`1ih` built, 7 crit) — `pytest tests/generate/test_render_views.py` green on: crit-1 matrix recompute (≥2 timepoints side-by-side, recomputed from `store.read`, latest-only FAILS); crit-2 projection ≥3-timepoint guardrail (present ≥3, trend-only at 2, projection-at-<3 FAILS); crit-3 five-element projection label set (the "naive projection from recent trend — not a clinical forecast" label + method + datapoint count + widening band + milestones); crit-4 store-state 1:1 map (pending→"pending" never a result; not-yet-answered→"not yet answered"; no-prior→no fabricated trend; read via the `loop_schema` published state set; pending-as-result / fabricated-single-point-trend FAILS) — the cross-boundary integration check; crit-5 local-only egress (`egress_guard.run` truthy over a REAL recompute + `rg` external-asset=0 + 0 model-bound dispatch + SEC-03 fail-injection flips guard FALSY); crit-6 size-budget pagination (worst-case view, each artifact `wc -c` < 500000 via the ADR-0004-T0 cap); crit-7 the suite passes. QA verifies.
- [ ] AC2 (consume-only, no redefinition — terminal sink publishes nothing) — `render_views` reads via `store.read` + the `loop_schema` published state set + `render.emit` + `component_set` shared markup + the ADR-0004-T0 cap; no second store key, no re-parsed NDJSON, no per-series component copies, no new egress surface; `render.py`/`component_set.py`/`loop_schema.py`/`store.py`/`keying.py`/`egress_guard.py` consumed read-only (unmodified).
- [ ] AC3 (W7 consumption-review decisions resolved) — the views map the `loop_schema` published states 1:1 including the `no-data` 0-timepoint marker (added S41) rendered distinctly; the ≥2-timepoint biomarker renders side-by-side in the matrix (crit-1). The beaded ≥2-timepoint `None`-sentinel question (`a-plus-maxing-bhc`) is DECIDED here: render the ≥2-timepoint state via the store sequence without amending `loop_schema` (read-only) — if a named marker is judged necessary, that is a `loop_schema` contract amendment → HALT + escalate (Architect review), never freelance the read-only module.
- [ ] AC4 (W7→Done checkpoint AS A GATE) — `pytest tests/generate/test_render_views.py` + the SEC-03 failing-capable egress injection into a `render_views` recompute (guard returns FAIL) + the cross-spec 2-decision contract (crit-4 1:1 map) + the ADR-0004-T0 cap (crit-6 each artifact <500000 bytes). No-go blocks "done".
- [ ] AC5 (no regression) — full `.venv/bin/python -m pytest` stays green (247→growing); all hook + shell baseline suites stay green.
- [ ] AC6 (three-tier review + lifecycle) — Tier-1 SE self-check → Tier-2 wave review (QA always; Architect for the consumed-contract surfaces; Security for the PII/egress boundary per the recurrence-watch) → Tier-3 `/review-pr` 6-agent with Phase-3 blind triage + Phase-7 blind verification each a dispatched profile-less agent (PF-S40-01) → fix every legitimate finding, priority-only never suppress (PF-S26-01) → `/merge`. `/execute-plan`, `/review-pr`, `/merge` each read FRESH via the Skill tool (PF-S39-01). S42 close sequenced AFTER the merge (PF-S25-01).

Files I WILL touch: `scripts/generate/render_views.py` (CREATE); `tests/generate/test_render_views.py` (CREATE); `HANDOFF.md` (contract + close rotation); `vault/sessions/session-42.md` (NEW); `vault/meta/overview.md` (wave-state); `vault/meta/log.md` (append); `vault/sessions/scope-contract-archive.md` (archive S41); `.beads/issues.jsonl` via `bd`; `docs/task-plan/.execution/` (gitignored). One `feature/render-matrix-projection-views` branch off `main`, one wave PR.

Files I will NOT touch: `scripts/generate/render.py` + `generate.py`; `vault/design/templates/component_set.py`; `scripts/store/loop_schema.py` + `store.py` + `keying.py`; `scripts/guard/egress_guard.py`; `scripts/plan/*`; `scripts/ingest/*`; the deployed roster `.claude/agents/*`; INVARIANTS.md + the audit scripts; the ADRs/specs/recipes as build inputs (read-only); `.claude/settings.json`; `main` directly; real operator-PII values.

NOT doing: any post-build-wave residual-bead work (`s38`/`r5l`/`pka`/`8j6`/`10h`/`20d`/`7lt`/`e3b`/`juc`…); the PII-guard-activation group (`3lv`/`2x1`/`rnm`/`fga`/`fmb`/`v13`); editing any consumed read-only interface (if the ≥2-timepoint marker needs `loop_schema` changed → HALT + escalate, not silent expansion); hand-rolling a per-task dispatch instead of `/execute-plan` WAVE mode (PF-S36-01); improvising `/review-pr`'s back-half phases (PF-S40-01); running `/review-pr`/`/merge` from cached context (PF-S39-01); the AskUserQuestion widget; defensive programming without stated motivation + approval; the LM-04 first-artifact generation (no real operator data).

Invariants at risk: INV-SCOPE-CONTRACT, INV-PF-ATTESTATION, INV-HO-ROTATION, INV-HO-NO-STALE-HASH, INV-BRANCH-NOT-MAIN, INV-TRUNK-COMPLETENESS, INV-ROLE-INLINING. Governing: `1ih` must preserve 0-egress / 0-model-bound (no labs/answers to model), the no-fabrication render (pending never a result; no-prior / single-point never a fabricated trend; projection only at ≥3 timepoints), the <500KB per-artifact cap, and inline-only (refuse-external-asset); it consumes the frozen upstream interfaces read-only.

### S42 Scope Contract Evaluation (2026-06-07, volatile)

- **AC1 (`1ih`/ADR-0007-T2 built, 7 crit) — PASS.** `scripts/generate/render_views.py` built via `/execute-plan` WAVE mode (3 TDD cycles): matrix recompute (≥2-tp side-by-side from `store.read`); projection ≥3-timepoint guardrail + 5-element "naive projection" label; store-state 1:1 map (the 5 published `loop_schema` markers + the ≥2-tp case via the store sequence); local-only egress + SEC-03 failing-capable injection; size-budget pagination. QA verified. `pytest tests/generate/test_render_views.py` 17→25 after the answered-over-time + Tier-3 fixes.
- **AC2 (consume-only, terminal sink publishes nothing) — PASS.** Renders THROUGH `render.emit` + the single `component_set` shared def; reads via `store.read` + the `loop_schema` published markers; no second key, no NDJSON re-parse, no per-series markup copies, no new egress surface; `render.py`/`component_set.py`/`loop_schema.py`/`store.py`/`keying.py`/`egress_guard.py` consumed read-only (unmodified).
- **AC3 (W7 consumption-review decisions) — PASS.** The ≥2-timepoint `None`-sentinel question (bead `bhc`) resolved by rendering via the store sequence with NO `loop_schema` change (Architect confirmed contract-sound); the `no-data` 0-timepoint marker mapped distinctly via the fail-closed `_STATE_DISPLAY` table.
- **AC4 (W7→Done checkpoint AS A GATE) — PASS.** Re-run GREEN post-Tier-3-fix: wave suite 25 passed; SEC-03 failing-capable egress injection flips the guard FALSY (4-way control); crit-4 1:1 map; crit-6 each artifact `wc -c` < 500000 (now also a failing-capable measured ceiling 24000). Full suite 272/2.
- **AC5 (no regression) — PASS.** Full `.venv/bin/python -m pytest` 247→**272 passed / 2 skipped** (+25 W7 build+review tests); the 2 skips are the pre-existing Linux-only egress-namespace tests; hook/shell baseline suites green.
- **AC6 (three-tier review + lifecycle) — PASS.** Tier-1 SE self-check → Tier-2 wave review (QA + Architect + Security; QA initial FAIL on the untested `answered-over-time` render branch → FIXED; Architect `5q5` bead for the `render.py` parallel-path; Security 0) → Tier-3 `/review-pr` #73 (6-agent; FAIL on a **dishonest mid-series projection** mislabeled "recent trend" (F2) + a **tautological size-cap assertion** (F4) + a degenerate-window bug + missing projection-value/escaping safety tests; blind triage **12 LEGITIMATE / 1 OUT_OF_SCOPE / 2 NOT_A_BUG**; 12 fixed + blind-verified **12/12 RESOLVED**) → rebase-merged via REST (GraphQL throttled). `/execute-plan` + `/review-pr` + `/merge` ALL invoked fresh via the Skill tool with references read fresh (PF-S39-01 HELD); the FULL `/review-pr` methodology incl. dispatched profile-less blind triage + blind verify ran (PF-S40-01 HELD). Close on `fix/s42-close` AFTER the merge (PF-S25-01).

### Drift checks (S42 close)

- **Task drift:** the contracted unit (build Wave 7 via `/execute-plan`, three-tier review, merge) delivered exactly. Build **17→18 leaves — the V1 build is COMPLETE.** Tier-3's remediation (one-projection-per-biomarker from the true recent trend + window rebalancing + 7 test-integrity additions) was larger than the recipe's literal GREEN steps but in-scope — it fixed shipped code that violated the task's OWN honest-projection / no-fabrication intent and closed safety-test gaps on the terminal PII sink, confirmed in-scope by the independent blind triage. Out-of-scope findings beaded (`byj` resulted-panel render handling, `5q5` `render.py` reconciliation), not built. No out-of-manifest edits.
- **Architecture drift:** toward LESS violation. The terminal render sink now renders at most ONE honest projection (from the true last-two-points trend, never a mid-series segment mislabeled "recent trend"), no degenerate rows, with a failing-capable per-page byte guard and an operator-text escaping/injection regression test on the terminal PII sink. All consumed contracts honored read-only; the V1 DAG terminal sink is built with 0 dangling references. INV-BRANCH-NOT-MAIN held (work on `feature/render-matrix-projection-views`, merged server-side; close on `fix/s42-close`); INV-TRUNK-COMPLETENESS verified at open + close; INV-ROLE-INLINING held (SE builder + 3 wave reviewers full-profile-inlined; 6 Tier-3 reviewers by registered profile-carrying type; blind triage + blind verify profile-less). INV-SCOPE-CONTRACT / INV-HO-ROTATION / INV-PF-ATTESTATION satisfied.
- **Vision drift:** none. What the system IS after S42: "a local-first health tracking + planning system whose V1 build is **COMPLETE (18/18)** — ingestion, NDJSON store + keying, egress/PII guards, the render engine, the no-train router, multi-domain plan assembly, the lab-loop store schemas, and the biomarker-matrix + projection render views — awaiting real operator data (LM-04) to generate its first artifact." Matches `design/vision.md`.

### PF attestation

S42 close (2026-06-07): **No new PF-class entries this session.** The full build→review→merge lifecycle ran cleanly with no user-caught process failure. Falsification windows HELD: **PF-S40-01** — S42's Tier-3 `/review-pr` ran its WHOLE phase list, dispatching a profile-less blind triage agent (no orchestrator self-triage; it scoped the resulted-panel finding OUT_OF_SCOPE (beaded `byj`) + 2 NOT_A_BUG (already-handled / beaded), sparing wrong/out-of-scope fixes) AND a profile-less blind verification agent (12/12 RESOLVED, independently driven). **PF-S39-01** — `/execute-plan`, `/review-pr`, `/merge` each invoked via the Skill tool with SKILL.md + references read fresh this session. **PF-S36-01** (`/execute-plan` wave mode, Wave 7 in order, W7→Done gated). **PF-S26-01** (every legitimate finding fixed or beaded; 0 suppressed). **PF-S25-01** (close on `fix/s42-close` after PR #73 merged). **PF-S13-01** + **PF-S37-01** held. **Observed, NOT promoted:** the **SIXTH consecutive wave** the layered Tier-3 review caught real defects the builder's green tests + all of Tier-2 missed — here a **dishonest projection** (a "naive projection from recent trend" computed from a mid-series segment — safety-adjacent on a health-render sink), a degenerate render row, a **tautological size-cap assertion** (25× headroom — could never red), and missing projection-value + operator-text-escaping safety tests. The process WORKED (all caught before `main`); not an orchestrator PF. Bead `pka` already tracks shifting the catch upstream — this sixth instance (now spanning every build wave S37-S42) strengthens the case for promoting `pka`'s structural fix: a recipe-template / Tier-2-checklist mandate for adversarial tests on the recurring surfaces (cross-stream/dedupe collisions, projection/no-fabrication honesty, byte-budget failing-capability, operator-text escaping). Recurrence-watch continues for one that ESCAPES all tiers — none has across the whole V1 build.

## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-41 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; S33 archived S32; S34 archived S33; S35 archived S34; S36 archived S35; S37 archived S36; S38 archived S37; S39 archived S38; S40 archived S39; S41 archived S40; S42 archived S41) to keep this handoff lean. The current (S42) scope contract is above; the archive holds the prior-session archaeology.
...
1. **V1 build COMPLETE (18/18) — the `/execute-plan` wave loop is DONE; there is NO Wave 8.** All 7 build-plan waves are built + merged. The next forward work is NOT a wave: it is (a) the **LM-04 first artifact** when Walter provides real operator data, (b) the **post-build-waves residual-bead group** (each its own short-lived `feature/`/`fix/` branch → `/review-pr` → `/merge`, NOT an `/execute-plan` wave), and (c) the **library-population `/aplus-research` track**. Do NOT reach for `/execute-plan` / a `v1-build` wave contract — the build plan is fully drained.
2. **read-before-invoke AND run-the-FULL-methodology (PF-S13-01 + PF-S17-01 + PF-S39-01 + PF-S40-01) — STANDING.** Still governs every future `/review-pr` (residual-bead fixes, library batches) and `/merge`: (a) read each gated skill fresh / invoke via the Skill tool (not from cached context); (b) run ALL of its phases — for `/review-pr`, Phase 3 ("**DO NOT triage findings yourself**" → dispatch the blind triage agent) + Phase 7 (blind verification) are non-optional profile-less independence layers, NOT to be improvised. **S42 ran the full methodology — PF-S40-01 HELD again** (blind triage scoped the resulted-panel finding OUT_OF_SCOPE [`byj`] + 2 not-a-bug). No `Workflow`/hand-roll substitution. `.venv/bin/python -m pytest`, never from memory.
3. **Three-tier review is load-bearing — across ALL SIX build waves (S37-S42) Tier-3 caught real safety/PII/honesty defects the builder's green tests + Tier-2 both missed (PF-S26-01/PF-S3-01).** S42: a **dishonest mid-series projection** (a "naive projection from recent trend" computed from a mid-series segment — safety-adjacent on a health render) + a **tautological size-cap assertion** (25× headroom, could never red) + missing projection-value/escaping safety tests. The process WORKED every time (caught before `main`; none ever ESCAPED all tiers). Bead `pka` tracks promoting a STRUCTURAL upstream fix (a recipe-template / Tier-2-checklist mandate for adversarial tests on the recurring surfaces); given the now-complete 6/6 pattern, prioritize `pka` before the library-population track (which mass-authors content). The full Tier-2 + Tier-3 + dispatched blind triage + blind verify stays non-optional for all post-build PRs.

**Demoted from prior Top-3:** the "Wave 7 run" framing — Wave 7 built + merged at S42 (PR #73); the V1 build is complete; there is no next wave.

## Current State (volatile)

- **S42 (2026-06-07) built Wave 7 via `/execute-plan` WAVE mode (the single open task) → V1 BUILD COMPLETE (18/18).** `1ih`/ADR-0007-T2 — biomarker-matrix + projection render-time views (`scripts/generate/render_views.py`), the terminal sink of the V1 DAG: recomputes the matrix + naive projections from `store.read` timepoints, maps the `loop_schema` 5-state contract 1:1, paginates under the ADR-0004-T0 cap. PF-S36-01 + PF-S39-01 + PF-S40-01 falsification windows HELD (skills invoked fresh via the Skill tool; wave in order; W7→Done checkpoint gated; the FULL `/review-pr` methodology incl. dispatched blind triage + blind verify ran).
- **The three-tier review caught + fixed defects the builder's tests + all of Tier-2 missed** (0 suppressed, PF-S26-01): Tier-2 (QA+Architect+Security) PASS after the untested `answered-over-time` branch was covered (`5q5` `render.py`-overlap bead); Tier-3 `/review-pr` #73 (6-agent) caught a **dishonest mid-series projection** mislabeled "naive projection from recent trend" (F2, safety-adjacent) + a **tautological size-cap assertion** (F4, 25× headroom) + a degenerate render row + missing projection-value/escaping safety tests → blind triage **12 LEGITIMATE / 1 OUT_OF_SCOPE (`byj`) / 2 NOT_A_BUG**; 12 fixed + blind-verified **12/12 RESOLVED**. PR #73 rebase-merged via REST (GraphQL throttled).
- **Build: 18/18 leaves — V1 COMPLETE.** All 7 waves built + merged (W1-W7). No artifact generates until `generate.run` is fed real operator data (LM-04, Walter-pending). Live phase-state in `vault/meta/overview.md`.
- **SINGLE TRUNK** unchanged (since S22). Suite **272 passed / 2 skipped** (+25 over the S41 247 baseline: W7 build + review tests; the 2 skips are pre-existing Linux-only egress-namespace tests); branch-completeness 0 violations (20 agents); all hook suites green. `main` at the Wave-7 merge as of 2026-06-07 S42 close + this S42 close PR.
- **Active landmarks:** no trigger windows opened. LM-04 still pending (the `generate.run` producer is live but unfed).

**Historical (kept for reference):** `vault/sessions/session-42.md` + `vault/meta/log.md` S42 entry.

## What Is Next (volatile)

### Resume checklist — next session (S43) open

**The V1 build is COMPLETE (18/18). `main` is at the Wave-7 merge + this close PR, tree clean, 0 open PRs. There is NO Wave 8 — the `/execute-plan` wave loop is finished.** The next session opens normally (Session Start Protocol steps 1-6 + `branch-completeness-audit.sh` at OPEN; baseline 272 passed / 2 skipped) and picks ONE forward track below. Do NOT write a `v1-build` wave contract or invoke `/execute-plan` — the build plan is fully drained; further build is residual-bead work on short-lived `feature/`/`fix/` branches → `/review-pr` (FULL methodology) → `/merge`.

1. **LM-04 — the first HTML artifact (highest value; Walter-pending on data).** When Walter provides real operator data exports, feed `generate.run` → the first artifact; validate against `vault/design/artifact-design-protocol.md`; archive in `vault/artifacts/<slug>/`. Producer is live; only the data is missing.
2. **Post-build-waves residual-bead group** (each its own branch → `/review-pr` → `/merge`): **`pka` P2 (prioritize)** — the structural upstream fix for the 6/6-wave Tier-3-catch pattern (recipe-template / Tier-2-checklist mandate for adversarial tests on the recurring surfaces), do BEFORE the library-population track; the **panel-result-writer pair** `s38` (store side) + `byj` (render side) — land TOGETHER per the Factory-to-Component Wiring rule; `5q5` `render.py` `emit_matrix_projection` reconciliation; `8j6` **P1** in-summary PII value-gate (before first real data); `10h`/`20d` (DEFENSIVE — needs approval)/`7lt`/`e3b`/`juc`/`r5l`; the **PII-guard-activation group** `3lv`/`2x1`/`rnm`/`fga`/`fmb`/`v13` + cross-hook matcher (before LM-04 / at V1-distribution).
3. **Library-population** via research-only `/aplus-research` sessions (parallel track; PF-S22-01 window: ONE merge target + `branch-completeness-audit.sh` + `wiki-lint.sh` at each batch close). V1 is a thin-library MVP and does not block on it.

**V1 published surface (all built):** `generate.run(artifact_name) -> Path` · `render.emit(template, store_read) -> Path` (no production PLAN-template producer yet — bead `4xe`) · `render_views(...)` (matrix + projection views) · the `Adapter` Protocol + 4 adapters · `scheduler.run()` · `ingest.run(adapter, export_file, root=…)` · `store.append`/`read` (cross-item `read_all`/`items` pending `4yk`) · `keying.py` (never redefine) · `egress_guard.run` · `pii_scan.scan(...)` · `router.summarize`/`router.dispatch` · `assemble(goal_set, summary, roster) -> plan` · `loop_schema` (5 published store-state markers, read 1:1 by `render_views`).
...
### Open beads carried
- **V1 build execution (label `v1-build`): ALL 18 CLOSED** (W1 `394`/`bez`/`qbb`; W2 `89a`/`e9m`; W3 `6be`/`gu4`/`xlu`/`br1`; W4 `n9h`/`3gp`/`yo6`/`ml1`/`ftm`; W5 `oaf`/`8cv`; W6 `1aa`; W7 `1ih`). Plus `e3d` (recipe doc-consistency, non-blocking) OPEN. **The build plan is fully executed.**
- **PR review beads open — S42 (Wave-7 review #73):** `byj` P2 (resulted-panel render handling — couples to `s38`), `5q5` P2 (`render.py` matrix-path reconciliation). **S41 (#71):** `s38` P2, `r5l` P3, `pka` P2, `a-plus-maxing-bhc`/W7 None-sentinel (RESOLVED in W7: ≥2-tp rendered via the store sequence — close at next bead pass). **S40 (#69):** `8j6` P1, `10h` P2, `20d` P2 (DEFENSIVE), `7lt` P2. **S39 (#66):** `e3b` P2, `juc` P2, `fmb` P3, `v13` P3, `fga` P3. **S38 (#64):** `3lv` P2, `rnm` P2, `2x1` P3, cross-hook matcher P3. **Earlier:** `1ww`/`ivt`/`z2u` (#44), `1vi` (#47), `4yk` P2 + `u8u` P3 (#55), T5/T9 (S37), `vvs` P2 (S36).
- **V1 spec/build-plan gaps (S29):** `kz6`, `12p`, `434`, `bpu`, `dv3`, `4xe`.
- **P2:** `dke`, `3v5`, `xg4`, `382`, `w3n`, `5bd`, `5l9`/`78p`, `pmp`, `h1z`, `rc1`, `d3w`.
...
- **Closed:** S42: `1ih` (Wave 7 via PR #73 — V1 18/18). S41: `1aa` (#71). S40: `oaf`/`8cv` (#69). S39: `yo6`/`ml1`/`ftm` (#66). S38: `xlu`, `br1` (#64). S37: `5wo`, `qwj`, `ko5`. S36: `orl`. S35: `3gp` (#55). S34: `gu4` (#53). S33: `n9h` (#50). S32: `8s6`/`6be`. S31: `89a`/`e9m`. S30: `394`/`bez`/`qbb`.

### Open project work
- First HTML artifact (LM-04) lands when `generate.run` is fed real operator data (Walter-pending) — the full render pipeline (`generate.run` / `render.emit` / `render_views`) is now built.

## Landmark window check (close step 8.7)

All 4 active landmarks (LM-01 doctor visit July 2026, LM-02 Oura/wearable, LM-03 23andMe, LM-04 first HTML artifact) — no trigger windows opened during S42 (2026-06-07; Wave-7 build session, no operator data ingested). LM-04 still pending: with the V1 build now COMPLETE (18/18), the full render pipeline (`generate.run`/`render.emit`/`render_views`) is live but no artifact generates until it's fed real operator data (Walter-pending) — LM-04 is now gated ONLY on the data, not on any build. LM-01's 14-day-before window still depends on the TBD July exact date (not within window as of 2026-06-07); LM-02/03 remain Walter-pending. No status flips due.
