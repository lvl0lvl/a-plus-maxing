...
Acceptance criteria:
- [ ] AC1 (data-model grounding): reviewed the real V1 data model + both current render surfaces (`generate.run` value-cards and `render_views` matrix/projection) so the mock reflects ACTUAL streams/fields (biomarkers, panels, watch-outs, feedback), not invented data.
- [ ] AC2 (mock produced): Pencil mock(s) for the unified dashboard covering all three zones — stats (biomarker values + trends + matrix/projections), plans (the surface that does not exist today, `1oh`), tracking (timepoints, pending panels, watch-outs); self-contained; presented to Walter.
- [ ] AC3 (sign-off): iterated to explicit Walter sign-off, OR the requested changes captured as the next mock iteration.
- [ ] AC4 (frozen target): approved design captured in a vault design note + each of the 6 dashboard beads (`i1t`/`1oh`/`i2yw`/`azf`/`y0h0`/`04uk`) annotated with how the mock resolves it.

Files I WILL touch: a `.pen` Pencil doc (NEW dashboard mock); a vault design note under `vault/design/`; `.beads/issues.jsonl` via `bd`; at close `HANDOFF.md`, `vault/sessions/session-47.md` (NEW).

Files I will NOT touch: ALL production code under `scripts/` + templates (no build this session — that is the post-sign-off phase); `.claude/hooks/*` + `settings.json` (no authorization); `INVARIANTS.md` rows; deployed roster `.claude/agents/*`; `main` directly; real operator-PII values.

NOT doing: building the dashboard fixes (`i1t` crash-fix etc. — post-sign-off build, separate scope/PR); Track-2 correctness; library population; LM-04 real-data artifact.

Invariants at risk: INV-SCOPE-CONTRACT, INV-PF-ATTESTATION, INV-HO-ROTATION, INV-HO-NO-STALE-HASH, INV-BRANCH-NOT-MAIN, INV-TRUNK-COMPLETENESS. A pure-mockup session changes no production behavior; nothing structural at risk.

### S47 Scope Contract Evaluation (2026-06-10, volatile)

- **AC1 (data-model grounding) — PASS.** Read the full V1 data model end-to-end before mocking: the store (`{item,timepoint,source,value}` per-item ndjson; `loop_schema` `biomarker::`/`panel::`/`watch-out::`/`feedback::` streams), both render surfaces (`generate.run`→`dashboard.py` value-cards that CRASH on string streams + leak the `biomarker::` prefix; `render_views.py` typed matrix/projection), and the plan layer (`router.summarize` de-id summary → `assemble()` attributed plan dict with NO render surface). The mock reflects actual fields, not invented data; honest gaps flagged (no units/range/polarity in the store; `state_for` is a name-hash placeholder; `_trend_token` RAISES on a directional change).
- **AC2 (mock produced) — PASS (exceeded).** A plan-forward Whoop/Apple-Health "Today" command center built in a NEW `.pen` (the operator's open canvas untouched). Final design has SEVEN zones (vs the 3 contracted): hero readiness rings → week calendar (month-expand) → today's plan (Workout/Nutrition/Supplements/Peptides app-screens with live tracking layered on the plan) → performance & trends (bar sparklines) → care team (ALL 16 domain specialists) → goals & progress → labs. Presented + iterated live with screenshots.
- **AC3 (sign-off) — PASS.** Walter: "good enough for v1 … iterate … in v2 … I saved it to disk." Reached across 4 feedback rounds (anemic→comprehensive Whoop/Apple; sparkline jank→bar columns; specialist count 9→all 16; workout/nutrition→live app-screens).
- **AC4 (frozen target) — PASS.** `vault/design/dashboard-v1-design.md` captures the approved design + load-bearing decisions + the build/data-model delta + the bead-resolution map; all 6 dashboard beads (`i1t`/`azf`/`1oh`/`i2yw`/`y0h0`/`04uk`) annotated with how the design resolves each + the note pointer.

### Drift checks (S47 close)
...
Scope contracts for Sessions 5-46 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; S33 archived S32; …; S44 archived S43; S46 archived S44 + S45 together — S44's entry had been missed at the S45 close; S47 archived S46) to keep this handoff lean. The current (S47) scope contract is above; the archive holds the prior-session archaeology.
...
1. **The v1 dashboard design is SIGNED OFF (S47); S48 BUILDS it — gated on a real DATA-MODEL DELTA, not a template tweak.** The approved target `vault/design/dashboard-v1-design.md` is a plan-forward command center. Building it needs, as an ordered sub-effort: per-biomarker metadata (units + reference_range + good-direction polarity — also replaces the `component_set.state_for` name-hash placeholder + unblocks `router._trend_token`, which currently RAISES on a directional change); a type-routed unified render surface (resolves `i1t` P1 crash / `i2yw` unify / `azf` label leak); plan-content schemas for workout/nutrition/supplements/peptides (`1oh`); the mixed-stream test (`04uk`); wearable→recovery/strain/sleep scoring (LM-02-gated); calendar + goal-progress models; per-specialist rollup for the 16 care-team cards. Do NOT band-aid `i1t` alone — BUT a minimal type-route crash-stop is a legitimate STANDALONE safety PR if a working dashboard is needed before the full build.
...
3. **read-before-invoke AND run-the-FULL-methodology (PF-S13/17/39/40) — STANDING.** Every `/review-pr` + `/merge` + `/aplus-research`: invoke FRESH via the Skill tool (cached ≠ fresh); run ALL phases — Phase 3 ("**DO NOT triage findings yourself**" → dispatch the blind triage agent) + Phase 7 (blind verify) are non-optional profile-less layers. No `Workflow`/hand-roll substitution. `.venv/bin/python -m pytest`, never from memory. **Editing `.claude/hooks/*` + `settings.json` needs EXPLICIT per-session authorization** (does NOT persist past the session that granted it). **Dashboard work is MOCKUP-FIRST and the v1 target is now LOCKED** (`vault/design/dashboard-v1-design.md`); v2 iterations also route through Pencil sign-off (`feedback-dashboard-pencil-mockups`).

## Current State (volatile)

- **S47 (2026-06-10) produced + LOCKED the v1 dashboard target design (mockup-first).** A plan-forward Whoop/Apple-Health "Today" command center mocked in Pencil across 4 iteration rounds to Walter's sign-off ("good enough for v1"). SEVEN zones: hero readiness rings → week calendar (month-expand) → today's plan (Workout/Nutrition/Supplements/Peptides as live app-screens — per-set completion, live HR/steps, planned-meal logging) → performance & trends (bar-column sparklines) → care team (ALL 16 domain specialists; the 4 internal build/review roles off) → goals & progress → labs (demoted). Operator saved the `.pen` to disk (operator-held).
- **Captured** as `vault/design/dashboard-v1-design.md` (frozen target + load-bearing design decisions + the build's DATA-MODEL DELTA + the bead-resolution map). All 6 dashboard beads (`i1t`/`azf`/`1oh`/`i2yw`/`y0h0`/`04uk`) annotated with how the design resolves each + the note pointer.
- **No production code, hooks, settings, INVARIANTS rows, or deployed agents touched** (pure design/docs). Suite **340 passed / 2 skipped** (unchanged); branch-completeness 0 (20 agents) at open + close.
- **Active landmarks:** no trigger windows opened. LM-04 still gated on real operator data — the mock used SYNTHETIC data only and produced no `vault/artifacts/` HTML artifact (the `.pen` is operator-held design, not a generated artifact).

**Historical (kept for reference):** `vault/sessions/session-47.md`.

## What Is Next (volatile)

### Resume checklist — next session (S48) open

**FIRST: merge the S47 close PR #89** (`fix/s47-dashboard-design` → `main`) — left OPEN at Walter's direction at S47 close; GraphQL was throttled, so use the REST rebase path per the merge memory. After that, `main` carries this close. V1 build COMPLETE (18/18); Track-1 PII/safety COMPLETE; the v1 DASHBOARD DESIGN is SIGNED OFF (`vault/design/dashboard-v1-design.md`). Open normally (Session Start Protocol steps 1-6 + `branch-completeness-audit.sh` at OPEN; baseline **340 passed / 2 skipped**). Commit-sequencing rule holds (`git add` + `git commit` separate calls); `git commit --no-verify` skips the bd auto-stage hook for a clean single-file commit (Top-3 #2). Each unit its own short-lived `fix/` branch → FULL `/review-pr` → `/merge`. **Editing `.claude/hooks/*` / `settings.json` needs EXPLICIT per-session authorization** (does not persist).

1. **BUILD THE DASHBOARD to the signed-off v1 target.** Read `vault/design/dashboard-v1-design.md` FIRST (the frozen design + its build implications). Gated on a DATA-MODEL DELTA — scope it as its own ordered sub-effort, not a template patch: (a) per-biomarker metadata (units + reference_range + good-direction polarity) → resolves `y0h0`, replaces the `state_for` name-hash placeholder, unblocks `router._trend_token`; (b) type-routed unified render surface → `i1t` (P1 crash) + `i2yw` (unify) + `azf` (clean labels); (c) plan-content schemas (workout/nutrition/supplement/peptide) + an `assemble()` render surface → `1oh`; (d) the mixed-stream test → `04uk`; (e) wearable→recovery/strain/sleep scoring (LM-02-gated); (f) calendar/event + goal-progress models; (g) per-specialist rollup for the 16 care-team cards. A minimal `i1t` crash-stop (type-route) is a legitimate STANDALONE safety PR if a working dashboard is needed sooner.
2. **Track 2 (V1 data-surface correctness — NOT started):** `s38`+`byj` panel-result-writer pair (land TOGETHER, Factory-to-Component rule); `5q5` (`render.py` `emit_matrix_projection` deprecation — superseded by `render_views`); `juc` (trend polarity — overlaps the biomarker-metadata work above); `7lt` (typed wired-set marker); `e3b` (clone-scoped store root); `r5l`/`bhc` caller-contract decisions; store beads `4yk`/`1ww`/`u8u`/`1vi`. `pka` (P2 — adversarial-test mandate) before the library track.
...
### Open beads carried (S47 deltas)
- **Closed S47:** none (design / sign-off session — no beads closed).
- **Annotated S47 (the 6 dashboard beads, all still open):** `i1t` (P1), `azf`/`1oh`/`i2yw`/`04uk` (P2), `y0h0` (P3) — each now carries the S47 design-resolution note + the `vault/design/dashboard-v1-design.md` pointer.
- **Carried (key):** `eb1` (P2, bd auto-stage commit scan), `2kk` (P3, two-line postal); `s38`/`byj`/`5q5`/`juc`/`7lt`/`e3b`/`r5l`/`bhc` (Track-2 V1 correctness); `pka` (adversarial-test mandate); `4yk`/`1ww`/`u8u`/`1vi` (store); `5i2`/`bie` (S43 #76 residuals); `21o`/`70j`/`f12`/`qkb` (S44 P3); `10h`; `e3d` + the S29 spec gaps (`kz6`/`12p`/`434`/`bpu`/`4xe`). Full set: `bd list --status open`.

## Landmark window check (close step 8.7)

All 4 active landmarks (LM-01 doctor visit July 2026, LM-02 Oura/wearable, LM-03 23andMe, LM-04 first HTML artifact) — no trigger windows opened during S47 (2026-06-10; a Pencil-design + docs session). LM-04 remains gated ONLY on real operator data: the S47 dashboard mock used SYNTHETIC data and produced no `vault/artifacts/` HTML artifact (the `.pen` is operator-held design, not a generated artifact) — did NOT trip it. LM-01's 14-day-before window depends on the TBD July exact date — not within window as of 2026-06-10. LM-02/03 remain Walter-pending. No status flips due.
