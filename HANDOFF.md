...
### S32 Scope Contract Evaluation (2026-06-05, volatile)
...
- **AC5 — PASS (this close).** `8s6` + `6be` CLOSED → frontier advanced (`6be` unblocked `n9h`). 4 close audits + branch-completeness green at `--session 32`; PF attestation; VOLATILE rotation; PR #46 (`feature/s32-store-hardening`, 13 legitimate fixed) rebase-merged at `9d476d7`, then PR #47 (`feature/ingest-shared-routine`, 22 legitimate fixed incl. 1 CRITICAL) rebase-merged at `5ca684e2`; the close runs on `fix/s32-close` AFTER both merges (PF-S25-01).
...
Scope contracts for Sessions 5-31 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32, 2026-06-05) to keep this handoff lean. The current (S32) scope contract is above; the archive holds the prior-session archaeology.
...
1. **read-before-invoke (PF-S13-01 + PF-S17-01) — STANDING; `execute` is MID-FLIGHT (data-IN layer BUILT: store + guard + ingestion; data-OUT + adapters + router + clone-init next).** Each task's recipe is the gate — READ it IN FULL before executing (HELD this session for the `8s6` + `ADR-0003-T1` recipes + `/review-pr` ×2 + `/merge` ×2). Wave-3 continuation builds the adapters (`n9h`) + scheduler (`oaf`) + the data-out layer (`gu4`/`xlu`/`br1`) — TDD with `.venv/bin/python -m pytest`, never from memory; no `Workflow` substitution.
2. **The V1 data-IN layer is BUILT, hardened, and path-safe.** The store is corruption-tolerant (skips malformed AND non-conformant lines, `STORE-SKIP:` stderr), crash-atomic (temp → fsync → `os.replace`), and **path-traversal-guarded** (`_item_path` rejects an `item` escaping the root — the SEC-001 CRITICAL fix). The ingestion routine (`scripts/ingest/`: `Adapter` Protocol + `run`/`manual_entry`/`import_csv`) writes through `store.append` only — single shared key, no model step, 0 egress proven. Enforcement-first holds: no plan-reasoning-over-PII task until the router (`ADR-0006-T0`/`T1`). **CARRIED (before V1 ships):** `qwj` P2 (vault-PROSE operator name + ADR-0005 clone-init); `1ww` P2 (concurrent-append — premature for single-operator V1); `1vi` P3 (re-ingest can't correct a value); `ivt`/`z2u` P3.
...
**Demoted from prior Top-3:** the "Wave 2 done / guard just-built / `8s6` open" framing (the store is now also corruption-tolerant + crash-atomic + path-safe; the ingestion routine that consumes the guard is built + merged; `8s6` closed).

## Current State (volatile)

- **THE V1 DATA-IN LAYER IS BUILT + HARDENED + MERGED (S32, 2026-06-05)** — two deliverables, two PRs. PR #46 (`8s6` store-hardening → `main` `9d476d7`) + PR #47 (`ADR-0003-T1` Wave-3 ingestion → `main` `5ca684e2`). Pipeline: …→ Wave 1 [S30] ✓ → Wave 2 [S31] ✓ → **store-harden + Wave-3 ingestion [S32] ✓** → Wave-3 adapters/scheduler + data-out (next).
- **The local store is corruption-tolerant + crash-atomic + path-safe:** `read`/`append` skip malformed AND valid-JSON-non-conformant lines (`STORE-SKIP: <path>:<lineno>` stderr); `append` rewrites via temp → fsync → `os.replace`; `_item_path` rejects any `item` whose resolved path escapes the root (the SEC-001 CRITICAL path-traversal fix). `append`/`read` signatures + `keying.py` unchanged.
- **The ingestion layer (`scripts/ingest/`):** `adapter.py` = the `Adapter` Protocol (`source_tag` + `read_readings`; `source_tag` is the per-adapter identity declaration — `run` does NOT call it, readings self-carry `source`). `ingest.py` = `run`/`manual_entry`/`import_csv` → one `_store_reading` → `store.append` (single shared keying dedupe, no second key, no model step, 0 egress proven). `import_csv` validates the whole CSV before any write (all-or-nothing).
- **Both 6-agent `/review-pr` runs earned their keep (PF-S3-01, 0 suppressed per PF-S26-01):** PR #46 — 13 legitimate (the corruption class was half-closed, `JSONDecodeError`-only — read/append still bricked on a non-conformant line); PR #47 — 22 legitimate incl. **1 CRITICAL (SEC-001 path traversal)**. All fixed + blind-verified RESOLVED.
- **SINGLE TRUNK** unchanged (since S22): `main` = complete project. Suite **72 passed / 2 skipped** (Linux-only egress).
- Beads `8s6`/`6be` **CLOSED**; 1 new bead `1vi` (P3 value-correction gap). `bd ready` frontier = Wave-3 continuation.
- **Active landmarks:** no trigger windows opened.

**Historical (kept for reference):** `vault/sessions/session-32.md` + `vault/meta/log.md` S32 entry.

## What Is Next (volatile)

### S32 hardened the store + built the Wave-3 ingestion routine. Next = Wave-3 adapters/scheduler + data-out.

**RESUMPTION POINT.** S32 fixed `8s6` (store durability + path safety) and built `ADR-0003-T1` (ingestion routine + adapter interface), both rebase-merged; closed `8s6`/`6be`. **Open the next session on `main`** (Start Protocol + `branch-completeness-audit.sh` at open).

**Next pipeline work — Wave-3 continuation + data-out:**
1. **Frontier:** `bd ready` surfaces `n9h` (`ADR-0003-T2` wired adapters — implement the `Adapter` Protocol for HealthKit/Oura/Garmin/Whoop; the 0-shared-routine-edit proof diffs `scripts/ingest/adapter.py` + `ingest.py`, so those stay stable); `oaf` (`ADR-0003-T3` scheduler — unattended `ingest.run`, delta-only, 0-egress) is BLOCKED on `n9h` and follows once it lands (NOT ready now). Parallel-ready (in `bd ready`): `gu4` (`ADR-0004-T1` render engine), `xlu` (`ADR-0005-T1` pre-commit hook — reuses `pii_scan.scan`; **resolve `qwj` first**), `br1` (`ADR-0006-T0` router spike). READ each recipe `docs/task-plan/<id>.md` IN FULL first (PF-S17-01), TDD with `.venv/bin/python -m pytest`, then `bd close`.
2. **Published surfaces the next wave consumes:** the `Adapter` Protocol (`source_tag` + `read_readings`) — `n9h`'s adapters implement it. `ingest.run(adapter, export_file, root=…)` — `oaf`'s scheduler invokes it. `store.append`/`read` (path-safe, corruption-tolerant). `keying.py` single key (never redefine — re-opens N3). `egress_guard.run(operation)` backs 0-egress checks. `pii_scan.scan` for `xlu`'s hook — **resolve `qwj` first** (≈924 `scan` hits across vault prose → the hook would block nearly every commit).
...
**Carried flags / beads (surfaced, not lost — PF-S26-01):**
- **NEW S32:** `1vi` P3 (re-ingest cannot correct a stored reading's value — dedupe key is `(item,timepoint,source)`, value excluded → first-write-wins; a render/edit-layer product decision, `ADR-0004+`, not a keying change).
...
### Open beads carried
- **V1 build execution (label `v1-build`):** 18 per-task beads, dependency-wired; **6 closed** (Wave 1: `394`/`bez`/`qbb`; Wave 2: `89a`/`e9m`; Wave 3: `6be`). READY now: `n9h` (Wave-3 cont.) + `gu4`/`xlu`/`br1`; `oaf` (`ADR-0003-T3` scheduler) is BLOCKED on `n9h`, not ready. Plus `e3d` (recipe-set doc-consistency, non-blocking).
- **PR review beads open:** `1ww`/`qwj`/`ivt`/`z2u` (PR #44), `1vi` (PR #47). **`8s6` CLOSED** (PR #46).
...
- **Closed:** S32: `8s6` (PR #46), `6be` (PR #47). S31: `89a`/`e9m` (PR #44). S30: `394`/`bez`/`qbb`. S29: `mo4` (PR #40). S28: `hv6` (PR #38). S27: `rg2`. S25: `fm4`/`hil`. S23: `bte`. S22: `gdw`. S21: `mhg`/`5ot`/`0be`.
