...
1. **The A′ gate-dispatch control-inversion (ADR-0028) is COMPLETE on main + the live `/generate-plan` run is EXECUTABLE — but a usable plan is blocked on EMPTY real data; S98 = get the INTAKE SCREENS working.** main @ the dated tip in Current State (S97 close): ADR-0028 (accepted, #255) → spec (#256) → build-plan (#257) → 5 recipes (#258) → the 5-task build + Tier-2 + full Tier-3 `/review-pr` + 4 fixes (#259) → close (#260). The skill drives the ONE shared driver via the `plan_step.py` step-harness, fulfilling every yielded typed request (AUTHOR/GATE/REAUTHOR/ADJUDICATOR) as a SUBSCRIPTION agent. Suite 1721/2; core-cap audit exit 0; EXTEND-NOT-REBUILD held (inner engine + `deid_in.py` numstat=0). **REFRAME (S97 close): a `/generate-plan` "live run" right now is only a SYNTHETIC machine-test — it can't produce the operator's actual plan because the local store is EMPTY (no real data ever ingested). The blocker is the INTAKE (only demographics + uploads capture today; the chat UI is absent + `converse` is a stub). Next = get the intake screens working so real data flows in (see What Is Next + bead `w3y8`'s scoping seed); the operator-present LIVE run is the FOLLOW-ON once real data is in.**
2. **Before the operator-present LIVE run with REAL PII: land the value-scan residuals + observe the live-dispatch 0-leak.** `6hts` (the de-id value-scan recall residuals — numeric-typed + two-line-postal PII, S96 Tier-2 finding) + `stsq` (SEC-3 — observe live-subscription-dispatch 0-raw-PII to a REAL agent, the one crown-jewel property the mock build defers to the operator-present run). The synthetic-first run (real dispatch + real de-id spend, 0 real PII) does NOT need these. Carried: `f0gh` (chat elicitation), `zsp5`, `myts` (PF-S96-01), `fbyr` (PF-S97-01).
3. **Standing mechanics (all HELD).** Build via the full pipeline (`/create-adr`→spec→build-plan→task-plan→`/execute-plan`; never hand-rolled, PF-S36-01) — each ran in full this session; **run the FULL gated `/review-pr` for any multi-file/security-relevant code PR (PF-S96-01, APPLIED on #259 — the 6-agent review caught a real crown-jewel defect a per-wave checkpoint missed)**; 3-tier review with profile-less blind-triage + EXECUTED blind-verify; EXTEND-NOT-REBUILD (inner engine numstat=0); full role profiles inlined VERBATIM on EVERY role dispatch (the hook DENIED 2 abbreviated dispatches this session); **review/verify agents use READ-ONLY git ONLY — never `git checkout`/`reset`/`commit` in the shared checkout (PF-S97-01, NEW: a review agent corrupted the build tree); mutate a `/tmp` copy or `git restore` a tracked file**; `/merge` via REST when GraphQL exhausted (full-40-char SHA guard on the reviewed+verified tip; poll GitHub to reflect the pushed SHA first); `git add` SEPARATE from `git commit`; commit branch-fixes in the main checkout on the feature branch, NOT a worktree (block-commit-main reads the starting cwd's repo); single trunk; stage close docs by EXPLICIT path; NEVER stop mid-loop to ask a settled question (PF-S92-01); per-task remediation not batched (PF-S93-01); re-run the close gate on final main + cite the SHA (PF-S74-01). Beads: `6hts`/`stsq` (live-run gates), `f0gh`, `myts`, `fbyr`, `zsp5`.
## Current State (volatile)

- **S97 (2026-06-25): designed + built + merged the ENTIRE ADR-0028 A′ gate-dispatch control-inversion — the live `/generate-plan` run is now executable.** A 0-spend dry-run of `/generate-plan` (S97 open) found the live run was NOT executable: the gate (quality judge + safety lenses) + the reauthor/adjudicator hooks were SYNCHRONOUS Python callables the skill couldn't fulfill with subscription agents (the Agent tool is the orchestrator's, not Python's). ADR-0028 (operator-approved option A) completes the control-inversion: `plan_driver.drive` yields a discriminated-union of typed dispatch-requests (AUTHOR/GATE direct-yield; REAUTHOR/ADJUDICATOR throw/replay-memo via a `BaseException` sentinel), the skill drives via the `plan_step.py` step-harness fulfilling each as a subscription agent — one driver, multiple consumers, no fork.
- **The full gated pipeline ran end-to-end, each stage through its own gate:** `/create-adr` (verify→judge→red-team→fix; the red-team caught AR-001 — reauthor/adjudicator fire inside the byte-frozen engine, forcing the throw/replay-memo mechanism; #255) → `/create-spec` (#256) → `/create-build-plan` (judge caught a missing no-fork release-grep; #257) → `/create-task-plan` (5 recipes; #258) → `/execute-plan` (5 tasks T1-T5; #259). All merged to `main` (the dated tip is in the Current State's last bullet).
- **The layered review caught a real crown-jewel defect at TWO layers PRE-MERGE.** Tier-2: the OQ-2 GATE violation (the build shipped the composed callable, not the raw-verdict producer the skill needs) → fixed + Architect re-review RESOLVED (executed). Tier-3 (the full 6-agent `/review-pr`): **BUG-1** — the `plan_step` step-harness infinite-loops on a `None` hook return, i.e. the SAFE-default outcome (a declined re-author / a declined adjudication → domain HELD) HANGS the production skill path forever — caught by 3 converging agents + RUN-confirmed, a defect all 4 per-wave checkpoints + the green 1718-test suite missed (the PF-S87-01 integration pattern); + BUG-2 (harness GATE-raise fail-closed path dead) + QUAL-1/QUAL-2 (stale doc refs). All 4 fixed + blind-verified RESOLVED with mutation→RED non-vacuity.
- **The V1 A′ runtime is now FULLY BUILT, INTEGRATED, and EXECUTABLE.** The shared no-fork driver, the composed fail-closed gate (raw-verdict producer + the ONE `compose_disposition` site), the step-harness round-based skill drive, the live de-id backend (mock-tested) + value-scanned, the yield-payload value-scan, the non-tautological replay self-test. Only the operator-present LIVE run remains (key injection + real data + spend).
- **Beads:** carry — `6hts` (de-id value-scan residuals, before real PII), `stsq` (SEC-3 live-dispatch 0-leak release gate), `f0gh` (chat elicitation), `zsp5`, `myts` (PF-S96-01), `fbyr` (PF-S97-01, new — review-agent git hygiene).
- **main @ `747ff39` (as of 2026-06-25 S97 close, PF-S74-01). Vault perpetually daemon-dirty (stage by explicit path).**

**Historical (kept for reference):** `vault/sessions/session-97.md`.
## What Is Next (volatile)

### S98 — get the INTAKE SCREENS working so the operator's REAL data flows in (the actual blocker to a usable plan)

**The reframe (S97 close, operator-directed):** the plan-generation engine + the live `/generate-plan` path are COMPLETE + executable (ADR-0028, main as of the Current State's dated tip). But a `/generate-plan` "live run" right now only does a SYNTHETIC machine-test — it can't produce the operator's actual plan, because **the local store is EMPTY: the operator has never ingested real data.** The blocker is the INTAKE: only Step-1 demographics + file uploads capture today; the rich planner-feeding data (goals/training/nutrition/supplements/peptides) is NOT capturable because (a) the wizard steps 2-5 render text-only "open the chat" panels with NO chat widget/JS/fetch, and (b) `_ClaudeNoTrainBackend.converse` is a `NotImplementedError` stub. So the next session makes the intake actually capture real data end-to-end.

**START S98 by scoping** (the grounded read-only scoping seed from S97 is captured on bead `w3y8` notes — confirm it against the live tree first). Then the directed work-list (file:function + bead — all on `w3y8`'s seed):
1. **Build the browser chat UI** — `vault/design/templates/intake.py` `_chat_panel`/`render` (a real turn-list + input + send + JS `fetch('POST /chat')`). Net-new UI → run it through the DESIGN AGENT TEAM + Pencil mockups + operator sign-off FIRST (PF-S68-01/PF-S49-01 — design via the team, not solo; the dashboard is design-LOCKED Clinical Light, but the INTAKE is separate + open). Highest leverage — without it nothing rich gets captured. `w3y8`.
2. **Implement live `converse`** — `scripts/model/client.py:201` (`NotImplementedError` stub), mirroring the working `deidentify`. `w3y8`.
3. **Wire the 4 ADR-0019 chat tokens** — `scripts/serve/chat.py` `_DOMAIN_TOKEN_GROUPING` + `_MINTABLE_TOKENS` (`f0gh`). **NOT naively:** the 4 band tokens are ALWAYS-SET (a fresh operator gets `not-discussed`/`none`/`moderate`); `chat._missing_fields` treats "present in summary" as captured, so a naive token-list edit makes the domain `domain_done` WITHOUT ever asking — the PF-S87-01 vacuous-done trap. The fix must treat the `not-discussed` sentinel as a GAP.
4. **Build the chat→plan usable-plan E2E** — `tests/plan/test_conversation_to_plan_e2e.py` per `docs/task-plan/ADR-0017-T2.md` (`recorded==True` + content-traceability; distinguish a usable plan from the honest no-plan state). NEVER built — the non-tautology gate (`8f1h`/PF-S87-01).
5. **Safety follow-ups before the live run:** the `chat.py:362` `_degraded_turn` echo (`s7qz`), the `router.py:623` summarize-scan uncap (`sc97`), the `multipart.py:247` repeated-field (`4oha`).

**THEN (after real data flows in):** land `6hts` (de-id value-scan residuals) before REAL PII → the operator-present LIVE run (`/generate-plan` over the operator's de-identified real data → a usable plan + doctor-visit queue + dashboard). The operator injects `export ANTHROPIC_API_KEY=$(security find-generic-password -s "quant-primary-api" -w)`; the agent NEVER touches the keychain; `stsq` (SEC-3 live-dispatch 0-leak) is the release gate. A SYNTHETIC machine-test (`/generate-plan` over fixtures, real dispatch + de-id spend, 0 real PII) is available anytime to prove the loop runs, but it does NOT produce a usable plan.

**Operational notes (S97 lessons):** (a) review/verify agents use READ-ONLY git ONLY — never `git checkout`/`reset`/`commit` in the shared checkout (PF-S97-01); (b) run the FULL gated `/review-pr` for any multi-file/security-relevant code PR (PF-S96-01) — it earns its keep (caught BUG-1, a per-wave-checkpoint-invisible integration defect); (c) the per-wave EXECUTED checkpoint verifies a task in isolation; the integrated final review catches cross-task defects — never skip it.

**Deferred / NOT blocking:** `zsp5`; the stale plan-gen docs reconcile (`78be`); `02m1` (daemon config); the local-model migration (North Star, post-V1).
## Landmark window check (close step 8.7)

S97 close (2026-06-25): re-read `vault/meta/landmarks.md` at close step 8.7 (unchanged this session — the ADR-0028 build is on SYNTHETIC fixtures + mock clients; no real operator data, no live API, no compound/biomarker page edits). LM-01 (First MD visit) **2026-07-13**: today (2026-06-25) is BEFORE the 14-day scoped-drift-audit window open (**2026-06-29**, 4 days out) → no trigger today. **NOTE for the next session on/after 2026-06-29: run the scoped drift audit on the LM-01 relevant_scopes** (operator-profile January-2026 section + `risk_tier: medium+`/experimental compounds + baseline-panel biomarkers + the doctor-handout queue); the 7-day MD-handoff-artifact window opens 2026-07-06. LM-02 (Apple Health) / LM-03 (23andMe) / LM-04 (first HTML artifact): not triggered. No landmark ACTIONS due this session.
## Open Issues
