...
- **AC4 (store-adversarial battery — 4 categories + mutation) — PASS, strengthened by review.** cross-stream (recovery≠strain), dedupe (idempotent re-run + distinct-day persistence), dedupe-key boundary (value + source), mutation (constant-timepoint→RED). The #136 review HARDENED it: TEST-1 (value-pin each of the 8 column→item mappings — transposed-mapping→RED) + TEST-2 (honest-zero retention — `not value`→RED), both mutation-proven.
