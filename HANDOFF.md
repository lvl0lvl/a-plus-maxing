...
Scope contracts for Sessions 5-39 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; S33 archived S32; S34 archived S33; S35 archived S34; S36 archived S35; S37 archived S36; S38 archived S37; S39 archived S38; S40 archived S39) to keep this handoff lean. The current (S40) scope contract is above; the archive holds the prior-session archaeology.
...
1. **Wave 6 via `/execute-plan` WAVE mode is the next action (PF-S36-01) — STANDING.** S41 = build-plan **Wave 6** (`1aa`/ADR-0007-T1 — lab-loop / watch-out / physician-feedback store schemas: pending / not-yet-answered / no-prior states defined + persisted, 0 signal automation). Then **Wave 7** (`1ih`/ADR-0007-T2 biomarker matrix/projection views). Build via `/execute-plan` in WAVE mode against `docs/build-plan/build-plan-v1-full.md` — complete the wave IN ORDER, gate on the W6→W7 checkpoint Go/No-Go, never hand-roll a per-task substitute. The `v1-build` contract cites W6 + attests the W5→W6 checkpoint passed. **Read `/execute-plan`'s SKILL.md + references IN FULL fresh first** (project path-mapping: recipes `docs/task-plan/`, build plan `docs/build-plan/`, roles `skills_library/roles/` inlined full).
2. **read-before-invoke AND run-the-FULL-methodology (PF-S13-01 + PF-S17-01 + PF-S39-01 + PF-S40-01) — STANDING.** Two halves: (a) read each gated skill fresh / invoke via the Skill tool (not from cached context); (b) run ALL of its phases — for `/review-pr`, Phase 3 ("**DO NOT triage findings yourself**" → dispatch the blind triage agent) and Phase 7 (blind verification) are non-optional independence layers, NOT to be improvised. PF-S40-01: I ran `/review-pr` Phases 0-2 then substituted my own triage+fix for Phases 3-7; the blind triage (once actually run) caught 3 findings I'd have wrongly fixed. No `Workflow`/hand-roll substitution. `.venv/bin/python -m pytest`, never from memory.
3. **Three-tier review is load-bearing — safety/PII surfaces hide in builder-blessed paths (PF-S26-01/PF-S3-01).** S40's Wave-5 build passed all 227 builder tests while the HALT hard-limit filter FAILED OPEN on a compound limit (a rec contradicting the 2nd clause shipped actionable) — caught only by Tier-3. This is the **FOURTH consecutive wave** a safety/PII surface hid behind green builder tests + survived Tier-2, caught by the layered review. For W6/W7 (`1aa` lab-loop store + `1ih` biomarker views — both store/PII-adjacent), the full Tier-2 + Tier-3 + blind triage + blind verify is non-optional; recurrence-watch for one that ESCAPES all tiers (none has).

**Demoted from prior Top-3:** the "Wave 5 run" framing — Wave 5 built + merged at S40 (PR #69); the forward risk is now the Wave 6 run.

## Current State (volatile)

- **S40 (2026-06-07) built Wave 5 via `/execute-plan` WAVE mode (the 2 open tasks).** `oaf`/ADR-0003-T3 (unattended delta-only scheduler) + `8cv`/ADR-0006-T2 (multi-domain plan assembly — the one task reasoning over the `ftm` router summary; the V1 PII trust + HALT safety task). PF-S36-01 + PF-S39-01 falsification windows HELD (sanctioned skills invoked fresh via the Skill tool, wave in order, W5→W6 checkpoint gated, no hand-roll).
- **The three-tier review caught + fixed a Critical HALT safety bypass the builder's 227 green tests + Tier-2 both missed** (0 suppressed, PF-S26-01): Tier-2 fixed a tautological PII test + HALT fail-closed-on-indeterminate; Tier-3 `/review-pr` #69 (6-agent) caught a **Critical** compound-hard-limit HALT fail-open (a rec contradicting the 2nd clause shipped actionable) + over-strike of limit-respecting recs + adapter-discovery crash + malformed-output crash → blind triage 18 LEGITIMATE / 3 rejected (BUG-5/QUAL-4/QUAL-6) / BUG-3 beaded; all fixed + blind-verified ALL 15 RESOLVED. PR #69 rebase-merged via REST (GraphQL throttled).
- **PF-S40-01 logged** — the Tier-3 FIX methodology was improvised (Phases 3-7 substituted) before Walter flagged it; course-corrected to the skill's blind-triage→fix→blind-verify.
- **Build: 16/18 leaves.** Wave 5 COMPLETE (`oaf`+`8cv`); **Wave 6** next (`1aa`) → **W7** (`1ih`). Live per-wave state in `vault/meta/overview.md`.
- **SINGLE TRUNK** unchanged (since S22). Suite **227 passed / 2 skipped** (+47 wave tests; the 2 skips are pre-existing Linux-only egress-namespace tests); all hook suites green. `main` @ 9c66ad4 (2026-06-07 Wave-5 merge) + this S40 close PR.
- **Active landmarks:** no trigger windows opened. LM-04 still pending (the `generate.run` producer is live but unfed).

**Historical (kept for reference):** `vault/sessions/session-40.md` + `vault/meta/log.md` S40 entry.

## What Is Next (volatile)

### Resume checklist — next session (S41) open

**Clean slate — nothing dangling from S40.** `main` is at the Wave-5 merge + this close PR, tree clean, 0 open PRs. The next session opens normally:

1. Run the Session Start Protocol (steps 1-6) + `branch-completeness-audit.sh` at OPEN. Baseline suite = 227 passed / 2 skipped.
2. **S41 is Phase C — Wave 6 via `/execute-plan` WAVE mode.** Cut a `feature/`-branch off `main`. Write + confirm a **`v1-build` scope contract** citing the build-plan wave (**W6**) AND attesting the W5→W6 checkpoint passed (run it: `.venv/bin/python -m pytest tests/ingest/test_scheduler.py tests/plan/test_assemble.py` + the cross-spec gates).
3. **Read `/execute-plan`'s SKILL.md + references IN FULL fresh first** (PF-S17-01). **AND when Tier-3 `/review-pr` runs, follow its Phases 3 (blind triage — do NOT triage yourself) + 7 (blind verification) in full (PF-S40-01).** Project path-mapping: recipes=`docs/task-plan/<task>.md`, build plan=`docs/build-plan/build-plan-v1-full.md`, roles=`~/Documents/Projects/skills_library/roles/<role>/agent.md` (inlined in full). Run via `/execute-plan` in WAVE mode — never a hand-rolled per-task SE dispatch (PF-S36-01).
4. **Build Wave 6** — `1aa` (ADR-0007-T1: lab-loop / watch-out / physician-feedback store schemas — pending / not-yet-answered / no-prior states defined and persisted; 0 signal automation; the lab loop's input edge is downstream of the Wave-5 plan assembly). Then run the **W6→W7 checkpoint AS A GATE** (incl. the SEC-03 failing-capable egress injection into a store/read cycle) → wave PR → `/review-pr` (FULL methodology) → `/merge`.
5. **Then W7** (`1ih` ADR-0007-T2 biomarker matrix/projection views — map store state 1:1, <500KB, 0 outbound). One wave ≈ one session. After W7 the V1 build is 18/18 (LM-04 still gated on real operator data).

### Follow-up bead sequencing (updated S40)

The router / HALT / PII residuals, with WHEN each is fixed (all LM-04-gated unless noted):

- **`8j6` (P1) — before LM-04.** Raw PII in a free-text summary field is surfaced verbatim by `assemble._personalize` (the router passes the 7 non-derived fields through verbatim). Value-level gate is a DESIGN decision: (a) closed-vocab store schema (touches `keying.py`) OR (b) an `assemble` PII-shape gate (needs `pii_scan`+identity-config). Characterization-test-pinned; Security-signed-off to carry. Decide before the first real operator data.
- **`10h` (P2) — before LM-04 / before live specialist roster.** HALT trusts specialist-supplied `category`; a mislabeled prohibited-class member escapes. Specialist→assemble metadata-integrity / category-attestation, beyond V1's trust model.
- **`20d` (P2 — DEFENSIVE, needs Walter approval) — before LM-04.** Adapter discovery (`scheduler._wired_adapters`) has no try/except around `importlib.import_module`; one module that errors at import aborts the whole unattended run. Fix is a try/except (skip+surface) — defensive, so decide fail-fast-and-document vs skip-and-surface with Walter first. (The non-defensive `issubclass`-before-instantiate half was already fixed in Wave 5.)
- **`7lt` (P2).** Replace the docstring-prose `"unwired"` wired-set marker with a typed/structural marker (`wired=False` attribute or a registry) — a spec amendment to the ADR-0003-T2/T3 adapter-discovery contract. (The live safety property is test-guarded; non-blocking.)
- **`e3b` (P2) — when `8cv` is wired to a real `router.summarize`.** Bind a clone-scoped store root at the `router.summarize` production call site (`assemble` correctly stays rootless). Amend the ADR-0006-T1 rootless `store.read(item)` contract at that wiring task.
- **`juc` (P2) — before plan-reasoning over changing labs.** `recent-trend-direction` needs per-item good-direction polarity metadata in `keying.py`; until then the router RAISES fail-closed on a determinable directional change.
- **`fga` (P3), `fmb` (P3), `v13` (P3), `rnm` (P2), `3lv` (P2)+`2x1` (P3)+cross-hook matcher (P3):** the PII-guard-activation + hardening group — `3lv` registers `block-pii-commit.sh` in `.claude/settings.json` (blocked on the `@gmail.com` fixture self-block + `2x1` case-insensitivity); `rnm` pins the scaffold-page convention; `fga` runtime scalar gate; `fmb` cap re-validation upstream; `v13` `@DARWIN_ONLY` egress-test markers. All before LM-04 / at V1-distribution.
...
**Published surfaces the next waves consume:** `generate.run(artifact_name) -> Path` · `render.emit(template, store_read) -> Path` (no production PLAN-template producer yet — bead `4xe`) · the `Adapter` Protocol + 4 adapters · `scheduler.run()` (unattended, data-driven wired set) · `ingest.run(adapter, export_file, root=…)` · `store.append`/`read` (cross-item `read_all`/`items` pending `4yk`) · `keying.py` (never redefine) · `egress_guard.run` · `pii_scan.scan(...)` · `router.summarize(store_read) -> dict` + `router.dispatch(summary)` · **`assemble(goal_set, summary, roster) -> plan` (multi-domain plan, fail-closed class-aware HALT, coverage-gap-honest) — `1ih`/W7 + ADR-0007 views read what the loop stores downstream.**
...
### Open beads carried
- **V1 build execution (label `v1-build`):** **16 closed** (W1 `394`/`bez`/`qbb`; W2 `89a`/`e9m`; W3 `6be`/`gu4`/`xlu`/`br1`; W4 `n9h`/`3gp`/`yo6`/`ml1`/`ftm`; W5 `oaf`/`8cv`). OPEN: W6 `1aa`; W7 `1ih`. Plus `e3d` (recipe doc-consistency, non-blocking).
- **PR review beads open — S40 (Wave-5 review #69):** `8j6` P1 (in-summary pass-through PII value-gate), `10h` P2 (mislabeled-category HALT integrity), `20d` P2 (adapter import isolation — DEFENSIVE), `7lt` P2 (typed wired-set marker). Carried from S39 (#66): `e3b` P2 (router clone-root wiring), `juc` P2 (trend polarity), `fmb` P3 (cap re-validation), `v13` P3 (DARWIN marker), `fga` P3 (router runtime scalar gate). **S38 (#64):** `3lv` P2, `rnm` P2, `2x1` P3, cross-hook matcher P3. **Earlier:** `1ww`/`ivt`/`z2u` (#44), `1vi` (#47), `4yk` P2 + `u8u` P3 (#55), T5/T9 (S37), `vvs` P2 (S36).
...
- **Closed:** S40: `oaf`/`8cv` (Wave 5 via PR #69). S39: `yo6`/`ml1`/`ftm` (#66). S38: `xlu`, `br1` (#64). S37: `5wo`, `qwj`, `ko5`. S36: `orl`. S35: `3gp` (#55). S34: `gu4` (#53). S33: `n9h` (#50). S32: `8s6`/`6be`. S31: `89a`/`e9m`. S30: `394`/`bez`/`qbb`.
