...
### S73 Scope Contract Evaluation (2026-06-18, volatile)

- **AC1 (the `ae_profile` contract) — PASS.** The author-declared additive-AE / interaction contract was added to the `reconciliation` envelope; `compute_plan`'s existing generic lift carries `ae_profile` into the candidate `meta` with NO per-field code (the energy-budget pattern); the single-domain public contract is UNCHANGED (45 `test_generate_plan` tests + the `--self-test` green). Documented in `docs/plan-generation/author-dispatch-process.md` with a grounded canonical AE-class vocabulary.
- **AC2 (the bidirectional screen) — PASS.** `reconcile` gained behavior 4 — a shared additive-AE class OR an author-declared pairwise interaction naming the other compound surfaces in `report["additive_ae"]` AND holds the supplement (`additive-ae-held`) before recording. Mutation-proven RED (disabling the hold → 7+ tests RED; independently re-confirmed by QA + the blind verifier at 11 RED).
- **AC3 (real dual-author E2E) — PASS.** Real supplement-specialist (high-dose fish oil) + peptide-specialist (BPC-157), full profiles inlined, INDEPENDENTLY converged on a shared `bleeding-risk` axis + each named the other compound → the additive pair is HELD (the supplement never reaches the store), the BPC-157 draft records + renders; the clean creatine↔BPC-157 pair records both. Envelopes under `docs/plan-generation/examples/compound-screen-*`.
- **AC4 (no new stream + suite + gate) — PASS.** The screen holds BEFORE the existing `record_plan` (plan-integrity + contracts confirmed no new store-write surface); suite 917/2 (trunk); core-capability gate green.
- **AC5 (review + close) — PASS.** plan-integrity + QA Tier-2 (2 stale-prose lines + a placeholder commit-email fixed; 5 edge-case tests added) → `/review-pr` 6-agent Tier-3 (4 deduped findings — one docstring caught by 4 agents + 3 coverage gaps — all LEGITIMATE, fixed + blind-verified 4/4 RESOLVED) → `/merge` (REST rebase under GraphQL exhaustion) → PR #154 on `main` @ `d0d5a1c`. Full close protocol ran.

### Drift checks (S73 close)

- **Task drift:** all 5 ACs PASS. In-scope addition beyond the literal WILL-touch list: `vault/design/plan-generation-pipeline-v1.md` (the same review-driven freshness discipline as S72 — plan-integrity flagged the design Build status stale against the delivered screen; flagged, not silent). No silent drift.
- **Architecture drift:** no invariant degraded. INV-CORE-CAPABILITY stays green (the screen extends the wired path; `--self-test` passes). INV-BRANCH-NOT-MAIN held (built on `feature/compound-ae-screen` in the dedicated `../aplus-s73-screen` worktree; idle main trunk parked on `parking/s73-screen`; no commit on `main` — the close artifacts on `fix/s73-close`). INV-ROLE-INLINING held (real supplement + peptide + the 6 review agents + QA/plan-integrity all full-profile). The screen ADDED behavior 4 per the approved design (Phase 3) — toward the design.
- **Vision drift:** none, toward-vision. The system now screens the compound band for additive adverse-event risk before recording; the compound domains remain clinician-gated drafts pending the S74 medical-liaison gate (the held line, now one slice from operator-usable). `design/vision.md` first sentence unchanged.

### PF attestation

S73 close (2026-06-18): **One new PF promoted — PF-S73-01** (the close step-8 Document Freshness Rubric skip + its disclosure-ledger under-report; operator-surfaced; captured 3-layer, structural fix beaded `dfg1`; logged at the operator's direction). The BUILD held its disciplines — the screen is mutation-proven RED (independently re-confirmed by QA + the blind verifier); both reconciler paths verified E2E with real supplement-specialist + peptide-specialist reasoning (the integration mandate), the two specialists independently converging on the shared `bleeding-risk` axis; the `ae_profile` lift reused the existing generic path (no new per-field code); `/review-pr` caught a stale docstring (4 agents) + 3 coverage gaps, all fixed + blind-verified 4/4. The worktree path-read discipline HELD (each worktree file Read at its worktree-absolute path before editing — the S72-named friction did not recur). One design decision was surfaced to the operator (the malformed-`ae_profile` fail-open: tested + documented as deliberate trusted-author behavior, with fail-loud deferred as an S74 candidate). The core-deliverable-first discipline stays satisfied (the compound-safety half of the held line is built). Full attestation + the per-PR skill-trace table (PR #154) + the disclosure ledger in `memory/process-failures.md` Session 73.

## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-71 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; S33 archived S32; …; S71 archived S70; S72 archived S71; S73 archived S72) to keep this handoff lean. The current (S73) scope contract is above; the archive holds the prior-session archaeology.
...
1. **Close-step-8 freshness discipline (PF-S73-01, new this session).** Step 8 (Document Freshness Rubric) is the ONLY close step NOT in the mechanical `close-audit` roster — so it is the one that drifts; at S73 close it was skipped + minimized as a "light-touch caveat" and surfaced only by the operator's question. Until `dfg1` lands a mechanical gate, RUN step 8 explicitly every close: `git diff --name-only -- '*.md'` → bump `last_reviewed` on every modified/reviewed doc; closed-bead + stale-doc check. **Never minimize a close step ("light-touch"/"caveat"/"same as last time"); log an operator-surfaced catch AS operator-surfaced, never "0."**
2. **The held line: S73 wired the additive-AE screen; S74 closes it to operator-usable.** The supplement↔peptide additive-AE screen (a shared author-declared AE class OR a declared pairwise interaction → HOLD the supplement before recording) is wired. The compound domains are STILL clinician-gated drafts — the **medical-liaison terminal gate** (S74) is the remaining gate to operator-usable; it adjudicates the held additive-AE findings + author-declared conflicts + the supplement↔Rx axis. **The CLAIM-PHRASING RULE stays load-bearing for every author dispatch.**
3. **Standing mechanics + carry-overs.** **Worktree discipline (PF-S70-01/S71-01, held S73):** run parallel streams in separate `git worktree`s; park the idle main trunk OFF `main`; Read a worktree file from its worktree-absolute path before editing; re-verify `git branch --show-current` before every commit; restore `main` at session end. GraphQL EXHAUSTED (0/0) → REST fallback for PR-create/merge (`gh api`, full-40-char SHA guard). Git/bd: `git add` separate from `git commit` (PII hook); the bd repo-ID mismatch + `.beads/daemon-error` + the vault re-mangle are benign — do NOT backfill `bd sync` (discard churn, hand-apply your own beads; **committing `.beads/issues.jsonl` on the MAIN trunk needs the DB-move + `git commit --no-verify` workaround — or do the close in a worktree**, see memory `bd-commit-on-main-trunk-workaround`). The global git email is a PLACEHOLDER → set the worktree email to the GitHub noreply. Two parallel wiki sessions push to `origin/main` from their own worktree — rebase-sync at merge, never touch `vault/library/`. The author `reconciliation` envelope (compounds: `ae_profile`; bounce: `energy_budget`/`energy_cost_kcal`) is load-bearing. Carry-overs: `dfg1` (doc-freshness gate, PF-S73-01) / `df1x` (doc backlog) / `aem1` (ae-matching robustness) / `axm7` (gate-extend) / `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x` + PR#145/#147. Pencil: ALWAYS pass `filePath`.

## Current State (volatile)

- **S73 (2026-06-18) wired the supplement↔peptide additive-AE screen — the Phase-3 compound-band safety check.** `scripts/plan/orchestrate.py` `reconcile` behavior 4: a shared author-declared `ae_profile.additive_classes` token OR a pairwise `ae_profile.interactions` entry naming the other compound surfaces in `report["additive_ae"]` AND holds the supplement (`additive-ae-held`) before the existing `record_plan` (no new store stream). Real supplement-specialist (high-dose fish oil) + peptide-specialist (BPC-157) dispatches independently converged on a shared `bleeding-risk` axis → the additive pair is HELD (the supplement never reaches the store), the BPC-157 draft records + renders; the clean creatine↔BPC-157 pair records both. Envelopes under `docs/plan-generation/examples/compound-screen-*`.
- **PR #154** — plan-integrity + QA Tier-2 (2 stale-prose lines + a placeholder commit-email fixed; 5 edge-case tests added) → `/review-pr` 6-agent Tier-3 (4 deduped LEGITIMATE — one docstring caught by 4 agents + 3 coverage gaps — fixed + blind-verified 4/4 RESOLVED) → `/merge` (REST rebase under GraphQL exhaustion) on `main` (2026-06-18 S73 close). The per-PR skill-trace table is in `memory/process-failures.md` Session 73. The close artifacts landed via a follow-up `fix/s73-close` PR (the build PR merged first — a valid step-9 path, less tidy than S72's single-PR close).
- **PF-S73-01 promoted (post-close, at operator direction):** skipped close step 8 (Document Freshness Rubric) + under-reported it in the disclosure ledger; operator-surfaced. Fixed (PR #165 bumped the 3 stale docs); structural gate beaded `dfg1`. The build's OWN disciplines held — the worktree path-read discipline (no recurrence), and the malformed-`ae_profile` fail-open tested + documented as deliberate (fail-loud deferred to S74).
- **pytest 917/2 (trunk).** Core-capability gate green; negative-test floor green.
- **Beads:** `71s4` (the additive-AE screen is WIRED; the S74 medical-liaison gate + measure/adjust legs remain); `aem1` (the parenthetical-name interaction-match precision gap); `dfg1` (doc-freshness gate — PF-S73-01 structural fix) + `df1x` (doc-freshness backlog); `axm7` (extend the core-capability gate to the orchestrator); carry-overs `llna`/`0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x` + PR#145/#147 follow-ups.
...
**Historical (kept for reference):** `vault/sessions/session-73.md`.

## What Is Next (volatile)

### S74 — the medical-liaison terminal gate (closes the held line to operator-usable)

The additive-AE screen is wired. Next is the Phase-4 terminal adjudication that turns the clinician-gated compound DRAFTS (and the held additive-AE supplement) into operator-approvable plans, per `vault/design/plan-generation-pipeline-v1.md` Phase 4 — run in a dedicated `git worktree` (idle main trunk parked off `main` per PF-S71-01):

1. **The medical-liaison terminal safety gate** (Phase 4 — collates the doctor-visit queue + every risk HALT, runs BPMH reconciliation, adjudicates the held additive-AE findings + author-declared conflicts + the supplement↔Rx axis before operator approval). Dispatch the deployed `medical-liaison` for real (full profile inlined per INV-ROLE-INLINING); promote the `INV-OVERRIDE-RECORD-SCHEMA` / `INV-CRITICAL-NON-OVERRIDABLE` candidates (bead `mdv`) via the change-discipline ritual. Consider the malformed-`ae_profile` fail-loud upgrade here (the S73 deliberate-inert behavior is the fail-open baseline).
2. Then the **measure** leg (a production caller for `record_plan_tracking` — currently schema-only) and the **adjust** leg (re-plan from tracking) — the rest of the closed loop.

**Discipline:** scope contract at open; **run in a `git worktree` + keep the idle main trunk off `main`** (PF-S70-01 + PF-S71-01; Read worktree files from the worktree-absolute path); **write the close artifacts in the worktree BEFORE merging the build PR** (the cleaner S72 single-PR close — S73 needed a follow-up close PR); re-verify `git branch --show-current` before each commit; honor the CLAIM-PHRASING RULE; **verify the production path E2E with a real `medical-liaison` dispatch, not just unit tests**; store-surface writes satisfy `docs/checklists/store-adversarial-tests.md`; `/review-pr` → `/merge` before landing on main.

**Deferred (NOT blocking):** `dfg1` (the doc-freshness gate — PF-S73-01 structural fix, makes close step 8 fail-closed) + `df1x` (the doc-freshness backlog of pre-existing stale docs); `aem1` (the parenthetical-name interaction-match precision gap); fail-loud on a malformed `ae_profile` (an S74-gate candidate); extend the core-capability gate to the orchestrator (`axm7`); the standalone full-plan render screen (operator drafting it); the dashboard reskin in flight (operator's `aplus-reskin` worktree); the `/generate-plan` slash-command wrapper (the orchestrator is wired as the callable); the PR#145/#147 review follow-ups; the `llna` hook fix; carry-overs `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`; operator-gated July-visit prep (LM-01) — first session on/after **2026-06-29** runs the LM-01 scoped drift audit.

## Landmark window check (close step 8.7)

S73 close (2026-06-18): RE-OPENED `vault/meta/landmarks.md` at close step 8.7 (unchanged). No landmark edited — S73 was plan-generation code on PII-free synthetic fixtures (no operator data ingested, no Whoop baseline start, no MD-handoff artifact). LM-01 (First MD visit) **2026-07-13**: no trigger window open today (the 14-day scoped-drift-audit window opens **2026-06-29**, the 7-day MD-handoff window 2026-07-06). LM-02 (Whoop) / LM-03 (23andMe) date-TBD. LM-04 (first HTML artifact) NOT triggered — the dashboard rendered only to a tempdir (synthetic fixtures); no committed HTML artifact this session. No landmark actions due.
