...
### S33 wired the per-source adapters. Next = the scheduler (`oaf`) + data-out.

**RESUMPTION POINT.** S33 built `ADR-0003-T2` (four adapters + the two 0-edit extensibility proofs), rebase-merged (PR #50); closed `n9h` → `oaf` unblocked. **Open the next session on `main`** (Start Protocol + `branch-completeness-audit.sh` at open).

**Next pipeline work — Wave-3 completion + data-out:**
1. **Frontier:** `bd ready` now surfaces `oaf` (`ADR-0003-T3` scheduler — unattended `ingest.run` over the WIRED adapter set [HealthKit+Oura+Garmin], delta-only, 0-egress; Whoop excluded; the literal `rg "whoop" scripts/ingest/scheduler.py = 0` Wave 4→5 boundary check lands HERE once `scheduler.py` exists — the PR #50 AC-5 deferral). Parallel-ready (in `bd ready`): `gu4` (`ADR-0004-T1` render engine — unblocks the most downstream: `3gp`/`1ih`/`8cv`), `xlu` (`ADR-0005-T1` pre-commit hook — reuses `pii_scan.scan`; **resolve `qwj` first**), `br1` (`ADR-0006-T0` router spike). READ each recipe `docs/task-plan/<id>.md` IN FULL first (PF-S17-01), TDD with `.venv/bin/python -m pytest`, then `bd close`.
2. **Published surfaces the next wave consumes:** the `Adapter` Protocol + the four adapters (`source_tag` + `read_readings`) — `oaf`'s scheduler assembles the wired set (`healthkit`/`oura`/`garmin`; `whoop` stays unwired). `ingest.run(adapter, export_file, root=…)` — `oaf` invokes it. `store.append`/`read` (path-safe, corruption-tolerant). `keying.py` single key (never redefine — re-opens N3). `egress_guard.run(operation)` backs 0-egress checks. `pii_scan.scan` for `xlu`'s hook — **resolve `qwj` first** (≈924 `scan` hits across vault prose → the hook would block nearly every commit).
