...
### S97 Scope Contract Evaluation (2026-06-25, volatile)

- **AC1 (ADR) — PASS.** `/create-adr` finished on the S97-drafted ADR-0028: VERIFY (PASS, 0 blocking) → RED-TEAM (caught the load-bearing AR-001 — reauthor/adjudicator fire inside the byte-frozen engine, so direct-yield contradicts numstat=0) → FIX (the two-mechanism resolution: AUTHOR/GATE direct-yield; REAUTHOR/ADJUDICATOR throw/replay-memo via a `BaseException` sentinel + fresh-scratch replay + charge-on-cache-miss, keeping the engine byte-frozen) → JUDGE (REVISE: a false `BaseException` justification citing a nonexistent `except Exception` + 3 imprecise citations) → bounded revise (recast to the true motivation; corrected refs). Inverse Related-Decisions edges backfilled into ADR-0022/0023/0024/0026/0027; `status: accepted`. Merged PR #255 → `main`.
- **AC2 (design pipeline) — PASS.** `/create-spec` (5 tasks, binary mock/fixture-satisfiable ACs; validate 11/11 + judge ACCEPT all 11 dims ≥9; constraint-preservation + live-repo grounding both 10 on independent re-measurement) → #256. `/create-build-plan` (4 waves T1→T2→T3→{T4,T5}; Architect-authored full-profile; validate 24/24 + judge REVISE→fixed: added the Wave-2 consumer no-fork release-grep the judge caught missing) → #257. `/create-task-plan` (5 TDD recipes, 5 SE authors full-profile + grounded; judge ACCEPT every recipe all-dims ≥9, no same-wave file collision, every crown-jewel/no-fork AC RED-capable) → #258. All merged.
- **AC3 (build) — PASS.** `/execute-plan` built all 5 tasks TDD-per-recipe on `feature/adr-0028-build`: T1 (`24d9b0b`→`4642ac6`), T2 (`3abbac7`), T3 (`feac2b3`), T4 (`4803027`), T5 (`994bf18`). Each wave's checkpoint EXECUTED green by the orchestrator (suite green, inner-engine numstat=0, no-fork 1/1/1, replay-safety equality mutation-proven, value-scan RED-capable, non-tautology proven). EXTEND-NOT-REBUILD held every wave; crown-jewel PII 0-leak; mock/fixture-tested, 0 live spend. (Wave-3 was finished from interrupted-SE partial work after the account switch — diagnosed sound + completed to GREEN.)
- **AC4 (review→merge) — PASS.** Tier-2 Wave-1 wave review caught a MUST-FIX (the GATE shipped the composed callable, violating ADR OQ-2 → the skill couldn't dispatch judge/lenses as agents) → fixed (GATE yields `(assembled_plan, gate_producer)` raw-verdict shape, composition extracted to the ONE `compose_disposition` site) → Architect re-review RESOLVED (executed-verified). A full Tier-3 `/review-pr` 6-agent over the integrated diff with independence INTACT (profile-less blind-triage + EXECUTED blind-verify) caught 4 LEGITIMATE findings — **BUG-1** (the step-harness infinite-loops on a `None` hook return, the safety-critical safe-default outcome; 3-agent convergence, RUN-confirmed) + BUG-2 (the harness GATE-raise fail-closed path dead) + QUAL-1/QUAL-2 (stale doc refs) — all fixed + blind-verified RESOLVED with mutation→RED non-vacuity → `/merge` PR #259 → `main` `747ff39`.
- **AC5 (close) — PASS** (this close, on final post-merge main `747ff39` as of 2026-06-25, SHA cited per PF-S74-01).

### Drift checks (S97 close)

- **Task drift — none (scope EXPANDED by the dry-run finding, anticipated by the directive).** The session delivered exactly the operator's "run the whole build pipeline to close" directive. The 0-spend dry-run that opened the work found the live run was NOT executable (the gate/hooks were synchronous Python callables) — so the scope became "design + build the fix (ADR-0028) to make it executable," which IS the direct path to the live run the operator wants. Not silent drift: the dry-run finding + the option-A choice were surfaced + operator-approved ("proceed with A") before the pipeline ran.
- **Architecture drift — none toward violation; net HARDENING.** The A′ control-inversion is completed (the skill can now dispatch the judge + safety lenses + reauthor/adjudicator as subscription agents, non-forkingly). NO-FORK held (1 composition site / 1 surface gate / 1 release site across all 4 consumers); EXTEND-NOT-REBUILD held (inner engine + `deid_in.py` numstat=0); the fail-closed disposition + the de-id value-scan (S96) preserved (Historical-Context confirmed). The layered review caught + fixed every defect PRE-MERGE (the OQ-2 GATE violation at Tier-2; BUG-1's safe-default-hang at Tier-3) — net safety-loop hardening.
- **Vision drift — none.** Still the local-first health tracking + planning system; this session made the operator-facing GENERATE front door genuinely executable. `design/vision.md` first sentence unchanged.

### Per-PR gated-skill invocation table (INV-SKILL-TRACE)

| PR | `/review-pr` invoked fresh | `/merge` invoked fresh | Outcome |
|----|----------------------------|------------------------|---------|
| #259 — the integrated ADR-0028 A′ build (5 tasks T1-T5) | YES — FULL 6-agent (Security/Bug-Hunter/Code-Quality/Test-Coverage/Contracts/Historical-Context) over the local diff `main..994bf18`, GraphQL exhausted; the `/review-pr` skill invoked in full (fail-closed pre-flight + Phase-0 context + Phase-1 6 agents + Phase-2 synthesis + Phase-3 profile-less blind-triage + Phase-5 fix + Phase-6 test gate + Phase-7 EXECUTED blind-verify); PF-S96-01 guard honored (a multi-file crown-jewel code PR → the FULL gated review, not a proportionate subset) | YES — `/merge` methodology via REST fallback (GraphQL exhausted): readiness + `git merge-tree` conflict-check + full-40-char SHA guard (`0e51628…`, the reviewed+verified post-fix tip) + rebase + branch cleanup local+remote | Merged → `main` `747ff39` (2026-06-25 S97); 4 LEGITIMATE (BUG-1 harness None-hang [Critical, 3-agent convergence] / BUG-2 harness GATE-raise dead path / QUAL-1 stale _safe_gate refs / QUAL-2 SKILL.md composition-site mislabel) fixed + blind-verified RESOLVED; Security PASS + Contracts PASS; quality gate PASS post-fix |
...
S97 close (2026-06-25): **One new PF promoted — PF-S97-01** (review agent ran `git checkout`/`reset` in the shared checkout, corrupting the build tree). During the Wave-1 Tier-2 review a review agent issued `git checkout`/`reset`/`commit` ("test probe" commits) in the SHARED main checkout while the build was active, moving HEAD off the build commit + leaving the tree at the pre-build state; QA caught it via reflog; recovered cleanly (the branch ref was intact), no work lost. Captured in ALL of this PF-log + `harvest.jsonl` + bead `a-plus-maxing-fbyr`. Guard: every review/verify agent dispatch carries an explicit READ-ONLY-git hygiene clause (applied for the rest of S97 — the Tier-3 6-agent + blind-triage + blind-verify all used read-only git cleanly, mutating only `/tmp` copies or `git restore`-ing tracked files). The **defining POSITIVE:** the comprehensive integrated Tier-3 review CAUGHT a real crown-jewel liveness/safety defect (BUG-1 — the SAFE-default outcome [declined re-author / declined adjudication → domain HELD] hangs the production skill path FOREVER) that all 4 per-wave EXECUTED checkpoints AND the green 1718-test suite missed — the exact PF-S87-01 "components pass their tests, the integration breaks" pattern, caught for $0 PRE-MERGE by the layered review working as designed (a per-wave checkpoint tests one task in isolation; only the integrated review composes all 5). Also the Tier-2 Architect MUST-FIX (the OQ-2 GATE violation) was caught + fixed + re-verified pre-merge. Observed but did NOT promote: (i) the account-switch-mid-build left interrupted Wave-3 SE partial work uncommitted in the tree — diagnosed sound + completed to GREEN (handled per the don't-trust-but-verify discipline, the harness was correct, no defect); (ii) the per-wave EXECUTED checkpoints missing BUG-1 is NOT a checkpoint failure — they verify each task in isolation by design; the integrated final review is the layer that catches cross-task integration defects, and it did. Prior lessons referenced DESCRIPTIVELY (the never-hand-roll-the-build discipline held — `/create-adr`/`/create-spec`/`/create-build-plan`/`/create-task-plan`/`/execute-plan`/`/review-pr`/`/merge` each in full; the run-the-FULL-gated-review-for-a-multi-file-code-PR discipline APPLIED [the lesson promoted last session]; the full-role-profile-inlining discipline — the role-inlining hook DENIED two abbreviated dispatches this session, re-run complete; the commit-on-the-feature-branch-in-the-main-checkout discipline; the per-task-not-batched remediation discipline; the autonomous-loop discipline — the entire design→build→review→merge→close ran continuously), never by bare token.
...
- The live-run-not-executable gap — a 0-spend dry-run found the gate + reauthor/adjudicator dispatches were synchronous Python callables the skill couldn't fulfill with subscription agents — detection: self (the dry-run I proposed + ran); surfaced_by: self. Surfaced to the operator as the option-A decision (approved "proceed with A"); the whole ADR-0028 build is the fix.
- The OQ-2 GATE violation — the Wave-1 build shipped the GATE as a composed callable (the ADR-rejected shape), so the skill couldn't dispatch the judge/lenses as agents — detection: gate (Tier-2 Architect Implementation-Review); surfaced_by: self. Fixed (raw-verdict producer; composition extracted to the ONE site) + Architect re-review RESOLVED (executed) before merge.
- BUG-1 (Critical) — the step-harness skips caching a `None` hook return, so the safe-default outcome (declined re-author / declined adjudication → domain HELD) infinite-loops the production skill path — detection: gate (Tier-3 Bug-Hunter + Test-Coverage + Historical-Context, 3-agent convergence, RUN-confirmed) + profile-less blind-triage LEGITIMATE; surfaced_by: self. Fixed (`_UNFULFILLED` sentinel, cache None) + blind-verified RESOLVED (mutation→RED) before merge.
- BUG-2 — the harness GATE-producer-raise fail-closed path is dead (`_send_round`'s BaseException branch unreachable; `_json_native` rejects exceptions → crash instead of graceful SAFETY_BLOCKED) — detection: gate (Tier-3 Bug-Hunter + Historical-Context); surfaced_by: self. Fixed (JSON-safe `{"__gate_raised__"}` marker → `gen.throw` → SAFETY_BLOCKED) + blind-verified RESOLVED before merge.
- QUAL-1 + QUAL-2 — stale `_safe_gate` comment refs (a symbol this PR deletes); SKILL.md mislabels `compose_gate_dispatch` as "the ONE composition site" (the no-fork governance term) — detection: gate (Tier-3 Code-Quality) + blind-triage LEGITIMATE; surfaced_by: self. Fixed + blind-verified (inspection-only) before merge.
...
**Historical (kept for reference):** `vault/sessions/session-97.md`.
...
1. **ADR-0029 SPA front-end intake wiring is COMPLETE + MERGED — the core capability is UNBLOCKED + mock-proven end-to-end; the ONLY remaining step is the operator-present LIVE run.** main @ `32706c8` (2026-06-26 S98 close): ADR-0029 (accepted) → spec → build-plan → 5 recipes → 3-wave build (T1 SPA render-view + GET / re-point + inline assets, T2 live converse, T3 SPA Upload form/chat wiring, T4 not-naive elicitation proof, T5 chat→usable-plan E2E) → Tier-2 reviews → Tier-3 6-agent `/review-pr` (4/4 LEGITIMATE → fixed → 4/4 RESOLVED) → rebase merge (PR #262). Suite 1760/3; frozen-engine numstat=0; core-cap audit exit 0. The SPA is served at GET /; the Upload chat POSTs the live `/chat`→`converse`; the E2E proves recorded==True + content-traceability (mock-tested). **REFRAME: the build is COMPLETE + MOCK-PROVEN; the operator-present LIVE run (real key + real data + spend) is the ONLY remaining step to a usable real plan — and the marquee BUG-1 fix means the live chat will actually work, not degrade every turn.**
2. **Before the operator-present LIVE run with REAL PII: land the value-scan residuals + observe the live-dispatch 0-leak + wire the live `author`.** `6hts` (de-id value-scan recall residuals) + `stsq` (SEC-3 live-subscription-dispatch 0-raw-PII). The live `author` leg (`_ClaudeNoTrainBackend.author` is STILL `NotImplementedError`; ADR-0015-T3; T5's E2E mock-tests it) is the remaining live-path wiring before the full real conversation→real plan works end-to-end. Safety follow-ups `s7qz`/`sc97`/`4oha` open.
3. **Standing mechanics (all HELD) + 2 NEW review-agent hardenings.** Full pipeline build (PF-S36-01, ran in full); FULL gated `/review-pr` for multi-file/security code (PF-S96-01 — APPLIED on #262, caught BUG-1); 3-tier review with profile-less blind-triage + EXECUTED blind-verify; EXTEND-NOT-REBUILD numstat=0; full role profiles inlined VERBATIM on EVERY role dispatch; **NEW PF-S98-01 (`9x5r`): review/verify agents are READ-ONLY + must NOT mutate system state — a review agent killed the operator's port-8765 server despite an explicit prohibition; harden review-agent dispatches (absolute read-only, forbid process-kill)**; **NEW PF-S98-02 (`6s8v`): set `OBJC_DISABLE_INITIALIZE_FORK_SAFETY=YES` on EVERY pytest run — the known fork-safety crash spammed the operator before mitigation**; `/merge` via REST when GraphQL exhausted (full-40-char SHA guard); `git add` SEPARATE from `git commit`; review/verify agents use READ-ONLY git ONLY (PF-S97-01); single trunk; stage close docs by EXPLICIT path; never stop mid-loop (PF-S92-01); re-run close gate on final main + cite SHA (PF-S74-01).

## Current State (volatile)

- **S98 (2026-06-26): ran the ENTIRE build pipeline (create-adr → spec → build-plan → task-plan → execute-plan → review-pr → merge) autonomously + merged ADR-0029 — the operator-approved SPA front-end is now WIRED to the live intake backend.** The session pivoted off the abandoned impeccable design tooling: the operator-approved app.html shell (hand-built earlier this session) became the V1 front-end `vault/design/templates/app_shell.py`, served at GET /, superseding the `intake.py` wizard realization.
- **The full gated pipeline, each stage through its own gate:** `/create-adr` (ADR-0029 + 8 bidirectional backfills; verify+judge caught the `images/body.png` inline-asset miss; red-team caught the fabricated-dashboard-data vs ADR-0009 D1/D2 tension → honest `tensions-with` + OQ-3) → `/create-spec` (live-repo-grounded: the backend was already built — only `converse` + the never-written E2E were the real gaps → right-sized to 5 tasks) → `/create-build-plan` (judge caught a broken prereq grep) → `/create-task-plan` (5 recipes, all ≥9) → `/execute-plan` (3 waves, per-wave EXECUTED checkpoints + Tier-2 QA/Security/Architect/design-reviewer, legitimate findings fixed) → Tier-3 `/review-pr` → rebase merge.
- **The Tier-3 6-agent review caught the marquee integration defect the mock-tested build missed:** BUG-1 — the live `converse` would forward an Anthropic-Messages-API-invalid `messages` array (a `role:"system"` dict at index 0) → every live chat turn 400s → degrades; the mock suite was blind (it bypasses `messages.create`). Fixed (the de-id context now rides as a JSON-string `user` turn — API-valid + PII-free) + a non-mock structural test added; blind-verified by EXECUTION. 3 other findings (duplicated pagehead, untested labs-loaded branch, stale docstring) fixed.
- **The core capability is now UNBLOCKED + mock-proven end-to-end:** SPA served → Upload chat POSTs `/chat` → live `converse` → de-id extraction → store → `summarize` → `generate_plan` → usable plan (recorded==True + content-traceability). Only the operator-present LIVE run remains.
- **Beads:** closed `w3y8`/`f0gh`/`8f1h` (the intake-wiring work, realized via ADR-0029). New: `9x5r` (PF-S98-01), `6s8v` (PF-S98-02). Carry: `6hts`/`stsq` (live-run gates), `s7qz`/`sc97`/`4oha` (safety follow-ups), `myts`/`fbyr`/`zsp5`.
- **main @ `32706c8` (as of 2026-06-26 S98 close, PF-S74-01). Vault perpetually daemon-dirty (stage by explicit path).**

**Historical (kept for reference):** `vault/sessions/session-97.md`.

## What Is Next (volatile)

### After S98 — the operator-present LIVE run is the remaining step to a usable real plan

The build is COMPLETE + mock-proven. To produce the operator's ACTUAL plan:
1. **Operator runs the intake live:** start the loopback server (`python -m scripts.serve` → 127.0.0.1:8765), open the SPA at GET /, fill the Upload Documents demographics + upload documents, and talk through goals/training/nutrition/supplements/peptides in the Upload chat. The operator injects `export ANTHROPIC_API_KEY=$(security find-generic-password -s "quant-primary-api" -w)`; the agent NEVER touches the keychain. (BUG-1's fix means the live chat now actually works rather than degrading every turn.)
2. **Before REAL PII / for the FULL real-conversation→real-plan path:** land `6hts` (de-id value-scan residuals) + observe `stsq` (SEC-3 live-dispatch 0-leak); wire the live `author` (`_ClaudeNoTrainBackend.author`, still `NotImplementedError` — ADR-0015-T3; T5 mock-tests it) so the plan-author leg runs live too.
3. **Then `/generate-plan`** over the operator's de-identified real data → a usable plan + doctor-visit queue + the dashboard made live (the Dashboard/Plan screens currently render honest awaiting-states — ADR-0029 OQ-3's per-surface live-wiring is the follow-on).

**Deferred / NOT blocking:** `zsp5`; the safety follow-ups `s7qz`/`sc97`/`4oha`; `02m1` (daemon config); the local-model migration (North Star, post-V1). The LM-01 scoped-drift-audit window opens 2026-06-29 (see Landmark check).

## Landmark window check (close step 8.7)

S98 close (2026-06-26): re-read `vault/meta/landmarks.md`. The ADR-0029 build is on mock clients + synthetic fixtures (no real operator data, no live API, no compound/biomarker page edits). LM-01 (First MD visit) **2026-07-13**: today (2026-06-26) is BEFORE the 14-day scoped-drift-audit window open (**2026-06-29**, 3 days out) → no trigger today. **NOTE for the next session on/after 2026-06-29: run the scoped drift audit on the LM-01 relevant_scopes** (operator-profile January-2026 section + `risk_tier: medium+`/experimental compounds + baseline-panel biomarkers + the doctor-handout queue); the 7-day MD-handoff window opens 2026-07-06. LM-02/03/04 not triggered. No landmark ACTIONS due this session.
