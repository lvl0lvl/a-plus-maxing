...
1. **AP-PROTOCOL-FROM-MEMORY / read-before-invoke (PF-S13-01 + PF-S17-01) — STANDING; `execute` is live and now MID-FLIGHT (Wave 1 done, Wave 2 next).** Each task's recipe is the gate — READ it IN FULL before executing (HELD this session for all 3 Wave-1 recipes). **Wave 2 builds REAL `pytest` production modules** (the store lib + PII guard under `scripts/`) — run each recipe's TDD cycle, never from memory. Run a wave from memory instead of its recipe and the class recurrence promotes.
2. **PII enforcement is the V1 critical path — Wave-1 DESIGN now FIXED (S30), but the GUARD ITSELF IS UNBUILT.** The egress mechanism (portable OS-level isolation w/ per-OS `sandbox-exec`/`unshare` dispatch, fail-closed, subprocess-catch) + the PII scan (biomarker-independent structural contents-search) are designed; Wave-2 `e9m` (`ADR-0001-T1`) builds `egress_guard.run` + `pii_scan.scan` failing-capable. **CARRIED FLAG:** `pii_scan.py` MUST search file CONTENTS (`xargs -0 rg`/args), with a failing-capable test injecting PII into a tracked file's CONTENTS (the blind-reverify caught a false-passing filename-stream scan). Enforcement-first holds: frontier = the PII guard `e9m` + store `89a`, before any plan-reasoning task.
3. **PF-S22-01 falsification window — still live for the parallel library-population track.** Research-only `/aplus-research` sessions remain the first parallelized track after S22. At each batch close: ONE merge target, `branch-completeness-audit.sh` + `wiki-lint.sh` green. Unchanged by Wave 1.

**Demoted from prior Top-3:** the "`execute` is the last UNSTARTED gated stage" framing of #1 (now mid-flight — Wave 1 executed); the "recipe-pinned but unbuilt" framing of #2 (Wave-1 design now FIXED + decision-recorded; the build is what remains).

## Current State (volatile)

- **WAVE 1 OF THE V1 BUILD IS EXECUTED (S30, 2026-06-05)** — the first execute-stage session. The 3 dependency-free design/measurement spikes ran per their recipes via dispatched Architect workers + independent review + blind re-verify: `ADR-0001-T0` (PII-boundary), `ADR-0002-T0` (store-keying), `ADR-0004-T0` (render-size). Pipeline: PRD→ADR→spec→build-plan→task-plan→**execute Wave 1 [S30] ✓** → Wave 2 (UNBLOCKED).
- **The deliverables are 3 GITIGNORED design reports** in the working-tree `docs/spec/.pipeline/` (consumed by Wave 2 from the working tree — not in git; they survive branch ops). Their decisions are recorded durably in `vault/sessions/session-30.md` (the provenance hedge): egress = OS-level network isolation w/ per-OS `sandbox-exec`/`unshare` dispatch (fail-closed, subprocess-catch); store = one NDJSON file/item under `vault/store/`, ISO-8601-to-second + `source`, fields `{item,timepoint,source,value}`, dedupe `(item,timepoint,source)`; render = **57741 bytes** worst-case (10×4), cap **16 series × 12 timepoints**.
- **The review chain caught + fixed 4 REAL findings on the PII critical path** (0 suppressed, PF-S26-01): a macOS-portability blocker (`unshare` absent → OS-level isolation w/ per-OS binding), 2 PII-scan coverage gaps (→ structural contents-search detector), and a false-passing scan command (→ `xargs -0 rg`). Mechanical checks passed all of these; the independent review + blind re-verify caught them.
- **SINGLE TRUNK** unchanged (since S22): `main` = complete project. **NO production code built this session** — the Wave-1 spikes are non-code design/measurement; tracked changes = HANDOFF + `session-30.md` + log + 3 bead closures, merged via the S30 close PR.
- Beads `394`/`bez`/`qbb` **CLOSED**; `bd ready` frontier = Wave 2 (`89a` ADR-0002-T1 store, `e9m` ADR-0001-T1 PII guard).
- **Active landmarks:** no trigger windows opened.

**Historical (kept for reference):** `vault/sessions/session-30.md` + `vault/meta/log.md` S30 entry.

## What Is Next (volatile)

### S30 executed Wave 1 (3 spikes). Next = Wave 2 — build the store lib + the PII guard.

**RESUMPTION POINT.** S30 fixed the Wave-1 design/measurement decisions (PII-boundary, store-keying, render-size) via 3 gitignored spike reports (decisions recorded in `vault/sessions/session-30.md`) + closed beads `394`/`bez`/`qbb`. **Open the next session on `main`** (Start Protocol + `branch-completeness-audit.sh` at open).

**Next pipeline work — execute Wave 2:**
1. **Frontier:** `bd ready` surfaces `89a` (`ADR-0002-T1` store + `keying.py`) and `e9m` (`ADR-0001-T1` egress/PII guard). Work each by READING its recipe `docs/task-plan/<id>.md` IN FULL first (PF-S17-01), running its TDD cycles — **Wave 2 builds REAL `pytest` production modules under `scripts/` now (no longer design-only)** — satisfying its Verification Checklist, then `bd close`. The build plan's Wave-2 boundary gates the next wave.
2. **The Wave-1 spike reports** are in the working-tree `docs/spec/.pipeline/` (gitignored); `ADR-0002-T1`/`ADR-0001-T1`'s entry states `test -f` them — they persist in this checkout. Their decisions are also in `session-30.md` if the working-tree copies are ever lost (e.g. `git clean -x`).
3. **Enforcement-first holds:** `e9m` (PII guard) + `89a` (store) are model-independent infrastructure; no plan-reasoning-over-PII task is buildable until the router (`ADR-0006-T0`/`T1`) lands in later waves. Wave 2 needs the egress-capture harness available (`sandbox-exec` on macOS — present) per `ADR-0001-T1`'s entry state.

**Carried flags for Wave 2** (surfaced, not lost — PF-S26-01):
- **`ADR-0001-T1` (`e9m`):** `pii_scan.py` MUST search file **CONTENTS** (`xargs -0 rg`/args form, per the corrected `ADR-0001-T0` command), with a **failing-capable test injecting operator-PII into a tracked file's CONTENTS** and asserting the scan catches it — guards against reintroducing the false-passing filename-stream scan the blind-reverify caught. The egress guard must catch out-of-process/subprocess egress + fail-closed (Security HIGH-1, per the corrected ADR-0001-T0 mechanism). **Recipe/spike drift to reconcile at build:** the `ADR-0001-T1` (+ `ADR-0004-T2`/`ADR-0007-T2`) recipe text still names the egress harness as the pre-remediation "syscall/socket interceptor OR offline-namespace run" menu — the build target is the spike's FINAL selection (OS-level network isolation w/ per-OS `sandbox-exec`/`unshare` binding; the interceptor was explicitly REJECTED), not the stale recipe menu.
- **`ADR-0002-T1` / `ADR-0003-T1`:** consume `keying.py` as the single shared key/field-set module — don't redefine the layout/field-set/dedupe-tuple without Architect review (re-opens N3).
- **`ADR-0004-T1`:** re-run the render-size measurement against the REAL template before `ADR-0004-T2`/`ADR-0007-T2` consume the cap (re-derive with the same headroom, or confirm the harness bound holds).
- **Carried from S29 (later waves):** the unnamed plan-`template` producer for `render.emit` (ADR-0005-T2 Deviation #2 + ADR-0006-T2 Upstream Flag, bead `4xe`) — resolve the Architect adjudication before the plan-render step.
...
**Operator-data preconditions** (Walter-pending; feed personalization at execute): 23andMe raw → `vault/dna/raw/`, the Oura/Apple-Watch/Garmin exports (the named first-cut adapters), meal-template content, January-2026 issue characterization. The three meta files (`operator-profile`/`current-state`/`goals`) remain `status: scaffold`.

### Open beads carried
- **V1 build execution (label `v1-build`):** 18 per-task beads, dependency-wired; 3 closed (Wave 1). READY now: Wave 2 — `89a` (`ADR-0002-T1` store) + `e9m` (`ADR-0001-T1` PII guard). Plus `e3d` (recipe-set doc-consistency normalization — review F3/F4/F5/F6, non-blocking).
- **V1 spec/build-plan gaps (NEW S29, surfaced during recipe authoring — separate follow-ups, NOT execution-blocking):** `kz6` (pytest bootstrap missing from task manifests), `12p` (build-plan dep-map omits ADR-0002-T1→ADR-0001-T1 gitignore edge), `434` (ADR-0004-T1 palette/color-distance design source), `bpu` (ADR-0005-T1 hook-contract under-spec), `dv3` (pre-push/CI pii_scan backstop), `4xe` (ADR-0005-T2 crit-3 / plan-template Wave ordering).
- **P2:** wiki-ingestion ADR-backfill (`dke`), `3v5`, `xg4`, `382`, `w3n`, `5bd`, `5l9`/`78p`, `pmp`, `h1z`, `rc1`.
- **P3:** `0oy` (Spec-B ADR-0006-T0 dep-map inconsistency), `ko5` (ratify the vault-git-tracking policy vs ADR-0005), `75t` (wiki-schema/source-whitelist `depends_on` freshness), `ae0`/`d6g`/`4ba`/`3v6`/`dip`, `t7z`/`fsr`/`8qe`, `r7t`/`7rm`/`60f`, `5jr`, `9c5`/`pnl`/`2n1`/`4h1`/`smw`, plus pre-existing `1ek`/`6ln`/`mdv`/`1rm`/`2gs`/`623`/`f2r`/`yfu`/`2qq`/`p47`/`o9y`/`7is`/`mdg`/`5by`/`1ox`/`9yk`.
- **Closed:** S30: `394`/`bez`/`qbb` (Wave-1 spikes — execute Wave 1). S29: `mo4` (task-plan stage — merged PR #40). S28: `hv6` (merged PR #38). S27: `rg2`. S25: `fm4`, `hil`. S23: `bte`. S22: `gdw`. S21: `mhg`/`5ot`/`0be`.

### Open project work (unchanged)
- Vault git-tracking decision deferred (`ko5`). First HTML artifact (LM-04) is architecturally placed by ADR-0004 (on-demand single-file generation) + recipe-pinned (`ADR-0004-T1`/`T2`/`T3`, `ADR-0007-T2`); built when execute reaches Waves 3-7.

## Landmark window check (close step 8.7)

All 4 active landmarks (LM-01 doctor visit July 2026, LM-02 Oura/wearable, LM-03 23andMe, LM-04 first HTML artifact) — no trigger windows opened during S30 (2026-06-05). LM-04 (first HTML artifact): the render-size spike `ADR-0004-T0` is now executed (it fixes the byte cap that the first artifact must respect); its generation tasks (`ADR-0004-T1`/`T2`/`T3`, `ADR-0007-T2`) remain recipe-pinned but unbuilt (Waves 3-7). LM-01's 14-day-before window still depends on the TBD July exact date (not yet within window as of 2026-06-05); LM-02/03 remain Walter-pending. No status flips due.
