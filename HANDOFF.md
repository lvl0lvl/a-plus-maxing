...
## Scope Contract — Session 84 (2026-06-21)

Goal: Execute the operator's standing P1 directive (bead `9qx5`) — run the independent PR-#205 review phases collapsed under context pressure in S83 (a profile-less BLIND TRIAGE [review Phase 3] + a profile-less EXECUTED BLIND VERIFY [review Phase 7] against merged `main`), in a FRESH session, then fix-forward or bead anything they surface.

Acceptance criteria:
- [x] AC1 (blind triage): a profile-less agent independently classifies each PR-#205 finding cluster against merged `main` (`8b52019`), with NO access to the S83 verdicts or the fix diffs.
- [x] AC2 (executed blind verify): a profile-less agent RUNS the behavior behind each claimed fix + reports RESOLVED / STILL_PRESENT with executed evidence; confirms the deferred items are genuinely still-open (not silently regressed).
- [x] AC2b (egress floor): the model-free + network-free ingest+render path re-confirmed by EXECUTION (sockets blocked).
- [x] AC3 (fix-forward or bead): any divergence fixed via a new PR or beaded; agreement recorded.
- [x] AC4 (close): full automatic close on final post-merge `main` + cite the SHA (PF-S74-01); `9qx5` closed.

Files I WILL touch: NONE in the product tree (no divergence surfaced); HANDOFF / `memory/process-failures.md` / beads / `vault/sessions/session-84.md` / `vault/meta/overview.md` at close.
Files I will NOT touch: the stranded `feature/apple-health-adapter` trunk checkout + its parallel-wiki uncommitted `vault/` changes; `scripts/ingest/{ingest,adapter,scheduler}.py`; the dashboard (design-LOCKED); `vault/library/`; `main` directly.
NOT doing: new ingest features; the labs adapter; the DNA clinical-SNP analysis; the interactive "B" build; loading real operator data; fix-forwarding the correctly-deferred review minors (`07f6`/`gzf7`/`dw8u`).
Invariants at risk: INV-SKILL-TRACE (this IS the remediation of the S83 INV-SKILL-TRACE deviation — the independence phases now run via SEPARATE profile-less agents); INV-CLOSE-AUDIT / INV-PF-ATTESTATION / INV-HARVEST-CAPTURE at close; INV-BRANCH-NOT-MAIN. **Core-capability-first gate (PF-S63-02): SATISFIED — the core capability is wired+invokable (`/generate-plan`, S81; the closed loop, S70-S80); S84 is a rigor-remediation of already-merged secondary work, the operator's explicit P1 first-task.** PF-S74-01: re-run the full close on final post-merge `main` + cite the SHA.

### S84 Scope Contract Evaluation (2026-06-21, volatile)

- **AC1–AC2b — PASS.** Two profile-less general-purpose agents (opus), each in its OWN clean worktree off `origin/main` @ `8b52019`, with no access to the S83 verdicts or fix diffs. BLIND TRIAGE (read-only): the 5 must-fix findings (TEST-001; the source-of-truth cluster QUAL-1/API-205-01/HIST-1; QUAL-2/API-205-03; API-205-02/HIST-3) independently confirmed RESOLVED in merged source (re-verified by execution + the 22/22 ingest suite, NOT by trusting commit `7376a53`); SEC-205-01 DEFERRED-OK; HIST-2 NOT_A_BUG (whoop IS wired — the reviewer misread the docstring); BUG-205-01 + TEST-003/004/005 + QUAL-3/4/5 DEFERRED-OK. EXECUTED VERIFY: items 1–7 RESOLVED with executed evidence — zip-slip member-escape False, filename HTML-escape holds, the `--source` test proven non-vacuous by a RED mutation, the congruence test fails loud both ways (bogus-add + UNWIRED-flip), `wearable_status` public+correct, intake self-contained, AND the full ingest+render path runs with sockets BLOCKED (egress-free by execution). 8a/8b confirm the deferred items genuinely still-open (no size cap; the `_dropzone` file-case raises `NotADirectoryError` — a loud crash, NOT a silent misclassification).
- **AC3 — PASS (no divergence).** Both independent passes converged: the S83 merge was SOUND. Nothing actionable beyond the already-correctly-deferred beads; no new escaped defect. `dw8u` sharpened with the executed BUG-205-01 repro; `9qx5` closed with the evidence.
- **AC4 (close) — PASS.** This close; the close gate re-ran on final post-merge `main` + the SHA cited (PF-S74-01).

### Drift checks (S84 close)

- **Task drift:** all ACs PASS. The session held EXACTLY to the bead-`9qx5` directive — no product code touched, no scope expansion; the one judgment call (NOT fix-forwarding the beaded minors) is per the audit-surfaced-fixes-are-exceptions discipline + both independent agents' deferral verdict, flagged not silent. No drift.
- **Architecture drift:** no invariant degraded — the session is verification-only; the product tree is byte-unchanged. The remediation RESTORES the independence guarantee INV-SKILL-TRACE exists to protect (the S83 deviation is now retroactively covered by the proper profile-less Phase 3/7). Toward the vision.
- **Vision drift:** none. The system's capability is unchanged; what changed is that the S83 merge is now INDEPENDENTLY CONFIRMED sound. `design/vision.md` first sentence unchanged. No drift.

### PF attestation

S84 close (2026-06-21): **No new PF-class entries this session.** The session executed the S83 remediation (bead `9qx5`) — the independent blind-triage + executed blind-verify the S83 disclosure flagged as collapsed — in a FRESH conversation, exactly the lesson the S83 recurrence-watch named; both passes confirmed the merge was sound. The verify-first / executed-evidence / profile-less-independence disciplines all HELD. Full attestation + the skill-trace note + the disclosure ledger in `memory/process-failures.md` Session 84.

**Historical (kept for reference):** `vault/sessions/session-83.md`.

## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-83 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; …; S82 archived S81; S83 archived S82; S84 archived S83) to keep this handoff lean. The current (S84) scope contract is above; the archive holds the prior-session archaeology.
...
1. **The S83 ingestion-on-ramp merge is now INDEPENDENTLY CONFIRMED sound (S84) — the remaining V1 gap is the rest of ingest + the actual operator data (PF-S63-02 satisfied).** S84 ran the independent PR-#205 review phases (bead `9qx5`) collapsed in S83: a profile-less BLIND TRIAGE + a profile-less EXECUTED VERIFY, each in its own clean worktree, no access to the S83 verdicts — both confirmed the merge SOUND (the 5 must-fix findings genuinely RESOLVED by execution; the deferred beads correctly open; HIST-2 + BUG-205-01 reviewer-misreads correctly not-actioned). **Do NOT propose new pipeline mechanism, a dashboard REDESIGN (design-LOCKED), `/generate-plan` (DONE S81), the Apple Health adapter (DONE S82), the ingest CLI / intake screen / README (DONE S83), or whoop-disable (rejected — coexistence) as next work.** The genuine next surface: the **labs adapter** (parser, none exists) + the **DNA clinical-SNP analysis** (the LM-03 step — DNA is LANDED-not-analyzed today; the sensitive variant-analysis pipeline is separate) + the filled operator-profile/current-state scaffolds + the **interactive ingest "B" build** (the upload web app + the bespoke ui-designer pass on intake steps 3-5, both deferred from S83) + LM-01 prep. **The operator loads their real Apple Health + DNA into their MAIN checkout post-merge (NOT a build worktree — it gets deleted); keep the original export files (the gitignored store has no git backup, ADR-0005).** Before any secondary scope, answer PF-S63-02 in writing + state WHY in the scope contract.
2. **PREFER A FRESH CONVERSATION PER WORK-UNIT — the S83→S84 lesson, now PROVEN.** S83 collapsed `/review-pr`'s independence phases (blind triage / SE fix / executed verify) under context pressure in one over-long conversation (an S82 close + a full PR-#199 review + the whole build), and DISCLOSED it as an INV-SKILL-TRACE deviation; S84 restored them in a FRESH session and confirmed the merge sound. **For any substantive PR: run review Phase 3 (blind triage) + Phase 7 (executed blind verify) via SEPARATE profile-less agents, never self-triage/self-verify; if context is exhausting mid-flow, STOP and resume in a fresh conversation rather than collapse the shape.** The executed-verify must RUN the behavior (zip-slip / escape / RED-mutation / socket-block), not read-and-declare.
3. **The disciplines that HELD + standing mechanics.** (a) **Executed-evidence verification** — the S84 verify agent RAN every claim (member-escape False; HTML-escape present; `--source` test RED under mutation; congruence test RED both ways; sockets blocked + path still completes), not source-reading. (b) **Profile-less independence** — the triage + verify agents had no access to prior verdicts/diffs, so they could not inherit the reasoning. (c) **Audit-surfaced fixes are exceptions** — the correctly-deferred minors (`07f6`/`gzf7`/`dw8u`) stayed beaded, not fix-forwarded into a verification session. (d) **Close on FINAL post-merge `main`** (`cvf1`): pytest + `close-audit --session N` + cite the SHA. Standing mechanics: **bd FIXED** (worktree+park is the fallback); review/merge off the **THREE-dot diff** off `origin/main` (NOT the stale local `main` — the trunk checkout is stranded on `feature/apple-health-adapter` with the parallel-wiki uncommitted `vault/` changes, untouched this session); GraphQL EXHAUSTED → REST (`gh api`, full-40-char SHA guard); `git add` SEPARATE from `git commit`; push as its own command; **two parallel wiki sessions push `origin/main` — close in a clean worktree off post-merge `main`**; **close-step-8 freshness un-gated until `dfg1`**. Carry-overs: `07f6`/`gzf7`/`dw8u` (S83 review beads) / `dfbi` / `6aen` / `cjgg`/`uerm`/`pwpr`/`sip9` / `cvf1` / `dfg1`/`df1x` / `aem1` / `axm7` / `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`/`5fw1`.

## Current State (volatile)

- **S84 (2026-06-21) ran the independent PR-#205 review phases collapsed in S83 (bead `9qx5`, P1) — the remediation the operator directed as the fresh-session first task.** Outcome: the S83 ingestion-on-ramp merge is INDEPENDENTLY CONFIRMED sound. NO product code changed — `main` is unchanged at `8b52019` save this docs-only close.
- **Two profile-less general-purpose agents (opus), each in its OWN clean worktree off `origin/main` @ `8b52019` (2026-06-21 S84), with no access to the S83 verdicts or fix diffs.** BLIND TRIAGE (read-only): the 5 must-fix findings (TEST-001; the source-of-truth cluster QUAL-1/API-205-01/HIST-1; QUAL-2/API-205-03; API-205-02/HIST-3) independently RESOLVED in merged source (re-verified by execution + the 22/22 ingest suite, not by trusting the commit message); SEC-205-01 DEFERRED-OK; HIST-2 NOT_A_BUG (whoop wired); BUG-205-01 + TEST-003/004/005 + QUAL-3/4/5 DEFERRED-OK. EXECUTED VERIFY: items 1–7 RESOLVED with executed evidence; 8a/8b the deferred items genuinely still-open.
- **No divergence, no new escaped defect.** `dw8u` sharpened with the executed BUG-205-01 repro (it raises `NotADirectoryError` — a loud crash, not a silent misclassification; one-line `exists()`→`is_dir()` fix; low realism, the product never creates a non-dir at the dropzone path). `9qx5` CLOSED with the evidence.
- **pytest 1182 passed / 3 skipped on `origin/main` (`8b52019`, 2026-06-21 S84 close)** in both verification worktrees — unchanged (verification-only session, product tree byte-unchanged).
- **Beads:** `9qx5` CLOSED; `dw8u` sharpened; `07f6` (SEC-205-01 cap — operator sign-off) / `gzf7` (test-coverage gaps) / `dw8u` (QUAL minors + BUG-205-01) remain correctly OPEN. Carry-overs `dfbi`/`cjgg`/`uerm`/`pwpr`/`sip9`/`cvf1`/`dfg1`/`df1x`/`aem1`/`axm7`/`6aen`/`0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`/`5fw1`.
- **The one operator-owned decision teed up:** SEC-205-01 / bead `07f6` — the DNA-export decompression cap is defensive programming (a small zip can land a huge file locally; the operator owns the file), so it needs the operator's binary sign-off: ADD the streamed-copy + byte-cap (the `07f6` recipe) or ACCEPT the self-DoS risk + close `07f6` won't-fix.
- **Active landmarks:** LM-01 (First MD visit) **2026-07-13** — the 14-day scoped-drift-audit window opens **2026-06-29** (no trigger today). LM-02 (Apple Health wearable baseline) / LM-03 (23andMe) — no real data ingested → no trigger.

**Historical (kept for reference):** `vault/sessions/session-84.md`.

## What Is Next (volatile)

### S85 — load the operator's real data, then the rest of ingest [operator-gated]

**The ingestion on-ramp is BUILT (S83) and now INDEPENDENTLY CONFIRMED sound (S84).** The operator can load real Apple Health + DNA via the CLI and see it in the intake screen, on a model-free/network-free local path. What's DONE (do not re-propose): the closed loop (S70-S80), `/generate-plan` (S81), the Apple Health adapter (S82), the ingest CLI + DNA landing + the intake screen + the tester README (S83), the S83-merge independent re-review (S84, bead `9qx5` CLOSED), the dashboard (design-LOCKED). Next:

1. **The operator loads their real data (operator action).** In the MAIN checkout (`a-plus-maxing`), NOT a build worktree: `python -m scripts.ingest <export.xml>` + `python -m scripts.ingest <23andme.zip>`, then `python -m scripts.generate.generate intake` → open `vault/artifacts/generated/intake.html`. Keep the original export files (the gitignored store has no git backup, ADR-0005). This is the first run on REAL data — validates the adapter/CLI on the operator's actual export. **NOTE for the next session:** the MAIN trunk checkout is currently stranded on `feature/apple-health-adapter` (the S82 branch, local `main` stale at `8fa7dc2` as of 2026-06-21 S84) with two parallel-wiki sessions' uncommitted `vault/` changes — sync it to `origin/main` (`8b52019`+, 2026-06-21 S84) before the operator loads there, WITHOUT disturbing the uncommitted wiki changes.
...
4. **The S83 review beads** — `07f6` (the decompression cap, needs operator sign-off — the one operator decision teed up above), `gzf7` (test-coverage gaps) + `dw8u` (QUAL minors + BUG-205-01, now carrying an executed repro).
...
**Discipline:** scope contract at open; **prefer a FRESH conversation per work-unit (the S83→S84 lesson, now proven — a single long conversation forced a disclosed review-shape deviation that S84 had to remediate)**; bd is FIXED (worktree+park is the fallback); for any store surface, the store-surface battery; for a visual artifact, Pencil + the design agents + operator sign-off FIRST (never solo-originate); for any new entry point that lists a discovered set, derive from discovery or add a congruence test (the S83 watch); `/review-pr` (off the THREE-dot diff vs **`origin/main`**, not stale local `main`) → `/merge`; **re-run the close gate on final post-merge `main` + cite the SHA (PF-S74-01)**.

**Deferred (NOT blocking):** `07f6`/`gzf7`/`dw8u` (S83 review beads); `dfbi` (the deferred Apple metrics); `6aen` (worktree-path edit guard); `cjgg`/`uerm`/`pwpr`/`sip9` (value-domain + interaction grounding); `dfg1`/`df1x` (the doc-freshness gate); `cvf1` (the mechanical post-merge-`main` verify); `aem1`; `axm7`; `5fw1`; the `llna` hook fix; carry-overs `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`; operator-gated July-visit prep (LM-01) — first session on/after **2026-06-29** runs the LM-01 scoped drift audit.

## Landmark window check (close step 8.7)

S84 close (2026-06-21): RE-OPENED `vault/meta/landmarks.md` at close step 8.7 (unchanged this session). No landmark edited — S84 was a VERIFICATION-ONLY remediation (the independent re-review of the S83 merge; no product code, no real operator data ingested). LM-01 (First MD visit) **2026-07-13**: no trigger window open today (the 14-day scoped-drift-audit window opens **2026-06-29**, the 7-day MD-handoff window 2026-07-06). LM-02 (Apple Health wearable baseline): no real data ingested → "baseline-start + 30 days" not open. LM-03 (23andMe): the DNA LANDING path exists (S83) but no real DNA ingested + the clinical-SNP ANALYSIS is unbuilt → the trigger has NOT opened. LM-04 (first HTML artifact) NOT triggered — the renderers emit to the gitignored `vault/artifacts/generated/`. No landmark ACTIONS due.
