...
- **Pass-3 pilot batch COMPLETE — 3 of 14 specialists DEPLOYED** (post-S16, integrator session). `medical-liaison` (Role 7 adjudicator; PR #8) + `labs-specialist` (PR #6) + `peptide-specialist` (PR #9) all rebase-merged to `main` (`origin/main` at `07900c3` as of 2026-05-29); **7 agents live** (4 foundation + 3 pilots). All 3 pass the corrected gate 0-viol; all carry no frontmatter (convention). **Gate recalibrated** (PR #7: R13-12 collation-only exempt + R13-3 token→WARN — pilot-surfaced). **`81.8%` authority-impersonation figure adjudicated CORRECT** (a builder flagged a JBDistill misattribution; integrator re-verified the Ekram full text — 9/11 = 81.8% — foundation docs unchanged; bead `amc` REJECTED; medical-liaison's own OQ-2/BC-3 withdrawn in-branch). Two PF-S16 entries logged (worktree-path hazard; rm-rf near-miss). **PILOT CHECKPOINT WITH WALTER NOW DUE — propose batch size for the 11 remaining; do NOT auto-start them.** Worktree teardown for the 3 merged pilots pending (offered to Walter). Open follow-up beads: `f2j` (EC-10 path), `1ek` (§13-spec reconcile rows 3/12 + R13-13 terminal-states), dispatch-call-site flip (medical-liaison now live), 2 labs hardening, medical-liaison design-doc-correction (substrate).
