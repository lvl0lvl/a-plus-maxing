...
3. **The follow-up tail:** `p5wx` (init_instance auto-provision `.venv` + `pip install -r requirements.txt`), `7may` (harden the provenance audit's `python -c` path interpolation), `po4x` (UPSTREAM toolkit harvest-gate/pf-ingest fix — framework-side); plus a beads-env cleanup for `.beads/daemon-error`.
