...
- **AC4 (key access) — PARTIAL.** The runtime-fetch discipline is documented (keychain fetch at run time, never tracked — PUBLIC repo); the exact keychain item NAME awaits one operator confirm (the broad keychain scan was guard-blocked — the correct outcome). The build loop fetches via `security find-generic-password -s "<name>" -w`.
...
2. **The keychain key + the surface-for-review items.** The build loop fetches the no-train API key at RUNTIME from the macOS keychain (`security find-generic-password -s "<NAME>" -w` — NAME pending the operator's confirm; the broad scan was correctly guard-blocked) — NEVER store/print/commit it (PUBLIC repo). Surface for operator review before the live build: the autonomously-added **ADR-0019** + the **RT-01 fail-closed contract** + the **Claude no-train default** (ADR-0015). Per PF-S87-01, the build's headline acceptance is a USABLE plan from the conversation, not just chat plumbing.
...
- **The keychain key:** the no-train API key is in the macOS keychain; the build loop fetches it at runtime (never tracked — PUBLIC repo). The exact item NAME is the one operator-confirm item (the broad scan was correctly guard-blocked).
...
4. **The LIVE end-to-end run** (the final, operator-present checkpoint): fetch the no-train API key at RUNTIME from the keychain (`security find-generic-password -s "<NAME>" -w` — confirm NAME with the operator; NEVER store/print/commit it), drive a real conversation → a real plan. **Per PF-S87-01 the acceptance is a USABLE plan, not just working chat plumbing.** Everything before this is mock/fixture-tested (no API spend — the operator's directive).
