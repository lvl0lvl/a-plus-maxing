...
## Scope Contract — Session 91 (2026-06-23)

Goal: Build **Wave B** of the conversational-intake build (demographic activation + the `SUMMARY_FIELD_SET` token vocabulary + the end-to-end usable-plan proof) via `/execute-plan`, through checkpoint → 3-tier review → merge → close. (AUTONOMOUS; self-confirmed.) **[RE-SCOPED mid-session by operator direction — see the evaluation: ADR-0017-T2 was superseded; the real plan-generation engine is a richer multi-agent pipeline that gets its own design pass.]**

**Build-plan wave:** Wave B of `docs/build-plan/build-plan-conversational-intake.md` (serial spine `ADR-0018-T1 → ADR-0019-T1 → ADR-0017-T2`). Prior-wave attestation: Wave A merged (PR #220), the wiki lane merged on top → `main` @ `990b83d`; the Wave-A checkpoint passed; Wave-A entry-state green (146).

Acceptance criteria:
- [ ] AC1 (plan-integrity grounding of Wave B against the live tree before building).
- [ ] AC2 (Wave B build via `/execute-plan`: ADR-0018-T1 demographic activation → ADR-0019-T1 token vocabulary → ADR-0017-T2 end-to-end).
- [ ] AC3 (Wave-B checkpoint, EXECUTED: orphan tokens lit + de-identified; per-token coarseness via independent output scan; disjointness tripwires green; store-adversarial battery; end-to-end usable plan; CONCERN-1 termination; post-conversation PII scan = 0; 0-shared-routine-edit; full regression; design-critic).
- [ ] AC4 (3-tier review → merge: Tier-1 SE; Tier-2 QA+Security+Architect+design-reviewer+plan-integrity; Tier-3 `/review-pr` independence intact; carried obligations NOTE-T3-1/NOTE-T2-2/PF-S90-01 verified).
- [ ] AC5 (close on final post-merge `main` + cite the SHA, PF-S74-01).

Files I WILL touch: (Wave B) `scripts/serve/capture.py`, `scripts/plan/router.py`, `vault/design/templates/intake.py`, `scripts/plan/generate_plan.py`/`scripts/serve/chat.py` (only for ADR-0017-T2's real-client wiring), `tests/*`; the recipes' `status`; the close docs (staged by EXPLICIT path — never `git add vault/`).
Files I will NOT touch: `scripts/ingest/*` (NFR-4); `scripts/store/store.py`+`keying.py` (byte-unchanged); `scripts/serve/route.py`; `scripts/plan/assemble.py`+`record_plan` (byte-unchanged); the dashboard (LOCKED); `vault/library/*`; `main` directly; the keychain.
NOT doing: the LIVE end-to-end run (operator-present, after Wave B); ANY live API call (mock/fixture-tested, 0 spend); an intake-UI redesign (Clinical Light LOCKED).
Invariants at risk: ADR-0001/ADR-0016 (per-token coarseness on the derived path; post-conversation PII scan); INV-CORE-CAPABILITY; the `router.py` disjointness tripwires + `WIRED_TOKENS⊆SUMMARY_FIELD_SET`; INV-BRANCH-NOT-MAIN; INV-ROLE-INLINING; the close gates. The `AP-SELF-REVIEW-UNDER-PROBES-INTERACTION-SURFACE` class (PF-S75→S90); PF-S90-01 (Canadian locale in the crown-jewel fixtures); PF-S74-01.

**Core-capability-first gate (PF-S63-02):** the foundation (intake + de-id token layer) advances toward the core; the real plan-generation engine (the operator's richer pipeline) is the explicit next design pass. Gate satisfied — this is the direct path, not secondary work.

**S91 self-confirmation (autonomous):** contract self-confirmed per the standing directive ("go"). **NOTE (process): this contract was first authored at session-OPEN but a sub-agent `git checkout`/`restore` during the build reverted the uncommitted HANDOFF edit; re-authored at close (the work matched the original plan modulo the operator-directed re-scope). Lesson: commit the scope contract before dispatching build agents — logged in the disclosure ledger.**

### S91 Scope Contract Evaluation (2026-06-23, volatile)

- **AC1 (plan-integrity grounding) — PASS.** `plan-integrity` ground all 3 Wave-B tasks against the live tree → CONDITIONAL GO; it CAUGHT the NOTE-T3-1 production-wiring gap (the planned end-to-end would prove a `generate_plan`-seam path production never runs — the PF-S63-02/PF-S87-01 trap) + the ADR-0018-T1 manifest gap (21 coupled test_router.py cases). The role earned its keep.
- **AC2 (Wave B build) — PASS for the foundation; ADR-0017-T2 CHANGED (operator-directed re-scope).** `ADR-0018-T1` (demographic activation; commit `d2f338b`; the SE correctly HALTED on the equipment-access-class reconciliation's 21-test coupling, the orchestrator adjudicated Option-A extend-manifest, the tests were UPDATED-not-weakened) + `ADR-0019-T1` (4 kind-2 chat-sourced tokens; commit `90d064d`) BUILT. **ADR-0017-T2 RE-SCOPED:** mid-session the operator clarified the real plan-generation engine is a richer multi-agent pipeline (de-id API wrapper + a subscription orchestrator running the specialists + judge + a review-pr-of-the-plan + revise + PII re-insertion + a maintained unified format) — so ADR-0017-T2's minimal-author premise is superseded; it does NOT build here, and the engine gets its own design pass (the next work). This is a CHANGED criterion, operator-directed + documented — NOT silent drift.
- **AC3 (Wave-B checkpoint) — CHANGED (the foundation's own gates PASS; the end-to-end gate is moot under the re-scope).** The foundation's gates are green: the disjointness tripwires + `WIRED_TOKENS⊆SUMMARY_FIELD_SET` green at module load; the per-token coarseness output scans (8 derived tokens) clean; the store-adversarial battery non-tautological (category-4 RED); `generate_plan.py`/`assemble.py`/`store.py`/`keying.py`/`ingest/*` byte-unchanged; full regression green. The end-to-end usable-plan + post-conversation PII scan are deferred to the plan-gen-engine design (they belong to the real pipeline, not the minimal author).
- **AC4 (3-tier review → merge) — PASS for the foundation.** Tier-1 per-task TDD (executed negative controls); Tier-3 `/review-pr` 7-agent (the 6 + a design-reviewer for the activated UI) with independence INTACT (profile-less blind-triage P3 + EXECUTED blind-verify P7): 12 findings → blind-triaged 9 LEGITIMATE + 2 OUT_OF_SCOPE/DEFERRED (HIST-1/2 → bead `f0gh`) + TEST-4 folded; 9 fixed (commits `950ca43`+`da43f14`) → 9/9 blind-verified RESOLVED; suite 1472/2. The carried Wave-A obligations: NOTE-T3-1 resolved by the Architect as a THIN forward + a spec amendment (`docs/spec/.wave-b-note-t3-1-amendment.md`, deferred to the engine); PF-S90-01 confirmed (Canadian locale moot for the reconciled equipment token). Merge via `/merge`.
- **AC5 (close) — PASS** (this close; close-audit + harvest-gate green on final post-merge `main`, SHA cited per PF-S74-01).

### Drift checks (S91 close)

- **Task drift:** One MAJOR operator-directed scope change (ADR-0017-T2 re-scoped to the plan-gen-engine design pass) — documented as a CHANGED criterion, NOT silent. The fix-forward of 9 review-LEGITIMATE findings (the deriver robustness + the BUG-1 honesty sentinel + test coverage) was in-scope foundation correctness; the 2 OUT_OF_SCOPE findings (the chat-elicitation wiring) were BEADED (`f0gh`), not absorbed. No silent drift.
- **Architecture drift:** the crown-jewel PII boundary HELD (Security: 0 findings, executed; the per-token coarseness on the derived path proven). INV-CORE-CAPABILITY: the foundation advances the de-id data layer the engine reads from; the real engine is the next design pass (the core capability is NOT yet end-to-end — honestly stated, the plan-gen-engine ADR is the path). The disjointness tripwires + byte-unchanged invariants held. **The vision EXPANDED (operator clarification): the plan-generation engine is a richer multi-agent pipeline mirroring the autonomous build pipeline — captured for the design pass, not drift but a sharpened target.**
- **Vision drift:** none in the wrong direction — the system gains the de-identified intake/token foundation; the operator's architecture clarification SHARPENED the plan-generation target (it was a stub; it's now a designed pipeline). `design/vision.md` first sentence unchanged.

### PF attestation

S91 close (2026-06-23): **No new PF-class entries this session.** The defining positives: the layered pipeline worked AS DESIGNED — `plan-integrity` caught the NOTE-T3-1 production-wiring gap (the exact PF-S63-02/PF-S87-01 trap the build exists to avoid) + the ADR-0018-T1 manifest gap BEFORE any wrong code merged; the SE correctly HALTED rather than weaken the 21 coupled tests; the `/review-pr` 7-agent caught 9 real foundation-robustness/honesty defects (the free-text derivers' value-domain gaps — the `AP-SELF-REVIEW-UNDER-PROBES-INTERACTION-SURFACE` class — + the always-set-token honesty gap) all fixed + blind-verified before merge; the operator's architecture clarification was engaged honestly (the minimal-author premise corrected, the real engine teed up as a design pass, not rubber-stamped — the anti-PF-S87-01 move). Observed-but-not-promoted (disclosure ledger): (a) the orchestrator's uncommitted S91 scope contract was reverted by a sub-agent git op — a known-class operational miss (commit the contract before dispatching), self-caught at close, re-authored + logged, no impact; (b) the `.beads` hook auto-stage on feature-branch commits (the documented benign bd-flush side-effect) recurred. Prior lessons referenced DESCRIPTIVELY (the never-hand-roll-the-build discipline; the read-the-gated-skill-in-full discipline; the verify-first + executed-blind-verify disciplines; the close-on-final-main re-verify; the single-trunk no-park topology), never by bare token.

**Historical (kept for reference):** `vault/sessions/session-91.md`.

## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-90 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; …; S88 archived S87; S89 archived S88; S90 archived S89; S91 archived S90) to keep this handoff lean. The current (S91) scope contract is above; the archive holds the prior-session archaeology.
...
1. **The conversational-intake FOUNDATION is complete + merged; the REAL plan-generation ENGINE is the next big design+build (operator-clarified architecture).** Wave A (model boundary + conversation path, S90) + the Wave-B foundation (demographic activation + the de-identified `SUMMARY_FIELD_SET` token vocabulary, S91) are merged. **The operator clarified the actual product architecture (do NOT treat plan-generation as a single model call):** a PII-wrapper boundary (the API model de-identifies in / re-inserts PII out) around a SUBSCRIPTION orchestrator that runs the SPECIALISTS as agents (like the autonomous build pipeline) → a JUDGE verify → a `/review-pr`-style multi-agent SAFETY review of the plan → revise → the API agent re-inserts PII + builds the (reviewed) tracking/testing → a unified maintained format (HTML) adjusted as wearables/labs arrive. **Next = the `plan-generation-engine` design pass: run the FULL autonomous build pipeline (refresh skills_library first — new commands) to author the ADR, then ADR→spec→build-plan→task-plan→execute-plan to build it.** Be extra-engaged monitoring the pipeline (it is the live reference model for the health-plan pipeline); bead any new-skill/command rough edges.
2. **The carried + newly-surfaced engine obligations (land in the engine, not the foundation).** **NOTE-T3-1 (Architect spec amendment `docs/spec/.wave-b-note-t3-1-amendment.md`):** thread a real `ModelClient` (THIN `client=` forward) through the PRODUCTION front door `pipeline.run_generation`→`orchestrate.generate_plans`→`compute_plan` (both sites, :530+:551) + assert the usable plan THROUGH that front door with a spy-backed placement assertion — NOT the bare `generate_plan` seam (the PF-S63-02/PF-S87-01 trap). **The chat ELICITATION wiring (`f0gh`):** the 4 ADR-0019 tokens are minted but `chat.py` never asks the nutrition/supplement/peptide/training domains — wire `_DOMAIN_TOKEN_GROUPING`+`_MINTABLE_TOKENS`; co-design with the engine's conversation stage. **The always-set DATA is honest (`not-discussed` sentinel landed):** "not elicited" ≠ "confirmed none" — the engine reasons over honest signals. **The de-id derivers are a real component** (the foundation hardened 5 free-text-parsing bugs — the engine's de-id stage must be robust). **DESIGN-ESCALATION:** the stale plan-gen docs describe the superseded runtime-A interactive-dispatch model (bead — reconcile with the engine's programmatic model).
3. **Standing mechanics (all HELD).** Build via the full pipeline (`/create-adr`→spec→build-plan→task-plan→`/execute-plan`; never hand-rolled, PF-S36-01); 3-tier review (Tier-1 SE → Tier-2 QA+Security+Architect+design-reviewer → Tier-3 `/review-pr` with profile-less blind-triage + EXECUTED blind-verify INTACT) + `plan-integrity` grounding/gating (it caught the NOTE-T3-1 trap + the manifest gap this session); store-surface tasks satisfy `docs/checklists/store-adversarial-tests.md`; **full role profiles inlined VERBATIM (the hook denies abbreviation); registered review-agent types carry their full profile**; `/review-pr` off the THREE-dot diff; `/merge` (REST fallback when GraphQL exhausted, full-40-char SHA guard); `git add` SEPARATE from `git commit` (the hook auto-stages `.beads` — benign); **single trunk — no park dance (PF-S71-01 retired); stage close docs by EXPLICIT path (never `git add vault/` — perpetual daemon churn)**; **COMMIT THE SCOPE CONTRACT BEFORE DISPATCHING BUILD AGENTS (a sub-agent git op reverted the uncommitted S91 contract — disclosure-ledger note)**; re-run the close gate on final post-merge `main` + cite the SHA (PF-S74-01). Carry-over beads: `f0gh` (chat elicitation), `j432`/`pqb0`/`zwow` (Wave-B follow-ups), `f22s`/`oves` (wiki), `02m1` (daemon config), `v70t` + the prior set.

## Current State (volatile)

- **S91 (2026-06-23): built + merged the conversational-intake WAVE-B FOUNDATION** (2 of the 3 planned tasks; the 3rd re-scoped by operator direction). `ADR-0018-T1` (commit `d2f338b`): activated the dead Step-1 demographic form (`vault/design/templates/intake.py`) into real POSTing inputs, lit the 4 orphan tokens (`training-age-band`/`sex-for-dosing`/`bodyweight-band`/`equipment-access-class`), reconciled `equipment-access-class` to one source (removed the postal derivation), form objective-only — the SE correctly HALTED on the 21 coupled `test_router.py` cases, the orchestrator adjudicated extend-manifest, the tests were UPDATED-not-weakened. `ADR-0019-T1` (commit `90d064d`): minted 4 kind-2 de-identified chat-sourced tokens (`dietary-pattern-class`/`supplement-stack-class`/`peptide-use-class`/`training-volume-band`), each coarseness-proven by an INDEPENDENT output scan. All mock/fixture-tested (0 live-API spend).
- **`/review-pr` (7-agent, the operator-requested gate before merge) ran with independence INTACT** (profile-less blind-triage + EXECUTED blind-verify): 12 findings → 9 LEGITIMATE (fixed, 9/9 blind-verified RESOLVED) + 2 OUT_OF_SCOPE/DEFERRED → bead `f0gh`. The 9 were real foundation-robustness/honesty defects: the free-text derivers under-probed their value-domain (malformed birth years, double-digit volumes, punctuation-only false-presence, dietary allergy-masking — the `AP-SELF-REVIEW-UNDER-PROBES-INTERACTION-SURFACE` class) + **the always-set tokens now carry a `not-discussed` sentinel** so "not elicited" ≠ "confirmed none" (data-honesty for the engine). Security: 0 findings (crown-jewel boundary proven clean, executed). Design-reviewer: PASS (on-theme, honest, accessible).
- **The operator clarified the REAL plan-generation architecture mid-session** (a richer multi-agent pipeline — see the Top-3 + What Is Next). `ADR-0017-T2` (the minimal end-to-end) was RE-SCOPED to the engine's design pass; the Architect's NOTE-T3-1 spec amendment (`docs/spec/.wave-b-note-t3-1-amendment.md`) captures the THIN production-wiring contract for the engine.
- **pytest 1472 passed / 2 skipped** on the merged foundation; the disjointness tripwires + `WIRED_TOKENS⊆SUMMARY_FIELD_SET` green; `generate_plan.py`/`assemble.py`/`store.py`/`keying.py`/`ingest/*` byte-unchanged; the store-adversarial battery non-tautological.
- **No new PF.** The layered pipeline worked as designed (plan-integrity caught the NOTE-T3-1 trap + the manifest gap; the SE HALTED rather than weaken tests; `/review-pr` caught 9 real defects before merge; the architecture clarification engaged honestly — the anti-PF-S87-01 move). Disclosure: the uncommitted S91 scope contract was reverted by a sub-agent git op (re-authored at close; lesson = commit it before dispatching).
- **Beads:** new this session: `f0gh` (chat elicitation wiring — P1, design-relevant). Carry-over: `j432`/`pqb0`/`zwow`/`v70t`/`f22s`/`oves`/`02m1` + the prior set.
- **Single clean trunk @ `main` (this session's merge SHA cited in the close attestation, PF-S74-01); no extra worktrees, no stashes; vault perpetually daemon-dirty (stage by explicit path).**

**Historical (kept for reference):** `vault/sessions/session-91.md`.

## What Is Next (volatile)

### S92 — the `plan-generation-engine` design pass + build (the operator's clarified architecture, via the FULL autonomous build pipeline)

The conversational-intake foundation is merged. The operator clarified the real product: plan generation is NOT a single model call — it is a **multi-agent pipeline mirroring our own autonomous build pipeline, with the health specialists as the agents.** The directed sequence:

1. **Refresh the autonomous build pipeline in `skills_library` FIRST** (the operator flagged NEW commands/updates). Read the current pipeline end-to-end so the build runs on the current commands; **bead any new-skill/command rough edges as they surface** (the operator expects some).
2. **Run the FULL pipeline to author the `plan-generation-engine` ADR** (`/create-adr` — likely an ADR). The architecture to capture (operator's words):
   - A **PII-wrapper boundary**: an API (no-train) model de-identifies operator PII IN; the same trusted API agent re-inserts the necessary PII OUT (the final personalized plan) + builds the (reviewed) tracking/testing.
   - A **subscription orchestrator** (Claude Code, like the build pipeline; API/local later) that runs the **SPECIALISTS as the authoring agents** over the de-identified summary → a **JUDGE verify** step (like the build pipeline) → a **`/review-pr`-style multi-agent SAFETY review** of the finished plan (medical-safety, edge-case, etc.) → **revise**.
   - A **unified maintained output** (HTML?) the API agent updates as wearables/labs arrive + adjusts.
   - Reconcile the existing deterministic seams: the de-id GATE (stronger than a model-de-id; keep as the hard guarantee) + the `assemble` safety filters + the medical-liaison adjudication; the THIN `client=` production-front-door wiring (Architect amendment `docs/spec/.wave-b-note-t3-1-amendment.md`); the chat ELICITATION wiring (`f0gh`).
3. **Then ADR→spec→build-plan→task-plan→`/execute-plan`** to build it, with the full 3-tier review on each wave. **Be EXTRA-ENGAGED monitoring the pipeline** — its structure is the live reference model for how the health-plan pipeline should be organized (the operator's explicit instruction).
4. **Mock/fixture-tested first (0 API spend); the LIVE end-to-end run is the operator-present checkpoint** (the operator injects `export ANTHROPIC_API_KEY=$(security find-generic-password -s "quant-primary-api" -w)`; the agent NEVER touches the keychain; never commit the key — PUBLIC repo). **DESIGN-ESCALATION to settle in the ADR:** the stale plan-gen docs (`/generate-plan` SKILL.md + `plan-generation-pipeline-v1.md` + `author-dispatch-process.md`) describe the superseded runtime-A interactive-dispatch model — reconcile to the programmatic-orchestrator model.

**Deferred / NOT blocking:** the carry-over beads (`f0gh` chat elicitation — fold into the engine's conversation stage; `j432`/`pqb0`/`zwow`; `f22s`/`oves` wiki; `02m1` daemon config); the local-model migration (North Star, post-V1).

## Landmark window check (close step 8.7)

S91 close (2026-06-23): RE-OPENED `vault/meta/landmarks.md` at close step 8.7 (unchanged this session). No landmark edited — S91 built the conversational-intake Wave-B foundation on PII-free SYNTHETIC fixtures + mock clients; no real operator data, no live API. LM-01 (First MD visit) **2026-07-13**: today (2026-06-23) is BEFORE the 14-day scoped-drift-audit window open (**2026-06-29**, 6 days out) → no trigger today; **NOTE for S92+: the first session on/after 2026-06-29 runs the scoped drift audit on the LM-01 relevant_scopes** (the 7-day MD-handoff window opens 2026-07-06). LM-02 (Apple Health) / LM-03 (23andMe): no real data ingested → not triggered. LM-04 (first HTML artifact): the renderers emit to the gitignored `vault/artifacts/generated/`; not triggered. No landmark ACTIONS due.
