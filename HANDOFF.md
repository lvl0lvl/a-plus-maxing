...
- **SINGLE TRUNK** unchanged (since S22): `main` = complete project. The data-in spec + S26 close were **rebase-merged to `main` via PR #31** (`/review-pr` Gate PASS); a follow-up **PR #33** then fixed the 3 Code-Quality findings I had initially mis-suppressed (PF-S26-01: legitimate = real → fix-or-bead, never suppress by severity). The 1 NOT_A_BUG finding stands. No code built — design only.
