...
## Scope Contract — Session 77 (2026-06-19)

Goal: Wire the doctor-visit-queue **data layer** — a `dvq::` store stream + a collation that records the plan-generation pipeline's adjudicated safety findings (additive-AE / conflict / rx-bpmh — each *cleared-via-override* OR *block-stands*) as **severity-ranked** queue entries in the SBAR §9.4 "flagged interactions/contraindications" form, verified by a real `medical-liaison` collation dispatch (the liaison owns the ranking side). The visual SBAR handout **render** is explicitly deferred to a design-led session (Pencil + operator sign-off + the design agents — never originated solo, per the design memory). Operator delegated S77 autonomously ("open S77, write the scope contract and proceed through the whole build with full reviews and rigor"). The LM-01-relevant surface (the MD handout's data), ahead of the 2026-06-29 window.

Acceptance criteria:
- [ ] AC1 (queue store stream): a new `scripts/store/queue_schema.py` with a `dvq::` stream — `record_doctor_visit_queue_entry` (dedupe identity `(item, timepoint=date, source=finding_id)`, value = entry dict) + `resolve_doctor_visit_queue` (reads + **severity-ranks**: CRITICAL/H1-H2 + non-overridable first, then HIGH, MEDIUM; cumulative latest-per-finding). **Mutation-proven RED.**
- [ ] AC2 (orchestrator collation): `collate_doctor_visit_queue(result, on_date, root)` records each adjudicated/held safety finding from `generate_plans`'s result (`adjudication` + `conflict_adjudications` + `rx_bpmh_adjudications`) as a queue entry `{finding_id, axis, caution (verbatim), held_domain, source_specialist, outcome, non_overridable, composite_band}`. No new behavior in `generate_plans`. **Mutation-proven RED.**
- [ ] AC3 (STORE-ADVERSARIAL BATTERY — the marquee gate): all 4 required tests on the `dvq::` stream per `docs/checklists/store-adversarial-tests.md` — cross-stream collision (`dvq::` vs `plan::`/`plan-track::`), same-timepoint-distinct-persist + same-finding idempotent, dedupe-key boundary, mutation-style (→ RED, reverted).
- [ ] AC4 (real liaison E2E): a real `medical-liaison` dispatch (full profile inlined) collates + severity-ranks a genuine queue — verifies the ranking discipline (watchlist-high first; GRADE tags; no clinical verdict). Captured as `doctor-visit-queue-*.example.json` (SYNTHETIC operator). Not a unit stub.
- [ ] AC5 (backward-compat + integration mandate): `generate_plans` behavior unchanged; all S70–S76 tests stay green; `--self-test` + core-capability gate green.
- [ ] AC6 (review + close): plan-integrity + QA (Tier-2; **the store-adversarial battery cited in the QA dispatch**; the design spec + author-dispatch doc LISTED in WILL-touch — MF-1) → `/review-pr` (Tier-3, 6-agent) → fix legitimate → `/merge`; full close, **re-run the close gate on the final post-merge `main`** + cite the `main` SHA (PF-S74-01).

Files I WILL touch: `scripts/store/queue_schema.py` (NEW — the `dvq::` stream); `scripts/plan/orchestrate.py` (`collate_doctor_visit_queue`); `tests/store/test_queue_schema.py` (NEW — the store-adversarial battery) + `tests/plan/test_orchestrate.py` (collation tests); new `docs/plan-generation/examples/doctor-visit-queue-*.example.json`; `vault/design/plan-generation-pipeline-v1.md` + `docs/plan-generation/author-dispatch-process.md` (queue data layer WIRED); HANDOFF/PF-log/harvest/beads/session-note at close.
Files I will NOT touch: `scripts/plan/adjudicate.py` + `scripts/plan/router.py` (the gate + PII boundary — unchanged); `scripts/store/store.py`/`keying.py` core (use the append/read API, never change the substrate); the deployed `.claude/agents/*` (inline for dispatch); the wiki / `vault/library/` (parallel sessions); `main` directly; the visual SBAR handout render (deferred — design-led).
NOT doing: the visual SBAR handout render (the one-page artifact — design-led, Pencil + operator sign-off + design agents, a separate session); the producer-side "every `risk_tier:medium+` write pushes to the queue" hook across all 14 specialists (this slice collates the plan-generation outputs); the measure/adjust legs; changing the `adjudicate` gate / `safety_finding` contract; any raw operator-medication ingest.
Invariants at risk: INV-BRANCH-NOT-MAIN (built in `../aplus-s77dvq` worktree off `main`; idle trunk parked on `parking/s77-dvq`; `.venv` provisioned); **store-surface battery (the NEW `dvq::` stream — the 4-test battery is MANDATORY, the marquee gate this session, at Tier-1 + the Tier-2 QA dispatch)**; INV-CORE-CAPABILITY (the queue is downstream of the wired path — stays green + `--self-test`); INV-ROLE-INLINING (real liaison + review agents, FULL profiles); INV-OVERRIDE-RECORD-SCHEMA + INV-CRITICAL-NON-OVERRIDABLE (the queue reads the adjudication outcomes — REUSED, not changed); the ADR-0006-T0 PII boundary (queue entries carry compounds + cautions, SYNTHETIC only — no raw operator PII); INV-CLOSE-AUDIT / INV-PF-ATTESTATION / INV-SKILL-TRACE / INV-HARVEST-CAPTURE at close. **PF-S76-01 + PF-S75-01 interaction-probes:** the `dvq::` stream's interaction with the existing store streams (cross-stream namespacing) + the severity-rank's boundary/ordering cases (the `dvq::` is LOCAL persistence — no model/PII/scan boundary, so the boundary-size-infra concern is N/A). Core-capability-first gate (PF-S63-02): SATISFIED — the core path is wired; the queue collates its safety outputs into the MD-handout data layer (the LM-01-relevant surface). **PF-S74-01 discipline: re-run the full close on the final post-merge `main` + cite the `main` SHA.**

### S77 Scope Contract Evaluation (2026-06-19, volatile)

- **AC1 (queue store stream) — PASS.** `scripts/store/queue_schema.py` — a `dvq::queue` stream (cumulative, deduped by `finding_id`, severity-ranked); `record_doctor_visit_queue_entry` + `resolve_doctor_visit_queue` + `read_doctor_visit_queue`. Writes THROUGH `store.append` + the one `keying.LINE_FIELDS`, no second key. Mutation-proven RED (the constant-source mutation collapses distinct findings). `_severity_rank` REFINED post-Tier-2 to lead with the OUTCOME tier (non-overridable > block-stands > cleared, then band) — fixing the safety-ordering inversion.
- **AC2 (orchestrator collation) — PASS.** `collate_doctor_visit_queue` records each adjudicated finding (additive-AE + conflict + rx-bpmh; cleared/block-stands) as a queue entry, rebuilt via the SAME safety_finding builders (verbatim finding_id/caution). `generate_plans` behavior unchanged. Mutation-proven RED. **Refined post-Tier-3:** `_adjudicate_with_band` threads the envelope's real `composite_band` + `harm_class` onto the outcome so a block-stands keeps its real band (BUG-1/BUG-2 fix).
- **AC3 (STORE-ADVERSARIAL BATTERY) — PASS.** All 4 categories on `dvq::queue` per `docs/checklists/store-adversarial-tests.md` — cross-stream collision, same-timepoint distinct-persist + idempotent, dedupe-key boundary, mutation-style (RED, reverted). Tier-3 historical-context confirmed the S41-class store-keying escapes did NOT recur.
- **AC4 (real liaison E2E) — PASS.** A real `medical-liaison` collation dispatch severity-ranked a genuine 3-finding queue; its MD-priority ranking agrees with the code on the safety-tier lead (the non-overridable warfarin auto-block first), refining intra-band clinically at dispatch. Captured `doctor-visit-queue-collated.example.json` (SYNTHETIC operator). No clinical verdict.
- **AC5 (backward-compat + integration mandate) — PASS.** `generate_plans` writes no `dvq::` stream; all S70–S76 tests stay green; suite 1063/3, floor 15/0, `--self-test` + core-capability green.
- **AC6 (review + close) — PASS.** plan-integrity (MF-1 module-docstring freshness — fixed) + QA (store-battery PASS 4/4 + 2 SHOULD-FIX incl. the safety-ordering inversion — fixed) Tier-2 → `/review-pr` 6-agent Tier-3 (BUG-1/BUG-2 band-derivation + QUAL-1 + TEST-1/3/4 fixed; CONTRACTS-DVQ-1 DECISION; blind-verified all RESOLVED executed) → `/merge` PR #175; full close, close gate re-run on final `main` + the `main` SHA cited (PF-S74-01).

### Drift checks (S77 close)

- **Task drift:** all 6 ACs PASS. The DATA-LAYER slice was built; the VISUAL SBAR handout render was explicitly deferred at open (design-led — Pencil + operator sign-off + design agents, per the design memory) — flagged in the contract, not silent. `_severity_rank` + the band derivation were REFINED mid-review (Tier-2 outcome-tier, Tier-3 band-threading) in response to gate findings — flagged. No silent drift.
- **Architecture drift:** no invariant degraded; `adjudicate.py` + `router.py` UNCHANGED. The NEW `dvq::queue` store stream carries the full store-adversarial battery (the marquee gate); the S41-class escapes did not recur. INV-CORE-CAPABILITY green; INV-BRANCH-NOT-MAIN held (built in `../aplus-s77dvq` off `main`, idle trunk parked `parking/s77-dvq`, `.venv` provisioned); INV-ROLE-INLINING held (real liaison + the 6 review agents + plan-integrity + QA full-profile). Toward the design (the MD-handout data layer landed).
- **Vision drift:** none, toward-vision. The doctor-visit queue now collates the pipeline's adjudicated safety findings into the MD-handout data layer — the first downstream consumer of the held-line, most relevant to the July MD visit. `design/vision.md` first sentence unchanged.

### PF attestation

S77 close (2026-06-19): **One new PF promoted — PF-S77-01** (Tier-1 self-review under-probed the new collation code's read of the existing `adjudicate` outcome contract; Tier-3 caught the queue band-derivation defect — recurrence 4 of self-review-under-probes-the-new-code-×-existing-contract; gate-caught before merge, fixed via the band-threading, mutation-proven + blind-verified RESOLVED; structural RUN-the-interaction-probe + contract-grounding discipline beaded `uerm`). The build's other disciplines held strongly — the store-adversarial battery (the marquee gate) built + mutation-proven RED across all 4 categories with no S41-class regression; the `dvq::queue` schema follows the `plan_schema` pattern; the collation reuses the S73-S76 builders verbatim; `generate_plans` unchanged; the design slice faithful (the visual render deferred as design-led). The close-on-final-`main` discipline applied. Full attestation + the per-PR skill-trace table (PR #175) + the disclosure ledger in `memory/process-failures.md` Session 77.


## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-76 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; …; S75 archived S74; S76 archived S75; S77 archived S76) to keep this handoff lean. The current (S77) scope contract is above; the archive holds the prior-session archaeology. (S78 will archive S77.)
...
1. **RUN the interaction-probe at Tier-1 — incl. CONTRACT-GROUNDING — don't just remember it (PF-S77-01, recurrence 4).** FOUR times now (S74 routing-key types on a fail-safe gate, S75 multi-concern hold composition, S76 a new list-field defeating the `pii_scan` 4096-cap, S77 the queue collation reading `composite_band` off the `adjudicate` OUTCOME — which carries it only in `override_record`/not at all for block-stands → a block-stands entry lost its real band) Tier-3 caught a defect in my new code's interaction with an EXISTING contract/assumption that my Tier-1 missed. The prior interaction-probe disciplines (beads `sip9` hold×hold, `pwpr` value×infra) were in this Top-3 but I did NOT apply them to the band derivation. **The fix is to RUN them, not pointer to them:** for any new code that READS a field off an existing return/contract, GROUND (by reading the source's actual return shape) that the source reliably carries that field for EVERY case the new code handles — before Tier-2/3 (bead `uerm`, pairs with `pwpr`/`sip9`; consider mechanizing since the discipline keeps being skipped).
2. **The disciplines that HELD and must keep holding.** (a) **Store-adversarial battery (mandate):** any new/changed `scripts/store/` surface carries all 4 categories (cross-stream / dedupe / boundary / mutation), mutation-proven RED, at Tier-1 + the Tier-2 QA dispatch — S77's `dvq::queue` did, no S41-class regression. (b) **Multi-concern hold model (`sip9`)** + **boundary-size×infra probe (`pwpr`):** the surfaces I DID probe held. (c) **Close on the FINAL post-merge `main` (`cvf1`):** re-run pytest + `close-audit --session N` on `main` AFTER merge+pull; the attestation CITES the post-merge `main` SHA; never claim "fully gated" before re-verifying the merged trunk.
3. **Standing mechanics + carry-overs.** **Worktree discipline:** separate `git worktree`s; park the idle trunk OFF `main`; Read worktree files at the worktree-absolute path; re-verify `git branch --show-current` before every commit; restore `main` at session end; **provision the worktree `.venv`**; **commit fixes PROMPTLY (`git checkout -- <file>` reverts an uncommitted fix to HEAD).** **Paste FULL role profiles verbatim — abbreviating one is denied by `enforce-role-inlining`.** **Design-led visual artifacts (the SBAR handout render, dashboards) are NEVER originated solo — Pencil + operator sign-off + the design agents (`feedback_design_via_agent_team`).** GraphQL EXHAUSTED → REST for PR-create/merge (`gh api`, full-40-char SHA guard). `git add` SEPARATE from `git commit` (PII hook denies the combined call). bd repo-ID mismatch in a worktree: `bd sync --flush-only` does NOT write `issues.jsonl` — surgically edit `issues.jsonl` (scrub new-bead owner → the `your-github-email@example.com` placeholder) + commit it in the worktree. Two parallel wiki sessions push to `origin/main` — rebase-sync at merge, never touch `vault/library/`. **Close-step-8 freshness un-gated until `dfg1` — RUN step 8 explicitly each close.** Carry-overs: `uerm` (RUN-the-interaction-probe + contract-grounding, PF-S77-01) / `pwpr` / `sip9` / `cvf1` / `dfg1`/`df1x` / `aem1` / `axm7` / `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`. Pencil: ALWAYS pass `filePath`.

## Current State (volatile)

- **S77 (2026-06-19) wired the doctor-visit-queue DATA LAYER — the medical-liaison's owned MD-handout surface (design §9.4), the first downstream consumer of the held-line.** `scripts/store/queue_schema.py` (NEW): a `dvq::queue` store stream (cumulative, deduped by `finding_id`, severity-ranked: non-overridable auto-block > block-stands > cleared, then band), writes THROUGH `store.append` + the one `keying.LINE_FIELDS`, carries the full store-adversarial battery (cross-stream / dedupe / mutation, mutation-proven RED — no S41-class regression). `scripts/plan/orchestrate.py`: `collate_doctor_visit_queue` records each ADJUDICATED safety finding (additive-AE + conflict + rx-bpmh; cleared-via-override OR block-stands) as a queue entry, rebuilt via the SAME safety_finding builders (verbatim finding_id/caution); `_adjudicate_with_band` threads the envelope's real band + harm_class onto the outcome (so a block-stands keeps its real band). `generate_plans` unchanged; `adjudicate.py`/`router.py` UNCHANGED. The visual SBAR handout RENDER is the design-led follow-on (deferred — Pencil + operator sign-off + design agents).
- **PR #175 → `main` (rebase merge; the S77 close re-run on the post-merge `main`, SHA cited in the close report).** Tier-2 plan-integrity (MF-1 module-docstring freshness — fixed) + QA (store-battery PASS 4/4 + the safety-ordering inversion fixed) → `/review-pr` 6-agent Tier-3 (**BUG-1/BUG-2 band-derivation** — the queue read `composite_band` off the `adjudicate` outcome, which does not carry it for block-stands — fixed via the band-threading, mutation-proven RED, **blind-verified all RESOLVED executed**; + QUAL-1 + TEST-1/3/4; CONTRACTS-DVQ-1 DECISION) → `/merge`. Verified over a real `medical-liaison` collation (`doctor-visit-queue-collated.example.json`) agreeing with the code on the safety-tier lead. Single-PR close.
- **PF-S77-01 promoted** (Tier-1 under-probed the new collation code's read of the existing `adjudicate` outcome contract; Tier-3 caught the band-derivation defect; recurrence 4 of self-review-under-probes-the-new-code-×-existing-contract; gate-caught before merge; structural RUN-the-interaction-probe + contract-grounding discipline beaded `uerm`).
- **pytest 1063/3 (worktree, native `.venv`).** Core-capability gate green; negative-test floor 15/0.
- **Beads:** `71s4` (doctor-visit-queue DATA LAYER WIRED; the SBAR handout RENDER + the measure/adjust legs remain); `uerm` (PF-S77-01) created; carry-overs `pwpr`/`sip9`/`cvf1`/`dfg1`/`df1x`/`aem1`/`axm7`/`llna`/`0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`.
...
**Historical (kept for reference):** `vault/sessions/session-77.md`.

## What Is Next (volatile)

### S78 — the SBAR handout render (DESIGN-LED) or the measure/adjust legs

The doctor-visit-queue DATA LAYER is wired; the adjudicated safety findings persist in `dvq::queue`, severity-ranked. Two candidate next surfaces (operator chooses):

1. **The SBAR handout RENDER** — the one-page MD handout (design §9.4: Situation / Background med-supplement table + flagged-interactions / Assessment / Recommendation) rendered from the `dvq::queue` stream + the operator BPMH. This is a **DESIGN-LED VISUAL artifact** — per the design memory it is NEVER originated solo: **Pencil mockup + operator sign-off on the visual design FIRST, then the design agents + the authoritative schema** (`feedback_design_via_agent_team`, `feedback_dashboard_pencil_mockups`, `feedback_build_from_authoritative_design_artifact`; PF-S68-01/S49-01 family). NOT a solo code build. Honor the render precondition (the entry may lack the liaison's GRADE/watchlist extras → degrade to the tier-only ordering). Highest LM-01 relevance.
2. **The measure leg** (a production caller for `record_plan_tracking` — currently schema-only) + the **adjust** leg (re-plan from tracking) — the rest of the closed loop. A code build (store-surface → the adversarial battery).

**Discipline:** scope contract at open (LIST the design spec + any doc in WILL-touch — MF-1); **worktree + idle trunk off `main` + provision the `.venv`**; **for any new/changed store surface, satisfy the store-adversarial battery**; **at Tier-1 RUN the interaction-probe incl. contract-grounding (PF-S77-01) — ground every field new code reads off an existing return against the source's actual return shape — AND the multi-concern (`sip9`) + boundary-size×infra (`pwpr`) probes**; **for a visual artifact, Pencil + operator sign-off FIRST (never solo)**; **write close artifacts in-worktree BEFORE merge**; **commit fixes promptly**; **paste FULL role profiles verbatim**; **verify the production path E2E with a real `medical-liaison` dispatch**; `/review-pr` → `/merge`; **re-run the close gate on the final post-merge `main` + cite the `main` SHA (PF-S74-01)**.

**Deferred (NOT blocking):** `uerm` (RUN-the-interaction-probe + contract-grounding — PF-S77-01) + `pwpr` (boundary-size×infra probe) + `sip9` (multi-concern probe); `dfg1` (the doc-freshness gate — PF-S73-01 structural fix) + `df1x` (the doc-freshness backlog); `cvf1` (the mechanical post-merge-`main` verify); `aem1` (the parenthetical-name interaction-match precision gap); the malformed-`ae_profile` fail-loud upgrade; `axm7` (extend the core-capability gate to the orchestrator); the standalone full-plan render screen + the dashboard reskin (operator's worktrees); the `/generate-plan` slash-command wrapper; the `llna` hook fix; carry-overs `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`; operator-gated July-visit prep (LM-01) — first session on/after **2026-06-29** runs the LM-01 scoped drift audit.

## Landmark window check (close step 8.7)

S77 close (2026-06-19): RE-OPENED `vault/meta/landmarks.md` at close step 8.7 (unchanged, `last_reviewed` 2026-06-19). No landmark edited — S77 was plan-generation code (the doctor-visit-queue DATA LAYER) on PII-free SYNTHETIC fixtures (no real operator data ingested, no Whoop baseline start, no MD-handoff artifact GENERATED — the queue is the data layer; the SBAR handout render is deferred). LM-01 (First MD visit) **2026-07-13**: no trigger window open today (the 14-day scoped-drift-audit window opens **2026-06-29**, the 7-day MD-handoff window 2026-07-06). NOTE: the doctor-visit-queue is the LM-01 handout's data foundation — building it now is ahead-of-window LM-01 prep. LM-02 (Whoop) / LM-03 (23andMe) date-TBD. LM-04 (first HTML artifact) NOT triggered (no committed HTML artifact). No landmark actions due.
