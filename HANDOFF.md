...
## Scope Contract — Session 76 (2026-06-19)

Goal: Wire the supplement↔Rx **BPMH axis (`rxbp`)** — the medical-liaison's marquee watchlist check — reading the operator medication list THROUGH the `router.summarize` PII de-identification boundary (ADR-0006-T0) as de-identified Rx-interaction-class tokens, then HOLDING a supplement whose author-declared interaction class stacks against a present Rx-class and routing it through the SAME S74 `adjudicate` gate via the established multi-concern hold model (PF-S75-01). The raw drug names are dropped at `summarize` — no pharmacology DB enters `scripts/`, no raw operator medication in any tracked artifact (the classes are operator/liaison-curated in the untracked store; tests + examples use synthetic Rx-class tokens). Operator delegated S76 autonomously ("proceed"); chosen as the highest-rigor remaining Phase-4 surface — it hardens the project's most security-load-bearing seam (the PII boundary) and produces the medication-interaction flags most relevant to the July MD visit (LM-01).

Acceptance criteria:
- [ ] AC1: `router.summarize`'s PII boundary extends to medications WITHOUT leaking raw drug names — a new de-identified field-set field (the curated Rx-interaction-class token-set) is added to `SUMMARY_FIELD_SET`, the raw medication-list source class is added to `EXCLUDED_RAW_PII`, the disjointness assertions still hold, and the whitelist gate still RAISES on any raw drug name reaching the payload. Adversarial PII test: a raw medication name never survives `summarize`; **mutation-proven** (break the derivation → a raw name leaks → the test goes RED). No pharmacology DB in `scripts/` (classes are operator/liaison-curated in the store).
- [ ] AC2: `reconcile` holds a supplement whose author-declared interaction class stacks against a present Rx-class in an INDEPENDENT `rx_bpmh_held` set; `generate_plans` routes each via the REUSED `adjudicate` gate (a `_rx_bpmh_safety_finding` → the S74 override-record schema + critical-non-overridable gate, `adjudicate.py` UNCHANGED); a content-valid override clears it; a domain records only when in NONE of `holds` / `conflict_held` / `rx_bpmh_held`. No new store stream. **Mutation-proven RED.**
- [ ] AC3: PF-S75-01 interaction-probe (Tier-1) — adversarially test the new hold's interaction with EVERY existing hold (additive-AE, conflict, energy-bounce, RED-S/LEA): a supplement held by additive-AE AND rx-bpmh clears each concern independently; >1 concurrent concern on one domain; clear-one-while-another-open — each tested BEFORE Tier-2/3.
- [ ] AC4: verified by a real `medical-liaison` dispatch (full profile inlined per INV-ROLE-INLINING) adjudicating a genuine supplement↔Rx BPMH finding E2E through `generate_plans` — a content-valid override clears + records, an invalid/refused one holds. Captured as `liaison-rxbp-*.example.json` using SYNTHETIC Rx-class tokens — no real operator medication, no raw PII in any artifact. Not a unit stub.
- [ ] AC5: backward-compat + integration mandate — no Rx-class present → no rx-bpmh hold; all S70–S75 tests + the PII-boundary/whitelist tests stay green; `--self-test` + core-capability gate green.
- [ ] AC6: plan-integrity + QA (Tier-2; **the design spec + the router PII docs LISTED in WILL-touch** — no MF-1 repeat) → `/review-pr` (Tier-3, 6-agent) → fix legitimate → `/merge`; full close, **re-run the close gate on the final post-merge `main`** + cite the `main` SHA (PF-S74-01 discipline).

Files I WILL touch: `scripts/plan/router.py` (the PII boundary: `SUMMARY_FIELD_SET` + `EXCLUDED_RAW_PII` + the Rx-class derivation + the disjointness assertions); `scripts/plan/orchestrate.py` (the `rx_bpmh_held` hold in `reconcile` + the rx-bpmh adjudication loop in `generate_plans` + `_rx_bpmh_safety_finding`); `tests/plan/test_router.py` + `tests/plan/test_orchestrate.py`; new `docs/plan-generation/examples/liaison-rxbp-*.example.json`; `vault/design/plan-generation-pipeline-v1.md` + `docs/plan-generation/author-dispatch-process.md` (rxbp WIRED freshness); HANDOFF/PF-log/harvest/beads/session-note at close.
Files I will NOT touch: `scripts/plan/adjudicate.py` (REUSE unchanged — the gate generalizes); `scripts/store/*` (read-only via `store_read`; no new stream; the raw med list lives in the untracked operator store); `assemble.py`/`generate_plan.py` core; the deployed `.claude/agents/*` (inline for dispatch); the wiki research-list / `vault/library/` (parallel sessions own it); `main` directly; any tracked file holding the operator's real medication list.
NOT doing: the doctor-visit-queue/SBAR artifact (the next surface, own session — a store-write); the measure/adjust legs; a real operator-medication ingest (rxbp reads the existing de-identified store summary); changing the `adjudicate` gate / override-record schema; a new store stream; a full drug-drug-interaction pharmacology database (author-declared classes + the curated Rx-class match only).
Invariants at risk: INV-BRANCH-NOT-MAIN (built in `../aplus-s76rxbp` worktree off `main`; idle trunk parked on `parking/s76-rxbp`; `.venv` provisioned); **ADR-0006-T0 PII boundary** (the security-load-bearing seam this extends — the adversarial PII test is the guard); the `block-pii-commit` hook + the standing no-operator-PII rule (synthetic Rx-class tokens only in tracked artifacts); INV-CORE-CAPABILITY (extends the wired path — stays green + `--self-test`); INV-ROLE-INLINING (real medical-liaison dispatch + the review agents — FULL profiles verbatim); INV-OVERRIDE-RECORD-SCHEMA + INV-CRITICAL-NON-OVERRIDABLE (REUSED, not changed); INV-CLOSE-AUDIT / INV-PF-ATTESTATION / INV-SKILL-TRACE / INV-HARVEST-CAPTURE at close; store-surface battery N/A (read-only via `store_read`). Core-capability-first gate (PF-S63-02): SATISFIED — the core plan-generation path is wired (INV-CORE-CAPABILITY green since S70); `rxbp` extends the held-line terminal gate to the Rx/BPMH axis (the liaison's marquee safety check), the most security-load-bearing remaining Phase-4 surface. **PF-S74-01 discipline: re-run the full close on the final post-merge `main` + cite the `main` SHA.**

### S76 Scope Contract Evaluation (2026-06-19, volatile)

- **AC1 (PII boundary extends, raw names never leak) — PASS.** `router.summarize` carries a new de-identified `rx-interaction-classes` field (curated class tokens); `medication-list` is in `EXCLUDED_RAW_PII`, never read (no `_RAW_TO_FIELD`/derivation entry); the disjointness assertions hold; the whitelist gate still RAISES on a raw drug field. Adversarial PII test mutation-proven (break the per-token scan → raw name leaks → RED). **Refined post-Tier-3:** the 8j6 backstop scans PER TOKEN (the whole-value scan was defeated by `scan_text`'s 4096-char cap once this first list-typed field exceeds it — SEC-1, the real PII leak Tier-3 caught + I fixed). No pharmacology DB in `scripts/`.
- **AC2 (BPMH hold → liaison gate) — PASS.** `reconcile` holds a class-stacking compound in an INDEPENDENT `rx_bpmh_held` set; `generate_plans` routes each via the REUSED `adjudicate` gate (`_rx_bpmh_safety_finding` → the S74 override-record schema + critical-non-overridable gate, `adjudicate.py` UNCHANGED); a content-valid override clears it; a domain records only when in NONE of `holds`/`conflict_held`/`rx_bpmh_held`. No new store stream. Mutation-proven RED.
- **AC3 (PF-S75-01 interaction-probe, Tier-1) — PASS.** Tested the new hold's interaction with additive-AE + conflict (each cleared independently; the triple-concern case — all three on one domain — added at Tier-2, mutation-proven RED) + the peptide-side multi-domain composition. The boundary-INFRASTRUCTURE interaction (the new field × the existing scan cap) was the axis I under-probed → SEC-1 (PF-S76-01).
- **AC4 (real liaison E2E) — PASS.** Two real deployed `medical-liaison` BPMH dispatches genuinely DISCRIMINATED on the medication: aspirin+fish-oil → HIGH/H3 cleared via a content-valid informed-refusal override; warfarin+fish-oil → CRITICAL/H2 non-overridable auto-block (the operator's informed-refusal NOT lowering the band, grounded against the project's own H2 classification of the vitamin-K/warfarin axis). Captured `liaison-rxbp-{cleared,blocked}.example.json` (SYNTHETIC Rx — no real operator medication). The cleared envelope was REVISED by the liaison under the real gate's reject feedback (Core Rule 8 repair).
- **AC5 (backward-compat + integration mandate) — PASS.** No Rx-class present → no rx-bpmh hold; all S70–S75 tests + the PII-boundary tests stay green; suite 1030/3, floor 15/0, `--self-test` + core-capability green.
- **AC6 (review + close) — PASS.** plan-integrity (2 doc-freshness SHOULD-FIX — SF-1/SF-2, both fixed) + QA (PASS + triple-concern + peptide-side coverage) Tier-2 → `/review-pr` 6-agent Tier-3 (**1 MUST-FIX SAFETY finding SEC-1** — a real PII leak on the boundary — + QUAL-1 + TEST-rxbp-2 fixed; 2 NOT_ACTIONABLE; blind-verified 3/3 RESOLVED executed) → `/merge` PR #174; full close, close gate re-run on final `main` + the `main` SHA cited (the close-on-final-`main` discipline).

### Drift checks (S76 close)

- **Task drift:** all 6 ACs PASS. In-scope generalization beyond the literal contract (flagged, not silent): the rxbp screen covers BOTH `supplements` AND `peptides` (the contract focused on supplements; the mechanism is symmetric and a peptide stacking with an anticoagulant is the same watchlist case). The 8j6 scan was REFINED to per-token mid-review in response to Tier-3 SEC-1 (flagged). No silent drift.
- **Architecture drift:** no invariant degraded; the S74 gate + its 2 invariants REUSED unchanged (`adjudicate.py` not in the diff). The ADR-0006-T0 PII boundary was EXTENDED, and the extension was hardened (the SEC-1 leak caught + fixed before merge — the boundary is stronger, not weaker). INV-CORE-CAPABILITY green; INV-BRANCH-NOT-MAIN held (built in `../aplus-s76rxbp` off `main`, idle trunk parked `parking/s76-rxbp`, `.venv` provisioned); INV-ROLE-INLINING held (2 real liaison + the 6 review agents + plan-integrity + QA full-profile). Toward the design (the BPMH axis closed).
- **Vision drift:** none, toward-vision. The BPMH axis now reads the operator medication surface de-identified and holds-and-adjudicates a class-stacking compound via the same terminal gate — the liaison's marquee safety check, most relevant to the July MD visit. `design/vision.md` first sentence unchanged.

### PF attestation

S76 close (2026-06-19): **One new PF promoted — PF-S76-01** (Tier-1/Tier-2 self-review under-probed the new PII-backstop's BOUNDARY-SIZE interaction with the existing `scan_text` 4096-char truncation; Tier-3's security-auditor caught a real PII leak on the ADR-0006-T0 boundary — recurrence 3 of self-review-under-probes-interactions; gate-caught before merge, nothing shipped, fixed via a per-token scan + mutation-proven + blind-verified RESOLVED; structural broaden-the-interaction-probe discipline beaded `pwpr`). The build's other disciplines held — both rxbp behaviors mutation-proven RED; a REAL pair of medical-liaison BPMH dispatches verified the gate (genuinely discriminating aspirin→cleared vs warfarin→auto-block); the gate REUSED unchanged; the integration mandate honored. The prior session's interaction-probe discipline HELD for the hold-composition axis (no multi-concern regression). The close-on-final-`main` discipline applied (re-run on the post-merge `main` + the `main` SHA cited). Full attestation + the per-PR skill-trace table (PR #174) + the disclosure ledger in `memory/process-failures.md` Session 76.


## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-75 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; …; S74 archived S73; S75 archived S74; S76 archived S75) to keep this handoff lean. The current (S76) scope contract is above; the archive holds the prior-session archaeology.
...
1. **Probe a new security-boundary value's BOUNDARY-SIZE + existing-shared-infra interaction at Tier-1 (PF-S76-01, recurrence 3).** Three times now (S74 routing-key input class, S75 multi-concern hold composition, S76 a new list-typed field defeating the existing `pii_scan.scan_text` 4096-char truncation → a real PII leak) Tier-3 caught a SAFETY/SECURITY-design hole my Tier-1/Tier-2 missed. The prior interaction-probe (bead `sip9`) covered hold×hold; it did NOT cover **a new value/field × the EXISTING SHARED INFRASTRUCTURE it flows through** (a scanner's length cap, a whitelist's shape assumption, a validator's contract). For any new value crossing a security boundary: probe the **BOUNDARY-SIZE case** (the largest the field can legitimately hold) against EVERY guarding validator, and ask whether the new value's SHAPE/SIZE violates an assumption that infra was built under (bead `pwpr`, pairs with `sip9`).
2. **The disciplines that HELD and must keep holding.** (a) **Multi-concern hold model (PF-S75-01, `sip9`):** any safety hold that COMPOSES is modeled as N-independent-reasons from the start; probe new-hold × every existing hold, >1 concern per entity, clear-one-while-another-open (S76's triple-concern probe held). (b) **Close on the FINAL post-merge `main` (PF-S74-01, `cvf1`):** every close re-runs pytest + `close-audit --session N` on `main` AFTER merge+pull and the attestation CITES the post-merge `main` SHA; never claim "fully gated" before re-verifying the merged trunk; log an operator-surfaced catch AS operator-surfaced.
3. **Standing mechanics + carry-overs.** **Worktree discipline:** separate `git worktree`s; park the idle trunk OFF `main`; Read worktree files at the worktree-absolute path; re-verify `git branch --show-current` before every commit; restore `main` at session end; **provision the worktree `.venv`**; **commit fixes PROMPTLY (`git checkout -- <file>` reverts an uncommitted fix to HEAD).** **Paste FULL role profiles verbatim — abbreviating one is denied by `enforce-role-inlining`.** GraphQL EXHAUSTED → REST for PR-create/merge (`gh api`, full-40-char SHA guard). `git add` SEPARATE from `git commit` (PII hook denies the combined call). bd repo-ID mismatch in a worktree: `bd sync --flush-only` does NOT write `issues.jsonl` — surgically edit `issues.jsonl` (scrub new-bead owner → the `your-github-email@example.com` placeholder) + commit it in the worktree. Two parallel wiki sessions push to `origin/main` — rebase-sync at merge, never touch `vault/library/`. **Close-step-8 freshness un-gated until `dfg1` — RUN step 8 explicitly each close.** Carry-overs: `pwpr` (boundary-infra interaction-probe, PF-S76-01) / `sip9` (multi-concern interaction-probe) / `cvf1` (close-gate-on-final-`main`) / `dfg1`/`df1x` (doc-freshness gate + backlog) / `aem1` / `axm7` / `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`. Pencil: ALWAYS pass `filePath`.

## Current State (volatile)

- **S76 (2026-06-19) wired the supplement↔Rx BPMH axis (`rxbp`) — the Phase-4 follow-on that reads the operator medication surface de-identified and adjudicates a class-stacking compound.** `scripts/plan/router.py`: a new de-identified `rx-interaction-classes` field carries operator/liaison-curated class tokens (the canonical AE-class vocabulary) through the `router.summarize` PII boundary; the raw `medication-list` is named-excluded and NEVER read (no pharmacology DB in `scripts/` — the drug-name→class de-identification is a store-layer curation step). The 8j6 PII backstop scans PER TOKEN (the whole-value scan was defeated by `pii_scan.scan_text`'s 4096-char cap once this first list-typed field exceeds it). `scripts/plan/orchestrate.py`: a compound (supplements OR peptides) whose declared `ae_profile.additive_classes` intersect the operator's present Rx-classes is held in an INDEPENDENT `rx_bpmh_held` set + routed through the REUSED S74 `adjudicate` gate (`adjudicate.py` unchanged); records only when in NONE of `holds`/`conflict_held`/`rx_bpmh_held`.
- **PR #174 → `main` (rebase merge; the S76 close re-run on the post-merge `main`, SHA cited in the close report).** Tier-2 plan-integrity (2 doc-freshness SHOULD-FIX fixed) + QA (PASS + triple-concern + peptide-side coverage) → `/review-pr` 6-agent Tier-3 (**1 MUST-FIX SAFETY finding** — SEC-1, a real PII leak: the 8j6 backstop's whole-value scan defeated by the `scan_text` 4096-char cap — fixed via a per-token scan, mutation-proven RED, **blind-verified 3/3 RESOLVED executed**; + QUAL-1 + TEST-rxbp-2; 2 NOT_ACTIONABLE) → `/merge`. Verified E2E over TWO real `medical-liaison` BPMH dispatches that DISCRIMINATED on the medication (aspirin→HIGH/H3 cleared; warfarin→CRITICAL/H2 auto-block); `liaison-rxbp-{cleared,blocked}.example.json`. Single-PR close.
- **PF-S76-01 promoted** (Tier-1/Tier-2 under-probed the new PII-backstop's BOUNDARY-SIZE interaction with the existing scan cap; recurrence 3 of self-review-under-probes-interactions; gate-caught before merge; structural broaden-the-interaction-probe discipline beaded `pwpr`).
- **pytest 1030/3 (worktree, native `.venv`).** Core-capability gate green; negative-test floor 15/0.
- **Beads:** `71s4` (rxbp WIRED; the doctor-visit-queue artifact + the measure/adjust legs remain); `rxbp` CLOSED; `pwpr` (PF-S76-01) created; carry-overs `sip9`/`cvf1`/`dfg1`/`df1x`/`aem1`/`axm7`/`llna`/`0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`.
...
**Historical (kept for reference):** `vault/sessions/session-76.md`.

## What Is Next (volatile)

### S77 — the doctor-visit-queue/SBAR artifact, then the measure/adjust legs

The additive-AE + conflict + Rx-BPMH axes all close to operator-usable via the terminal gate. Next, per `vault/design/plan-generation-pipeline-v1.md` Phase 4 — in a dedicated `git worktree`, idle trunk off `main`, with a provisioned `.venv`:

1. **The doctor-visit-queue collation + SBAR handout artifact** — the medical-liaison's owned queue surface (`artifacts/_doctor-visit-queue` + its SBAR skeleton; the queue is populated incrementally by every `risk_tier:medium+` write + every risk HALT). This is a SUBSTRATE WRITE surface — it MUST satisfy `docs/checklists/store-adversarial-tests.md` (the adversarial cross-stream + dedupe-collision + mutation battery), checked at Tier-1 + the Tier-2 QA dispatch. Highest July-MD-visit relevance (LM-01): the handout is the first downstream consumer of the held-line + the BPMH surface.
2. Then the **measure** leg (a production caller for `record_plan_tracking` — currently schema-only) + the **adjust** leg (re-plan from tracking) — the rest of the closed loop.

**Discipline:** scope contract at open (LIST the design spec + any doc in WILL-touch — MF-1); **worktree + idle trunk off `main` + provision the `.venv`**; **for the queue's substrate write, satisfy the store-adversarial battery**; **at Tier-1, probe any new security-boundary value's BOUNDARY-SIZE + existing-shared-infra interaction (PF-S76-01) AND any new composable hold's multi-concern interaction (PF-S75-01)**; **write close artifacts in-worktree BEFORE merge**; **commit fixes promptly**; **paste FULL role profiles verbatim**; **verify the production path E2E with a real `medical-liaison` dispatch**; `/review-pr` → `/merge`; **re-run the close gate on the final post-merge `main` + cite the `main` SHA (PF-S74-01)**.

**Deferred (NOT blocking):** `pwpr` (broaden the Tier-1 interaction-probe — PF-S76-01) + `sip9` (the multi-concern interaction-probe); `dfg1` (the doc-freshness gate — PF-S73-01 structural fix) + `df1x` (the doc-freshness backlog); `cvf1` (the mechanical post-merge-`main` verify); `aem1` (the parenthetical-name interaction-match precision gap); the malformed-`ae_profile` fail-loud upgrade; `axm7` (extend the core-capability gate to the orchestrator); the standalone full-plan render screen + the dashboard reskin (operator's worktrees); the `/generate-plan` slash-command wrapper; the `llna` hook fix; carry-overs `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`; operator-gated July-visit prep (LM-01) — first session on/after **2026-06-29** runs the LM-01 scoped drift audit.

## Landmark window check (close step 8.7)

S76 close (2026-06-19): RE-OPENED `vault/meta/landmarks.md` at close step 8.7 (unchanged, `last_reviewed` 2026-06-19). No landmark edited — S76 was plan-generation code (rxbp) on PII-free SYNTHETIC operator/Rx fixtures (no real operator data ingested, no Whoop baseline start, no MD-handoff artifact). LM-01 (First MD visit) **2026-07-13**: no trigger window open today (the 14-day scoped-drift-audit window opens **2026-06-29**, the 7-day MD-handoff window 2026-07-06). LM-02 (Whoop) / LM-03 (23andMe) date-TBD. LM-04 (first HTML artifact) NOT triggered (no committed HTML artifact). No landmark actions due.
