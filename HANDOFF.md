...
Goal: Merge the owed S57 close PR #121, then design-then-build the 30-day care-team rollup (Zone 5) — a `juc`-style decision note locking the specialist→store-stream mapping + the 30-day freshness status/color rule + the sparse-domain honesty handling, a read-model aggregating the existing streams into a per-specialist 30-day status, and the Zone 5 render carrying the colored per-domain status line — advancing the no-placeholder-dashboard ("done") path through a full gated `/review-pr` + `/merge` lifecycle.

Acceptance criteria:
- [ ] AC1: PR #121 (S57 close docs) reviewed (3-agent docs subset, full methodology) + REST rebase-merged; both `/review-pr` and `/merge` invoked fresh via the Skill tool; baseline green after (785/2).
- [ ] AC2: a `juc`-style vault decision note (`vault/decisions/`) locking (i) the specialist→store-stream mapping, (ii) the 30-day freshness status/color rule (green = touched ≤30d, amber = stale 30–60d, grey = none), (iii) the sparse-domain honesty handling (option (a): all 16 cards, honest grey on the ~9 streamless domains).
- [ ] AC3: a read-model aggregating the existing streams (`plan`/`plan-track`/`panel`/`watch-out`/`calendar`/…) into a per-specialist 30-day status; tests for the 30-day window boundary, the no-data handling, and cross-stream isolation (store-read surface — `docs/checklists/store-adversarial-tests.md` applies for the categories that fit a read-model; flagged at Tier-1 + Tier-2 QA).
- [ ] AC4: Zone 5 render (`_care_team_zone`) carries the colored per-domain status line per the signed card anatomy; honest empty states hold (streamless domains grey; all-empty store keeps the current "no rollup yet"); demo'd with synthetic data; locked sets untouched.
- [ ] AC5: full 6-agent `/review-pr` + `/merge` on the implementation PR; every legitimate finding fixed + blind-verified, 0 suppressed; suite green after merge.
- [ ] AC6: full session close per protocol — all FIVE audits incl. `skill-trace-audit.sh --session 58`, 6-clause rotation, PF attestation with the per-PR table.

Files I WILL touch: a new rollup read-model module under `scripts/store/` (or `scripts/`); `vault/design/templates/dashboard.py` (Zone 5 `_care_team_zone` render + status wiring); `tests/store/` + `tests/generate/` (Zone 5); `vault/decisions/` (the decision note); `vault/components/` (component doc); `.beads/issues.jsonl` via `bd`; HANDOFF/vault at close. `vault/design/templates/component_set.py` ONLY if a shared status-dot/card helper is genuinely needed (flag if so).

Files I will NOT touch: `.claude/hooks/*` + `.claude/settings.json` (no hook auth this session); INVARIANTS.md rows; `.claude/agents/*`; `main` directly; `PALETTE`/`SERIES`/`ACCENTS` values (ADR-0009 locked); `SUMMARY_FIELD_SET`+`EXCLUDED_RAW_PII` (Security MEDIUM-2 lock); `report.py`; the other dashboard zones (1–4, 6, 7); `goal_schema`/`calendar_schema`/`plan_schema`/`loop_schema` write paths (unless a shared read util genuinely needs it — flag).

NOT doing: an "on track / off track" status rule (needs unsigned per-domain targets — overreach); the correctness/governance tail (`b6um`/`e3b`/`02pe`/`r3pq`/`5zfk`/`rn3v`/`imev`/`tdre`/`vjsw`/`dt0t`/`1ww`) — exception: `b6um` folds in ONLY if it touches the exact AA gate I extend (flagged, not pre-adopted); `pq7m`; `dqyv` + the archive-SHA-check bead (governance follow-ups, opportunistic); LM-02/LM-04; the library-population track.

Invariants at risk: none structurally. INV-SKILL-TRACE binds PR #121 + the implementation PR (per-PR table at close). Store-read surface: the rollup read-model triggers `docs/checklists/store-adversarial-tests.md` consideration (Tier-1 + Tier-2 QA). PF-S49-01: build strictly from the signed Zone 5 anatomy. PF-S6-01: the decision note's mapping verified against the live streams before building. AC4 reduces the awaiting-zone count to zero (except wearable, LM-02-gated).

### S58 Scope Contract Evaluation (2026-06-13, volatile)

- **AC1 (PR #121 S57-close reviewed + REST rebase-merged; baseline green) — PASS.** Docs 3-agent subset, full methodology; all three agents (code-quality/contracts/historical) returned 0 findings, each git-verifying the rotation completeness + the on-main/off-main SHA classification + counts/beads; 0 suppressed; both skills fresh; rebase-merge on-main `93c4f1c` (close `55a1ab0`); baseline 785/2 after.
- **AC2 (decision note locking mapping + freshness rule + sparse handling) — PASS.** `vault/decisions/2026-06-13-care-team-rollup-zone5-mapping.md` records Decision 1 (the specialist→store-stream mapping — 6 of 16 mapped by existing wiring, 10 streamless), Decision 2 (the 30-day freshness status/color rule: green ≤30d / amber 31–60d / grey >60d-or-none; future events EXCLUDED per the review refinement), Decision 3 (option (a): all 16 cards, honest grey on the streamless — Walter-confirmed "'a' sounds right"). The mapping was verified against the live streams before building (PF-S6-01).
- **AC3 (read-model aggregating existing streams + tests) — PASS.** `scripts/store/care_team_rollup.py` is a pure read-model (no new store stream): `resolve_rollup(store_read, today)` attributes each reading by prefix/calendar-category and reduces to a per-specialist 30-day freshness from the latest RECORDED-DATA timepoint (future excluded). Store-adversarial battery applicable categories present + non-tautological — cross-stream isolation (constructed + real `store.read_all` end-to-end) + the 30/60-day threshold boundary; mutation battery proven RED per the per-test docstrings (CURRENT_MAX_DAYS=999, dropped-prefix, last-write-wins, dropped future-exclusion — all run RED + reverted); dedupe categories N/A for a read-model (documented).
- **AC4 (zone-5 render colored per-domain status; honest empty; locked sets) — PASS.** `_care_team_zone(rollup)` carries a DATA-STATE status dot (`.sdot-*` = PALETTE good/watch/muted — non-text chrome, ADR-0009 D3; the name's category glyph dot untouched) + a muted recency caption; all 16 cards render, the streamless read "no data yet" grey, an all-empty store keeps the static "no rollup yet"; demo'd with synthetic data. No new AA-gated text pair (`b6um` untriggered). `PALETTE`/`SERIES`/`ACCENTS` + `SUMMARY_FIELD_SET`/`EXCLUDED_RAW_PII` byte-unchanged.
- **AC5 (full 6-agent `/review-pr` + `/merge` on #122) — PASS.** Security 0, Contracts 0 (layering/chrome-data-state/locked-constants/call-sites/note↔code fidelity all PASS). 3 LEGITIMATE fixed + blind-verified 4/4 (QUAL-1 stale note anchors; TEST-1 order-blind latest-wins test; HIST-1 module-docstring mutation-scope overclaim — the S56 #118 class); QUAL-2 NOT_ACTIONABLE; BUG-1 DECISION-by-rule but ORCHESTRATOR-ELECTED to refine for honesty (future events excluded from freshness — "updated today" for a scheduled event was a real misstatement); 0 suppressed; rebase-merge on-main `afd40a8`; suite 821/2.
- **AC6 (full session close per protocol) — PASS** (this close; five mandatory audits incl. `skill-trace-audit.sh --session 58`, 6-clause rotation, PF attestation with the #121/#122 per-PR table; DOCUMENT_RUBRIC run for real at step 8 — the S57 lesson).

Flagged additions (none silent): the BUG-1 future-event freshness exclusion (a review-driven honesty refinement of my OWN same-session decision note — the triage classified it DECISION, I elected to refine the note for honesty; surfaced to Walter); the `component_set.py` four `.sdot` CSS rules (the contract's conditionally-allowed "if a shared helper is genuinely needed" — the data-state status dot); the new `vault/components/care-team-rollup.md` component-interface doc.

### Drift checks (S58 close)

- **Task drift:** all 6 ACs PASS. The BUG-1 future-exclusion was a review-driven honesty refinement of the same-session decision note (flagged, not silent — the blind triage classified it DECISION; the orchestrator elected to refine the note because the "updated today" wording for a future scheduled event is a real misstatement, and the "decision" it relitigated was this session's own note). The `.sdot` CSS + the component doc are surfaced. No silent drift.
- **Architecture drift:** none degraded. A new read-model `care_team_rollup` reads the SAME `store_read` the render already groups — a PARALLEL read strictly downstream of the render's fail-loud routing guard (verified by the contracts review), not a routing replacement. ADR-0009 D3 chrome/data-state separation honored (the freshness dot is PALETTE data-state, NOT an accent, and is a SEPARATE element from the name's category glyph dot). `PALETTE`/`SERIES`/`ACCENTS` byte-identical; PII boundary untouched (`report.py` not touched; `SUMMARY_FIELD_SET`/`EXCLUDED_RAW_PII` byte-identical). The `_timepoint_date` try/except matches the ESTABLISHED `component_set.reading_date` timepoint-tolerance contract (the existing unparseable-timepoint tests require it) — not new defensive programming, and store-layer-local to avoid a template-layer import inversion. INV-SKILL-TRACE bound #121+#122; INV-TRUNK-COMPLETENESS green at open + close.
- **Vision drift:** none. After S58 the same local-first health tracking + planning system, V1 complete, whose dashboard Care Team zone now renders real per-specialist 30-day recorded-data freshness (a data-state status dot + recency caption, honest grey on the streamless domains) — the THIRD and LAST of the three remaining "awaiting" zones now populates from data. The dashboard is no-placeholder except the wearable surface (LM-02, operator-gated). Matches the first sentence of `design/vision.md`.

### PF attestation

S58 close (2026-06-13): No NEW PF-class entries PROMOTED this session. One observed process issue NOT promoted (a PF-S51-01-shape concurrency window): the 6 Phase-1 review agents ran in the SHARED working tree, and several (test-coverage, historical) ran mutation batteries concurrently — one reviewer momentarily OBSERVED another's un-reverted `plan::workout` drop; the final tree was verified pristine (`git status` clean, all reverts completed, suite 821/2), so no harm, but the race window is the exact PF-S51-01 shape. Mitigation/watch: review agents that run mutation batteries should get an isolated worktree (or be told read-only); the blind triage + blind verification this session WERE told read-only and stayed clean. The PF-S13-01/PF-S37-01 falsification window HELD this close — DOCUMENT_RUBRIC was RUN for real at step 8 with cited evidence (the 2 modified `.md` files carry today's `last_reviewed`; the decision note's `ektw` reference is a resolves-pointer; the PF log is append-only). Full attestation with the per-PR invocation table (#121, #122, all YES) and non-promoted observations in `memory/process-failures.md` (Session 58 section).

## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-57 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; S33 archived S32; …; S55 archived S54; S56 archived S55; S57 archived S56; S58 archived S57) to keep this handoff lean. The current (S58) scope contract is above; the archive holds the prior-session archaeology.
...
2. **The dashboard "done" path is COMPLETE — all three awaiting-zone data models shipped (goal/zone-6 S56, calendar/zone-2 S57, care-team-rollup/zone-5 S58); the dashboard is no-placeholder except the wearable surface (LM-02, operator-gated on Walter's Oura data).** So the FORWARD work shifts off the done-path to the correctness/governance tail (`b6um` sub-AA legend [the `watch-text` token is the fix vehicle, when the AA gate is next touched]; `e3b`/`02pe`/`r3pq`/`5zfk`/`rn3v`/`imev`/`tdre`/`vjsw`/`dt0t`/`1ww`) and the deferred follow-ups (`dqyv` promote `loop_schema._reading`/`_content_tag` public — 3-consumer trigger fired; the archive-SHA-check bead). `SUMMARY_FIELD_SET`/`EXCLUDED_RAW_PII` locked (Security MEDIUM-2); `PALETTE`/`SERIES`/`ACCENTS` locked (ADR-0009). Standing mechanics: REST `gh api` with the full-40-char SHA for PR ops (GraphQL + a low core quota both rate-limit under heavy sessions — budget the calls, background-wait the reset, never poll-loop); stage with `git add` separately from `git commit`; `.claude/hooks/*` edits need per-session authorization.
3. **Two active process watches.** (a) **Off-main-archive-SHA (count=2):** the repo rebase-merges, so the close-time `scope-contract-archive.md` entry must cite ON-MAIN SHAs (verified via `git merge-base --is-ancestor`), NOT the pre-rebase `fix/*` tips — promote + build the archive-SHA-check at N=3. (b) **Concurrent-mutation-in-shared-tree (PF-S51-01 shape):** Phase-1 `/review-pr` agents that run mutation batteries (test-coverage, historical) do so in the SHARED working tree — give such agents an isolated worktree or tell them read-only (the blind triage/verification agents WERE told read-only and stayed clean); always verify `git status` pristine after the review.

## Current State (volatile)

- **S58 (2026-06-13) ran two full PR lifecycles, both merged to `main` (at `afd40a8` as of 2026-06-13 S58 close); suite 821 passed / 2 skipped (+36 from the 785 at session open).** Merged: #121 (S57 close docs — 3-agent subset, 0 findings, all git-verified), #122 (the care-team rollup — full 6-agent review). 0 findings suppressed; both PRs invoked `/review-pr` + `/merge` fresh via the Skill tool (REST rebase-merge under GraphQL + low-core throttle).
- **The 30-day care-team rollup (dashboard zone 5) SHIPPED (`ektw` closed) — the LAST awaiting zone:** `scripts/store/care_team_rollup.py` is a new READ-MODEL (no new store stream) attributing each store reading to a specialist (by prefix / calendar category) and reducing to a per-specialist 30-day RECORDED-DATA freshness (green ≤30d / amber 31–60d / grey >60d-or-none; future events excluded). The dashboard `_care_team_zone` renders a data-state status dot (PALETTE good/watch/muted) + a muted recency caption; all 16 cards show, the ~10 streamless read honest grey "no data yet", an all-empty store keeps "no rollup yet". Built per the signed zone-5 anatomy + the new decision note (`vault/decisions/2026-06-13-care-team-rollup-zone5-mapping.md`); Walter confirmed option (a) (all-16, honest grey). **The dashboard is no-placeholder except the wearable surface (LM-02).**
- **The #122 review was load-bearing on the orchestrator's own work:** the 6-agent panel (Security 0, Contracts 0) caught a real HONESTY defect in my SAME-SESSION decision note — a future scheduled event made a specialist read "updated today" — so I refined the freshness rule to exclude future events (recorded-data recency only) and flagged it to Walter; plus 3 doc/test nits fixed + blind-verified 4/4 (stale note anchors, an order-blind latest-wins test, a module-docstring mutation-scope overclaim = the S56 #118 class). A PF-S51-01-shape concurrent-mutation race was observed (review agents mutating the shared tree) — tree verified pristine, no harm.
- **Beads:** closed `ektw` (merge provenance #122). Still OPEN and next-up: the correctness/governance tail (`b6um`, `e3b`, `02pe`, `r3pq`, `5zfk`, `rn3v`, `imev`, `tdre`, `vjsw`, `dt0t`, `1ww`); `pq7m` (store-schema-gated); `dqyv` (3-consumer promotion); the archive-SHA-check follow-up.
- **Active landmarks:** no trigger windows opened (LM-01 July visit date still TBD).

**Historical (kept for reference):** `vault/sessions/session-58.md`.

## What Is Next (volatile)

### Resume checklist — next session (S59) open

**FIRST: merge the S58 close PR** (`fix/s58-close` → `main`; docs 3-agent `/review-pr` subset → REST rebase `/merge`, BOTH invoked fresh via the Skill tool; `skill-trace-audit.sh --session 59` runs at close per INV-SKILL-TRACE). Open normally (Session Start Protocol; `branch-completeness-audit.sh` at OPEN; baseline on `main` **821 passed / 2 skipped**).

**S59 plan (the done-path dashboard is COMPLETE — forward work shifts to correctness/governance + follow-ups; operator to prioritize at open, NOT pre-adopted):**

1. **The correctness/governance tail.** `b6um` (sub-AA dashboard legend — the `watch-text` token is the fix vehicle, a quick win the next time the render AA gate is touched); `e3b`/`02pe`/`r3pq`/`5zfk`/`rn3v`/`imev`/`tdre`/`vjsw`/`dt0t`/`1ww` — pick by priority, each through a full review/merge lifecycle. Verify-first each bead against the live code before acting (the S55 stale-bead-path lesson; bead text can name pre-refactor paths).
2. **The deferred follow-ups.** `dqyv` (promote `loop_schema._reading`/`_content_tag` to a public store constructor — the 3-consumer trigger fired at S57); the **archive-SHA-check** bead (mechanize the on-main-SHA check for `scope-contract-archive.md` — the off-main-SHA class is count=2; promote + build at N=3). Opportunistic / when an adjacent surface is touched.
3. **Operator-gated.** `pq7m` store-schema-gated (not a read-model retry); LM-02 (Oura wearable — the last dashboard surface) + LM-04 (first real artifact archive) gated on Walter's data; the library-population `/aplus-research` track operator-deferred.

## Landmark window check (close step 8.7)

S58 close (2026-06-13): re-read `vault/meta/landmarks.md`. No `active` landmark's trigger window opened during S58 (LM-01 July visit — exact date still TBD, >14 days out even from July 1; LM-02/LM-03 date-TBD pending Walter; LM-04 not triggered — S58 built a backend read-model + a dashboard zone render demo'd with SYNTHETIC data, with no operator-facing artifact generated or archived to `vault/artifacts/`; the first real face-sheet/dashboard generation for the July visit remains the LM-04 trigger). No landmark actions due.
