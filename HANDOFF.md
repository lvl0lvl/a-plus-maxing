...
## Scope Contract — Session 95 (2026-06-25)
...
### S95 Scope Contract Evaluation (2026-06-25, volatile)

- **AC1 (Wave 2) — PASS.** ADR-0026-T2 composed `gate_dispatch` built (`scripts/plan/gate_dispatch.py` + `tests/plan/test_gate_dispatch.py`) TDD-per-recipe. Wave-2 checkpoint EXECUTED green: exact 3-key disposition `{accept, safety_passed, revise_domains}`; fail-closed-on-malformed (a raised / non-dict / missing-key judge or review → `safety_passed` NOT boolean-True → SAFETY_BLOCKED; 0 safety_passed-True over the malformed family); both gates run (judge ≥1 + review ≥2 lenses); AC-2 non-tautological (a structural-deduction REVISE → the SPECIFIC localized domain, a dimension-level REVISE → ALL run-set); the composer-driver seam (real composer output → `plan_driver` disposition-read → promote-on-accept / SAFETY_BLOCKED-on-malformed). EXTEND-NOT-REBUILD held (inner engine + gate callables + driver numstat=0).
- **AC2 (Wave 3) — PASS.** ADR-0026-T3 (the `/generate-plan` A′ skill front-door — reconciled to drive `plan_driver.drive` over the de-identified summary, no fork at the skill level; de-id-IN via `ModelClient.deidentify`) + ADR-0026-T4 (the `core-capability-audit.sh` repoint onto the A′ spine — CALLER → `plan_driver.py`, RUN_GEN_HOST → `plan_orchestrator.py`; the new `scripts/plan/_a_prime_self_test.py` drives the REAL composer + `run_orchestrated`, asserting promotion-on-accept + 0-plans-on-safety-not-True, non-tautological via the `A_PRIME_SELF_TEST_BROKEN_SPINE` self-probe). Checkpoint EXECUTED green: the audit exits 0 on the wired spine, NON-ZERO on a broken spine; the F-007 negative test still RED on an unwired path (now BOTH structural + behavioral legs — Tier-3 added case E). The repoint CLOSES the `71s4`/PF-S63-02 build leg (the mechanical INV-CORE-CAPABILITY guard pins the A′ spine).
- **AC3 (review→merge) — PASS.** Waves 2+3 through Tier-1 SE-TDD + Tier-2 wave review (QA + Security + Architect, RUN-IT verified; Wave 2 clean, Wave 3 one LOW ARCH-1 doc fix) + Tier-3 FULL 6-agent `/review-pr` over the combined Wave-2+3 diff (independence intact — profile-less blind-triage + EXECUTED blind-verify). 5 findings (ALL impact-2; 4 of 6 agents `findings: []` confirmed by execution): blind-triage classified all 5 LEGITIMATE; fixed + blind-verified RESOLVED (mutation→RED non-vacuity proofs) → `/merge` PR #250 → `main` `9785e48`.
- **AC4 (close) — PASS** (this close, on final post-merge main `9785e484` as of 2026-06-25, SHA cited per PF-S74-01).

### Drift checks (S95 close)

- **Task drift — none.** The session delivered exactly the S94-deferred scope: Waves 2-3 built + 3-tier-reviewed + merged. The plan-generation engine live-wiring is now COMPLETE on `main` (all 3 waves); only the operator-present LIVE run remains (correctly out of scope — operator-gated). No silent change; the S94 evaluation pre-declared this as the S95 plan.
- **Architecture drift — none toward violation; net HARDENING.** The merged code wired the composed fail-closed `gate_dispatch` (the driver consumes its 3-key disposition; `safety_passed is True` strict) + the A′ skill front-door + the audit repoint that STRENGTHENS the PF-S63-02 / INV-CORE-CAPABILITY guard (now pins the A′ spine the live path actually takes, non-tautologically — Historical-Context confirmed the repoint strengthens, not weakens). EXTEND-NOT-REBUILD held (inner engine + gate callables + driver numstat=0). The Tier-3 fixes were polish (a dead defensive branch removed, a test-portability fix, a behavioral-RED negative case added, honest test/recipe labels, a frontmatter description refresh) — net quality gains, none architecture-affecting.
- **Vision drift — none.** Still the local-first health tracking + planning system; Waves 2-3 wired the runtime's plan-disposition gate + the operator-facing GENERATE front door + the mechanical core-capability proof. `design/vision.md` first sentence unchanged.

### Per-PR gated-skill invocation table (INV-SKILL-TRACE)

| PR | `/review-pr` invoked fresh | `/merge` invoked fresh | Outcome |
|----|----------------------------|------------------------|---------|
| #250 — Waves 2+3 of the live-wiring (gate_dispatch composer + skill front-door + core-capability-audit repoint + A′ self-test) | YES — FULL 6-agent (Security/Bug-Hunter/Code-Quality/Test-Coverage/Contracts/Historical-Context) over the combined Wave-2+3 local diff `0d177b1..2e28ced`, GraphQL exhausted; profile-less blind-triage + EXECUTED blind-verify intact | YES — REST fallback (full-40-char SHA `-f sha=` guard, head `2f63385…`), rebase, branch deleted local+remote | Merged → `main` `9785e48` (2026-06-25 S95); 5 LEGITIMATE (all impact-2: QUAL-1 stale skill desc / QUAL-2 dead revise-domain fallback / TEST-1 relative self-test interpreter / TEST-2 mislabeled fail-loud arm + recipe / TEST-3 missing behavioral-RED shell case) fixed + blind-verified RESOLVED; quality gate PASS |

### PF attestation

S95 close (2026-06-25): No new PF-class entries this session. The session executed cleanly — Waves 2-3 built TDD-per-recipe via `/execute-plan`, each checkpoint EXECUTED green, Tier-2 wave-reviewed, then a combined Tier-3 6-agent `/review-pr` over the Wave-2+3 diff with both independence phases intact. The defining positive: the layered review worked exactly as designed — 4 of 6 review lenses returned `findings: []` (confirmed by execution: 34 tests run, broken-spine exit 1, numstat=0, no PII/key literals), and the 5 surfaced findings were ALL low-severity polish (impact-2), every one blind-triaged LEGITIMATE, fixed, and blind-verified RESOLVED with mutation→RED non-vacuity proofs BEFORE merge. Observed but did NOT promote: (i) the block-commit-main hook false-positives on a WORKTREE commit (it reads the starting cwd's repo — the main checkout on main — not the worktree's actual feature branch), so the worktree-fix path was blocked; resolved cleanly by transferring the tested edits via patch and committing on the feature branch checked out in the main checkout (the established build pattern) — a hook worktree-blindness nuance, no defect escaped, the work landed correctly. (ii) the early Test-Coverage Tier-3 dispatch was DENIED by the enforce-role-inlining hook (the QA profile was inlined missing `## Communication` + `## Context Loading`) — the hook working as designed; re-dispatched with the complete 11-section profile, no coverage lost. Prior lessons referenced DESCRIPTIVELY (the never-hand-roll-the-build discipline held — `/execute-plan` + `/review-pr` + `/merge` each in full; the layered-review-catches-defects discipline; the per-task-not-batched remediation discipline; the autonomous-loop discipline — the build + review + merge + close ran continuously to the live-test boundary without a stop-to-ask), never by bare token.
...
- QUAL-1 — the `/generate-plan` SKILL.md frontmatter `description` still carried the pre-A′ framing while the body was reflowed to the A′ driver model (the highest-visibility surface, loads into context by default) — detection: gate (Tier-3 Code-Quality) + blind-triage LEGITIMATE; surfaced_by: self. Refreshed the description + blind-verified (inspection-only).
- QUAL-2 — a dead `section.get("domain") or domain` defensive fallback in `gate_dispatch._localized_revise_domains` (assemble always writes a truthy section domain = the run-set key) — detection: gate (Tier-3 Code-Quality); surfaced_by: self. Reduced to bare `domain` + blind-verified RESOLVED (mutation→RED).
- TEST-1 — the broken-spine chain test invoked a RELATIVE `.venv/bin/python` (FileNotFoundError from a cwd lacking a root `.venv`) — detection: gate (Tier-3 Test-Coverage, hit empirically in a scratch worktree); surfaced_by: self. Abs-pathed via `_REPO_ROOT` + blind-verified RESOLVED (runs from `/tmp` now).
- TEST-2 — `test_malformed_composite_never_safety_passed_true` was mislabeled as guarding the `safety_passed` assignment when all 3 inputs RAISE (fail-loud); the assignment is guarded by the missing-key test; the T2 recipe claimed an impossible "raised judge propagates safety_passed:True" shape — detection: gate (Tier-3 Test-Coverage, proven by mutation); surfaced_by: self. Relabeled honestly + removed dead `_MissingKeyReviewDispatch` + corrected the recipe + blind-verified RESOLVED (assignment-guard non-vacuity preserved).
- TEST-3 — the audit's F-007 shell negative test had no BEHAVIORAL-RED case (only structural omissions; the self-test-FAILED→nonzero branch was asserted only by the Python test) — detection: gate (Tier-3 Test-Coverage); surfaced_by: self. Added case E (`A_PRIME_SELF_TEST_BROKEN_SPINE=1` over the wired spine, assert exit 1) + blind-verified RESOLVED (mutation→RED: only case E catches a swallowed exit).

**Historical (kept for reference):** `vault/sessions/session-95.md`.
...
1. **The plan-generation engine LIVE-WIRING is COMPLETE on main; only the operator-present LIVE run remains.** All 3 waves are merged to `main` @ `9785e48` (as of 2026-06-25 S95 close): the live no-train de-id backend + the keystone control-inversion driver (Wave 1, #248), the composed `gate_dispatch` (Wave 2), and the `/generate-plan` A′ skill front-door + the core-capability-audit repoint onto the A′ spine (Wave 3, #250). Suite 1670/2. The `71s4`/PF-S63-02 build leg is CLOSED (the audit mechanically pins the A′ spine, non-tautologically). **Next = the operator-present LIVE end-to-end run** (`/generate-plan` over a PII-free SYNTHETIC summary FIRST, then real data): the operator injects `export ANTHROPIC_API_KEY=$(security find-generic-password -s "quant-primary-api" -w)`; the agent NEVER touches the keychain; never commit the key (PUBLIC repo).
2. **Land the two release-gate beads BEFORE the live run with real PII.** (a) `8d8r` (SEC-1 de-id value-scan hardening — add a value-level `pii_scan.scan_text` over the de-id summary VALUES, mirror `router.summarize`, so a faithless model echoing raw PII into an in-set key's value is caught; mock-testable). (b) `stsq` (SEC-3 — observe live-subscription-dispatch 0-raw-PII to a REAL agent; the one crown-jewel property the mock build correctly DEFERS to the operator-present run). Carried for the live half: the chat ELICITATION wiring (`f0gh`); `zsp5` (TEST-2 _safe_gate cap-branch test, S94 Wave 1). Everything up to the live run is mock/fixture-tested (0 spend).
3. **Standing mechanics (all HELD this session).** Build via the full pipeline (`/create-adr`→spec→build-plan→task-plan→`/execute-plan`; never hand-rolled, PF-S36-01) — `/execute-plan` + `/review-pr` + `/merge` each in full; Tier-1 SE-TDD + Tier-2 wave review (QA+Security+Architect, RUN-IT) + Tier-3 6-agent `/review-pr` with profile-less blind-triage + EXECUTED blind-verify INTACT (#250 — 5 impact-2 findings, all LEGITIMATE, fixed + blind-verified with mutation→RED proofs PRE-MERGE); EXTEND-NOT-REBUILD (inner engine + gate callables + driver numstat=0); **full role profiles inlined VERBATIM on EVERY dispatch (the enforce-role-inlining hook denies abbreviation — it DENIED a QA Tier-3 dispatch missing 2 sections this session; re-dispatched complete)**; `/review-pr` off the local diff; `/merge` (REST fallback, full-40-char SHA guard); `git add` SEPARATE from `git commit`; single trunk; stage close docs by EXPLICIT path (never `git add vault/`); **NEVER stop mid-loop to ask a settled question (PF-S92-01)**; PER-TASK remediation not batched (PF-S93-01); re-run the close gate on final main + cite the SHA (PF-S74-01). **NEW build-execution note (S95): the block-commit-main hook reads the STARTING cwd's repo, so it false-blocks a WORKTREE commit (the main checkout is on main) — commit branch-fixes on the feature branch checked out in the main checkout, or transfer tested edits via patch; do NOT rely on a worktree for the commit step.** Beads: `8d8r`/`stsq` (live-run release gates), `zsp5`/`f0gh`; carry-over `j432`/`f22s`/`02m1`/`v70t`/`s4i4`.

## Current State (volatile)

- **S95 (2026-06-25): BUILT + merged WAVES 2-3 of the live-wiring — the engine live-wiring is now COMPLETE on main.** Wave 2: ADR-0026-T2 the composed `gate_dispatch` (quality_judge ∥ review_plan → the 3-key `{accept, safety_passed, revise_domains}` disposition the keystone driver consumes; fail-closed-on-malformed). Wave 3: ADR-0026-T3 the `/generate-plan` A′ skill front-door (drives `plan_driver.drive` over the de-identified summary, no fork at the skill level) ∥ ADR-0026-T4 the `core-capability-audit.sh` repoint onto the A′ spine + the A′-inversion `--self-test`. Tier-1+2+3 reviewed → `/merge` PR #250 → `main` `9785e48`. Suite 1670/2.
- **The Tier-3 layered review worked as designed.** 6-agent `/review-pr` over the combined Wave-2+3 diff: 4 of 6 lenses `findings: []` (confirmed by execution — 34 tests run, broken-spine exit 1, numstat=0, no PII/key literals); 5 findings surfaced (ALL impact-2 polish), all blind-triaged LEGITIMATE, fixed + blind-verified RESOLVED with mutation→RED non-vacuity proofs BEFORE merge (QUAL-1 stale skill desc, QUAL-2 dead fallback, TEST-1 relative interpreter, TEST-2 mislabeled fail-loud arm + recipe, TEST-3 missing behavioral-RED shell case).
- **The V1 runtime model (A′ skill-as-orchestrator, control-flow-inverted) is now FULLY BUILT + WIRED.** The shared driver `plan_driver.drive` (the ONE generator-coroutine, no fork); `run_orchestrated` drives it (API/test path) AND the `/generate-plan` skill drives the SAME generator (the production front door); the composed `gate_dispatch` emits the 3-key disposition the driver consumes; the core-capability-audit mechanically pins the A′ spine (closes the PF-S63-02 build leg). The live de-id backend (`_ClaudeNoTrainBackend.deidentify`) is BUILT (mock-tested).
- **Only the operator-present LIVE run remains** (key injection + real data + spend). Land `8d8r` (SEC-1 value-scan) + observe `stsq` (SEC-3 0-leak) before real PII.
- **Beads:** carry — `8d8r` (SEC-1, land before live run), `stsq` (SEC-3 release gate), `zsp5` (TEST-2), `f0gh` (chat elicitation), `s4i4` (PF-S93-01), `j432`/`f22s`/`02m1`/`v70t`. No new beads this session (Tier-3 findings all fixed inline).
- **main @ `9785e48` (as of 2026-06-25 S95 close, PF-S74-01). Vault perpetually daemon-dirty (stage by explicit path).**

**Historical (kept for reference):** `vault/sessions/session-95.md`.

## What Is Next (volatile)

### Next — the operator-present LIVE end-to-end run (the engine live-wiring is COMPLETE)

All 3 waves of the live-wiring are BUILT + merged to `main` `9785e48` (as of 2026-06-25 S95 close). The plan-generation engine now has a live front door (`/generate-plan`), the composed fail-closed gate, the shared no-fork driver, the live no-train de-id backend (mock-tested), and a mechanical core-capability proof. The ONLY remaining step is the operator-present LIVE run. Directed next steps:

1. **Land `8d8r` (SEC-1 de-id value-scan hardening) BEFORE the live run** — add a value-level `pii_scan.scan_text` over the de-id summary VALUES (mirror `router.summarize`), so a faithless model echoing raw PII into an in-set key's value is caught. Mock-testable (0 spend).
2. **The operator-present LIVE run** — `/generate-plan` over a PII-free SYNTHETIC summary FIRST (the integration-mandate E2E: real specialist dispatch → de-id → composed gate → driver → render), THEN real operator data. The operator injects `export ANTHROPIC_API_KEY=$(security find-generic-password -s "quant-primary-api" -w)`; the agent NEVER touches the keychain; never commit the key (PUBLIC repo). `stsq` (SEC-3 live-dispatch-0-raw-PII observation) is the release gate.
3. **After the live run proves the loop:** the chat ELICITATION wiring (`f0gh`); the conversational intake agent epic (`w3y8`, the 2026-06-22 vision pivot); the local-model migration (North Star, post-V1).

**Build-execution note (S95 lesson, for any future branch-fix commit):** the block-commit-main hook reads the STARTING cwd's repo (the main checkout, on main), so it false-blocks a WORKTREE commit. Commit branch-fixes on the feature branch checked out in the main checkout (the established build pattern), or transfer tested edits via patch — do NOT rely on a worktree for the commit step.

**Deferred / NOT blocking:** `j432`/`f22s` (wiki); `02m1` (daemon config); `v70t` (PII-gate locale); the stale plan-gen docs reconcile (`78be`).

## Landmark window check (close step 8.7)

S95 close (2026-06-25): re-read `vault/meta/landmarks.md` at close step 8.7 (unchanged this session — the live-wiring build is on PII-free SYNTHETIC fixtures + mock clients; no real operator data, no live API, no compound/biomarker page edits). LM-01 (First MD visit) **2026-07-13**: today (2026-06-25) is BEFORE the 14-day scoped-drift-audit window open (**2026-06-29**, 4 days out) → no trigger today. **NOTE for the next session on/after 2026-06-29: run the scoped drift audit on the LM-01 relevant_scopes** (operator-profile January-2026 section + `risk_tier: medium+`/experimental compounds + baseline-panel biomarkers + the doctor-handout queue); the 7-day MD-handoff-artifact window opens 2026-07-06. LM-02 (Apple Health) / LM-03 (23andMe) / LM-04 (first HTML artifact): no real data ingested, no artifact generated → not triggered. No landmark ACTIONS due this session.
