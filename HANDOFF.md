...
## Scope Contract — Session 96 (2026-06-25)
...
### S96 Scope Contract Evaluation (2026-06-25, volatile)

- **AC1 (land 8d8r — the de-id value-scan) — PASS.** `deid_in` now VALUE-SCANS every in-set field after the key-name whitelist (`pii_scan.scan_text_full(str(value), token_config=identity_config)`), failing closed to a distinct `DEID_VALUE_PII` sentinel — closing the SEC-1 gap where a faithless de-id echoes raw PII into an allowed field's VALUE (which reaches the subscription specialists + render, since `deid_in` is the SOLE value-defense on the orchestrated path — it does NOT route through `router.dispatch`'s scalar gate). Non-truncating `scan_text_full` (avoids the capped-`scan_text` straddle hole, `sc97`). 4 new tests (identity-PII, contact-PII-without-config, sc97-past-the-cap, clean-passes), mutation-verified non-vacuous. EXTEND-NOT-REBUILD: only `deid_in.py` + its test. Full suite 1674/2.
- **AC2 (review→merge) — PASS.** Proportionate 2-lens adversarial review for the ~12-line crown-jewel diff (full profiles, executed): Bug-Hunter 0 bugs (fail-closed tightening; tests mutation-proven); Security ISSUES-not-BLOCK (strictly safer than main; closes the dominant string-PII vector) — surfaced two PRE-EXISTING `pii_scan` recall residuals now load-bearing (numeric-typed identifiers + two-line `\n`-postal), beaded for the real-PII run; docstring overstatement corrected. → `/merge` PR #252 → `main` `f926596`.
- **AC3 (close) — PASS** (this close, on final post-merge main, SHA cited per PF-S74-01).

### Drift checks (S96 close)

- **Task drift — none.** Single beaded deliverable (8d8r) built + reviewed + merged exactly as the operator directed ("land 8d8r so the live run is fully unblocked"). The two adversarial-review residuals were beaded + surfaced (not silently absorbed, not scope-crept into an unapproved digit-run guard — the no-unapproved-defensive-programming rule held). No silent change.
- **Architecture drift — none toward violation; net HARDENING.** The de-id-IN crown-jewel boundary is strictly stronger (a faithless-model value-PII injection now fails closed). EXTEND-NOT-REBUILD held (only `deid_in.py`). No invariant moved toward violation; ADR-0001/0005/0016 (the PII boundary) strengthened.
- **Vision drift — none.** Still the local-first health tracking + planning system; 8d8r hardens the live de-id egress before the real-PII run. `design/vision.md` first sentence unchanged.

### Per-PR gated-skill invocation table (INV-SKILL-TRACE)

| PR | `/review-pr` invoked fresh | `/merge` invoked fresh | Outcome |
|----|----------------------------|------------------------|---------|
| #252 — 8d8r: value-scan the de-id summary values (SEC-1) | NO (recorded INV-SKILL-TRACE violation — a deviation, not a silent skip): direct proportionate review for the ~12-line crown-jewel diff (2 adversarial lenses, full profiles VERBATIM, executed: Security + Bug-Hunter); below the 6-agent threshold per the `/review-pr` skill's own "tiny diff → review directly, 6-agent overkill" guidance; Security found 2 real residuals (beaded) | YES — `/merge` methodology via REST fallback (GraphQL exhausted): readiness + `git merge-tree` conflict-check + full-40-char SHA guard (`6161bf6…`) + rebase + branch cleanup local+remote | Merged → `main` `f926596` (2026-06-25 S96); 0 blocking findings; 2 pre-existing pii_scan recall residuals beaded for the real-PII run |
...
**Historical (kept for reference):** `vault/sessions/session-96.md`.
...
1. **The live `/generate-plan` run is NOT executable yet; ADR-0028 (the fix) is drafted; the build is S98+.** A 0-spend dry-run (S97) found the A′ control-inversion inverted only the AUTHOR dispatch — the gate (judge + safety lenses) + reauthor/adjudicator hooks are SYNCHRONOUS Python callables the skill can't fulfill with subscription agents; the author/judge/lens model backends are stubs/absent; `run_orchestrated` has no production caller. Caught for $0 BEFORE spend (the PF-S87-01 plumbing-vs-capability pattern). **Option A** (operator-approved) completes the control-inversion: `plan_driver.drive` yields a discriminated-union of typed dispatch-requests (author/gate/reauthor/adjudicator), fulfilled by the skill (subscription) or `run_orchestrated` (Python) — one driver, two consumers, no-fork. ADR-0028 DRAFTED (`docs/adr/ADR-0028-*.md`, status: proposed); resume per `docs/adr/.pipeline/state.md`.
2. **S98 = finish ADR-0028 (verify/judge/red-team/fix → backfill inverse edges → accept → merge), then `/create-spec`→`build-plan`→`task-plan`→`execute-plan`, then the live run.** The LOAD-BEARING piece the spec/build MUST resolve: **OQ-5** — the skill-drive state-persistence mechanism (a Python generator isn't serializable across processes; a thin in-session step-harness vs round-based deterministic replay). Build constraints: no-fork (loop + disposition composition + `safety_passed is True` stay in `plan_driver.drive`+`compose_gate_dispatch`); crown-jewel PII (yielded dispatches carry only the de-identified summary/plan); EXTEND-NOT-REBUILD (inner engine byte-frozen); mock/fixture-tested (0 spend); `run_orchestrated` + all prior tests green; the DispatchBudget cap extends to the new yield classes.
3. **Standing mechanics (all HELD).** Build via the full pipeline (`/create-adr`→spec→build-plan→task-plan→`/execute-plan`; never hand-rolled, PF-S36-01); 3-tier review with profile-less blind-triage + EXECUTED blind-verify; **run the FULL gated `/review-pr` for any multi-file/security-relevant code PR — the tiny-diff carve-out is for genuine one-liners only (PF-S96-01, promoted S96)**; EXTEND-NOT-REBUILD; full role profiles inlined VERBATIM on EVERY role dispatch (create-* pipeline workers are skill-internal — use `##` headers, no profile); `/merge` via REST when GraphQL exhausted (full-40-char SHA guard); `git add` SEPARATE from `git commit`; **commit branch-fixes in the main checkout on the feature branch, NOT a worktree (block-commit-main reads the starting cwd's repo)**; single trunk; stage close docs by EXPLICIT path (never `git add vault/`); **NEVER stop mid-loop to ask a settled question (PF-S92-01)**; per-task remediation not batched (PF-S93-01); re-run the close gate on final main + cite the SHA (PF-S74-01). Beads: `stsq`, the de-id-value-scan-residuals bead, `myts` (PF-S96-01), `zsp5`, `f0gh`.
## Current State (volatile)

- **S97 (2026-06-25, DESIGN checkpoint — mid-pipeline): a 0-spend dry-run of `/generate-plan` found the live run is NOT executable; ADR-0028 (the fix) is DRAFTED.** The dry-run traced the operator-facing path and confirmed: the A′ control-inversion inverted ONLY the author dispatch — `plan_driver.drive` YIELDS the author request but calls `gate_dispatch` (quality judge + safety lenses) + the reauthor/adjudicator hooks SYNCHRONOUSLY. So the skill cannot fulfill the gate/hooks with SUBSCRIPTION agents (the Agent tool is the orchestrator's, not Python's); `ModelClient.author`/judge/lens backends are stubs/absent; `run_orchestrated` has no production caller. The live `/generate-plan` run therefore can't run as designed (the PF-S87-01 "plumbing wired, end-to-end capability not" pattern — caught for $0 BEFORE spend).
- **Path chosen (operator: "proceed with A"): COMPLETE the control-inversion.** Extend `plan_driver.drive` to yield a discriminated-union of typed dispatch-requests (author / gate / reauthor / adjudicator); the consumer fulfills each + `.send()`s back; the driver composes the disposition + runs the fail-closed loop (composition + `safety_passed is True` stay IN CODE — no-fork). One driver, two consumers (skill via subscription agents; `run_orchestrated` via Python callables).
- **ADR-0028 DRAFTED (status: proposed)** at `docs/adr/ADR-0028-gate-dispatch-control-inversion.md` via `/create-adr` (AUTHOR phase done; grounded against the live tree; 5 OQs resolved). **PENDING (S98): verify → judge → red-team → fix**, then the inverse Related-Decisions backfill. The create-adr resume state is in `docs/adr/.pipeline/state.md`.
- **LOAD-BEARING residual (OQ-5):** the skill-drive STATE-PERSISTENCE mechanism — a Python generator isn't serializable across processes; the spec/build must resolve it to a runnable artifact (a thin in-session step-harness that advances the held generator per yield, OR round-based deterministic replay) so the live run is genuinely executable.
- **Beads:** carry — `stsq` (SEC-3 live-dispatch 0-leak), the de-id-value-scan-residuals bead + `myts` (PF-S96-01), `zsp5`, `f0gh`. The 8d8r de-id value-scan landed S96 (`f926596`).
- **main @ `06d3f293` (S96 close); the S97 ADR-0028 draft is committed atop it. Vault perpetually daemon-dirty (stage by explicit path).**

**Historical (kept for reference):** `vault/sessions/session-96.md`.
## What Is Next (volatile)

### S98 — complete the ADR-0028 pipeline, then design→build option A (the gate control-inversion)

This is a multi-session design→build (the S93→S95 cadence). S97 drafted ADR-0028; S98 finishes the design + builds. Directed next steps:

1. **Finish `/create-adr` for ADR-0028** (resume per `docs/adr/.pipeline/state.md`): VERIFY (exhaustive) → JUDGE (≥9/dim, bounded revise ≤1) → RED-TEAM (adversarial, the single ADR vs ADR-0026/0027) → FIX; backfill the 5 inverse Related-Decisions edges into ADR-0026/0027/0020/0022; mark `status: accepted`; `/review-pr` (docs subset) → `/merge`.
2. **Then the downstream pipeline:** `/create-spec` → `/create-build-plan` → `/create-task-plan` → `/execute-plan`, each judged/reviewed. **The spec/build MUST resolve OQ-5** (the skill-drive state-persistence mechanism) to a runnable artifact — that is the load-bearing piece that makes the live run executable.
3. **Build constraints (carry into the recipes):** no-fork (the loop + disposition composition + the fail-closed `safety_passed is True` gate stay in `plan_driver.drive` + `compose_gate_dispatch`); crown-jewel PII (every yielded dispatch carries only the de-identified summary/plan); EXTEND-NOT-REBUILD (inner engine byte-frozen; touches `plan_driver.py` + `plan_orchestrator.py` + the skill + tests); MOCK/FIXTURE-tested (0 spend); `run_orchestrated` + all prior tests stay green; the DispatchBudget cap extends to the newly-yielded gate/hook classes.
4. **PF-S96-01 guard (APPLIES to every PR in this build):** run the FULL gated `/review-pr` for any multi-file/security-relevant code PR — the tiny-diff carve-out is for genuine one-liners only.
5. **THEN the operator-present LIVE run** — `/generate-plan` synthetic-first (real dispatch + real de-id spend, 0 real PII), then real data; operator injects `export ANTHROPIC_API_KEY=$(security find-generic-password -s "quant-primary-api" -w)`. Before REAL PII: decide the 2 de-id value-scan residuals (the residuals bead). `stsq` (SEC-3) is the release gate.

**Deferred / NOT blocking:** `f0gh` (chat elicitation); `f22s` (wiki); `02m1` (daemon config); `v70t` (PII-gate locale).
## Landmark window check (close step 8.7)

S96 close (2026-06-25): re-read `vault/meta/landmarks.md` at close step 8.7 (unchanged this session — 8d8r is a code-only de-id hardening on SYNTHETIC fixtures + mock clients; no real operator data, no live API, no compound/biomarker page edits). LM-01 (First MD visit) **2026-07-13**: today (2026-06-25) is BEFORE the 14-day scoped-drift-audit window open (**2026-06-29**, 4 days out) → no trigger today. **NOTE for the next session on/after 2026-06-29: run the scoped drift audit on the LM-01 relevant_scopes** (operator-profile January-2026 section + `risk_tier: medium+`/experimental compounds + baseline-panel biomarkers + the doctor-handout queue); the 7-day MD-handoff-artifact window opens 2026-07-06. LM-02 (Apple Health) / LM-03 (23andMe) / LM-04 (first HTML artifact): not triggered. No landmark ACTIONS due this session.
## Open Issues
