...
### S72 Scope Contract Evaluation (2026-06-18, volatile)

- **AC1 (compute/record split) — PASS.** `compute_plan` extracted from `generate_plan` (compute the candidate, NO record); `generate_plan` = `compute_plan` + `record_plan`, its public contract UNCHANGED (45 `test_generate_plan` tests pass; `--self-test` green). plan-integrity confirmed the signature + return shape preserved.
- **AC2 (orchestrator) — PASS.** `scripts/plan/orchestrate.py` `generate_plans(authors, store_read, root, *, plan_date, gates, reauthor)` computes every domain's candidate, runs `reconcile`, records the survivors via the existing `record_plan`.
- **AC3 (energy bounce) — PASS.** `sustains:false` → bounce → re-author once under the sustainable-energy ceiling → record reduced, else HOLD (never an un-fuelable load). Each path mutation-proven RED (reauthor-resolves / over-ceiling-unresolved / no-hook-held / ceiling-absent-no-crash).
- **AC4 (overlap/conflict + RED-S/LEA cross-domain) — PASS.** Cross-domain overlap (distinct-domain dedup) + author-declared conflict detection (multi-author accumulation, attribution non-spoofable); plus the RED-S/LEA cross-domain short-circuit (a tripped nutrition screen also holds the workout — design Phase 0.5), mutation-proven RED.
- **AC5 (real-dispatch E2E, both paths) — PASS.** Real personal-trainer + nutritionist (full profiles inlined): the no-bounce path (130-kcal deferred session + `sustains:true`) AND the bounce path (700-kcal cleared session → the nutritionist's real `sustains:false` ceiling 300 → re-author to 260 kcal) both run through `generate_plans` to a rendered dashboard. Envelopes captured under `docs/plan-generation/examples/cross-domain-*`.
- **AC6 (no new stream + suite + gate) — PASS.** Records via the existing `record_plan` (plan-integrity confirmed no new store-write stream); suite 899/3; core-capability gate green.
- **AC7 (review + close) — PASS.** plan-integrity + QA Tier-2 (7 findings fixed) → `/review-pr` 6-agent Tier-3 (10 LEGITIMATE fixed + blind-verified 10/10 RESOLVED; 1 OUT_OF_SCOPE → bead `axm7`) → `/merge`; full close protocol ran (recorded at close, pre-merge; the per-PR skill-trace table in `memory/process-failures.md` Session 72).

### Drift checks (S72 close)

- **Task drift:** all 7 ACs PASS. One in-scope addition beyond the WILL-touch list — `vault/design/plan-generation-pipeline-v1.md` (a review-driven freshness fix: plan-integrity flagged the design's "Deferred to build-out" stale against the delivered bounce; flagged, not silent). No other drift.
- **Architecture drift:** no invariant degraded. INV-CORE-CAPABILITY stays green (the refactor preserved the wired path + the `--self-test`). INV-BRANCH-NOT-MAIN held (build on `feature/cross-domain-reconciler` in the dedicated `../aplus-s72-reconciler` worktree; idle main trunk parked on `parking/s72-reconciler`; no commit on `main`). INV-ROLE-INLINING held (real trainer + nutritionist + the 6 review agents + QA/plan-integrity all dispatched with full profiles). AC6's "no new store stream" verified by plan-integrity. The reconciler EXTENDED the RED-S/LEA screen to cross-domain (design Phase 0.5) — toward the design, not a degradation.
- **Vision drift:** none, toward-vision. The system now reconciles the four domain plans into one adjudicated set (the integration terminal function); the compound domains remain clinician-gated drafts pending the S73 medical-liaison gate (the held line). `design/vision.md` first sentence unchanged.

### PF attestation

S72 close (2026-06-18): **No new PF-class entries this session.** The build held its disciplines — the two safety behaviors (energy bounce; RED-S/LEA cross-domain short-circuit) mutation-proven RED; both reconciler paths verified E2E with real author reasoning (the integration mandate), not stubs; the refactor preserved `generate_plan`'s contract; `/review-pr` caught a real dict-spread bug (the conflict `from` override) + four safety-path coverage gaps, all fixed + blind-verified. The worktree path-read friction (editing a worktree file read only at the main-trunk path during the pre-worktree session-open) recurred and was caught each time by the harness's per-path file-state gate — the same worktree/cwd-resolution family the prior session's disclosure ledger noted; tooling friction, no loss, no new class. Full attestation + the per-PR skill-trace table (PR #149) + the disclosure ledger in `memory/process-failures.md` Session 72.

## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-71 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; S33 archived S32; …; S70 archived S69; S71 archived S70; S72 archived S71) to keep this handoff lean. The current (S72) scope contract is above; the archive holds the prior-session archaeology.
...
1. **Worktree session discipline (PF-S70-01 + PF-S71-01) — held this session.** Run parallel streams in separate `git worktree`s; keep the idle main trunk parked OFF `main` (the `block-commit-main` launch-cwd false-block, bead `llna`); re-verify `git branch --show-current` before every commit; **restore `main` at session end.** Worktree path corollary (recurred + harness-caught this session): when editing a file in the worktree, Read it from the WORKTREE-absolute path first — the harness tracks file-state per path, so a main-trunk-path read does not satisfy a worktree-path Edit.
2. **The held line: S72 wired the reconciler; S73 closes it to operator-usable.** The cross-domain reconciler (energy bounce + RED-S/LEA cross-domain short-circuit + overlap/conflict DETECTION) is wired. The compound domains are STILL clinician-gated drafts — the **supplement↔peptide additive-AE screen + the medical-liaison terminal gate** (S73) are the gate to operator-usable; the reconciler's overlap/conflict output stays detect+report until the liaison gate adjudicates it. **The CLAIM-PHRASING RULE stays load-bearing for every author dispatch.**
3. **Standing mechanics + carry-overs.** GraphQL EXHAUSTED (0/0; REST core had budget) → REST fallback for PR-create/merge (`gh api`, full-40-char SHA on the merge guard). Git/bd: `git add` separate from `git commit` (PII hook); `.beads/daemon-error` + the bd repo-ID mismatch + the `vault/approaches/` re-mangle are benign — do NOT run a backfill `bd sync`. The author `reconciliation` envelope contract (workout `energy_cost_kcal`; nutrition `energy_budget`) is load-bearing for the bounce. Carry-overs: `axm7` (gate-extend) / `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x` + PR#145/#147 review follow-ups. Pencil: ALWAYS pass `filePath`.

## Current State (volatile)

- **S72 (2026-06-18) wired the cross-domain reconciler — the step-4 integration terminal function.** `scripts/plan/orchestrate.py` `generate_plans` computes all four domain candidates (`compute_plan` — the compute/record split, `generate_plan`'s contract unchanged) → `reconcile` (the RED-S/LEA cross-domain short-circuit + the nutrition→workout energy bounce + overlap/conflict detection) → records the reconciled set via the existing `record_plan`. Real personal-trainer + nutritionist dispatches verified BOTH paths E2E: no-bounce (130-kcal deferred session, `sustains:true`) and bounce (700-kcal cleared → `sustains:false` ceiling 300 → re-author to 260). Envelopes under `docs/plan-generation/examples/cross-domain-*`.
- **PR #149** — plan-integrity + QA Tier-2 (7 fixed) → `/review-pr` 6-agent Tier-3 (10 LEGITIMATE fixed + blind-verified 10/10; 1 OUT_OF_SCOPE → bead `axm7`) → `/merge` (REST rebase under GraphQL exhaustion) lands on `main`. Recorded at close, pre-merge; the per-PR skill-trace table is in `memory/process-failures.md` Session 72.
- **No PF promoted.** The worktree path-read friction was caught each time by the harness's per-path file-state gate (the worktree/cwd-resolution family); tooling friction, no loss.
- **pytest 899/3.** Core-capability gate green; negative-test floor green.
- **Beads:** `71s4` (the reconciler is WIRED; the S73 compound-safety + medical-liaison slice remains); `axm7` (extend the core-capability gate to the orchestrator); carry-overs `llna`/`0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x` + PR#145/#147 follow-ups.
...
**Historical (kept for reference):** `vault/sessions/session-72.md`.

## What Is Next (volatile)

### S73 — the compound-safety + clinical-adjudication slice (closes the held line to operator-usable)

The cross-domain reconciler is wired. Next is the slice that makes the compound drafts operator-approvable, per `vault/design/plan-generation-pipeline-v1.md` Phases 3–4 — run in a dedicated `git worktree` (idle main trunk parked off `main` per PF-S71-01):

1. **The supplement↔peptide two-pass mutual interaction / additive-AE screen** (Phase 3 compound band — bidirectional; members each pass single-domain filters but the combination isn't cross-checked for additive AE risk). The reconciler's overlap/conflict DETECTION feeds this.
2. **The medical-liaison terminal safety gate** (Phase 4 — collates the doctor-visit queue + every risk HALT, runs BPMH reconciliation, adjudicates blocks before operator approval). Dispatch the deployed `medical-liaison` for real (full profile inlined); promote the `INV-OVERRIDE-RECORD-SCHEMA` / `INV-CRITICAL-NON-OVERRIDABLE` candidates (bead `mdv`) via the change-discipline ritual.
3. Then the **measure** leg (a production caller for `record_plan_tracking` — currently schema-only) and the **adjust** leg (re-plan from tracking) — the rest of the closed loop.

**Discipline:** scope contract at open; **run in a `git worktree` + keep the idle main trunk off `main`** (PF-S70-01 + PF-S71-01; Read worktree files from the worktree-absolute path); re-verify `git branch --show-current` before each commit; honor the CLAIM-PHRASING RULE; **verify by running the production path E2E with a real dispatch, not just unit tests**; store-surface writes satisfy `docs/checklists/store-adversarial-tests.md`; `/review-pr` → `/merge` before landing on main.

**Deferred (NOT blocking):** extend the core-capability gate to the orchestrator (`axm7`); the standalone full-plan render screen (operator drafting it); the dashboard reskin in flight (operator's `aplus-reskin` worktree); the `/generate-plan` slash-command wrapper (the orchestrator is wired as the callable); the PR#145/#147 review follow-ups; the `llna` hook fix; carry-overs `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`; operator-gated July-visit prep (LM-01) — first session on/after **2026-06-29** runs the LM-01 scoped drift audit.

## Landmark window check (close step 8.7)

S72 close (2026-06-18): RE-OPENED `vault/meta/landmarks.md` at close step 8.7 (unchanged). No landmark edited — S72 was plan-generation code on PII-free synthetic fixtures (no operator data ingested, no Whoop baseline start, no MD-handoff artifact). LM-01 (First MD visit) **2026-07-13**: no trigger window open today (the 14-day scoped-drift-audit window opens **2026-06-29**, the 7-day MD-handoff window 2026-07-06). LM-02 (Whoop) / LM-03 (23andMe) date-TBD. LM-04 (first HTML artifact) NOT triggered — the dashboard rendered only to a tempdir (synthetic fixtures); no committed HTML artifact this session. No landmark actions due.
