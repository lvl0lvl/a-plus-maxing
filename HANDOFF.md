...
3. **The adoption PR is pending + the rigor pull cadence is now live.** `feature/rigor-toolkit-adoption` (S64) needs `/review-pr` (6-agent) + `/merge` fresh via the Skill tool (INV-SKILL-TRACE) — run after the heartbeat sequencing decision. New standing discipline (Discipline 11): at the drift-audit boundary, diff `rigor_version` (1.0.0) vs `frameworks/rigor/VERSION`, read the CHANGELOG delta, pull (re-run `toolkit/tests/run-all-tests.sh` first). Standing mechanics unchanged: REST `gh api` full-40-char SHA for PR ops; stage `git add` separately from `git commit`; `.claude/hooks/*` edits need per-session authorization.
...
Open normally (Session Start Protocol; `branch-completeness-audit.sh` at OPEN; baseline `main` 833/2 — note Wave A lives on `feature/rigor-toolkit-adoption`, unmerged). Read `docs/rigor-adoption-log.md` before continuing the adoption.

**S65 forward options (operator prioritizes at open, NOT pre-adopted):**

1. **Land Wave A:** `/review-pr` (6-agent) + `/merge` the `feature/rigor-toolkit-adoption` PR fresh via the Skill tool (INV-SKILL-TRACE). Decide heartbeat sequencing first (`0qf6`): wire it as part of this PR (+ update `test_settings_hook_paths.sh` + ensure the review's own dispatches carry the clause), or merge Wave A and wire heartbeat in Wave D.
2. **The core deliverable (`71s4`, P1) — the actual point.** Build the plan-generation path end-to-end (a model/API client + a production caller for `assemble` + a `plan` render target + real operator meta-inputs) under the re-tightened rigor + add the core-capability-first session gate (Wave B). This is what PF-S63-02 says to do.
3. **Wave B/C/D of the adoption:** B (approaches + disclosure ledgers + `falsification-scan` advisory + the core-capability gate), C (`plan-integrity` role + INVARIANTS rows via the change-discipline ritual), D (global `~/.claude/skills`+`commands` sync; fix `d1kc`).
