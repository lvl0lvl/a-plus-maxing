...
### S33 Scope Contract Evaluation (2026-06-05, volatile)
...
Scope contracts for Sessions 5-32 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; S33 archived S32) to keep this handoff lean. The current (S33) scope contract is above; the archive holds the prior-session archaeology.
...
1. **read-before-invoke (PF-S13-01 + PF-S17-01) — STANDING; `execute` is MID-FLIGHT (data-IN layer now: store + guard + ingestion + WIRED ADAPTERS; scheduler + data-out + router + clone-init next).** Each task's recipe is the gate — READ it IN FULL before executing (HELD this session for the `ADR-0003-T2` recipe + `/review-pr` + `/merge`). Next builds the scheduler (`oaf`, now unblocked) then the data-out layer (`gu4`/`xlu`/`br1`) — TDD with `.venv/bin/python -m pytest`, never from memory; no `Workflow` substitution.
2. **The V1 data-IN layer is BUILT, hardened, path-safe, and now SOURCE-WIRED.** Four per-source adapters (`scripts/ingest/adapters/`: HealthKit/Oura/Garmin wired + Whoop unwired scaffold) implement the frozen `Adapter` Protocol (`source_tag` + `read_readings`) and run through the UNCHANGED `ingest.run`; the ADR-0003 0-shared-routine-edit extensibility criterion is **mechanically proven by a genuinely-falsifiable gate** (a tautological HEAD-relative baseline was caught + fixed to fork-point; a committed-probe falsifiability test was added + teeth-checked). 0-egress held (adapters import only `json`/`pathlib`/`typing`). Enforcement-first holds: no plan-reasoning-over-PII task until the router (`ADR-0006-T0`/`T1`). **CARRIED (before V1 ships):** `qwj` P2 (vault-PROSE operator name + ADR-0005 clone-init); `1ww` P2 (concurrent-append — premature for single-operator V1); `1vi` P3 (re-ingest can't correct a value); `ivt`/`z2u` P3.
3. **PF-S22-01 falsification window — still live for the parallel library-population track.** Research-only `/aplus-research` sessions remain the first parallelized track after S22. At each batch close: ONE merge target, `branch-completeness-audit.sh` + `wiki-lint.sh` green. Unchanged this session.

**Demoted from prior Top-3:** the "data-IN built but not source-wired / `n9h` open" framing (the four adapters are now built + merged, the 0-edit extensibility proof is mechanically falsifiable, `n9h` closed, `oaf` unblocked).

## Current State (volatile)

- **THE V1 DATA-IN LAYER IS NOW SOURCE-WIRED + MERGED (S33, 2026-06-05)** — one deliverable, one PR. PR #50 (`n9h`/`ADR-0003-T2` wired adapters → `main` `aab93cc`). Pipeline: …→ Wave 1 [S30] ✓ → Wave 2 [S31] ✓ → store-harden + Wave-3 ingestion [S32] ✓ → **Wave-3 wired adapters [S33] ✓** → scheduler (`oaf`) + data-out (next).
- **Four per-source adapters (`scripts/ingest/adapters/{healthkit,oura,garmin,whoop}.py`):** each implements exactly `source_tag()` + `read_readings(export_file)` (the frozen `ADR-0003-T1` Protocol — `isinstance(adapter, Adapter)` holds for each). HealthKit/Oura/Garmin are WIRED; Whoop is the registered-but-UNWIRED scaffold (Risk N2 → `ingest.manual_entry` until a later task wires it). All per-source format specifics live inside each `read_readings`; `ingest.py` + `adapter.py` are byte-untouched (the 0-shared-routine-edit proof).
- **The two 0-edit extensibility proofs are GENUINELY FALSIFIABLE:** each diffs `ingest.py`+`adapter.py` against a DISTINCT committed-tree baseline built over the FORK-POINT tree (`git merge-base HEAD origin/main`). A tautological HEAD-relative baseline (would miss a committed shared-routine edit) was caught by orchestrator independent verification + fixed BEFORE review; a committed-probe falsifiability test was added (orchestrator teeth-checked: RED under a neutered `_baseline_ref`).
- **The 6-agent `/review-pr` earned its keep on the single-SE build (PF-S3-01, 0 suppressed per PF-S26-01):** 7 legitimate findings fixed + **7/7 blind-verified RESOLVED** — HIST-001 (adapter path is a new untrusted-`item` sink with no SEC-001 traversal regression test → added), TEST-001 (falsifiability proof exercised `--no-index`, not the real gate path → committed-probe test), TEST-002 (Whoop unwired-gate regex missed `import …whoop as w`), TEST-004 (cross-source non-dedupe), 3 QUAL docstring/import. 5 findings correctly no-action (failed the legitimacy test, NOT severity-suppressed).
- **SINGLE TRUNK** unchanged (since S22): `main` = complete project. Suite **88 passed / 2 skipped** (Linux-only egress).
- Bead `n9h` **CLOSED**; `oaf` (`ADR-0003-T3` scheduler) now **READY**. `bd ready` frontier = `oaf` + `gu4`/`xlu`/`br1`.
- **Active landmarks:** no trigger windows opened.

**Historical (kept for reference):** `vault/sessions/session-33.md` + `vault/meta/log.md` S33 entry.

## What Is Next (volatile)

### S33 wired the per-source adapters. DECIDED next = `gu4` (ADR-0004-T1 render engine).

**RESUMPTION POINT.** S33 built `ADR-0003-T2` (four adapters + the two 0-edit extensibility proofs), rebase-merged (PR #50); closed `n9h` → `oaf` unblocked. **Open the next session on `main`** (Start Protocol + `branch-completeness-audit.sh` at open), then scope **`gu4`** — the next build is decided (see #1).

**Next pipeline work — `gu4` is the decided next; `oaf`/`xlu`/`br1` are the standing frontier:**
1. **DECIDED NEXT (S33) — `gu4` / `ADR-0004-T1` (render engine + template component library).** Recipe `docs/task-plan/ADR-0004-T1.md` — READ IN FULL first (PF-S17-01). **Why `gu4` over `oaf`:** it PUBLISHES the V1 render interface (`render.emit(template, store_read)` + `vault/design/templates/component_set.py`) consumed by `ADR-0004-T2`/`T3` + `ADR-0006-T2` + `ADR-0007-T2`, so it **unblocks the most downstream** (`3gp` + `ftm` + `8cv` directly, contributes to `1ih`); it is on the critical path to the July MD-handoff artifact (LM-01/LM-04 need the render engine); and its **entry state is already met** (prerequisites `ADR-0002-T1` store + `ADR-0001-T1` egress guard both merged S31). **Deliverable (5-file manifest):** `scripts/generate/render.py` (publishes `render.emit`) + `vault/design/templates/{component_set,dashboard,report}.py` + `tests/generate/test_render.py`; branch `feature/generate-render-engine`. **Load-bearing gates:** crit-5 (a real `emit` wrapped in ONE closure under `egress_guard.run` is TRUTHY → 0 egress), crit-6 (`emit` RAISES on a non-`data:` external asset — refuses a network-dependent file), crit-3 (the accessibility gate is the Wave 3→4 go/no-go and must report MEASURED NUMERIC WCAG-AA values, never a boolean); read `vault/design/artifact-design-protocol.md` (palette / single-file / accessibility sections only) as the read-only design source. **Carried flags:** after `gu4` builds the real template, re-run the render-size measurement before `ADR-0004-T2`/`ADR-0007-T2` consume the cap; the plan-`template` producer for `render.emit` (bead `4xe`).
2. **Standing frontier (after / parallel to `gu4`):** `oaf` (`ADR-0003-T3` scheduler over the WIRED set [HealthKit+Oura+Garmin], delta-only, 0-egress, Whoop excluded — completes data-IN; the literal `rg "whoop" scripts/ingest/scheduler.py = 0` Wave 4→5 boundary check lands HERE once `scheduler.py` exists, the PR #50 AC-5 deferral; it is a deferrable LEAF — `ingest.run` already works, so unattended scheduling can follow `gu4`), `xlu` (`ADR-0005-T1` pre-commit hook — reuses `pii_scan.scan`; **resolve `qwj` first**), `br1` (`ADR-0006-T0` router spike). READ each recipe `docs/task-plan/<id>.md` IN FULL first (PF-S17-01), TDD with `.venv/bin/python -m pytest`, then `bd close`.
3. **Published surfaces the next wave consumes:** `render.emit(template, store_read)` (the V1 render interface `gu4` publishes — `ADR-0004-T2`/`T3`/`ADR-0006-T2`/`ADR-0007-T2` consume it); the `Adapter` Protocol + the four adapters (`source_tag` + `read_readings`) — `oaf`'s scheduler assembles the wired set (`healthkit`/`oura`/`garmin`; `whoop` stays unwired). `ingest.run(adapter, export_file, root=…)` — `oaf` invokes it. `store.append`/`read` (path-safe, corruption-tolerant). `keying.py` single key (never redefine — re-opens N3). `egress_guard.run(operation)` backs 0-egress checks. `pii_scan.scan` for `xlu`'s hook — **resolve `qwj` first** (≈924 `scan` hits across vault prose → the hook would block nearly every commit).
4. **Enforcement-first holds:** no plan-reasoning-over-PII task is buildable until the router (`ADR-0006-T0`/`T1`) lands in later waves.

**Carried flags / beads (surfaced, not lost — PF-S26-01):** S33's code-PR (#50) review produced 0 new carry-forward beads (all 7 legitimate findings fixed in-PR; the 5 no-action findings failed the 6-condition legitimacy test, not severity); the close-PR (#51) docs review filed `orl` P3 (narrative-vs-DAG wave-naming reconciliation, sibling of `e3d`). Still open from prior reviews:
- **PR #47:** `1vi` P3 (re-ingest cannot correct a stored reading's value — dedupe key is `(item,timepoint,source)`, value excluded → first-write-wins; a render/edit-layer product decision, `ADR-0004+`, not a keying change).
- **PR #44 review beads (still open):** `1ww` P2 (concurrent-append non-atomic dedupe — premature for single-operator V1), `qwj` P2 (PII-free-trunk vault-prose operator name + ADR-0005 clone-init — **before V1 ships**), `ivt` P3 (recipe `ADR-0001-T1` names the rejected interceptor), `z2u` P3 (gitignore-test isolation).
- **`ADR-0004-T1`:** re-run the render-size measurement against the REAL template before `ADR-0004-T2`/`ADR-0007-T2` consume the cap. **Carried from S29:** the plan-`template` producer for `render.emit` (bead `4xe`).
...
### Open beads carried
- **V1 build execution (label `v1-build`):** 18 per-task beads, dependency-wired; **7 closed** (Wave 1: `394`/`bez`/`qbb`; Wave 2: `89a`/`e9m`; Wave 3: `6be`/`n9h`). READY now: `oaf` (`ADR-0003-T3` scheduler, unblocked by `n9h`) + `gu4`/`xlu`/`br1`. Plus `e3d` (recipe-set doc-consistency, non-blocking).
- **PR review beads open:** `1ww`/`qwj`/`ivt`/`z2u` (PR #44), `1vi` (PR #47). **`n9h` CLOSED** (PR #50).
...
- **P3:** `orl` (narrative-vs-DAG wave-naming, NEW S33), `0oy`, `ko5` (vault-git-tracking vs ADR-0005), `75t`, `ae0`/`d6g`/`4ba`/`3v6`/`dip`, `t7z`/`fsr`/`8qe`, `r7t`/`7rm`/`60f`, `5jr`, `9c5`/`pnl`/`2n1`/`4h1`/`smw`, plus pre-existing `1ek`/`6ln`/`mdv`/`1rm`/`2gs`/`623`/`f2r`/`yfu`/`2qq`/`p47`/`o9y`/`7is`/`mdg`/`5by`/`1ox`/`9yk`.
- **Closed:** S33: `n9h` (PR #50). S32: `8s6` (PR #46), `6be` (PR #47). S31: `89a`/`e9m` (PR #44). S30: `394`/`bez`/`qbb`. S29: `mo4` (PR #40). S28: `hv6` (PR #38). S27: `rg2`. S25: `fm4`/`hil`. S23: `bte`. S22: `gdw`. S21: `mhg`/`5ot`/`0be`.

### Open project work (unchanged)
- Vault git-tracking decision deferred (`ko5`). First HTML artifact (LM-04) is architecturally placed by ADR-0004 + recipe-pinned (`ADR-0004-T1`/`T2`/`T3`, `ADR-0007-T2`); built when execute reaches the data-out waves.

## Landmark window check (close step 8.7)

All 4 active landmarks (LM-01 doctor visit July 2026, LM-02 Oura/wearable, LM-03 23andMe, LM-04 first HTML artifact) — no trigger windows opened during S33 (2026-06-05). LM-04 (first HTML artifact): still unbuilt (the render task `gu4`/`ADR-0004` lands in the data-out wave); the wired adapters merged this session are an upstream data-IN prerequisite, not the artifact. LM-01's 14-day-before window still depends on the TBD July exact date (not yet within window as of 2026-06-05); LM-02/03 remain Walter-pending. No status flips due.
