...
Scope contracts for Sessions 5-33 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; S33 archived S32; S34 archived S33) to keep this handoff lean. The current (S34) scope contract is above; the archive holds the prior-session archaeology.
...
1. **read-before-invoke (PF-S13-01 + PF-S17-01) — STANDING; `execute` is MID-FLIGHT (data-IN wired + data-OUT RENDER ENGINE now published; scheduler + render consumers + router + clone-init next).** Each task's recipe is the gate — READ it IN FULL before executing (HELD this session for the `ADR-0004-T1` recipe + design source + `/review-pr` + `/merge`). Next: `oaf` (scheduler, READY) or the render consumers `ADR-0004-T2`/`T3`, or `xlu`/`br1` — TDD with `.venv/bin/python -m pytest`, never from memory; no `Workflow` substitution.
2. **The V1 data-OUT render layer is PUBLISHED + accessibility-gated + PII-boundaried.** `render.emit(template, store_read) -> Path` (`scripts/generate/render.py`) writes ONE inlined self-contained HTML file (0 external refs via a hardened HTMLParser scanner; raises on external assets), reads only `store_read`, < 500KB; `component_set.py` + dashboard/report templates draw the colorblind-safe palette. The AC-3 accessibility gate is **MECHANICALLY FALSIFIABLE** — it catches a member-collision palette regression (the review caught + fixed the original tautology; re-verified by mutation). The ADR-0005 PII-free-trunk boundary now covers the render output dir (`vault/artifacts/generated/` gitignored). **CARRIED:** `5wo` (F12 — `render.emit` single-Path vs `ADR-0004-T2` ≥2-path pagination, Architect amendment before T2); `qwj` P2 / `1ww` P2 / `1vi` P3 / `ivt`/`z2u` P3; the render-size re-measure before `ADR-0004-T2`/`T0007-T2` consume the cap.
3. **PF-S3-01 layered-review is load-bearing on single-builder tasks — proven AGAIN this session.** The 6-agent `/review-pr` caught the AC-3 tautology that the orchestrator's OWN pre-review probe missed (the probe drifted to a non-member hex caught by membership; the review drifted to an existing-member hex the subset-check couldn't catch). For any single-SE build, the full 6-agent `/review-pr` + blind triage + blind verify is non-optional; when self-verifying a falsifiability gate, test EVERY failure mode it must catch, not one.

**Demoted from prior Top-3:** the "data-IN source-wired / `n9h` open" framing (gu4's render engine is now built + merged, the AC-3 accessibility gate is mechanically falsifiable, `gu4` closed; the data-OUT render interface is published).

## Current State (volatile)

- **THE V1 DATA-OUT RENDER ENGINE IS PUBLISHED + MERGED (S34, 2026-06-05)** — one deliverable, one PR. PR #53 (`gu4`/`ADR-0004-T1` render engine + component library → `main` `aa0d9e8` as of 2026-06-05 S34 close). Pipeline: …→ Wave 2 [S31] ✓ → store-harden + Wave-3 ingestion [S32] ✓ → Wave-3 wired adapters [S33] ✓ → **Wave-3 render engine [S34] ✓** → scheduler (`oaf`) + render consumers (`T2`/`T3`) + router (next).
- **`render.emit(template, store_read) -> Path` (`scripts/generate/render.py`):** writes ONE self-contained inlined HTML file, returns the path, reads operator data ONLY from `store_read`, raises on external assets (a hardened `HTMLParser`-based scanner that catches `@import`/`srcset`/`poster`/`meta-refresh` and does NOT false-trip on benign operator `url(…)` text), < 500KB, offline. `component_set.py` = the single inline-CSS/SVG component library + the colorblind-safe palette; `dashboard.py` + `report.py` assemble it.
- **The AC-3 accessibility gate is MECHANICALLY FALSIFIABLE:** computes WCAG contrast + per-adjacent-pair CIEDE2000 ΔE over the RENDERED colors, asserts role→hex equality + mutual distinctness vs the independently-recorded `vault/decisions/2026-06-05-render-colorblind-safe-palette.md` palette + floor. The original gate shipped non-falsifiable for member-collision drift; the 6-agent review caught it (the orchestrator's own probe missed it), fixed + re-verified by mutation (good→watch-hex collision now FAILS).
- **The 6-agent `/review-pr` earned its keep on the single-SE build (PF-S3-01, 0 suppressed per PF-S26-01):** 12 legitimate findings fixed + **12/12 blind-verified RESOLVED** — standouts: the AC-3 tautology (TEST), the ADR-0005 PII-trunk gitignore gap (SEC/HIST), the external-asset scan misses + the benign-`url(`-text false-positive (SEC/BUG), the UTF-8 write. 1 beaded (`5wo` pagination contract), 3 NOT_A_BUG (failed the legitimacy test, NOT severity-suppressed).
- **SINGLE TRUNK** unchanged (since S22): `main` = complete project. Suite **108 passed / 2 skipped** (Linux-only egress).
- Bead `gu4` **CLOSED**; `5wo` (F12) filed P2. `bd ready` frontier = `oaf` (scheduler, READY) + `xlu`/`br1` + the gu4-unblocked render consumers (`ADR-0004-T2` blocked-pending `5wo`; `ADR-0004-T3` = the first-artifact producer).
- **Active landmarks:** no trigger windows opened. LM-04 (first HTML artifact) still pending — `gu4` published the RENDER ENGINE but no artifact is generated yet (that lands with `ADR-0004-T3` `generate.run`).

**Historical (kept for reference):** `vault/sessions/session-34.md` + `vault/meta/log.md` S34 entry.

## What Is Next (volatile)

### S34 published the render engine. Frontier: `oaf` (READY) · `ADR-0004-T3` (first-artifact producer) · `ADR-0004-T2` (blocked-pending `5wo`).

**RESUMPTION POINT.** S34 built `ADR-0004-T1` (render engine + component library + 2 templates + the falsifiable AC-3 gate), rebase-merged (PR #53); closed `gu4`; filed `5wo` (the F12 pagination contract gap). **Open the next session on `main`** (Start Protocol + `branch-completeness-audit.sh` at open), then scope the next task — READ its recipe `docs/task-plan/<id>.md` IN FULL first (PF-S17-01). No single decided-next is pinned; the frontier (priority is a next-session call):
...
2. **`ADR-0004-T3` (`generate.run` entry point) — the first-ARTIFACT producer.** Invokes `render.emit` for cron + on-demand; this is the task that writes the FIRST actual HTML artifact (LM-04) into `vault/artifacts/generated/` (now gitignored). On the critical path to the July MD-handoff. Consumes the `render.emit` published this session.
3. **`ADR-0004-T2` (matrix/projection render) — BLOCKED pending `5wo` (bead dep `yo6`→`5wo` wired, so `bd ready` agrees — T2 drops back in once `5wo` closes).** Before T2 is built, resolve `5wo` (F12): `render.emit`'s single-`Path` return vs T2 AC-2's required ≥2-path pagination (Architect amendment to the T1 Interface Contract OR caller-orchestrated pagination with a per-page discriminator). Do NOT silently resolve inside T2.
4. **`xlu` (`ADR-0005-T1` pre-commit hook — reuses `pii_scan.scan`; resolve `qwj` first) · `br1` (`ADR-0006-T0` router spike).** READ each recipe IN FULL first, TDD with `.venv/bin/python -m pytest`, then `bd close`.

**Published surfaces the next wave consumes:** `render.emit(template, store_read) -> Path` (NEW S34 — `ADR-0004-T2`/`T3`/`ADR-0006-T2`/`ADR-0007-T2` consume it; the path-return semantics are the `5wo` open question for T2) + `component_set.py` (the shared component library + palette, reused by T2/T0007-T2). The `Adapter` Protocol + four adapters — `oaf`'s scheduler assembles the wired set. `ingest.run(adapter, export_file, root=…)` — `oaf` invokes it. `store.append`/`read`, `keying.py` single key (never redefine), `egress_guard.run(operation)`. `pii_scan.scan` for `xlu` — **resolve `qwj` first** (≈924 `scan` hits across vault prose). Enforcement-first holds: no plan-reasoning-over-PII task buildable until the router (`ADR-0006-T0`/`T1`).

**Carried flags / beads (surfaced, not lost — PF-S26-01):** S34's code-PR (#53) review filed `5wo` P2 (the F12 `render.emit` pagination contract gap — Architect amendment before `ADR-0004-T2`); all 12 other legitimate findings fixed in-PR. Still open from prior reviews:
...
### Open beads carried
- **V1 build execution (label `v1-build`):** 18 per-task beads, dependency-wired; **8 closed** (Wave 1: `394`/`bez`/`qbb`; Wave 2: `89a`/`e9m`; Wave 3: `6be`/`n9h`/`gu4`). READY now: `oaf` (`ADR-0003-T3` scheduler) + `xlu`/`br1` + the gu4-unblocked render consumers (`ADR-0004-T2` blocked-pending `5wo`; `ADR-0004-T3` first-artifact producer). Plus `e3d` (recipe-set doc-consistency, non-blocking).
- **PR review beads open:** `1ww`/`qwj`/`ivt`/`z2u` (PR #44), `1vi` (PR #47), `5wo` (PR #53, F12 pagination contract). **`gu4` CLOSED** (PR #53).
...
- **Closed:** S34: `gu4` (PR #53). S33: `n9h` (PR #50). S32: `8s6` (PR #46), `6be` (PR #47). S31: `89a`/`e9m` (PR #44). S30: `394`/`bez`/`qbb`. S29: `mo4` (PR #40). S28: `hv6` (PR #38). S27: `rg2`. S25: `fm4`/`hil`. S23: `bte`. S22: `gdw`. S21: `mhg`/`5ot`/`0be`.
...
All 4 active landmarks (LM-01 doctor visit July 2026, LM-02 Oura/wearable, LM-03 23andMe, LM-04 first HTML artifact) — no trigger windows opened during S34 (2026-06-05). LM-04 (first HTML artifact): still pending — `gu4` published the RENDER ENGINE (`render.emit`) but no artifact is generated yet; the first artifact lands with `ADR-0004-T3` (`generate.run`), which writes to the now-gitignored `vault/artifacts/generated/`. LM-01's 14-day-before window still depends on the TBD July exact date (not yet within window as of 2026-06-05); LM-02/03 remain Walter-pending. No status flips due.
