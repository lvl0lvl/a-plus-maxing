...
3. **AP-CROSS-ROLE-CONTRACT-DRIFT** — frozen-design-doc defects to reconcile via change discipline BEFORE the relevant audit scripts go LIVE or specialists inherit them. **`dcy` (XR-S15-01) CLOSED post-S15** (PR #4 → `main` `0da8788`; top-level `harm_class` is the auto-block key, §12.3 + deployed exemplars now populate it; the row-5 h1h2-autoblock script is no longer blocked). Remaining: `1rm` (XR-S15-02, P3 — `probe_floor` two-name), plus carried `p47`/`o9y`/`7is`/`9u6`/`7m1`/`z8i`.
...
- **Beads:** `4ej` (XR-002) CLOSED at AC0; `dcy` (XR-S15-01) CLOSED post-S15 (PR #4 → `main` `0da8788` — top-level `harm_class` is the auto-block key, exemplars now populate it). Open from S15: `1rm` (P3, XR-S15-02 — `probe_floor` two-name).
