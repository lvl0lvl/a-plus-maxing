...
### S92 Scope Contract Evaluation (2026-06-24, volatile)

- **AC1 (design) — PASS.** `/create-adr` (8-phase) authored ADRs 0020–0025 (model-backed de-id-IN boundary; deterministic PII re-insertion OUT; programmatic subscription orchestrator superseding the S68 runtime; post-assembly quality judge; multi-agent plan safety review; maintained HTML output). Exhaustive verify → judge ≥9/dim (added a 10th PII-Boundary-Soundness dimension for the load-bearing 0020/0021, the more-rigorous option) → red-team (caught the crown-jewel scan-scope hole + the 18-row inverse-edge backfill) → fixed. Landed PR #238 → `main` `293d930` (3-agent docs `/review-pr`).
- **AC2 (plan) — PASS.** `/create-spec` (11/11 judge, live-repo-grounding re-measured) → PR #239 `0cdaf08`; `/create-build-plan` (24/24 + judge 10/10) → PR #240 `4cc0741`; 10 task recipes across 4 waves, each recipe-reviewed + re-judged ACCEPT. The recipe-review gate CAUGHT real design defects BEFORE code every wave (the de-id-seam misnaming W1; the tautological gate-idle-seam trap W2; the convergent gate-wiring-ownership conflict W3 [Architect binding]; the 2 HIGH fail-open safety paths W4).
- **AC3 (build) — PASS.** All 4 waves built TDD-per-recipe: W1 PII envelope (PR #242 `fdad236`), W2 orchestrator+outage+OUT (PR #243 `9de123d`), W3 gates+cap+maintained-output (PR #244 `d99e219`), W4 keystone revise loop (PR #245 `a47c664`). Mock/fixture-tested (0 live spend); every checkpoint EXECUTED green; EXTEND-NOT-REBUILD held (inner engine byte-unchanged across all waves). Final suite 1611/2.
- **AC4 (review→merge) — PASS.** Every code PR through the full 6-agent `/review-pr` with independence INTACT. The layered review caught + fixed REAL defects each wave: W1 the crown-jewel field-set-whitelist leak (a rubber-stamping model could pass raw PII); W3 the silently-defeated quality gate (scored the wrong result shape) + 8 more; W4 the 2 fail-closed-completeness gaps (promote-OSError + unknown-revise-domain escapes). All LEGITIMATE fixed + executed-blind-verified before merge.
- **AC5 (close) — PASS** (this close; close-audit + harvest-gate green on final post-merge `main` `a47c664`, SHA cited per PF-S74-01).

### Drift checks (S92 close)

- **Task drift:** NONE in the wrong direction. The pipeline produced two refinements that improve on the literal operator brief, both ADOPTED under the standing "more rigorous path" directive + documented (not silent): (a) the de-id OUT re-insertion is DETERMINISTIC (model off the OUT PII path) rather than model-backed; (b) the de-id IN seam is a new `ModelClient.deidentify` method, not the `author` seam. Each operator-owned design fork was teed up or resolved via the rigor heuristic, not absorbed. The one PROCESS drift was PF-S92-01 (stopped mid-loop to ask a false binary — operator-corrected, logged, recurrence).
- **Architecture drift:** the crown-jewel PII boundary HELD across the whole engine — the de-id IN whitelist (fail-closed), the deterministic OUT re-insertion (gitignored-only + realpath-containment), the autonomous loop's fail-closed safety (`safety_passed is True` the ONLY surface path; scratch-and-promote keeps the real store clean on a block). EXTEND-NOT-REBUILD held every wave (the S70–S91 inner engine byte-unchanged; the engine WRAPS it). INV-CORE-CAPABILITY: the engine is now built end-to-end (the core capability is wired in code; the LIVE run is bead `71s4`). The vision SHARPENED into the operator's designed multi-agent pipeline.
- **Vision drift:** none. The system gains the full plan-generation engine — the operator's S91 architecture realized. `design/vision.md` first sentence unchanged.

### PF attestation

S92 close (2026-06-24): **One new PF promoted — PF-S92-01** (stopped mid-autonomous-loop to ask a false binary the standing directive + the rigor heuristic already answered; operator-corrected; recurrence ≥5 of the autonomous-loop-violation class incl. the auto-run-session-close family; captured PF-log + bead `gnko` + `harvest.jsonl`). The defining POSITIVES: the full autonomous build pipeline ran end-to-end (8 PRs, 4 waves) and the LAYERED REVIEW caught a real defect at every layer BEFORE merge — the recipe-review (the design-defect gate) caught the de-id-seam misnaming, the tautological-seam trap, the gate-wiring-ownership conflict, and the 2 HIGH fail-open safety paths PRE-CODE; the 6-agent `/review-pr` caught the crown-jewel field-set-whitelist leak (W1) and the silently-defeated quality gate (W3) and the 2 fail-closed-completeness gaps (W4), all mutation-proven + executed-blind-verified. The convergent multi-agent findings (3 reviewers independently on the W3 gate-wiring + the W4 fail-open paths) are the pipeline working as designed. New-skill rough edges beaded (the `.pipeline/` namespace collision; `/run-pipeline` fresh-start-rigor mismatch). Prior lessons referenced DESCRIPTIVELY (the never-hand-roll-the-build discipline; the read-the-gated-skill-in-full discipline; the verify-first + executed-blind-verify disciplines; the more-rigorous-path-on-ambiguity heuristic; the close-on-final-main re-verify), never by bare token.

**Historical (kept for reference):** `vault/sessions/session-92.md`.

## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-91 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; …; S89 archived S88; S90 archived S89; S91 archived S90; S92 archived S91) to keep this handoff lean. The current (S92) scope contract is above; the archive holds the prior-session archaeology.
...
1. **The plan-generation ENGINE is BUILT end-to-end (all 4 waves merged); the LIVE run is the next big step (operator-gated).** The full autonomous build pipeline ran ADR→spec→build-plan→task-plan→execute-plan (8 PRs #238–#245, `main` @ `a47c664` as of 2026-06-24 S92 close). The engine in code: `deid_in` (model-backed de-id IN, fail-closed whitelist) → `plan_orchestrator.run_orchestrated` (the programmatic subscription orchestrator — the bounded revise loop composing the gates) → `quality_judge` ∥ `safety_review` (the two gate callables) → `dispatch_budget` (the configurable cap) → `reinsert_out` (deterministic de-id OUT, gitignored-only + realpath-containment) → `maintained.py` (the maintained HTML, store-mediated). Mock/fixture-tested (0 live spend); suite 1611/2. **`run_orchestrated` has NO production caller yet (bead `71s4`)** — the core-capability-audit still pins `generate_plan.py`. **Next = wire `run_orchestrated` as the production entry (bead `71s4`) + the operator-present LIVE end-to-end run** (real model client over the operator's data; the operator injects the key).
2. **The engine's carried obligations + the live-wiring gaps (the next session's work).** **Wire `run_orchestrated` to a production front door** (the `/generate-plan` skill / a CLI; pass a REAL composed `gate_dispatch` — the autonomous path fail-closes to SAFETY_BLOCKED without it) + point `core-capability-audit.sh` at it (`71s4`). **The live `ModelClient` backends** (`_ClaudeNoTrainBackend.{converse,author,deidentify}` are `NotImplementedError` stubs — lit at the operator checkpoint). **The chat ELICITATION wiring (`f0gh`):** the conversation stage still doesn't ask the nutrition/supplement/peptide/training domains. **The DESIGN-ESCALATION:** the stale plan-gen docs (`/generate-plan` SKILL.md + `plan-generation-pipeline-v1.md`) describe the superseded runtime-A interactive-dispatch model — reconcile to the now-built programmatic orchestrator (the design doc carries a `superseded_by: ADR-0022` pointer; the skill text still needs updating). The SEC-01 caller-precondition beads (the ADR-0025-T1 realpath-resolve-before-write; the `_repo_root` explicit pass) land with the production caller.
3. **Standing mechanics (all HELD).** Build via the full pipeline (`/create-adr`→spec→build-plan→task-plan→`/execute-plan`; never hand-rolled, PF-S36-01); the recipe-review (QA+Security+Architect, the design-defect gate — caught a real defect PRE-CODE every wave) → judge ACCEPT before build; Tier-3 `/review-pr` 6-agent with profile-less blind-triage + EXECUTED blind-verify INTACT (caught a real defect PRE-MERGE every wave); EXTEND-NOT-REBUILD (the inner engine is WIRED not rewritten — numstat=0 every wave); store-surface tasks satisfy `docs/checklists/store-adversarial-tests.md`; **full role profiles inlined VERBATIM on EVERY dispatch (the hook denies abbreviation)**; `/review-pr` off the THREE-dot diff; `/merge` (REST fallback when GraphQL exhausted, full-40-char SHA guard); `git add` SEPARATE from `git commit` (the combined form trips block-pii-commit; the hook auto-stages `.beads` — benign); single trunk; stage close docs by EXPLICIT path (never `git add vault/` — perpetual daemon churn); **NEVER stop mid-autonomous-loop to ask a question the standing directive + the more-rigorous-path heuristic already answer (PF-S92-01) — a milestone is a report-while-continuing checkpoint, not a stop-and-ask**; re-run the close gate on final post-merge `main` + cite the SHA (PF-S74-01). New beads this session: `71s4`-adjacent (the engine live-wiring), the `.pipeline/` namespace-collision + `/run-pipeline` fresh-start-rigor-mismatch (skill rough edges), the SEC-01 caller-preconditions, `gnko` (PF-S92-01). Carry-over: `f0gh`/`j432`/`pqb0`/`zwow`/`f22s`/`oves`/`02m1`/`v70t`.

## Current State (volatile)

- **S92 (2026-06-24): designed + BUILT the plan-generation ENGINE end-to-end via the FULL autonomous build pipeline.** ADR→spec→build-plan→task-plan→execute-plan, 8 PRs merged to `main` @ `a47c664`: ADR set #238 (0020–0025), spec #239, build-plan #240, Wave-1-prep #241, Wave 1 #242 (PII envelope: de-id-IN boundary + the scan-scope fix), Wave 2 #243 (orchestrator + outage-halt + deterministic de-id-OUT), Wave 3 #244 (quality judge + safety review + dispatch cap + maintained HTML), Wave 4 #245 (the keystone bounded revise loop + the full E2E). All mock/fixture-tested (0 live spend); suite 1611/2.
- **The crown-jewel PII boundary HELD across the whole engine.** de-id IN: a model-backed `ModelClient.deidentify` + a fail-closed field-set whitelist (the W1 6-agent review caught a real leak — a rubber-stamping model could pass raw PII; fixed). de-id OUT: DETERMINISTIC re-insertion (model off the OUT path), gitignored-only + realpath-containment (the W3 review caught a symlink-bypass pre-code). The autonomous loop: FAIL-CLOSED safety — `safety_passed is True` is the ONLY surface path; scratch-and-promote keeps the real store clean on any block; INV-CRITICAL-NON-OVERRIDABLE holds (the W4 review caught the 2 HIGH fail-open paths).
- **The layered pipeline caught a REAL defect at every layer before merge.** Recipe-review (the design-defect gate, PRE-CODE): the de-id-seam misnaming (W1), the tautological gate-idle-seam trap (W2), the convergent gate-wiring-ownership conflict (W3, Architect binding), the 2 HIGH fail-open safety paths (W4). 6-agent `/review-pr` (PRE-MERGE): the field-set-whitelist leak (W1), the silently-defeated quality gate that scored the wrong result shape (W3), the promote-OSError + unknown-revise-domain escapes (W4) — all mutation-proven + executed-blind-verified. EXTEND-NOT-REBUILD held every wave (the S70–S91 inner engine byte-unchanged; the engine WRAPS it).
- **`run_orchestrated` has no production caller yet** (bead `71s4`; the core-capability-audit still pins `generate_plan.py`) — the LIVE run (real backends + operator data) is the operator-present next step. The live `ModelClient` backends are `NotImplementedError` stubs.
- **One new PF — PF-S92-01** (stopped mid-autonomous-loop to ask a false binary the standing directive already answered; operator-corrected; recurrence ≥5 of the autonomous-loop-violation class; bead `gnko` + harvest record).
- **Beads:** new — `gnko` (PF-S92-01), the `.pipeline/`-namespace-collision + `/run-pipeline`-fresh-start-rigor-mismatch (skill rough edges), the 2 SEC-01 ADR-0025-T1 caller-preconditions. Carry-over: `71s4` (the engine live-wiring), `f0gh`/`j432`/`pqb0`/`zwow`/`f22s`/`oves`/`02m1`/`v70t` + the prior set.
- **Single clean trunk @ `main` `a47c664` (as of 2026-06-24 S92 close, PF-S74-01); no extra worktrees, no stashes; vault perpetually daemon-dirty (stage by explicit path).**

**Historical (kept for reference):** `vault/sessions/session-92.md`.

## What Is Next (volatile)

### S93 — wire the engine to a production front door + the operator-present LIVE run

The plan-generation engine is BUILT end-to-end (mock/fixture-tested). The core capability is wired in code but has no production caller and no live model client. The directed next steps:

1. **Wire `run_orchestrated` as the production entry (bead `71s4`).** Point a front door (the `/generate-plan` skill / a CLI) at it; pass a REAL composed `gate_dispatch` (the autonomous path fail-closes to SAFETY_BLOCKED without it — that's by design). Repoint `scripts/core-capability-audit.sh` from `generate_plan.py` to `run_orchestrated` (the INV-CORE-CAPABILITY assertion). Land the 2 SEC-01 caller-preconditions (the ADR-0025-T1 realpath-resolve-before-write; the explicit `_repo_root` pass).
2. **Light the live `ModelClient` backends** (`_ClaudeNoTrainBackend.{converse,author,deidentify}` — currently `NotImplementedError` stubs). Then the **operator-present LIVE end-to-end run**: the operator injects `export ANTHROPIC_API_KEY=$(security find-generic-password -s "quant-primary-api" -w)`; the agent NEVER touches the keychain; never commit the key (PUBLIC repo). Mock/fixture-tested up to that point (0 spend).
3. **Reconcile the stale plan-gen docs (the DESIGN-ESCALATION):** `/generate-plan` SKILL.md + `plan-generation-pipeline-v1.md` (the latter carries the `superseded_by: ADR-0022` pointer; the skill text + `author-dispatch-process.md` still describe the superseded runtime-A interactive-dispatch model) → update to the now-built programmatic orchestrator.
4. **The chat ELICITATION wiring (`f0gh`):** the conversation stage still doesn't ask nutrition/supplements/peptides/training — fold into the engine's conversation stage.

**Deferred / NOT blocking:** the skill rough edges beaded this session (the `.pipeline/` namespace collision; the `/run-pipeline` fresh-start-rigor mismatch — fix upstream in skills_library); `j432`/`pqb0`/`zwow`; `f22s`/`oves` (wiki); `02m1` (daemon config); the local-model migration (North Star, post-V1).
