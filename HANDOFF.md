...
1. **AP-PROTOCOL-FROM-MEMORY / read-before-invoke (PF-S13-01 + PF-S17-01) — STANDING; the one remaining gated pipeline stage is `execute` (all planning stages are done).** Execution runs wave by wave against the 18 recipes; each recipe's TDD steps + the build plan's wave checkpoints are the gates — READ the recipe IN FULL before executing each task (HELD this session for `/create-task-plan` + `/review-pr` + `/merge`). Run a wave from memory instead of its recipe and the class recurrence promotes.
2. **PII enforcement is the V1 critical path — DECIDED (ADR-0001) + SPECCED + SCHEDULED + now RECIPE-PINNED enforcement-first (S29 bead DAG), but UNBUILT.** The bead DAG wires `ADR-0006-T0` (W3) → `ADR-0006-T1` (W4) → `ADR-0006-T2` (W5) and `ADR-0001-T1` (W2) → all 8 data-out 0-egress consumers; `bd ready` cannot surface a plan-reasoning-over-PII task before its router prerequisites close. Execute MUST honor this: no plan-reasoning-over-PII task built before `ADR-0006-T0`/`T1` lands; store/ingestion/generation stay model-independent. The recipes pin the failing-capable egress/PII/HALT RED tests that prove it at build time.
3. **PF-S22-01 falsification window — still live for the parallel library-population track.** Research-only `/aplus-research` sessions remain the first parallelized track after S22. At each batch close: ONE merge target, `branch-completeness-audit.sh` + `wiki-lint.sh` green. Unchanged by the task-plan stage.

**Demoted from prior Top-3:** the "`mo4` task-plan is next" framing of #1 (now re-points at `execute` — the last gated stage); the "scheduled but not recipe-pinned" framing of #2 (now pinned in the S29 recipes + bead DAG).

## Current State (volatile)

- **THE V1 TASK-PLAN STAGE IS COMPLETE (S29, 2026-06-04).** Merged on `main` (PR #40): **18 per-task implementation recipes** in `docs/task-plan/` (`status: approved`) + **the executable per-task bead DAG** (18 `v1-build` beads, 41 dependency edges). Pipeline: PRD (S24) → ADR (S25) → spec [S26+S27] → build-plan [S28] → **task-plan [S29] ✓** → execute (now UNBLOCKED — the last gated stage).
- **The recipes:** one per build task, 8-section template (Entry State, Context Load, Risk Pre-Check, TDD Steps, Interface Contracts, Verification Checklist, Commit, Rollback). Every spec acceptance criterion maps to a failing-capable RED target; the security-sensitive recipes (ADR-0001-T1 egress/PII guard, ADR-0005-T1 content-scan hook, ADR-0006-T1 no-train router, ADR-0006-T2 plan-assembly HALT) carry the SEC-03 fail-injection + the health-safety floors. Each passed a fresh independent judge (all 10 dims ≥9).
- **The bead DAG:** `bd ready` frontier = exactly the 3 Wave-1 spikes (`ADR-0001-T0`, `ADR-0002-T0`, `ADR-0004-T0`); 15 blocked downstream. Enforcement-first is recipe-pinned in the edges: `ADR-0006-T0`→`T1`→`T2` and `ADR-0001-T1` → all 8 data-out 0-egress consumers (Contracts review confirmed 0 missing/extra/reversed vs the 41-edge build-plan DAG).
- **SINGLE TRUNK** unchanged (since S22): `main` = complete project. Task plan **rebase-merged via PR #40** (`/review-pr` 3-agent docs subset, Gate PASS — 6 legitimate findings: 2 fixed + blind-verified RESOLVED, 4 beaded `e3d`; 1 NOT_A_BUG; **0 suppressed**, matrix priority-only per PF-S26-01). No code built — design only.
- **`mo4` CLOSED** (task plan delivered); **execute UNBLOCKED**.
- **Active landmarks:** no trigger windows opened.

**Historical (kept for reference):** `vault/sessions/session-29.md` + `vault/meta/log.md` S29 entry.

## What Is Next (volatile)

### S29 completed the task-plan stage (merged). Next = `execute` the build wave by wave.

**RESUMPTION POINT.** S29 delivered the 18 V1 implementation recipes (`docs/task-plan/`, each judge ACCEPT all-dims-≥9) + the executable per-task bead DAG, reviewed (`/review-pr` #40 Gate PASS — 2 fixed + blind-verified, 4 beaded) and rebase-merged to `main`. The task-plan stage (`mo4`) is **COMPLETE** — all planning stages are done. **Open the next session on `main`** (Start Protocol + `branch-completeness-audit.sh` at open).

**Next pipeline work — `execute`:**
1. **Start at the readiness frontier:** `bd ready` surfaces the 3 Wave-1 spikes (`ADR-0001-T0`, `ADR-0002-T0`, `ADR-0004-T0`). Work each task by READING its recipe `docs/task-plan/<task-id>.md` IN FULL first (PF-S17-01), running its TDD cycles, satisfying its Verification Checklist, then `bd close` it — which unblocks the next wave's beads. The build plan's wave checkpoints gate each wave boundary.
2. **Preserve enforcement-first** (the bead DAG enforces it, but verify): no plan-reasoning-over-PII task (`ADR-0006-T2`) built before `ADR-0006-T0`/`T1`; the egress guard `ADR-0001-T1` (W2) before all 8 data-out 0-egress consumers. The recipes' SEC-03 fail-injection + health-safety RED tests must turn red on a broken guard/HALT.
3. **Honor the spike→impl handoffs:** the 4 spikes (`ADR-0001-T0`, `ADR-0002-T0`, `ADR-0004-T0`, `ADR-0006-T0`) produce gitignored `docs/spec/.pipeline/` reports the downstream impl recipes' Entry States HALT-gate on — execute the spike + record its report before its consumers.

**Carried recipe-level flags for execute** (recorded in the recipes, surface at their wave): the unnamed plan-`template` producer for `render.emit` (ADR-0005-T2 Deviation #2 + ADR-0006-T2 Upstream Flag, bead `4xe`) — resolve the Architect adjudication before the plan-render step; the ADR-0005-T2 crit-3 Wave-ordering scoping (bead `4xe`).
...
### Open beads carried
- **V1 build execution (label `v1-build`, NEW S29):** 18 per-task beads, dependency-wired. READY now: the 3 Wave-1 spikes. Plus `e3d` (recipe-set doc-consistency normalization — review F3/F4/F5/F6, non-blocking).
...
- **Closed:** S29: `mo4` (task-plan stage — merged PR #40). S28: `hv6` (merged PR #38). S27: `rg2`. S25: `fm4`, `hil`. S23: `bte`. S22: `gdw`. S21: `mhg`/`5ot`/`0be`.
