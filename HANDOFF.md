...
## Scope Contract — Session 81 (2026-06-20)

Goal: Build `/generate-plan` — the operator-facing INVOCATION path for the closed loop's GENERATE leg (the missing front door). A project-local skill that walks the orchestrator (runtime A) through the plan-generation lifecycle — dispatch each `PLAN_DOMAINS` specialist (full profile) over the de-identified summary + gated wiki → build the `authors` dict → run the reconciled pipeline (`orchestrate.generate_plans` with the energy-bounce re-author + medical-liaison adjudicator dispatch hooks) → collate the `dvq::queue` → render — backed by a thin TESTED production seam (`generate_plans` had no production caller). Proven end-to-end on synthetic data.
...
### S81 Scope Contract Evaluation (2026-06-20, volatile)

- **AC1–AC6 — PASS.** `run_generation` stitches `generate_plans` → `collate_doctor_visit_queue` (closing the no-production-caller gap; `orchestrate.py` byte-unchanged); the `/generate-plan` skill + command codify the dispatch lifecycle (full-profile specialists, the reauthor + adjudicator hooks, collate, render); the safety floor is reused (held → adjudicate, the clearance gate); 13 pipeline tests incl. the cleared-held-finding stitch (mutation-proven vs the no-adjudicator control), block-stands queuing with the real band, multi-axis + rx-bpmh collation, the gates/reauthor forwarding, the store battery; a REAL personal-trainer dispatch (full profile) over a synthetic trainee authored a workout → `run_generation` recorded it (load dropped, clearance not granted) → the dashboard rendered all 5 exercises; `generate_plans`/`collate`/`compute_plan`/`reconcile`/`adjudicate`/`generate` UNCHANGED; full suite 1145/2 on final `main`, core-capability gate green.
- **AC7 (review + close) — PASS.** Tier-1 → Tier-2 plan-integrity INTEGRITY-CLEAN + QA (1 MUST FIX + 2 SHOULD FIX fixed — the seam's gates/reauthor forwarding was untested, proven silently severable) → Tier-3 `/review-pr` 6-agent (security/bug-hunter/code-quality CLEAN; contracts PASS; blind-triaged 3 LEGITIMATE seam-collation gaps fixed + blind-verified RESOLVED, 2 NOT_A_BUG/NOT_ACTIONABLE, 1 DECISION pinned; historical HIST-1 procedural — the rebase-merge + the issues.jsonl revert resolved it) → `/merge` rebase PR #191 → `main` @ `2b946e8`; full close, close gate re-run on final `main` + the SHA cited (PF-S74-01).

### Drift checks (S81 close)

- **Task drift:** all 7 ACs PASS. The seam-forwarding tests (Tier-2) + the multi-axis/rx-bpmh/band collation tests (Tier-3) were ADDED at review — caught by the layered review, strengthening AC5's coverage, not silent drift. The bd daemon auto-swept a cosmetic `issues.jsonl` re-encode into the feat commit — caught at the merge gate (the HIST-1 lens) + reverted so the PR carried only the 5 code files. No silent drift.
- **Architecture drift:** no invariant degraded. `orchestrate.py` + the pipeline modules UNCHANGED (REUSE only — `run_generation` is a thin production caller; security/bug/historical confirmed no S41 cross-stream / clearance-bypass regression); the safety floor is reused (held → adjudicate, never bypassed — plan-integrity + security confirmed no direct record path). INV-CORE-CAPABILITY green (`run_generation` is the cross-domain terminal's production caller, `--self-test` PASS); INV-ROLE-INLINING held (the `enforce-role-inlining` hook CAUGHT + I corrected an abbreviated Tier-2 profile mid-session; the real personal-trainer dispatch + the 6 review agents full-profile). Toward the vision.
- **Vision drift:** none, toward-vision. The closed loop now has a FRONT DOOR — `/generate-plan` produces a followable, safety-gated, specialist-authored plan end-to-end on synthetic data. `design/vision.md` first sentence unchanged. No drift.

### PF attestation

S81 close (2026-06-20): **No new PF-class entries this session** — every finding was caught by the layered review or a mechanical guard BEFORE merge (nothing escaped). Observed-but-not-promoted: (a) the Tier-2 dispatch ABBREVIATED the plan-integrity + QA role profiles (the be-concise bias overriding the role-profile mandate); the `enforce-role-inlining` hook BLOCKED it and I corrected by reading + inlining the full verbatim profiles — a recurrence of the documented abbreviate-the-profile pattern caught by its existing mechanical guard, zero escaped impact (the guard working as designed); (b) the bd daemon auto-swept a cosmetic `issues.jsonl` re-encode into the feat commit — caught at the merge gate (the same bd-auto-sweep mechanics as the prior design-commit), reverted so the PR carried only the 5 code files; (c) the seam's gates/reauthor forwarding + the multi-axis/rx-bpmh collation were grounded at Tier-2/Tier-3 review rather than the Tier-1 self-check — the layered-review mechanism working (the seam-test-the-forwarding discipline, a recurrence-watch on Tier-1 wiring-contract tests for a new production caller). The disciplines that HELD: **specialist-reasoned-not-solo** (a REAL personal-trainer dispatch authored the plan; the skill originates no plan content — plan-integrity + security confirmed no invented logic); **the safety floor reused** (held → adjudicate, the clearance gate — never bypassed); **the store-surface battery**; **REUSE-only** (the pipeline modules UNCHANGED); **close-on-final-`main`** (re-run on `main` @ `2b946e8`). Full attestation + the per-PR skill-trace table (PR #191) + the disclosure ledger in `memory/process-failures.md` Session 81.

## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-80 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; …; S79 archived S78; S80 archived S79; S81 archived S80) to keep this handoff lean. The current (S81) scope contract is above; the archive holds the prior-session archaeology.
...
1. **The closed loop is COMPLETE *and* INVOKABLE — the only remaining V1 work is REAL operator data (PF-S63-02 satisfied + a front door).** S81 added `/generate-plan` (the GENERATE front door); with the full plan→act→measure→adjust loop (S70-S80) it now runs end-to-end on SYNTHETIC data, specialist-authored. **Do NOT propose new pipeline mechanism, a dashboard REDESIGN (design-LOCKED, not happening), `/generate-plan` (DONE S81), or the #179 merge (MERGED) as next work.** The genuine next surface is operator-data ingest (operator-gated, the unlock from synthetic→real) + LM-01 prep. Before any secondary scope, the PF-S63-02 gate answer is "YES — wired AND invokable end-to-end on synthetic"; state WHY the secondary work is right in the scope contract.
2. **A new production caller that FORWARDS params to a terminal must TEST the forwarding at Tier-1 (the S81 watch).** S81's `run_generation` forwards `gates`/`reauthor`/`adjudicator` to `generate_plans`; QA proved severing them at the seam left all tests GREEN (the Factory-to-Component Wiring class — a signature param whose forwarding nothing asserts). **For a new seam/caller: each forwarded param gets a test that goes RED if severed (a non-default value that the callee acts on — e.g. `clearance_granted=True` keeps load), and the seam's NEW behavior (here the collate multi-entry / rx-bpmh / band-threading paths) gets seam-level tests, not just the inner-layer coverage.** Caught at Tier-2/Tier-3, nothing escaped.
3. **The disciplines that HELD + standing mechanics.** (a) **Specialist-reasoned-not-solo:** any plan/adjustment is authored by a REAL specialist dispatch (full profile inlined), never invented in `scripts/` (the skill is the wiring + the dispatch lifecycle; plan-integrity + security confirmed zero invented logic). (b) **Paste FULL role profiles VERBATIM** — the `enforce-role-inlining` hook BLOCKED an abbreviated Tier-2 dispatch this session; the be-concise bias does NOT apply to role profiles (read the whole `~/Documents/Projects/skills_library/roles/<role>/agent.md` + paste it). (c) **Store-surface battery** on every store read/write surface; **REUSE-only** for pipeline modules. (d) **Close on FINAL post-merge `main`** (`cvf1`): re-run pytest + `close-audit --session N` + cite the SHA. Standing mechanics: **bd is FIXED** (S81-prep `bd quit; rm -f .beads/beads.db*; bd init --prefix a-plus-maxing` resolved the repo-ID mismatch — bd works, commit hooks pass on a feature branch in the main trunk; the worktree+park dance is now only the FALLBACK if bd breaks again); **bd still auto-sweeps a cosmetic `issues.jsonl` re-encode into commits → restore `issues.jsonl` to main's version + re-commit `--no-verify` so a code PR carries only code files**; review/merge off the **THREE-dot diff** (`main...branch`) not two-dot — a branch behind main shows PHANTOM deletions of wiki entries main added (HIST-1); GraphQL EXHAUSTED → REST for PR-create/merge (`gh api`, full-40-char SHA guard); `git add` SEPARATE from `git commit`; the compound `git push && gh api` trips the block-dangerous parser → push as its own command; two parallel wiki sessions push `origin/main` — never touch `vault/library/` AND leave their uncommitted `vault/` changes untouched; **close-step-8 freshness un-gated until `dfg1` — RUN step 8 each close**. Carry-overs: `6aen` / `cjgg`/`uerm`/`pwpr`/`sip9` / `cvf1` / `dfg1`/`df1x` / `aem1` / `axm7` / `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`/`5fw1`.

## Current State (volatile)

- **S81 (2026-06-20) built `/generate-plan` — the GENERATE-leg INVOCATION path (PR #191 → `main` @ `2b946e8`, rebase).** The closed loop now has a FRONT DOOR: a followable, safety-gated, specialist-authored plan generates end-to-end on synthetic data.
- **`scripts/plan/pipeline.py` `run_generation` (NEW):** the thin production caller stitching `orchestrate.generate_plans` (compute → reconcile → adjudicate-held → record) + `collate_doctor_visit_queue` into one recorded GENERATE pass, returning `{**result, dvq_entries}`. REUSE only — it closes the "`generate_plans` had no production caller" gap (the INV-CORE-CAPABILITY pattern applied to the cross-domain terminal); `orchestrate.py` byte-unchanged. The **`/generate-plan` skill + command** (`.claude/skills/generate-plan/`) codify the dispatch lifecycle: de-identified inputs → dispatch each `PLAN_DOMAINS` specialist (full profile, INV-ROLE-INLINING) → build `authors` → call the seam with the `reauthor` (energy-bounce re-dispatch) + `adjudicator` (medical-liaison gate) hooks → collate → render. Reasoning is the specialists' (runtime A), never invented in code.
- **Real-dispatch E2E:** a live personal-trainer (full profile) authored a workout for a synthetic trainee under `clearance_granted=False`; `run_generation` recorded it (load dropped by the gate); the dashboard rendered all 5 exercises. Captured: `docs/plan-generation/examples/generate-plan-workout-author-output.example.json`.
- **Review (PR #191):** Tier-2 plan-integrity INTEGRITY-CLEAN + QA (1 MUST FIX + 2 SHOULD FIX fixed — the seam's `gates`/`reauthor` forwarding was untested, QA proved it silently severable) → Tier-3 `/review-pr` 6-agent (security/bug-hunter/code-quality CLEAN; contracts PASS; blind-triaged 3 LEGITIMATE seam-collation gaps — multi-axis, rx-bpmh, block-stands band — fixed + blind-verified RESOLVED; 2 NOT_A_BUG/NOT_ACTIONABLE; 1 DECISION pinned; historical HIST-1 procedural, rebase-merge resolved) → `/merge`.
- **No new PF this session** — every finding caught by the layered review or a mechanical guard before merge (the `enforce-role-inlining` hook caught an abbreviated Tier-2 profile; corrected). Watch: a new production caller's param-forwarding gets a Tier-1 test (the seam's forwarding was grounded at review, not Tier-1).
- **pytest 1145/2 on final post-merge `main` (`2b946e8`, 2026-06-20 S81 close).** Core-capability gate green (`--self-test` PASS).
- **bd FIXED (S81-prep):** the main-trunk repo-ID mismatch was resolved via `bd quit; rm -f .beads/beads.db*; bd init --prefix a-plus-maxing` (fingerprint verified; bd works, commit hooks pass on a feature branch in the trunk).
- **Beads:** no `/generate-plan` bead existed (it was a deferred "later convenience"); none created (the work is done). Carry-overs `cjgg`/`uerm`/`pwpr`/`sip9`/`cvf1`/`dfg1`/`df1x`/`aem1`/`axm7`/`6aen`/`0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`/`5fw1`.
- **Active landmarks:** LM-01 (First MD visit) **2026-07-13** — the 14-day scoped-drift-audit window opens **2026-06-29**. The closed loop is now COMPLETE *and invokable* end-to-end on synthetic data; the remaining LM-01 gap is REAL operator data. LM-02 Whoop / LM-03 23andMe date-TBD.

**Historical (kept for reference):** `vault/sessions/session-81.md`.

## What Is Next (volatile)

### S82 — the Apple Health ingestion adapter (the first real-data step) [operator-chosen]

**The V1 closed loop is COMPLETE *and invokable* on SYNTHETIC data** (`/generate-plan` + the plan→act→measure→adjust loop + the SBAR handout + face sheet). What's DONE (do not re-propose): the closed loop, `/generate-plan` (S81), the design PR #179 (merged), the dashboard (design-LOCKED). The operator chose the first REAL-data adapter as the next task:

1. **Build the Apple Health ingestion adapter — bead `v3ml`** (the operator uses Apple Health, NOT Whoop). Add `scripts/ingest/adapters/apple_health.py` implementing the ADR-0003-T1 contract (`source_tag()` + `read_readings`); the scheduler auto-discovers it with 0 edits to the routine/scheduler (the ADR-0003-T2 invariant). The new work vs the existing Whoop adapter: parse `export.xml` (streamed via `xml.etree.iterparse`) not SQLite, and **aggregate Apple's RAW timestamped samples to one value per day** (the store's (item, day, source) key) — the main new logic. Metric map: `HeartRateVariabilitySDNN`→hrv, `RestingHeartRate`→rhr, `OxygenSaturation`→spo2, `RespiratoryRate`→resp-rate, `AppleSleepingWristTemperature`→skin-temp-dev, sleep-stage records→sleep-efficiency (the fiddliest); recovery/strain do NOT exist in Apple Health (honest absence — and consistent with the personal-trainer no-black-box-composites stance). Plus tests (mirror the Whoop adapter test + the store-adversarial battery for `source='apple-health'`) + a short ADR (the export.xml mechanism + the daily-aggregation decision + the operator-owns-the-export license path, same as Whoop) + flip `whoop.py` to `UNWIRED = True` (instead-of, not alongside). Build/test on a synthetic `export.xml` fixture first; the operator's real export validates it. **Full scope in bead `v3ml`.**
2. **The rest of operator-data ingest** (after the adapter): the filled `operator-profile.md`/`current-state.md` scaffolds, the labs + 23andMe adapters (no parsers exist yet — only the framework + the new Apple Health adapter), the MD-visit outcome — all operator-gated (the PII boundary + ADR-0005 → a gitignored local store, never committed).
3. **LM-01 (July 13 MD visit) prep** — the 14-day scoped-drift-audit window opens **2026-06-29**; the 7-day MD-handoff-GENERATION window opens **2026-07-06**. The SBAR render + face sheet + `/generate-plan` are all BUILT — the remaining LM-01 gap is REAL operator data + an MD-handoff artifact GENERATED from it.

**Discipline:** scope contract at open; bd is FIXED so a `feature/*`/`fix/*` branch in the main trunk commits cleanly (no worktree needed unless bd breaks again); for any store surface, the store-surface battery; **at Tier-1, value-domain-ground every field new code branches on (PF-S78-01) + a new caller's param-forwarding gets a RED-if-severed test (the S81 watch)**; for specialist-reasoning, a REAL specialist dispatch (full profile, paste it VERBATIM); for a visual artifact, Pencil + the design agents + operator sign-off FIRST (never solo); `/review-pr` (off the THREE-dot diff) → `/merge`; **re-run the close gate on the final post-merge `main` + cite the SHA (PF-S74-01)**.
...
S81 close (2026-06-20): RE-OPENED `vault/meta/landmarks.md` at close step 8.7 (unchanged this session). No landmark edited — S81 built `/generate-plan` (the GENERATE invocation path) on PII-free SYNTHETIC data (a synthetic trainee; no real operator data ingested, no Whoop baseline, no MD-handoff artifact GENERATED from real data). LM-01 (First MD visit) **2026-07-13**: no trigger window open today (the 14-day scoped-drift-audit window opens **2026-06-29**, the 7-day MD-handoff window 2026-07-06). The closed loop is now COMPLETE *and invokable* end-to-end on synthetic data (`/generate-plan` → the plan→act→measure→adjust loop + the SBAR handout + the face sheet) — the remaining LM-01 gap is REAL operator data. LM-04 (first HTML artifact) NOT triggered — the renderers emit to the gitignored `vault/artifacts/generated/` (renderers, not COMMITTED artifacts). LM-02 (Whoop) / LM-03 (23andMe) date-TBD. No landmark actions due.
