...
## Scope Contract — Session 87 (2026-06-22)

Goal: Enable + evaluate the operator's real use of the intake website (sync the stranded trunk so they can run it), audit the intake's end-to-end wiring, resolve the intake's product direction, and set up the next session to build that direction autonomously. (An EMERGENT session — no upfront contract; this is retroactive.)

Acceptance criteria:
- [x] AC1 (trunk sync, WIP-preserving): the operator's stranded MAIN checkout (`feature/apple-health-adapter` `d99e437`, 41 behind) synced to `origin/main` @ `dfd44e8` so `python -m scripts.serve` runs the merged intake code; the parallel-wiki vault WIP PRESERVED (full backup `stash@{0}`; 6 clean files re-laid; 3 diverged `bpc-157` files left at main's version for operator reconcile) — never touched the WIP destructively.
- [x] AC2 (server smoke test): the loopback intake server started + verified serving the 6-step wizard (HTTP 200, capture fields) from the operator's real checkout.
- [x] AC3 (intake-wiring audit): full audit — only 6 signals reach the planner; Step-1 demographics DEAD placeholders; 4 ORPHAN model tokens; most fields record-only (never reach the planner).
- [x] AC4 (direction resolved + recorded): operator-confirmed VISION PIVOT to a conversational intake agent (form = demographics+uploads; rich sections = chat; no-train API V1 → local model; ADR-0001 boundary relaxation signed off). Recorded — user memory `project_vision_pivot_conversational_intake` + epic bead `w3y8`.
- [x] AC5 (autonomous next-session setup): the S88 scope + autonomous directive written into "What Is Next" so the next session runs the pivot build autonomously.
- [x] AC6 (close): full S87 close on final post-merge `main` + cite the SHA (PF-S74-01).

Files I WILL touch: HANDOFF / PF-log / harvest / beads / session-note / overview (the close docs); the user memory (the pivot). NO repo CODE this session.
Files I will NOT touch: the operator's MAIN-checkout vault WIP (preserved in `stash@{0}`, never destructively touched); any repo code (the build is S88); `main` directly.
NOT doing: building the conversational intake agent (S88, autonomous); the ADR (S88); the `design/vision.md` update (S88, with the ADR); patching the form's dead Step-1 / orphan tokens (SUPERSEDED by the pivot — the form's rich sections are being replaced).
Invariants at risk: ADR-0001 (the zero-egress boundary is DELIBERATELY relaxed by the confirmed pivot [raw conversation → no-train API], operator-signed-off + recorded, ADR lands S88 — flagged, NOT silent; the store stays de-identified); INV-CORE-CAPABILITY (PF-S63-02 — the pivot is the path to the first model client → real plan generation); INV-CLOSE-AUDIT / INV-PF-ATTESTATION / INV-SKILL-TRACE / INV-HARVEST-CAPTURE at close; INV-BRANCH-NOT-MAIN. **Core-capability-first gate (PF-S63-02): the pivot DIRECTLY targets the core (first model integration).**

### S87 Scope Contract Evaluation (2026-06-22, volatile)

- **AC1–AC6 — PASS.** Trunk synced (WIP preserved in a labeled stash + re-laid); server smoke-tested; the wiring audit surfaced the form's lossiness; the operator confirmed the conversational-intake pivot (durably recorded); the S88 autonomous directive is set; this close ran on final `main` + cites the SHA.

### Drift checks (S87 close)

- **Task drift:** S87 was EMERGENT (no upfront scope contract) — it began as "sync so I can test," became an audit, then a vision pivot. Each expansion was operator-directed (sync → "can't input" → audit → "it's too rigid" → pivot), not silent. The retroactive contract documents it. (Process note: an emergent session without an upfront contract is itself a minor deviation from the start-protocol; acceptable here because the work was operator-led discovery, recorded in full.)
- **Architecture drift:** YES — deliberately. The confirmed pivot RELAXES ADR-0001's zero-egress (the crown-jewel boundary) and introduces the first model client — the most significant architecture change since inception. Signed off + recorded (memory + epic bead `w3y8`), with the ADR to formalize it in S88. Flagged loudly, NOT silent; the relaxation is scoped (raw conversation → no-train API; the store stays de-identified).
- **Vision drift:** YES — deliberately. The vision is PIVOTING (local-first tracker/planner → conversational personal-health-agent). Operator-confirmed. `design/vision.md`'s first sentence is now STALE pending the S88 update (with the ADR) — recorded so the next session updates it.

### PF attestation

S87 close (2026-06-22): One new PF — PF-S87-01 (plumbing-reviewed-not-end-to-end-capability, operator-caught): the S86 Wave-B intake passed its 3-tier review on PII plumbing + the wired fields, but no review verified the feature's END-TO-END capability (does the captured intake produce a usable plan?) — so the dead Step-1 inputs + 4 orphan model tokens + record-only-everything surfaced only on operator use. Lessons: a non-functional input is a functional defect (not a cosmetic "minor"); apply the core-capability-first gate to a feature's end-to-end output, wired forward into the conversational-intake build's acceptance. The vision pivot itself is an operator-confirmed PRODUCT decision (boundary relaxation signed off), a decision not a failure. Full attestation + skill-trace + disclosure ledger in `memory/process-failures.md` Session 87.

**Historical (kept for reference):** `vault/sessions/session-87.md`.

## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-84 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; …; S83 archived S82; S84 archived S83; S85 archived S84; S86 archived S85; S87 archived S86) to keep this handoff lean. The current (S87) scope contract is above; the archive holds the prior-session archaeology.
...
1. **The intake direction PIVOTED (operator-confirmed 2026-06-22) — build the CONVERSATIONAL intake agent (epic `w3y8`), do NOT patch the form.** The rigid form is being replaced for the rich sections (it's too lossy — only 6 signals reach the planner; Step-1 demographics are dead; 4 orphan model tokens). Form keeps demographics + content-uploads ONLY; the rich sections (goals/training/nutrition/supplements/peptides) become a chat with an intake agent that writes de-identified facts to the store. **Do NOT fix the form's dead Step-1 / orphan-token gaps (SUPERSEDED).** The genuine next surface: the conversational intake agent — ADR → `vision.md` → design → build. Model: no-train API V1 → local model (North Star). Full grounding: epic `w3y8` + user memory `project_vision_pivot_conversational_intake` + `vault/sessions/session-87.md`.
2. **S88 runs AUTONOMOUSLY + the boundary is relaxed + verify END-TO-END capability.** The next session is authorized to write its own scope contract (self-confirm) and DRIVE CONTINUOUSLY through ADR→design→build without pausing for status (the drive-through-boundaries discipline). ADR-0001 zero-egress is RELAXED (raw conversation → no-train API; signed off; the store stays de-identified) — the first model client. Per PF-S87-01, verify the feature's END-TO-END output (a usable plan), not just that the chat plumbing works. The PII boundary is still the crown jewel: only the live conversation reaches the no-train API; everything PERSISTED stays de-identified.
3. **Standing mechanics + the operator's data state.** Build via **`/execute-plan`** (never hand-rolled); ADRs via **`/create-adr`**; design via the **design agents** (never solo-originate); full role profiles inlined VERBATIM (the hook denies abbreviation); `/review-pr` off the **THREE-dot diff** vs `origin/main` with BOTH independence phases INTACT → `/merge` (REST fallback when GraphQL exhausted, full-40-char SHA guard); `git add` SEPARATE from `git commit`; re-run the close gate on final post-merge `main` + cite the SHA (PF-S74-01). **The operator's trunk is now SYNCED to `origin/main` @ `dfd44e8` (as of 2026-06-22 S87 close);** their parallel-wiki vault WIP is in `stash@{0}` + 3 `bpc-157` files to reconcile (their call — never touch destructively). The basic-memory daemon re-adds `permalink:` frontmatter (benign churn).

## Current State (volatile)

- **S87 (2026-06-22): the intake's product direction PIVOTED.** Built+merged the form (Wave A+B, S85+S86), then by actually using it found the rigid form is too lossy — and the operator confirmed a pivot to a **conversational intake agent**. The form is being replaced for the rich sections.
- **The operator can now run the code:** their stranded MAIN trunk is synced to `origin/main` @ `dfd44e8` (2026-06-22 S87 close); the parallel-wiki vault WIP is preserved (full backup `stash@{0}`; 6 files re-laid; 3 `bpc-157` files at main's version to reconcile). The basic-memory daemon churn (`permalink:` frontmatter) is benign.
- **Intake-wiring audit:** only 6 signals reach the planner (goal-domains/goal-targets/goal-priority-order/hard-limits + recovery-status-band + train-around→active-issue-class). Step-1 demographics are DEAD static placeholders; 4 model-bound tokens (`sex-for-dosing`/`bodyweight-band`/`equipment-access-class`/`training-age-band`) are ORPHANS; most fields route record-only and never reach the planner.
- **The PIVOT (operator-confirmed, recorded):** form = demographics + content-uploads ONLY; rich sections = a conversational intake agent (targeted Qs + back-and-forth) → de-identified facts → the store. Model: **no-train API V1 → local model (North Star)**. Introduces the FIRST model client + relaxes ADR-0001 zero-egress (raw conversation → no-train API; signed off; store stays de-identified); finally addresses the core-capability gap. Recorded: user memory `project_vision_pivot_conversational_intake` + epic bead `w3y8`.
- **pytest 1264 passed / 3 skipped on `main` @ `dfd44e8` (2026-06-22 S87 close).** No repo code changed this session (sync + audit + decision).
- **One new PF: PF-S87-01** (plumbing-reviewed-not-end-to-end-capability, operator-caught).
- **Beads:** epic `w3y8` (the pivot, P1) + `8f1h` (PF-S87-01, P2); the S86 Wave-B follow-ups (`sc97`/`4oha`/`opka`/`67xq`/`y0eo`/`twrn`) are mostly moot under the pivot — triage during the design.

**Historical (kept for reference):** `vault/sessions/session-87.md`.

## What Is Next (volatile)

### S88 — AUTONOMOUS: build the conversational intake agent (ADR → vision → design → build)

**This session runs AUTONOMOUSLY per the operator's standing directive (2026-06-22): "run autonomously in the next session; write the scope contract and proceed through the process following the autonomous-loop protocols."** You are authorized to WRITE THE SCOPE CONTRACT AND SELF-CONFIRM IT (no operator confirmation needed) and then DRIVE CONTINUOUSLY through the pipeline — do NOT pause at boundaries for status (the drive-through-boundaries discipline). Reserve stopping ONLY for a genuine operator-owned decision the recorded decision does not already settle.

**The decision is MADE (do not re-litigate):** form = demographics + content-uploads (DNA/HealthKit/labs) ONLY; the rich sections (goals/training/nutrition/supplements/peptides) = a conversational intake agent (targeted questions + real back-and-forth) that writes de-identified facts to the existing store; model = **no-train API for V1 → local model (North Star, deferred but committed)**; ADR-0001 zero-egress RELAXED (raw conversation → a no-train API, operator-signed-off; the store + everything persisted stays de-identified). Full grounding: epic bead `w3y8` + user memory `project_vision_pivot_conversational_intake` + `vault/sessions/session-87.md` (the intake-wiring audit).

Sequence (use the GATED SKILLS IN FULL — read each before invoking):
1. **Write the S88 scope contract** (self-confirm) per the recorded decision; append it to HANDOFF.
2. **ADR via `/create-adr`** — the conversational-intake architecture: the FIRST model client (a no-train API; designed SWAPPABLE at the boundary for the local-model future, which also turns your API conversations into the local-model eval corpus); the ADR-0001 boundary relaxation (scope it: raw conversation → no-train API; the store + everything persisted stays de-identified); the **conversation → de-identified-store extraction contract** (how dialogue becomes the structured facts the planner reads — the load-bearing seam); the form/chat split; **whether the same model client also wires core plan-generation** (likely YES — PF-S63-02 / bead `71s4`, the first real model integration). Then update `design/vision.md` to the conversational-agent direction (its first sentence is STALE).
3. **Design pass** — via the design/architecture agents (NEVER solo-originate), grounded against the planner code: the question strategy, the chat transport (a chat endpoint on the existing loopback server), the extraction, the PII handling.
4. **Build** via `/execute-plan` + the 3-tier review. The PII boundary is the crown jewel (the store stays de-identified; only the live conversation reaches the API). **Per PF-S87-01, the acceptance MUST verify END-TO-END capability — a usable plan from the conversation, not just that the chat plumbing works.**
5. **`/review-pr`** (independence phases INTACT) → **`/merge`** → **close** (full protocol + the close gate on final post-merge `main` + cite the SHA).

**Deferred / NOT blocking:** patching the form's dead Step-1 + orphan-token gaps (SUPERSEDED — do not touch the form); the S86 Wave-B follow-up beads (`sc97`/`4oha`/`opka`/`67xq`/`y0eo`/`twrn` — mostly moot under the pivot; triage during the design); the operator's `bpc-157` WIP reconcile (their call); the local-model migration (North Star, post-V1); the operator loading real data (once the chat intake exists).

## Landmark window check (close step 8.7)

S87 close (2026-06-22): RE-OPENED `vault/meta/landmarks.md` at close step 8.7 (unchanged this session). No landmark edited — S87 synced the trunk + audited + decided the pivot on PII-free work; no real operator data ingested. LM-03 (23andMe) / LM-02 (Apple Health): the upload path exists but no real data ingested → not triggered. LM-01 (First MD visit) **2026-07-13**: no trigger today (the 14-day scoped-drift-audit window opens **2026-06-29**, the 7-day MD-handoff window 2026-07-06). LM-04 (first HTML artifact): renderers emit to the gitignored `vault/artifacts/generated/`; not triggered. No landmark ACTIONS due.
