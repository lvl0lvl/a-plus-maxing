...
3. **Correctness/governance tail (filler or one cleanup batch):** `e3b` (OPEN — close criterion is ADR-0006-T2 wiring verification), `b6um` (sub-AA legend P2 bug), S52-discovered `rn3v`/`imev`/`tdre`/`vjsw`/`dt0t`; carried `1ww`. Then LM-04 (Walter-pending real data) + the library-population `/aplus-research` track (explicitly deferred from this plan by the operator). Tail: `bd ready`.
