...
3. **Wave A is LANDED (#139); the rigor pull cadence is now live; the adoption tail is beaded.** `/review-pr` (6-agent → blind triage → blind verify; 18 findings → 14 fixed+verified incl. a real fail-open F1) + `/merge` landed Wave A on `main`. New standing discipline (Discipline 11): at the drift-audit boundary, diff the root `rigor_version` (1.0.0) vs `${SKILLS_LIBRARY}/frameworks/rigor/VERSION`, read the CHANGELOG delta, pull (re-run `toolkit/tests/run-all-tests.sh` first). Adoption tail beaded: Wave B `eyn4`, C `4hmo`, D `ckl1`, heartbeat `0qf6`, `d1kc`, `po4x`. Standing mechanics unchanged: REST `gh api` full-40-char SHA for PR ops; stage `git add` separately from `git commit`; `.claude/hooks/*` edits need per-session authorization.
...
- **Beads:** `71s4` (P1, core plan-generation + core-capability gate), `0qf6` (P2, heartbeat wiring), `d1kc` (P2, provenance test `jsonschema`). Waves B/C/D to be beaded at their start.
