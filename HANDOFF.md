...
Scope contracts for Sessions 5-34 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; S33 archived S32; S34 archived S33; S35 archived S34) to keep this handoff lean. The current (S35) scope contract is above; the archive holds the prior-session archaeology.
...
1. **read-before-invoke (PF-S13-01 + PF-S17-01) — STANDING; `execute` is MID-FLIGHT (data-IN wired + data-OUT render engine + the cron/on-demand ENTRY POINT now published; scheduler + matrix/projection render + router + clone-init next).** Each task's recipe is the gate — READ it IN FULL before executing (HELD this session for the `ADR-0004-T3` recipe + consumed surfaces + `/review-pr` + `/merge`). Next: `oaf` (scheduler, READY) or `ADR-0004-T2` (still blocked on `5wo`), or `xlu`/`br1` — TDD with `.venv/bin/python -m pytest`, never from memory; no `Workflow` substitution.
2. **The V1 generation ENTRY POINT is PUBLISHED + merged.** `generate.run(artifact_name, *, _root, _out_dir) -> Path` (`scripts/generate/generate.py`) is the thin on-demand + unattended-cron entry over `render.emit`: one code path reads the cross-item store, selects dashboard/report, drives exactly ONE `render.emit`, returns the path; no daemon / listening socket / stdin prompt; writes to the gitignored `vault/artifacts/generated/`. The AC-3 NG-4 gate is **MECHANICALLY FALSIFIABLE ACROSS THE PROCESS TREE** (catches an in-process, an `os.fork`-child, AND a `subprocess`-child listener). **CARRIED:** `4yk` (P2 — publish `store.read_all`/`items` + collapse `_read_store`; ADR-0002 amendment, the store on-disk coupling the review surfaced); `5wo` (still — `render.emit` single-Path vs `ADR-0004-T2` pagination); `u8u` P3; `qwj`/`1ww`/`1vi`/`ivt`/`z2u`; `4xe` (plan-`template` + render-size re-measure).
3. **PF-S3-01 layered-review is load-bearing on single-builder tasks — proven AGAIN (3rd consecutive: `n9h` S33, `gu4` S34, `3gp` S35).** The 6-agent `/review-pr` caught a REAL NG-4 falsifiability hole the orchestrator's OWN probe missed — the AC-3 gate was blind to a CHILD-PROCESS listener (the realistic "runs a server/daemon" breach); my probe tested only the in-process case. Fixed + independently re-verified by mutation (both child shapes → RED). For any single-SE build, the full 6-agent `/review-pr` + blind triage + blind verify is non-optional; when self-verifying a falsifiability gate, test EVERY failure mode INCLUDING the cross-process one, not just the obvious one.

**Demoted from prior Top-3:** the "render engine published / `gu4` open" framing (the render ENTRY POINT is now built + merged, the AC-3 gate is falsifiable across the process tree, `3gp` closed; the first-artifact producer is live — though no artifact is generated until `generate.run` is invoked with real operator data, LM-04 still pending).

## Current State (volatile)

- **THE V1 GENERATION ENTRY POINT IS PUBLISHED + MERGED (S35, 2026-06-06)** — one deliverable, one PR. PR #55 (`3gp`/`ADR-0004-T3` `generate.run` → `main` `9cfb607` as of 2026-06-06 S35 close). Pipeline: …→ Wave-3 wired adapters [S33] ✓ → Wave-3 render engine [S34] ✓ → **Wave-4 generation entry point [S35] ✓** → scheduler (`oaf`) + matrix/projection render (`T2`, blocked on `5wo`) + router (next).
- **`generate.run(artifact_name, *, _root, _out_dir) -> Path` (`scripts/generate/generate.py`):** one code path for on-demand AND unattended/cron; assembles the cross-item store read model (via `_read_store`, which enumerates the store on-disk layout — see `4yk`), selects dashboard/report by name, drives exactly ONE `render.emit`, returns the written path; reads no stdin, prompts for nothing, opens no server / binds no listening socket; writes to the engine-owned gitignored `vault/artifacts/generated/`. `main`/`__main__` = the cron-/operator surface (`python -m scripts.generate.generate <name>`). `render.py` consumed read-only (unmodified — no ADR-0004-T2 collision).
- **The AC-3 NG-4 gate is MECHANICALLY FALSIFIABLE ACROSS THE PROCESS TREE:** a `sitecustomize` listen-recorder drives the real cron surface as a subprocess + 3 positive controls (same-interpreter, `os.fork` child, fresh-interpreter) + a negative control. The original gate was blind to a child-process listener (the realistic server breach); the 6-agent review caught it (the orchestrator's own probe missed it), fixed + re-verified by mutation (both child shapes → RED).
- **The 6-agent `/review-pr` earned its keep on the single-SE build (PF-S3-01, 0 suppressed per PF-S26-01):** 12 deduped findings (0 from Security) → **9 fixed + 9/9 blind-verified RESOLVED**, 2 beaded (`4yk` P2 store surface), 1 DEFERRED-beaded (`u8u` P3), 1 NOT_ACTIONABLE (F10). Standout: the AC-3 child-process falsifiability hole (TEST), the store on-disk coupling (API/HIST → `4yk`).
- **SINGLE TRUNK** unchanged (since S22): `main` = complete project. Suite **120 passed / 2 skipped** (Linux-only egress).
- Bead `3gp` **CLOSED**; `4yk` (P2) + `u8u` (P3) filed. `bd ready` frontier = `oaf` (scheduler, READY) + `xlu`/`br1`; `ADR-0004-T2` (`yo6`) still blocked on `5wo`.
- **Active landmarks:** no trigger windows opened. LM-04 (first HTML artifact) still pending — the ENTRY POINT is published but no artifact is generated until `generate.run` is invoked with real operator data.

**Historical (kept for reference):** `vault/sessions/session-35.md` + `vault/meta/log.md` S35 entry.

## What Is Next (volatile)

### S35 published the generation entry point. Frontier: `oaf` (READY) · `ADR-0004-T2` (blocked-pending `5wo`) · `xlu`/`br1`.

**RESUMPTION POINT.** S35 built `ADR-0004-T3` (`generate.run` — the on-demand + unattended-cron entry over `render.emit`), rebase-merged (PR #55); closed `3gp`; filed `4yk` (publish `store.read_all`/`items`) + `u8u` (store dir-edge). **Open the next session on `main`** (Start Protocol + `branch-completeness-audit.sh` at open), then scope the next task — READ its recipe `docs/task-plan/<id>.md` IN FULL first (PF-S17-01). No single decided-next is pinned; the frontier (priority is a next-session call):

1. **`oaf` / `ADR-0003-T3` (scheduler) — READY, the readiest leaf.** Unattended `ingest.run` over the WIRED adapter set (HealthKit+Oura+Garmin), delta-only, 0-egress, Whoop excluded — completes the data-IN layer. The literal `rg "whoop" scripts/ingest/scheduler.py = 0` Wave 4→5 boundary check lands HERE once `scheduler.py` exists (the PR #50 AC-5 deferral). No blockers.
2. **`ADR-0004-T2` (matrix/projection render) — BLOCKED pending `5wo` (bead dep `yo6`→`5wo` wired).** Resolve `5wo` first: `render.emit`'s single-`Path` return vs T2 AC-2's required ≥2-path pagination (Architect amendment to the T1 Interface Contract OR caller-orchestrated pagination). Do NOT silently resolve inside T2. **Consider `4yk` alongside** — `generate.run`'s `_read_store` store-layout coupling collapses to one call once the published `store.read_all`/`items` surface lands.
3. **`xlu` (`ADR-0005-T1` pre-commit hook — reuses `pii_scan.scan`; resolve `qwj` first) · `br1` (`ADR-0006-T0` router spike).** READ each recipe IN FULL first, TDD with `.venv/bin/python -m pytest`, then `bd close`.
4. **The first ARTIFACT (LM-04)** lands when `generate.run` is invoked with real operator data (Walter-pending exports) — the producer is now live; only the data is missing.

**Published surfaces the next wave consumes:** `generate.run(artifact_name) -> Path` (NEW S35 — the cron/on-demand entry; a thin leaf, no downstream interface consumers plan against it). `render.emit(template, store_read) -> Path` + `component_set.py` (S34 — `ADR-0004-T2`/`ADR-0006-T2`/`ADR-0007-T2` consume them; path-return is the `5wo` open question for T2). The `Adapter` Protocol + four adapters — `oaf`'s scheduler assembles the wired set. `ingest.run(adapter, export_file, root=…)` — `oaf` invokes it. `store.append`/`read` (per-item; cross-item `read_all`/`items` pending `4yk`), `keying.py` single key (never redefine), `egress_guard.run(operation)`. `pii_scan.scan` for `xlu` — **resolve `qwj` first** (≈924 `scan` hits across vault prose). Enforcement-first holds: no plan-reasoning-over-PII task buildable until the router (`ADR-0006-T0`/`T1`).

**Carried flags / beads (surfaced, not lost — PF-S26-01):** S35's code-PR (#55) review filed `4yk` P2 (publish `store.read_all`/`store.items` + collapse `generate._read_store` — ADR-0002 amendment; the store on-disk-layout coupling, independently bead-and-proceed'd by the Contracts review per the `5wo` precedent) + `u8u` P3 (`store.read` raises `IsADirectoryError` on a dir-named `<x>.ndjson`, pre-existing store.py); the other 9 legitimate findings fixed in-PR, 0 suppressed. Still open from prior reviews:
- **`ADR-0004-T1` carried:** re-run the render-size measurement against the REAL template before `ADR-0004-T2`/`ADR-0007-T2` consume the cap; the plan-`template` producer for `render.emit` (bead `4xe`).
- **PR #53:** `5wo` P2 (the F12 `render.emit` single-Path vs `ADR-0004-T2` pagination — Architect amendment before T2).
- **PR #47:** `1vi` P3 (re-ingest cannot correct a stored reading's value — first-write-wins; a render/edit-layer product decision, not a keying change).
- **PR #44 review beads (still open):** `1ww` P2 (concurrent-append — premature for single-operator V1), `qwj` P2 (PII-free-trunk vault-prose operator name + ADR-0005 clone-init — **before V1 ships**), `ivt` P3, `z2u` P3.
...
### Open beads carried
- **V1 build execution (label `v1-build`):** per-task beads, dependency-wired; **9 closed** (Wave 1: `394`/`bez`/`qbb`; Wave 2: `89a`/`e9m`; Wave 3: `6be`/`n9h`/`gu4`; Wave 4: `3gp`). READY now: `oaf` (`ADR-0003-T3` scheduler) + `xlu`/`br1`. BLOCKED: `ADR-0004-T2` (`yo6`) pending `5wo`. Plus `e3d` (recipe-set doc-consistency, non-blocking).
...
All 4 active landmarks (LM-01 doctor visit July 2026, LM-02 Oura/wearable, LM-03 23andMe, LM-04 first HTML artifact) — no trigger windows opened during S35 (2026-06-05/06). LM-04 (first HTML artifact): still pending — `ADR-0004-T3` (`generate.run`) now published the first-artifact PRODUCER (writes to the gitignored `vault/artifacts/generated/`), but its trigger is "on generation" and no artifact is generated until `generate.run` is invoked with real operator data (Walter-pending exports). LM-01's 14-day-before window still depends on the TBD July exact date (not yet within window as of 2026-06-06); LM-02/03 remain Walter-pending. No status flips due.
