...
1. **Worktree session discipline (PF-S70-01 + PF-S71-01) — held this session.** Run parallel streams in separate `git worktree`s; keep the idle main trunk parked OFF `main` (the `block-commit-main` launch-cwd false-block, bead `llna`); re-verify `git branch --show-current` before every commit; **restore `main` at session end** (done). Worktree path corollary: Read a worktree file from its WORKTREE-absolute path before editing it (the harness tracks file-state per path) — this discipline HELD this session (the S72-named friction did not recur).
...
3. **Standing mechanics + carry-overs.** GraphQL EXHAUSTED (0/0; REST core had budget) → REST fallback for PR-create/merge (`gh api`, full-40-char SHA on the merge guard). Git/bd: `git add` separate from `git commit` (PII hook); `.beads/daemon-error` + the bd repo-ID mismatch + the `vault/approaches/` + bpc-157 re-mangle are benign — do NOT run a backfill `bd sync` (discard the churn, hand-apply only your own beads). The global git email is a PLACEHOLDER (`your-github-email@example.com`) — set the worktree email to the GitHub noreply before committing. Two parallel wiki sessions push to `origin/main` from their own worktree — rebase-sync at merge, never touch the research-list / `vault/library/` surface. The author `reconciliation` envelope (compounds: `ae_profile`; bounce: `energy_budget`/`energy_cost_kcal`) is load-bearing. Carry-overs: the new ae-matching-robustness bead (parenthetical-name precision) / `axm7` (gate-extend) / `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x` + PR#145/#147 follow-ups. Pencil: ALWAYS pass `filePath`.
...
- **No PF promoted.** The worktree path-read discipline held (no recurrence). The malformed-`ae_profile` fail-open was tested + documented as deliberate trusted-author behavior (fail-loud deferred to S74).
- **pytest 917/2 (trunk).** Core-capability gate green; negative-test floor green.
- **Beads:** `71s4` (the additive-AE screen is WIRED; the S74 medical-liaison gate + measure/adjust legs remain); the new ae-matching-robustness bead (the parenthetical-name interaction-match precision gap); `axm7` (extend the core-capability gate to the orchestrator); carry-overs `llna`/`0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x` + PR#145/#147 follow-ups.
