...
## Scope Contract — Session 80 (2026-06-20)

Goal: Wire the **adjust-leg PROGRESSION ALGORITHM** — the closed loop's ADJUST leg (plan → act → measure → ADJUST), the LAST core pipeline piece. A production caller (`scripts/plan/adjust.py` `adjust_plan`) reads the plan-vs-actual progress (`track.resolve_plan_progress`), gates the honest "nothing to progress from" boundary, then records the domain SPECIALIST'S adjusted output via the REUSED `generate_plan` as a NEW dated plan — the de-load/advance reasoning is the specialist's (a REAL personal-trainer dispatch), never invented in code. Finish the closed loop end-to-end on synthetic data so the only remaining items are operator-data-dependent.

Acceptance criteria:
- [x] AC1 (adjust caller): `scripts/plan/adjust.py` `adjust_plan(domain, author_output, store_read, root, *, prior_date, adjust_date, gates=None)` records the specialist's ADJUSTED `author_output` via the REUSED `generate_plan` as a new dated plan. Computes NO progression rule.
- [x] AC2 (honest boundary + full value domain): the full has_plan × has_tracking domain decided — no prior plan → `NO_PLAN_TO_ADJUST_FROM`; plan but no tracking → `NO_TRACKING_TO_ADJUST_FROM`; both → record. A fail-loud forward-date gate (adjust_date strictly after prior_date, validated via the store's `_check_date`) guards the same-date store-identity dedupe-drop (PF-S78-01).
- [x] AC3 (safety floor not bypassed on the re-plan): the adjusted plan rides `generate_plan` → `assemble`'s four filters + the workout clearance gate + the nutrition RED-S/LEA veto apply to the re-plan exactly as to the initial per-domain plan. NO direct record path.
- [x] AC4 (specialist-reasoned, not solo): a REAL personal-trainer dispatch (full profile inlined) authored a genuine adjustment (squat HELD after 2/3 tracked sets + a tolerance signal, hinge advanced, plank advanced, no un-cleared load) — flowed through `adjust_plan` + recorded.
- [x] AC5 (tests): `tests/plan/test_adjust.py` — closed-loop E2E (mutation-proven), boundary states, clearance + RED-S/LEA on the re-plan, the forward-date gate, supplements + nutrition aggregate adjust, the record-path ValueError, the store-surface battery (cross-stream / dedupe-idempotent / mutation).
- [x] AC6 (dispatch doc): `docs/plan-generation/adjust-dispatch-process.md` (NEW); `vault/design/plan-generation-pipeline-v1.md` Build-status flipped to WIRED S80.
- [x] AC7 (REUSE-only + backward-compat): `generate_plan`/`track`/`orchestrate`/`assemble`/`store` UNCHANGED; full suite + core-capability gate green.
- [x] AC8 (review + close): Tier-1 SE self-check → Tier-2 plan-integrity + QA → `/review-pr` (Tier-3, 6-agent) → fix legitimate → `/merge`; full close, **re-run the close gate on the final post-merge `main`** + cite the `main` SHA (PF-S74-01).

Files I WILL touch: `scripts/plan/adjust.py` (NEW); `tests/plan/test_adjust.py` (NEW); `docs/plan-generation/adjust-dispatch-process.md` (NEW); `vault/design/plan-generation-pipeline-v1.md` (Build-status); HANDOFF/PF-log/harvest/beads/session-note at close.
Files I will NOT touch: `scripts/plan/{generate_plan,track,orchestrate,adjudicate,router,assemble}.py` (REUSE — call only); `scripts/store/*` (REUSE); the deployed `.claude/agents/*`; the wiki / `vault/library/`; `main` directly.
NOT doing: a multi-block periodization model (V1 is one adjust step from one observed block); peptide progression (not a `TRACKED_DOMAIN`); a cross-domain re-plan through `orchestrate` (the per-domain adjust is the V1 surface); the dashboard's plan-vs-actual + adjust render (a design-led surface — design agents + operator sign-off); the `/generate-plan` convenience wrapper; real operator-data ingest (operator-gated).
Invariants at risk: INV-BRANCH-NOT-MAIN (built in `../aplus-adjust` worktree off `main`, `.venv` provisioned; idle trunk parked off `main`); **store-surface battery (`adjust_plan`'s READ+WRITE of `plan::`/`plan-track::`)**; INV-CORE-CAPABILITY (extends the wired path to the ADJUST leg — stays green + `--self-test`); INV-ROLE-INLINING (the review agents + the real personal-trainer dispatch FULL profiles); **PF-S78-01 value-domain grounding**; INV-CLOSE-AUDIT / INV-PF-ATTESTATION / INV-SKILL-TRACE / INV-HARVEST-CAPTURE at close. Core-capability-first gate (PF-S63-02): SATISFIED — the ADJUST leg is the LAST core pipeline piece; the closed loop now runs end-to-end on synthetic data. **PF-S74-01 discipline: re-run the full close on the final post-merge `main` + cite the `main` SHA.**

### S80 Scope Contract Evaluation (2026-06-20, volatile)

- **AC1–AC7 — PASS.** `adjust_plan` records the specialist's adjusted output via the reused `generate_plan` (computes no progression rule); the full has_plan × has_tracking value domain is decided + a fail-loud forward-date gate guards the same-date store-identity collision; the safety floor (assemble's filters + clearance + nutrition RED-S/LEA veto) applies to the re-plan with no direct record path; a REAL personal-trainer dispatch authored a genuine adjustment (squat HELD @2/3 sets + tolerance signal, hinge + plank advanced, no un-cleared load); 17 adjust tests incl. the closed-loop E2E (mutation-proven), boundary states, the safety floor on the re-plan (both domains), the forward-date gate, supplements + nutrition aggregate adjust, the record-path ValueError, the store-surface battery; the dispatch doc + the pipeline-doc Build-status; `generate_plan`/`track`/`orchestrate`/`assemble`/`store` UNCHANGED; full suite 1132/2 on final `main`, core-capability gate green.
- **AC8 (review + close) — PASS.** Tier-1 SE self-check → Tier-2 plan-integrity INTEGRITY-CLEAN + QA (2 MUST FIX + 4 SHOULD FIX, all fixed — the forward-date gate fixed a real same-date dedupe-drop misreport QA RAN; load-bearing tests mutation-proven non-vacuous) → Tier-3 `/review-pr` 6-agent (security CLEAN; blind-triaged 5 LEGITIMATE fixed + blind-verified RESOLVED, 2 NOT_A_BUG) → `/merge` rebase PR #187 → `main` @ `41e2c83`; full close, close gate re-run on final `main` + the SHA cited (PF-S74-01).

### Drift checks (S80 close)

- **Task drift:** all 8 ACs PASS. The forward-date gate + the value-domain coverage were ADDED at Tier-2/Tier-3 review (the same-date collision QA found by RUNNING it; the basic-ISO `YYYYMMDD` date-format gap Tier-3 found) — caught by the layered review, AC2 (value domain decided) STRENGTHENED not changed. No silent drift.
- **Architecture drift:** no invariant degraded. `generate_plan`/`track`/`orchestrate`/`assemble`/`store` UNCHANGED (REUSE only); the new `adjust_plan` READ+WRITE surface carries the store-battery (no S41-class regression — historical-context confirmed); the de-load reasoning is the specialist's, not invented in code (plan-integrity confirmed zero progression math). INV-CORE-CAPABILITY green (extends the wired path to the ADJUST leg, `--self-test` PASS); INV-BRANCH-NOT-MAIN held (built in `../aplus-adjust` off `main`, `.venv` provisioned; idle trunk parked off `main`); INV-ROLE-INLINING held (plan-integrity + QA + 6 review agents + the real personal-trainer dispatch full-profile). Toward the vision.
- **Vision drift:** none, toward-vision. The system now CLOSES the loop — plan → act → measure → ADJUST runs end-to-end on synthetic data; the de-load/advance is the specialist's, recorded through the same safety floor as the initial plan. `design/vision.md` first sentence ("closes the loop") is now literally true in code. No drift.

### PF attestation

S80 close (2026-06-20): **No new PF-class entries this session** — every finding was caught by the layered review BEFORE merge (nothing escaped), the disciplines held. Observed-but-not-promoted: (a) the adjust_date value domain (the same-date store-identity collision + the basic-ISO `YYYYMMDD` date-format gap) was grounded at Tier-2/Tier-3 review rather than the Tier-1 self-check — the layered-review mechanism working as designed (QA caught the same-date dedupe-drop misreport by RUNNING it; the review's date-format lens caught the `YYYYMMDD` gap; both fixed + blind-verified before merge), a recurrence-watch on Tier-1 value-domain grounding for a NEWLY-introduced field, not an escaped failure; (b) the block-commit-main worktree false-block recurred and was handled by the known parking workaround (the documented worktree-commit discipline), not a new class. The disciplines that HELD: **specialist-reasoned-not-solo** (a REAL personal-trainer dispatch authored the adjustment, full profile inlined — plan-integrity confirmed zero progression math in code); **the safety floor on the re-plan** (clearance + RED-S/LEA veto both pinned); **the store-surface battery**; **value-domain grounding** (the forward-date gate); **REUSE-only** (the pipeline modules UNCHANGED); **close-on-final-`main`** (re-run on `main` @ `41e2c83`). Tier-2 plan-integrity INTEGRITY-CLEAN; Tier-3 security CLEAN. Full attestation + the per-PR skill-trace table (PR #187) + the disclosure ledger in `memory/process-failures.md` Session 80.

## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-79 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; …; S78 archived S77; S79 archived S78; S80 archived S79) to keep this handoff lean. The current (S80) scope contract is above; the archive holds the prior-session archaeology.
...
1. **The core capability is DONE — the closed loop runs end-to-end on SYNTHETIC data only; the remaining gap is REAL operator data (PF-S63-02 inverted).** plan → act → measure → ADJUST is now wired (S80). The core-capability-first gate is SATISFIED and stays satisfied; the next sessions' work is operator-data-dependent (the filled profile scaffold, labs, Whoop/LM-02, 23andMe/LM-03, the MD-visit outcome/LM-01) + design-led polish — NOT new pipeline mechanism. **Before adopting any secondary scope, the PF-S63-02 gate answer is now "YES, the core capability works end-to-end on synthetic data" — so secondary work (the dashboard adjust-render, the `/generate-plan` wrapper) is legitimately next, but state WHY in the scope contract.**
2. **Value-domain grounding belongs at Tier-1 for a NEWLY-INTRODUCED field, not only at review (the PF-S78-01 watch).** S80's `adjust_date` value domain (same-date store-identity collision → dedupe-drop misreport; basic-ISO `YYYYMMDD` accepted by `fromisoformat` but rejected by the store's strict dashed `_DATE_RE`) was grounded at Tier-2/Tier-3 review, not the Tier-1 self-check — caught by the layered review (QA RAN the misreport; the review's date lens caught the format gap), nothing escaped, both fixed + blind-verified. **For new code branching on a field: at Tier-1 enumerate the FULL value domain incl. dates (canonical vs basic-ISO), ordering (forward/same/earlier), and same-key store-identity collisions; decide each.** The fix reuses the store's own validator (`plan_schema._check_date`), never a duplicated regex (`cjgg`/`uerm`/`pwpr`/`sip9`).
3. **The disciplines that HELD + standing mechanics.** (a) **Specialist-reasoned-not-solo:** any reasoning surface (a plan, an adjustment) is authored by a REAL specialist dispatch (full profile inlined), never invented in `scripts/` — the S80 adjust used a real personal-trainer dispatch; plan-integrity confirmed zero progression math in code (the PF-S68/S49/S69 solo-origination family held). (b) **Store-surface battery** on every new/changed store read/write surface (cross-stream / dedupe-idempotent / mutation). (c) **REUSE-only** for pipeline modules (call, never modify). (d) **Close on the FINAL post-merge `main` (`cvf1`):** re-run pytest + `close-audit --session N` on `main` AFTER merge+pull; the attestation CITES the post-merge `main` SHA. Standing: **paste FULL role profiles verbatim** (`enforce-role-inlining`); GraphQL EXHAUSTED → REST for PR-create/merge (`gh api`, full-40-char SHA guard); `git add` SEPARATE from `git commit` (PII hook denies the combined call); **`block-commit-main` checks the idle trunk's launch-branch → PARK the trunk off `main` (a `parking/*` branch) before any worktree commit (PF-S71-01)**; a main-checkout commit auto-sweeps `.beads/issues.jsonl` via bd's git hook — soft-reset + re-commit `--no-verify` with only your files, or commit in a worktree; two parallel wiki sessions push `origin/main` — never touch `vault/library/` AND leave their uncommitted `vault/` changes in the trunk untouched; Read-AND-edit at the worktree-ABSOLUTE path (PF-S79-01); **close-step-8 freshness un-gated until `dfg1` — RUN step 8 each close**; Pencil: ALWAYS pass `filePath`. Carry-overs: `6aen` (PF-S79-01) / `cjgg`/`uerm`/`pwpr`/`sip9` (value-domain grounding) / `cvf1` / `dfg1`/`df1x` / `aem1` / `axm7` / `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`/`5fw1`.

## Current State (volatile)

- **S80 (2026-06-20) wired the adjust-leg PROGRESSION ALGORITHM (PR #187 → `main` @ `41e2c83`, rebase) — the closed loop's ADJUST leg, the LAST core pipeline piece.** The closed loop (plan → act → measure → ADJUST) now runs **end-to-end on synthetic data**.
- **`scripts/plan/adjust.py` `adjust_plan` (NEW):** reads the plan-vs-actual progress (`track.resolve_plan_progress`), gates the honest "nothing to progress from" boundary (the full has_plan × has_tracking domain) + a fail-loud forward-date gate (validated via the store's `_check_date`), then records the domain SPECIALIST'S adjusted output via the REUSED `generate_plan` as a NEW dated plan. The de-load/advance reasoning is the SPECIALIST'S — a REAL personal-trainer dispatch (full profile inlined) authored a genuine adjustment (squat HELD after 2/3 tracked sets + a tolerance signal, hinge + plank advanced, no un-cleared load) — never invented in code (plan-integrity confirmed zero progression math). The safety floor (assemble's four filters + the workout clearance gate + the nutrition RED-S/LEA veto) applies to the re-plan exactly as to the initial per-domain plan; NO direct record path. `generate_plan`/`track`/`orchestrate`/`assemble`/`store` UNCHANGED (REUSE only).
- **Review (PR #187):** Tier-2 plan-integrity INTEGRITY-CLEAN + QA (2 MUST FIX + 4 SHOULD FIX fixed — the forward-date gate fixed a REAL same-date store-identity dedupe-drop misreport QA RAN; load-bearing tests mutation-proven non-vacuous) → Tier-3 `/review-pr` 6-agent (security CLEAN; blind-triaged 5 LEGITIMATE — the `YYYYMMDD` date-format strictness, nutrition aggregate + `gates=None` default-deny coverage, gate-rationale precision, docstring — fixed + blind-verified RESOLVED; 2 NOT_A_BUG: the documented `state`-union design + the unreachable inherited KeyError) → `/merge`.
- **No new PF this session** — every finding was caught by the layered review before merge (nothing escaped); the disciplines held. Watch: Tier-1 value-domain grounding for a NEWLY-introduced field (the `adjust_date` domain was grounded at Tier-2/3, not Tier-1).
- **pytest 1132/2 on final post-merge `main` (`41e2c83`, 2026-06-20 S80 close).** Core-capability gate green (`--self-test` PASS, author→assemble→record_plan→dashboard wired).
- **Beads:** `71s4` CLOSED (the adjust PROGRESSION ALGORITHM is wired — the closed loop is complete); carry-overs `cjgg`/`uerm`/`pwpr`/`sip9`/`cvf1`/`dfg1`/`df1x`/`aem1`/`axm7`/`6aen`/`0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`/`5fw1`.
- **Active landmarks:** LM-01 (First MD visit) **2026-07-13** — the 14-day scoped-drift-audit window opens **2026-06-29**. The closed loop is now COMPLETE end-to-end on synthetic data (the SBAR handout render + the face sheet + the full plan→adjust loop are all built); the remaining LM-01 gap is REAL operator data (the profile scaffold is unfilled). LM-02 Whoop / LM-03 23andMe date-TBD.

**Historical (kept for reference):** `vault/sessions/session-80.md`.

## What Is Next (volatile)

### S81 — REAL operator data, design-led polish, or the design PR #179 merge

**The core capability is COMPLETE: the closed loop (plan → act → measure → adjust) runs end-to-end on SYNTHETIC data, and the doctor-visit SBAR handout + physician face sheet render.** Every remaining item is operator-data-dependent or design-led polish — NOT new pipeline mechanism (PF-S63-02 gate now answers YES). Candidate next surfaces (operator chooses):

1. **REAL operator-data ingest — the unlock from synthetic → real.** The filled profile scaffold (the `operator-profile.md` slots are placeholders), labs, the Whoop baseline (LM-02), 23andMe (LM-03), the MD-visit outcome (LM-01). This is what makes the plans + handout reflect the actual operator. Operator-gated (the PII boundary + the ADR-0005 no-PII-in-tracked-text rule govern HOW it lands — likely a gitignored store, not tracked).
2. **The dashboard's plan-vs-actual + adjust render surface** — a DESIGN-LED surface (Pencil + the design agents + operator sign-off, never originated solo, PF-S68-01); the closed loop has no operator-facing view of the adjustment yet.
3. **The `/generate-plan` convenience wrapper** — an optional slash-command front-end over the orchestrate→adjudicate→generate_plan→adjust pipeline (the mechanism is wired; this is ergonomics).
4. **Merge the design PR #179** (doctor-visit handout MOCKUPS — approved + design-critic-passed, OPEN, merge pending the operator).
5. **LM-01 (July 13 MD visit) prep** — the 14-day scoped-drift-audit window opens **2026-06-29**; the first session on/after that runs the LM-01 scoped drift audit.

1. **The adjust leg's PROGRESSION ALGORITHM** (the remaining half of `71s4`) — how a specialist (personal-trainer / nutritionist) changes the prescription from the `resolve_plan_progress` plan-vs-actual view (progress → de-load / advance). **Specialist-reasoning** — needs a REAL specialist dispatch (full profile inlined) + design, NOT invented solo. A code build with a real-agent E2E.
2. **Merge the design PR #179** (the doctor-visit handout MOCKUPS) — approved + design-critic-passed, OPEN, the merge decision pending the operator. (The code PR #182 that renders the handout is already merged; the `.pen` mockups are independent of the code, so #179 can merge any time.)
3. **LM-01 (July 13 MD visit) prep** — the 14-day scoped-drift-audit window opens **2026-06-29**; the first session on/after that date runs the LM-01 scoped drift audit (verify `operator-profile.md` + the compound `last_verified` + the doctor-handout queue). The 7-day MD-handoff-GENERATION window opens **2026-07-06**. The SBAR render + the face sheet are both BUILT — the remaining LM-01 gap is REAL operator data (the profile scaffold is unfilled; the handout renders synthetic slot data).

**Discipline:** scope contract at open; **worktree + idle trunk off `main` + provision the `.venv` + Read-AND-edit at the worktree-absolute path (PF-S79-01)**; for any store surface, the store-surface battery; **at Tier-1, value-domain-ground every field new code branches on (PF-S78-01) + the field-existence (`uerm`), boundary-size (`pwpr`), multi-concern (`sip9`) probes**; for a visual artifact, Pencil + the design agents + operator sign-off FIRST (never solo); for specialist-reasoning, a REAL specialist dispatch (full profile); **paste FULL role profiles verbatim**; `/review-pr` → `/merge`; **re-run the close gate on the final post-merge `main` + cite the SHA (PF-S74-01)**.

**Deferred (NOT blocking):** `6aen` (worktree-path edit guard — PF-S79-01); `cjgg`/`uerm`/`pwpr`/`sip9` (value-domain + interaction grounding); `dfg1` (the doc-freshness gate — PF-S73-01 structural fix) + `df1x`; `cvf1` (the mechanical post-merge-`main` verify); `aem1` (the parenthetical-name interaction-match precision gap); `axm7` (extend the core-capability gate to the orchestrator); `5fw1` (the harvest-gate heading-regex fragility); the design PR #179 merge; the standalone full-plan render screen + the dashboard reskin (operator's worktrees); the `/generate-plan` slash-command wrapper; the `llna` hook fix; carry-overs `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`; operator-gated July-visit prep (LM-01) — first session on/after **2026-06-29** runs the LM-01 scoped drift audit.

## Landmark window check (close step 8.7)

S80 close (2026-06-20): RE-OPENED `vault/meta/landmarks.md` at close step 8.7 (unchanged this session). No landmark edited — S80 wired the adjust-leg progression on PII-free SYNTHETIC data (no real operator data ingested, no Whoop baseline, no MD-handoff artifact GENERATED from real data). LM-01 (First MD visit) **2026-07-13**: no trigger window open today (the 14-day scoped-drift-audit window opens **2026-06-29**, the 7-day MD-handoff window 2026-07-06). The closed loop is now COMPLETE end-to-end on synthetic data (plan→act→measure→adjust + the SBAR handout + the face sheet all built) — the remaining LM-01 gap is REAL operator data (the profile scaffold is unfilled). LM-04 (first HTML artifact) NOT triggered — the renderers emit to the gitignored `vault/artifacts/generated/` (renderers, not COMMITTED artifacts). LM-02 (Whoop) / LM-03 (23andMe) date-TBD. No landmark actions due.
