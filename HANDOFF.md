...
Scope contracts for Sessions 5-62 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; S33 archived S32; …; S60 archived S59; S61 archived S60; S62 archived S61; S63 archived S62) to keep this handoff lean. The current (S63) scope contract is above; the archive holds the prior-session archaeology.
...
1. **Gated-skill discipline — invoke `/review-pr` AND `/merge` fresh via the Skill tool for EVERY PR (S63 HELD the line).** PR #136 (the WHOOP adapter) got BOTH gated skills fresh via the Skill tool (6-agent `/review-pr` → blind triage → blind verify; `merge-methodology.md` read fresh; REST rebase under GraphQL throttle, full-40-char-SHA guard); the close PR likewise. The rule stands: every PR gets both gated skills fresh via the Skill tool; a right-size needs explicit operator sign-off + disclosure, never a unilateral skip. `scripts/skill-trace-audit.sh --session <N>` is the 5th mandatory close audit; every NO cell carries the word "violation".
2. **Forward work: the WHOOP adapter's real-data tail + the remaining S61 plans + the correctness/governance tail.** (a) **WHOOP/noop adapter** (`mdzq`) — **BUILT + WIRED at S63 (#136)**: read-only `sqlite3` read of noop's `whoop.sqlite` `dailyMetric` (mechanism resolved OQ-2/OQ-3 = sqlite, not MCP), replaces the fabricated scaffold. **Open tail: end-to-end validation against the operator's REAL `whoop.sqlite`** — gated on the operator installing noop, **bonding the strap to noop**, and offloading a data sample. Plus 2 review-beaded follow-ups: `crgz` (doc-freshness — supersede the stale "Whoop registered-but-unwired" gates in spec/build-plan/T2-T3 recipes) + `ienx` (multi-device fail-loud hardening). (b) **wiki-population research plan** (`docs/research-plan/`) — a PARALLEL session; gated on its §8 D3 (minimum context-file fill). (c) **local-model eval plan** (`docs/model-eval/`) — gated on operator inputs D2/D3/D5; result is an ADR superseding ADR-0001 (now carries §3.5 noop-AI-Coach integration prior art). The correctness/governance tail stands: `02pe`, `dqyv`, `ofn0` + P3s (`pq7m`/`a94f`/`dt0t`/`rn3v`/`imev`/`tdre`/`20d`/`vvs`); `d3w` left to the external rigor-framework project (Walter S60); `10h`/`1ww`/`e3b` deferred-by-design (re-read their `bd show` notes). Locked: `PALETTE`/`SERIES`/`ACCENTS` (ADR-0009), `SUMMARY_FIELD_SET`/`EXCLUDED_RAW_PII` (Security MEDIUM-2). Standing mechanics: REST `gh api` full-40-char SHA for PR ops (GraphQL throttles ~hourly + low core quota — budget the calls, never poll-loop); stage `git add` separately from `git commit`; `.claude/hooks/*` edits need per-session authorization.
3. **Two process watches.** (a) **Off-main-archive-SHA (count=2):** the repo rebase-merges, so the `scope-contract-archive.md` close entry cites ON-MAIN commits (`git merge-base --is-ancestor`-verified), NOT pre-rebase `fix/*` tips; HELD at S63 (the S62 contract archived at its on-main #133-close commit, recorded in the archive + PF log); `ofn0` mechanizes it — build at N=3. (b) **Cross-cutting registry/enum edits have a test blast radius (S63 observation):** adding `biomarker_meta` rows for the new wearable markers broke 5 dashboard/trend tests that use those markers as *unregistered* exemplars — caught by running the suite, reverted + beaded. Before a flagged-optional registry/enum expansion, grep for tests that depend on the marker's *current* (un)registered state. (c) **Close-step-8/8.7 DocRubric+landmark-from-memory (S57/S59):** did NOT recur at S60-S63 (all re-opened from the files); `d3w` mechanizes it. **PF-S51-01** (review agents read-only) HELD — the #136 review/triage/verify panels were read-only, tree pristine.

## Current State (volatile)

- **S63 (2026-06-14) ran one full PR lifecycle (#136) + this close PR, merged to `main` (at `9cfbfef` as of 2026-06-14 S63 close); suite 833 passed / 2 skipped.** Walter: "build the whoop. also, update the model-eval plan to consider noop's local AI Coach approach."
- **The WHOOP/noop adapter is BUILT + WIRED (#136), replacing the fabricated scaffold.** `scripts/ingest/adapters/whoop.py` now reads noop's `whoop.sqlite` read-only (`sqlite3` `mode=ro` via `Path.as_uri()`) and maps the `dailyMetric` table → `source: whoop` store readings (recovery / strain[0–21] / hrv / rhr / sleep-efficiency / spo2 / resp-rate / skin-temp-dev), NULLs skipped. **Mechanism (OQ-2/OQ-3) resolved at build = read-only sqlite, NOT the MCP subprocess** (the file-based `read_readings(export_file)` contract fits with 0 edits to the ADR-0003 shared routine/scheduler; MCP is the documented fallback). Wired by removing the `UNWIRED` marker — `scheduler._wired_adapters()` on main now returns `['garmin','healthkit','oura','whoop']`. **Store readings carry `source: whoop`** (device provenance, dedupe-distinct); the biomarker-page `source: wearable` enum (ADR-0011 D4) is the separate page layer.
- **The #136 6-agent `/review-pr` was load-bearing.** It caught a real read-only-bypass (the URI `mode=ro` defeated by a `?`/`#` in the path — a `#` opened the WRONG file; fixed via `as_uri()`) and two mutation-survive-green test holes (TEST-1: 6 of 8 column→item mappings pinned by presence not value → a transposed mapping ships a fabricated value; TEST-2: the `is None` NULL-skip untested → `not value` silently drops honest zeros) — all 7 LEGITIMATE fixed + blind-verified 7/7, both new guards mutation-proven.
- **`docs/model-eval/local-model-evaluation-plan.md` updated** (§3.5 + §4b/§8): noop's AI Coach as prior art for the local-inference integration seam — a BYO-provider protocol + an OpenAI-compatible local endpoint (Ollama/LM-Studio à la `:11434/v1`) + consent-gated compact-summary context over the same on-device data the adapter ingests; license-honest (reference, not vendor — PolyForm).
- **Beads:** created `crgz` (doc-freshness — supersede stale whoop-unwired gates in spec/build-plan/recipes) + `ienx` (multi-device fail-loud hardening); closed `1uav` (overtaken-by-events — its `test_whoop_not_invoked` target was replaced). **`mdzq` stays OPEN** — the build + fixture tests + adversarial battery merged, but the bead's real-data E2E validation tail remains (gated on the operator bonding the strap to noop + a sample). Correctness/governance tail still open: `02pe`/`dqyv`/`ofn0` + P3s; `d3w` external; `10h`/`1ww`/`e3b` deferred-by-design.
- **Active landmarks:** LM-01 (First MD visit) **2026-07-13**. No trigger window open yet — the **14-day scoped-drift-audit window opens 2026-06-29**, the 7-day MD-handoff window 2026-07-06. LM-02 is the **Whoop** baseline (date-TBD — the adapter build does NOT start the baseline; that needs real strap data offloaded into noop).

**Historical (kept for reference):** `vault/sessions/session-63.md`.

## What Is Next (volatile)

### Resume checklist — next session (S64) open

No owed close PR (the S63 close PR merges this session). Open normally (Session Start Protocol; `branch-completeness-audit.sh` at OPEN; baseline on `main` **833 passed / 2 skipped**).

**S64 forward options (operator prioritizes at open, NOT pre-adopted):**

1. **WHOOP adapter — real-data E2E validation (`mdzq` tail).** GATED on the operator: install noop on the M2 Studio, **bond the strap to noop** (takes the BLE bond from the official WHOOP app), let it offload a baseline, then point the adapter at the real `~/Library/Application Support/OpenWhoop/whoop.sqlite` and validate end-to-end (the fixtures were documented-schema synthetic). This also unblocks the LM-02 Whoop baseline. Until the operator does this, the adapter is built+tested but unvalidated against real data.
2. **The two review-beaded follow-ups (P3).** `crgz` — Document-Freshness supersession of the stale "Whoop registered-but-unwired" gates in `docs/spec/adr-0001-adr-0003-spec.md` + `docs/build-plan/build-plan-v1-full.md` (the Wave-4 checkpoint is now a vacuously-passing gate) + the T2/T3 recipes. `ienx` — multi-device fail-loud hardening for the adapter.
3. **The remaining S61 plans.** (a) **wiki-population research** — a PARALLEL session; resolve its §8 D3 (minimum context-file fill so gate 2.75 doesn't HALT). (b) **local-model eval** — gated on operator inputs D2/D3/D5; result is an ADR superseding ADR-0001 (now carries §3.5 noop-AI-Coach integration prior art).
4. **The correctness/governance tail.** `02pe`/`dqyv`/`ofn0` + P3s; `d3w` external (Walter S60); `10h`/`1ww`/`e3b` deferred-by-design.
5. **Operator-gated.** July-visit prep (LM-01 / `c6k`) — date-fixed (2026-07-13; 14-day window opens **2026-06-29**, the first session on/after runs the LM-01 scoped drift audit).

## Landmark window check (close step 8.7)

S63 close (2026-06-14): RE-OPENED `vault/meta/landmarks.md` from the file at close step 8.7 (the S57/S59 from-memory miss did NOT recur — S60-S63 all re-opened). No landmark edited this session — the WHOOP adapter build does NOT start the LM-02 Whoop baseline (that needs real strap data offloaded into noop, an operator step). LM-01 (First MD visit) date **2026-07-13**. No `active` landmark's trigger window is open as of today 2026-06-14 — LM-01's 14-day scoped-drift-audit window opens **2026-06-29** (the 7-day MD-handoff window 2026-07-06); LM-02 Whoop baseline date-TBD pending the strap-data start; LM-03 date-TBD; LM-04 (first HTML artifact) not triggered (S63 shipped an ingestion adapter + docs, no operator-facing artifact archived to `vault/artifacts/`). No landmark actions due THIS session; the first session on/after 2026-06-29 runs the LM-01 scoped drift audit.
