...
Scope contracts for Sessions 5-40 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; S33 archived S32; S34 archived S33; S35 archived S34; S36 archived S35; S37 archived S36; S38 archived S37; S39 archived S38; S40 archived S39; S41 archived S40) to keep this handoff lean. The current (S41) scope contract is above; the archive holds the prior-session archaeology.
...
7. Read the most recent session note in `vault/sessions/` — currently `session-41.md`
...
1. **Wave 7 via `/execute-plan` WAVE mode is the next action (PF-S36-01) — STANDING.** S42 = build-plan **Wave 7** (`1ih`/ADR-0007-T2 — biomarker-matrix + projection render-time views; the LAST leaf → V1 build 18/18). Build via `/execute-plan` in WAVE mode against `docs/build-plan/build-plan-v1-full.md` — complete the wave IN ORDER, gate on the W7→Done checkpoint Go/No-Go (incl. the SEC-03 failing-capable egress injection into a `render_views` recompute), never hand-roll a per-task substitute. The `v1-build` contract cites W7 + attests the W6→W7 checkpoint passed. **Read `/execute-plan`'s SKILL.md + references IN FULL fresh first** (project path-mapping: recipes `docs/task-plan/`, build plan `docs/build-plan/`, roles `skills_library/roles/` inlined full).
2. **read-before-invoke AND run-the-FULL-methodology (PF-S13-01 + PF-S17-01 + PF-S39-01 + PF-S40-01) — STANDING.** Two halves: (a) read each gated skill fresh / invoke via the Skill tool (not from cached context); (b) run ALL of its phases — for `/review-pr`, Phase 3 ("**DO NOT triage findings yourself**" → dispatch the blind triage agent) and Phase 7 (blind verification) are non-optional independence layers, dispatched as profile-less agents, NOT to be improvised. **S41 ran the full methodology — PF-S40-01 HELD** (the blind triage scoped out 4 findings a self-triage might have mis-handled). No `Workflow`/hand-roll substitution. `.venv/bin/python -m pytest`, never from memory.
3. **Three-tier review is load-bearing — safety/PII surfaces hide in builder-blessed paths (PF-S26-01/PF-S3-01).** S41's Wave-6 build passed all builder tests AND all of Tier-2 (QA+Architect+Security) while a **cross-stream store namespace collision** (`read_panel` returned a fabricated biomarker value) + a **same-timepoint carry-forward dedupe-drop** (a dropped contraindication answer — a safety surface) hid in the loop-schema — caught only by Tier-3. This is the **FIFTH consecutive wave** a safety/PII surface hid behind green builder tests + survived Tier-2, caught by the layered review. For W7 (`1ih` biomarker views — store-reading / PII-adjacent), the full Tier-2 + Tier-3 + dispatched blind triage + blind verify is non-optional; bead `pka` tracks shifting the catch upstream (mandate adversarial cross-stream/dedupe tests at build+Tier-2); recurrence-watch for one that ESCAPES all tiers (none has).

**Demoted from prior Top-3:** the "Wave 6 run" framing — Wave 6 built + merged at S41 (PR #71); the forward risk is now the Wave 7 run.

## Current State (volatile)

- **S41 (2026-06-07) built Wave 6 via `/execute-plan` WAVE mode (the single open task).** `1aa`/ADR-0007-T1 — lab-loop / watch-out / physician-feedback store schemas (`scripts/store/loop_schema.py`): the published 4+1-state contract (`pending`/`not-yet-answered`/`no-prior`/`answered-over-time` + the Tier-3-added `no-data`) that W7's render views read 1:1. PF-S36-01 + PF-S39-01 + PF-S40-01 falsification windows HELD (sanctioned skills invoked fresh via the Skill tool with references read fresh; wave in order; W6→W7 checkpoint gated; the FULL `/review-pr` methodology incl. dispatched blind triage + blind verify ran).
- **The three-tier review caught + fixed defects the builder's tests + Tier-2 both missed** (0 suppressed, PF-S26-01): Tier-2 (QA+Architect+Security all PASS) surfaced 2 non-blocking (value-pin FIXED; ≥2-timepoint sentinel beaded for W7); Tier-3 `/review-pr` #71 (6-agent) caught a **cross-stream namespace collision** (`read_panel` returned a fabricated biomarker value) + a **same-timepoint carry-forward dedupe-drop** (a safety surface) + a 0-timepoint `None`-overload + weak AC-5 assertions → blind triage **9 LEGITIMATE / 3 OUT_OF_SCOPE / 1 NOT_ACTIONABLE / 1 NOT_A_BUG / 1 by-design**; 9 fixed + blind-verified **9/9 RESOLVED**. PR #71 rebase-merged via REST (GraphQL throttled).
- **Build: 17/18 leaves.** Wave 6 COMPLETE (`1aa`); **Wave 7** next (`1ih`/ADR-0007-T2) → V1 18/18. Live per-wave state in `vault/meta/overview.md`.
- **SINGLE TRUNK** unchanged (since S22). Suite **247 passed / 2 skipped** (+20 over the S40 227 baseline: +10 build + +10 review-fix tests; the 2 skips are pre-existing Linux-only egress-namespace tests); branch-completeness 0 violations (20 agents); all hook suites green. `main` at the Wave-6 merge as of 2026-06-07 S41 close + this S41 close PR.
- **Active landmarks:** no trigger windows opened. LM-04 still pending (the `generate.run` producer is live but unfed).

**Historical (kept for reference):** `vault/sessions/session-41.md` + `vault/meta/log.md` S41 entry.

## What Is Next (volatile)

### Resume checklist — next session (S42) open

**Clean slate — nothing dangling from S41.** `main` is at the Wave-6 merge + this close PR, tree clean, 0 open PRs. The next session opens normally:

1. Run the Session Start Protocol (steps 1-6) + `branch-completeness-audit.sh` at OPEN. Baseline suite = 247 passed / 2 skipped.
2. **S42 is Phase C — Wave 7 (the LAST leaf) via `/execute-plan` WAVE mode.** Cut a `feature/`-branch off `main`. Write + confirm a **`v1-build` scope contract** citing the build-plan wave (**W7**) AND attesting the W6→W7 checkpoint passed (run it: `.venv/bin/python -m pytest tests/store/test_loop_schema.py` + the cross-spec store-API-reuse + the `rg` 0-automation floor).
3. **Read `/execute-plan`'s SKILL.md + references IN FULL fresh first** (PF-S17-01). **AND when Tier-3 `/review-pr` runs, dispatch its Phase-3 blind triage (do NOT triage yourself) + Phase-7 blind verification as profile-less agents (PF-S40-01 — S41 HELD; keep it HELD).** Project path-mapping: recipes=`docs/task-plan/<task>.md`, build plan=`docs/build-plan/build-plan-v1-full.md`, roles=`~/Documents/Projects/skills_library/roles/<role>/agent.md` (inlined in full). Run via `/execute-plan` in WAVE mode — never a hand-rolled per-task SE dispatch (PF-S36-01).
4. **Build Wave 7** — `1ih` (ADR-0007-T2: biomarker-matrix + projection render-time views — a ≥2-timepoint biomarker renders side-by-side in the matrix; a projection renders only at ≥3 timepoints and is trend-only at 2, carrying the "naive projection" label + method + count + widening band + milestones; **pending / not-yet-answered map 1:1 from the ADR-0007-T1 store state — the cross-boundary integration check, and decide the W7 consumption questions below**; recompute local-only, 0 external requests, 0 lab/answer to model; a worst-case view paginates so each artifact `wc -c` < 500000). Then run the **W7→Done checkpoint AS A GATE** (incl. the SEC-03 failing-capable egress injection into a `render_views` recompute) → wave PR → `/review-pr` (FULL methodology) → `/merge`.
5. **After W7 the V1 build is 18/18** (LM-04 still gated on real operator data). Then the post-build-waves residual-bead group + the library-population `/aplus-research` track.

**W7 consumption-review decisions (from S41 Tier-3, surface at the `1ih` build):** `1ih` reads ADR-0007-T1's published store-state contract — including the **≥2-timepoint biomarker `None` sentinel** (bead `a-plus-maxing-bhc`/the W7 None-sentinel flag: decide whether the ≥2-timepoint state needs a named marker = a contract amendment per the change-control clause) and the new **`no-data` 0-timepoint marker**. The cross-boundary 1:1 mapping (ADR-0007-T2 crit 4) is the integration test that closes the store-schema↔render-view contract.

### Follow-up bead sequencing

New from S41 (Wave-6 review #71): **`s38` (P2)** — panel **result-writer** (order-independent pending→result resolution + a distinct result source-tag so the result can't dedupe-collide with the pending marker + a result-lands test); a downstream task, build when the operator-result-entry path lands. **`r5l` (P3)** — `derive_watchout_questions(None)` caller-contract decision (valid "no protocols" input vs caller violation; no defensive code without approval). **`pka` (P2)** — strengthen the builder/Tier-2 adversarial-coverage mandate for store cross-stream + dedupe-collision safety cases (the 5-consecutive-wave recurrence-watch — shift the catch upstream of Tier-3).

Carried residuals (all LM-04-gated unless noted): **`8j6` (P1)** in-summary free-text PII value-gate (closed-vocab store schema vs `assemble` PII-shape gate — decide before first real operator data); **`10h` (P2)** mislabeled-category HALT integrity; **`20d` (P2, DEFENSIVE — needs Walter approval)** adapter import isolation; **`7lt` (P2)** typed wired-set marker; **`e3b` (P2)** clone-root binding at the future `router.summarize` production call site (`assemble` stays rootless); **`juc` (P2)** trend-polarity metadata in `keying.py`; the **PII-guard-activation group** `3lv`/`2x1`/`rnm`/`fga`/`fmb`/`v13` + cross-hook matcher (before LM-04 / at V1-distribution).
...
**Published surfaces the next wave consumes:** `generate.run(artifact_name) -> Path` · `render.emit(template, store_read) -> Path` (no production PLAN-template producer yet — bead `4xe`) · the `Adapter` Protocol + 4 adapters · `scheduler.run()` · `ingest.run(adapter, export_file, root=…)` · `store.append`/`read` (cross-item `read_all`/`items` pending `4yk`) · `keying.py` (never redefine) · `egress_guard.run` · `pii_scan.scan(...)` · `router.summarize(store_read) -> dict` + `router.dispatch(summary)` · `assemble(goal_set, summary, roster) -> plan` · **`loop_schema` — `record_pending_panel`/`read_panel`, `record_watchout_answer`/`read_watchout`/`read_watchout_answers`, `record_biomarker`/`read_biomarker`, `record_physician_feedback`/`read_physician_feedback`, `derive_watchout_questions`; published store-state markers `pending`/`not-yet-answered`/`no-prior`/`answered-over-time`/`no-data` (stream-isolated item-id namespacing internal) — `1ih`/W7 reads these 1:1.**
...
### Open beads carried
- **V1 build execution (label `v1-build`):** **17 closed** (W1 `394`/`bez`/`qbb`; W2 `89a`/`e9m`; W3 `6be`/`gu4`/`xlu`/`br1`; W4 `n9h`/`3gp`/`yo6`/`ml1`/`ftm`; W5 `oaf`/`8cv`; W6 `1aa`). OPEN: W7 `1ih` (the last leaf). Plus `e3d` (recipe doc-consistency, non-blocking).
- **PR review beads open — S41 (Wave-6 review #71):** `s38` P2 (panel result-writer), `r5l` P3 (derive(None) caller-contract), `pka` P2 (builder/Tier-2 adversarial-coverage), `a-plus-maxing-bhc`/W7 None-sentinel flag. **S40 (#69):** `8j6` P1, `10h` P2, `20d` P2 (DEFENSIVE), `7lt` P2. **S39 (#66):** `e3b` P2, `juc` P2, `fmb` P3, `v13` P3, `fga` P3. **S38 (#64):** `3lv` P2, `rnm` P2, `2x1` P3, cross-hook matcher P3. **Earlier:** `1ww`/`ivt`/`z2u` (#44), `1vi` (#47), `4yk` P2 + `u8u` P3 (#55), T5/T9 (S37), `vvs` P2 (S36).
