...
1. **ADR-0031 Local-Extraction-First PDF Ingestion + Genetics is COMPLETE + MERGED — the operator's real DNA report can now be ingested; the ONLY remaining step is the operator-present LIVE run.** main @ `c781e8e8` (2026-06-28 S100 close): the live DNA-PDF failure was root-caused (array-rooted structured-output schema + max_tokens truncation + genetics density) → ADR-0031 (accepted; amends ADR-0030) → spec (6 tasks) → build-plan (3 waves) → 6 recipes → 3-wave build → Tier-2 reviews → Tier-3 6-agent `/review-pr` (2 LEGITIMATE → fixed → blind-verified RESOLVED; 2 DEFERRED-beaded) → rebase merge (PR #268). A PDF is extracted LOCALLY (pdftotext/marker subprocess, zero network) and ONLY the extracted TEXT (chunked, never truncating) reaches the no-train lane; genetics is captured as DURABLE GENOTYPE FACTS (item=gene+rsID, value=alleles) so current science re-interprets the 2019 report. Suite 1906/7 + 2 env; frozen-engine numstat=0; core-cap audit exit 0; crown-jewel raw-binary-stays-local HELD by execution. **REFRAME: ingestion now handles the operator's real genetics PDF; the operator-present LIVE run (real key + real data + spend, ADR-0031 OQ-1) is the ONLY remaining step to exercise it.**
2. **Before / as part of the operator-present LIVE run: the standing live-run gates + the NEW ADR-0031 follow-ups.** Standing: `6hts` (de-id value-scan residuals) + `stsq` (SEC-3 live-dispatch 0-leak) + the live `author` leg (`_ClaudeNoTrainBackend.author` still `NotImplementedError`, ADR-0015-T3) + `by30` (F4 /upload CSRF DECISION). NEW from the Tier-3 review: **`0ai1` (P3): extract_chunked silently drops a non-conformant reading at the swappable ADR-0015 client seam** — not production-reachable (ModelClient validates conformance upstream + raises first), fail-closed-hardening for a future non-validating backend. **`tjz9` (P3): pdf_extract subprocess decode is locale-strict** (UnicodeDecodeError on µ/°/± medical chars under an explicitly ASCII-forcing locale → silent degrade) — unreachable on the operator's macOS default; trivial fix (`encoding="utf-8", errors="replace"`). Carry `36as` (W1 Security LOW: subprocess abspath/timeout).
3. **Standing mechanics (all HELD) + the recurred PF this session.** Full pipeline build never hand-rolled (PF-S36-01, each stage in full); FULL gated `/review-pr` for multi-file/security code (PF-S96-01 — APPLIED on #268, caught the hard-split integration defect); 3-tier review with profile-less blind-triage (corrected the Bug-Hunter's literal repro) + EXECUTED blind-verify (reproduced the fix by running it); EXTEND-NOT-REBUILD numstat=0; full role profiles inlined VERBATIM on EVERY role dispatch; review/verify agents READ-ONLY + scratch-`/tmp` mutations only (PF-S97-01/PF-S98-01) + never touched port 8765; `OBJC_DISABLE_INITIALIZE_FORK_SAFETY=YES` on EVERY pytest; crown-jewel raw-binary-local-only HELD by execution; `/merge` via REST when GraphQL exhausted (full-40-char SHA guard); `git add` SEPARATE from `git commit`; single trunk; stage close docs by EXPLICIT path; re-run close gate on final main + cite SHA (PF-S74-01). **RECURRED PF-S92-01 (#5, `0ls9`): over-stopped mid-loop to ask an Architect-adjudicated/reversible/beaded question (the `%PDF` magic-byte sniff) — caught by the operator; an adjudicated-to-ACCEPT guard with a determined next step is NOT an authorized turn-ender.**

## Current State (volatile)

- **S100 (2026-06-28): root-caused the operator's live DNA-report-PDF upload failure, then ran the ENTIRE build pipeline (create-adr → spec → build-plan → task-plan → execute-plan → review-pr → merge) autonomously to make PDF ingestion LOCAL-EXTRACTION-FIRST (ADR-0031).** Triggered by the operator's real 42-page/107-SNP genetics report failing with "No new data landed" ("getting a read on your DNA is a key feature of this app").
- **Root cause (3 live bugs, foundation-fixed on-branch):** (1) the Anthropic `output_config.format.json_schema` rejects an array-rooted schema → object-rooted (0ee9a5d); (2) `max_tokens=2048` truncated the JSON → 16384 (e7461ab); (3) the genetics report is too dense to fit even 16384 in one structured-output call → the real fix is LOCAL-EXTRACTION-FIRST.
- **ADR-0031 (Local-Extraction-First PDF Ingestion + Genetics):** a PDF is extracted LOCALLY (pdftotext primary / marker fallback, subprocess, 0 network) and ONLY the extracted TEXT — chunked so the structured output never truncates — reaches the no-train lane (`text/plain`, never the raw `document` block). The raw binary NEVER leaves the machine (a privacy TIGHTENING over ADR-0030's send-raw-PDF). Genetics is captured as DURABLE GENOTYPE FACTS (item=gene+rsID, timepoint=sample-date, source=dna-report, value=alleles) so current science re-interprets the 2019 report. Amends ADR-0001/0009/0016/0027/0030; mock/fixture-tested, 0 live spend.
- **The Tier-3 6-agent review caught the integration defect the mock-tested per-wave build missed:** the `_split_chunks` hard-split overlap-drop — a reading straddling a >chunk_chars line's hard-split boundary was lost from BOTH chunks while `extract_all` reported `complete=True` (silent data loss + false-complete, the honest-completeness marquee). Three independent agents (Bug-Hunter / Test-Coverage / regression-history) converged; profile-less blind-triage corrected the literal repro; fixed (overlap-safe hard-split, pieces bounded at `chunk_chars - overlap`) + a regression test (RED on old code, proven) + the route Returns docstring; blind-verified RESOLVED by execution. B (non-conformant drop) + C (locale-strict decode) DEFERRED-beaded.
- **Beads:** new `0ai1` (review-B non-conformant drop), `tjz9` (review-C locale decode), `0ls9` (PF-S92-01 recurrence). Carry: `36as` (W1 Security LOW), `6hts`/`stsq`/`by30` (live-run gates), `2xu4`/`2l69` (S99 PFs), `02m1` (daemon config), and the standing safety set.
- **main @ `c781e8e8` (as of 2026-06-28 S100 close, PF-S74-01). Vault perpetually daemon-dirty (stage by explicit path).**

**Historical (kept for reference):** `vault/sessions/session-99.md`.

## What Is Next (volatile)

### After S100 — the operator-present LIVE run is THE remaining step (now incl. the genetics PDF)

The build (intake + plan engine + format-agnostic ingestion + now local-extraction-first PDF/genetics) is COMPLETE + mock-proven. To produce the operator's ACTUAL plan:
1. **Operator runs it live (incl. the DNA report):** restart the loopback server against the merged main (`python -m scripts.serve` → 127.0.0.1:8765 — the operator's prior server runs the pre-merge main, so restart to pick up ADR-0031), open the SPA at GET /, save the no-train key via the Profile screen (→ keychain `a-plus-maxing-api-key`), then upload the genetics PDF on Upload Documents — it now extracts LOCALLY (pdftotext) → text/plain → the no-train lane → genotype facts → confirm → land — and confirm the extracted readings. The agent NEVER touches the keychain. (ADR-0031 OQ-1: the first real pdftotext + real-key + real-spend run is operator-gated.)
2. **Before REAL PII / for the FULL real path:** land `6hts` (de-id value-scan residuals) + observe `stsq` (SEC-3 live-dispatch 0-leak); wire the live `author` (`_ClaudeNoTrainBackend.author`, still `NotImplementedError` — ADR-0015-T3); decide `by30` (the /upload same-origin gate). Optionally pull forward `tjz9` (locale-strict decode, trivial) before a non-default-locale launch.
3. **Then `/generate-plan`** over the operator's de-identified real data (now including the genotype facts) → a usable plan + doctor-visit queue + the dashboard made live (the per-surface live-wiring is the follow-on).

**Deferred / NOT blocking:** `0ai1` (swappable-seam fail-closed hardening); `tjz9`/`36as` (pdf_extract hardening); the safety follow-ups; `02m1` (daemon config); the local-model migration (North Star, post-V1). The LM-01 scoped-drift-audit window is now OPEN (opened 2026-06-29) — see Landmark check.

## Landmark window check (close step 8.7)

S100 close (2026-06-28): re-read `vault/meta/landmarks.md`. The ADR-0031 build is on mock clients + synthetic fixtures (no real operator data, no live API, no compound/biomarker page edits) — no landmark ACTION triggered by the build itself. LM-01 (First MD visit) **2026-07-13**: today (2026-06-28) is one day BEFORE the 14-day scoped-drift-audit window opens (**2026-06-29**) → no trigger today, but it opens TOMORROW. **NOTE for the next session on/after 2026-06-29: run the LM-01 scoped drift audit** on the relevant_scopes (operator-profile January-2026 section + `risk_tier: medium+`/experimental compounds + baseline-panel biomarkers + the doctor-handout queue); the 7-day MD-handoff window opens 2026-07-06. LM-02/03/04 not triggered.
