...
## Scope Contract — Session 86 (2026-06-22)

Goal: Build Wave B — the interactive intake wizard capture (steps 2-6) — completing the operator-requested intake website so the operator fills their profile point-and-click, then drive autonomously (no boundary pauses) through 3-tier review → merge → close.

Acceptance criteria:
- [x] AC1 (Wave B build, via `/execute-plan`): `scripts/serve/capture.py::persist_capture` routes steps 2-6 fields BY DATA CLASS — de-identified `WIRED_TOKENS` (`goal-domains`/`goal-priority-order`/`goal-targets`/`hard-limits`/`recovery-status-band`) → `store.append(source="intake")`; raw/rich → the gitignored `vault/scaffold/filled/`; `intake.py` steps 2-6 markup on the CURRENT theme; `server.py` `do_POST` field routing. Recipe `docs/task-plan/ADR-0014-T1.md`. PR #212.
- [x] AC2 (design-critic, operator-required): the design-reviewer ran over steps 2-6 — theme CONSISTENT (the operator's "stick with the current theme") + field semantics reviewed for honesty (the over-promising Step-5 subtitle was caught + reworded).
- [x] AC3 (PII boundary, EXECUTED): `rx-interaction-classes` routes RECORD-ONLY (never the model — curation surface deferred to ADR-0014 OQ-2); free-text wired tokens full-value PII-scanned (no 4096-cap / window-straddle evasion); server-side enum validation; 0-shared-routine-edit numstat EMPTY; no raw field reaches a wired token / `summarize` — Security + the blind-verifier executed the exploits.
- [x] AC4 (3-tier review → merge): Tier-1 TDD; Tier-2 (QA+Security+Architect+design-reviewer, EXECUTED) caught the CRITICAL raw-rx→model + fixed; Tier-3 `/review-pr` 6-agent with BOTH independence phases intact (profile-less blind-triage P3 + executed blind-verify P7) caught the F1 postal-straddle PII leak + 5 lower legitimate findings — all fixed, 6/6 blind-verified RESOLVED → `/merge` (PR #212 → `main` @ `7c374ad`).
- [x] AC5 (close): full automatic close on final post-merge `main` + cite the SHA (PF-S74-01).

Files I WILL touch: `scripts/serve/capture.py` (new) + `server.py` (do_POST routing); `vault/design/templates/intake.py` (steps 2-6 markup + a11y); `scripts/guard/pii_scan.py` (additive `scan_text_full`, default `scan_text` byte-unchanged); `tests/serve/*` + `tests/generate/test_intake.py`; `docs/task-plan/ADR-0014-T1.md` + `docs/spec/adr-0013-adr-0014-spec.md` + `docs/build-plan/build-plan-ingest-website.md` (rx record-only reconciliation); HANDOFF / PF-log / harvest / beads / session-note / overview at close.
Files I will NOT touch: the shared ingest routine (`ingest.py`/`adapter.py`/`scheduler.py`); `scripts/plan/*` (`router`/`summarize`/`store`/`generate` — REUSE only); the dashboard (design-LOCKED); `vault/library/`; the stranded `feature/apple-health-adapter` main-checkout + its parallel-wiki uncommitted `vault/` changes; `main` directly.
NOT doing: the ADR-0014 OQ-2 `rx-interaction-classes` curation surface (deferred/beaded); the multi-select `goal-domains` repeated-field parser fix (beaded); the `summarize()` 8j6 backstop uncap (defense-in-depth, beaded); the capture-path O(n^2) length cap (beaded — defensive, needs operator sign-off); loading real operator data; the labs adapter; the DNA clinical-SNP analysis.
Invariants at risk: ADR-0001 (zero PII to the model — the crown jewel; the review caught + EXECUTED-fixed a real postal-straddle leak before merge); ADR-0005 (operator data → gitignored scaffold); ADR-0003-T2 0-shared-routine-edit (HELD, numstat-gated); INV-CORE-CAPABILITY (untouched — Wave B is ingest UX; the plan path stays wired); INV-ROLE-INLINING (every role dispatch full-profile / registered-type — the hook caught + I corrected one abbreviated SE dispatch, zero downstream impact); INV-CLOSE-AUDIT / INV-PF-ATTESTATION / INV-SKILL-TRACE / INV-HARVEST-CAPTURE at close; INV-BRANCH-NOT-MAIN. **Core-capability-first gate (PF-S63-02): SATISFIED — core wired+invokable (`/generate-plan`, S81); Wave B is the operator-requested ingest UX, the genuine remaining V1 gap.** PF-S74-01: re-run the full close on final post-merge `main` + cite the SHA.

### S86 Scope Contract Evaluation (2026-06-22, volatile)

- **AC1–AC4 — PASS.** Wave B built via `/execute-plan` (recipe ADR-0014-T1; capture by data class). The design-reviewer confirmed the theme CONSISTENT + caught the Step-5 honesty gap. Tier-2 (executed exploits) caught the CRITICAL `rx-interaction-classes` raw→model path → fixed record-only (curation deferred OQ-2). Tier-3 `/review-pr` (full 6-agent + profile-less blind-triage + EXECUTED blind-verify, independence NOT collapsed) caught the F1 postal-address PII straddle past the scan cap (an ADR-0001 crown-jewel leak, reproduced E2E through the live server) + 5 lower findings (F3 vacuous-enum, F5 doc-reconciliation, F6 over-promise, F9 hint, F10 test-gap); all 6 fixed + 6/6 blind-verified RESOLVED. Merged PR #212 → `main` @ `7c374ad`. Suite 1264 passed / 3 skipped; 0-shared-routine-edit numstat empty.
- **AC5 (close) — PASS.** This close; the close gate re-ran on final post-merge `main` + the SHA cited (PF-S74-01).

### Drift checks (S86 close)

- **Task drift:** none silent. Wave B is the build-plan's planned wave (the S85 contract deferred it to S86). The mid-review scope was fix-forward of triaged-LEGITIMATE findings (6 fixed) with the real-but-unbounded/out-of-scope items BEADED (the OQ-2 curation surface, the multi-select parser, the backstop uncap, the O(n^2) cap, two design-polish items), never silently absorbed or suppressed.
- **Architecture drift:** the PII boundary (ADR-0001) was STRESSED and HELD — the review caught a real raw-PII-to-model leak (the postal straddle) and fixed it before merge, executed-verified closed; the 8j6 summarize backstop's shared cap is beaded (defense-in-depth, no live vector once the capture leak is fixed). 0-shared-routine-edit HELD (numstat empty; `pii_scan.py` gained only an additive helper). INV-CORE-CAPABILITY untouched. Toward the vision (the operator can now fill their profile point-and-click into the de-identified store).
- **Vision drift:** none. The system gains the interactive intake wizard — the operator-requested front-door for de-identified profile capture. `design/vision.md` first sentence unchanged.

### PF attestation

S86 close (2026-06-22): **No new PF-class entries this session.** The continuous-drive discipline HELD (the prior-session pause-at-autonomous-boundary lesson applied — I drove fix → checkpoint → Tier-3 review → merge → close without a status pause). The review pipeline worked AS DESIGNED (caught + fixed a real CRITICAL crown-jewel PII leak before merge, blind-verified). Observed-but-not-promoted: I ABBREVIATED the SE role profile on one fix-cycle dispatch — the `enforce-role-inlining` hook CAUGHT it (denied the dispatch), and I re-dispatched with the full canonical profile, zero downstream impact; a known class with an active mechanical guard that fired, so a disclosure-ledger item, not a new PF. Full attestation + the per-PR skill-trace table + the disclosure ledger in `memory/process-failures.md` Session 86.

**Historical (kept for reference):** `vault/sessions/session-86.md`.

## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-84 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; …; S83 archived S82; S84 archived S83; S85 archived S84; S86 archived S85) to keep this handoff lean. The current (S86) scope contract is above; the archive holds the prior-session archaeology.
...
1. **The interactive intake website is COMPLETE + MERGED (Wave A upload S85 + Wave B wizard capture S86, PR #212 → `main` @ `7c374ad` as of 2026-06-22 S86 close).** `python -m scripts.serve` (loopback-only) serves the full steps-1-6 wizard → captures the operator's profile by data class (de-identified → the store; raw/rich → the gitignored scaffold) + accepts browser uploads → the unchanged ingest seam. **Do NOT re-propose the ADRs / the upload server / the wizard capture / `/generate-plan` (S81) / the dashboard (design-LOCKED).** The genuine next surface: **the operator loading REAL data** — sync the stranded MAIN trunk to `origin/main` first (preserve the parallel-wiki uncommitted `vault/` changes), then `serve` → upload/fill; then the labs adapter + the DNA clinical-SNP analysis (LM-03).
2. **Under an explicit autonomous `/goal`, DRIVE THROUGH the protocol's boundaries (lesson HELD this session).** When the operator sets an autonomous goal ("do it autonomously, don't stop"), run fix → checkpoint → review → merge → close → next continuously — do NOT pause at wave/context boundaries for status. The harness's compaction + the resume bead carry continuity; the SE/review sub-agents run in FRESH contexts so build/review quality is preserved regardless of orchestration depth. Reserve stopping for a genuine operator-owned decision. (S86 applied this — no pause; the prior recurrence is closed-watch.)
3. **The PII boundary is the crown jewel — the review must EXECUTE, not reason.** Wave B's Tier-3 caught a real raw-PII-to-model leak (a postal address straddling the scan-window boundary past the 4096 cap) ONLY because the security + blind-verify agents reproduced it END-TO-END through the live server; a source-read review would have rubber-stamped it. Standing mechanics: build via **`/execute-plan`** (never hand-rolled — the methodology-substitution failure); ADRs via **`/create-adr`**; design via the **design agents** (current theme, never solo-originate); `/review-pr` off the **THREE-dot diff** vs `origin/main` with BOTH independence phases (blind-triage P3 + EXECUTED blind-verify P7) INTACT; `/merge` REST fallback when GraphQL is exhausted (full-40-char SHA guard); `git add` SEPARATE from `git commit`; **full role profiles inlined VERBATIM (the hook denies abbreviation)**. Carry-over beads: the OQ-2 rx-curation surface / the multi-select `goal-domains` parser / the 8j6-backstop uncap / the capture O(n^2) cap / two design-polish items / `xpev` (now DONE) / `07f6` / `gzf7`/`dw8u` / `0qf6`/`po4x`/`p5wx`/`7may`/`5fw1`.

## Current State (volatile)

- **S86 (2026-06-22) built Wave B — the interactive intake wizard capture (steps 2-6) — MERGED to `main` @ `7c374ad` (PR #212).** The intake website is now COMPLETE: `python -m scripts.serve` (loopback 127.0.0.1) serves the full wizard; on submit, `capture.py::persist_capture` routes each field by data class — de-identified `WIRED_TOKENS` → `store.append(source="intake")` (reaching the no-train model only via `summarize`); everything raw/rich → the gitignored `vault/scaffold/filled/` (ADR-0005), never the model.
- **PII boundary (crown jewel) held under stress.** `rx-interaction-classes` routes RECORD-ONLY (raw drug names never reach the model; the curation surface is deferred to ADR-0014 OQ-2). Free-text wired tokens are PII-scanned over the FULL value (the Tier-3 review caught + fixed a postal-address leak that straddled the old 4096-char scan window; `pii_scan.py` gained an additive non-truncating `scan_text_full`). Server-side enum validation on the bounded fields. 0-shared-routine-edit HELD (numstat empty).
- **Reviews:** Tier-1 TDD; Tier-2 (QA+Security+Architect+design-reviewer, EXECUTED exploits — caught the CRITICAL raw-rx→model path); Tier-3 `/review-pr` 6-agent + design-reviewer with **independence intact** (profile-less blind-triage P3 + EXECUTED blind-verify P7 — caught the F1 postal-straddle PII leak + 5 lower, all 6 fixed + 6/6 blind-verified RESOLVED). Beaded follow-ups: the OQ-2 rx-curation surface (P2), the `summarize` 8j6-backstop uncap (P2, defense-in-depth), the multi-select `goal-domains` parser (P2), the capture O(n^2) length cap (P3), two design-polish items (P3).
- **pytest 1264 passed / 3 skipped on `main` @ `7c374ad` (2026-06-22 S86 close).**
- **No new PF this session** (the continuous-drive discipline held; one abbreviated-SE-profile dispatch was hook-caught + corrected — a disclosure-ledger item).
- **The operator's MAIN trunk (`a-plus-maxing`) is still stranded on `feature/apple-health-adapter`** (local `main` stale) with the parallel-wiki uncommitted `vault/` changes — must be synced to `origin/main` (preserving those changes) before the operator can `serve`/upload/fill.
...
### S87 — the operator loads REAL data (the intake website is COMPLETE)

**The intake website is DONE (Wave A upload + Wave B wizard capture, merged S85+S86).** What's DONE (do not re-propose): the ADRs + the upload server + the wizard capture; `/generate-plan` (S81); the dashboard (design-LOCKED). Next:

1. **The operator loads real data.** Sync the stranded MAIN trunk (`a-plus-maxing` on `feature/apple-health-adapter`) to `origin/main` FIRST — preserving the parallel-wiki uncommitted `vault/` changes (NEVER touch those) — then `python -m scripts.serve` → upload Apple Health / 23andMe + fill the wizard. This is the operator-facing milestone the whole intake build served.
2. **The rest of operator-data ingest:** the labs adapter (a parser — none exists); the DNA clinical-SNP analysis (the LM-03 action — DNA *lands* via the upload route but is not analyzed); the filled `operator-profile`/`current-state`/`goals` scaffolds.
3. **Beaded Wave-B follow-ups (none blocking):** the ADR-0014 OQ-2 `rx-interaction-classes` curation surface (de-identified class tokens → the model-bound item); the `summarize` 8j6-backstop full-value scan (defense-in-depth, single-sourced with the capture scan); the multi-select `goal-domains` repeated-field parser; the capture-path O(n^2) length cap (needs operator sign-off — defensive); two design-polish items (the stepper-nav affordance + chip checked-state styling).
4. **LM-01 (July 13 MD visit) prep** — the 14-day scoped-drift-audit window opens **2026-06-29**.

**Discipline:** under the autonomous `/goal`, DRIVE THROUGH boundaries (close → open → next) — do not pause for status (the lesson held S86); build via `/execute-plan` (never hand-rolled); the design-critic for any new UI; `/review-pr` off the THREE-dot diff vs `origin/main` with the independence phases INTACT (never collapsed) → `/merge`; full role profiles inlined verbatim (the hook denies abbreviation); re-run the close gate on final post-merge `main` + cite the SHA (PF-S74-01).

**Deferred (NOT blocking):** the beaded Wave-B follow-ups above; `07f6` (decompression cap, operator-owned); the Wave-A LOW-security hardening bead; `gzf7`/`dw8u`; the `SUMMARY_FIELD_SET` extension for nutrition/supplements (future ADR-0006-T0); `0qf6`/`p5wx`/`7may`/`po4x`/`5fw1`; operator-gated July-visit prep (LM-01) — first session on/after **2026-06-29** runs the LM-01 scoped drift audit.

## Landmark window check (close step 8.7)

S86 close (2026-06-22): RE-OPENED `vault/meta/landmarks.md` at close step 8.7 (unchanged this session). No landmark edited — S86 built the intake wizard capture (Wave B) on PII-free SYNTHETIC fixtures; no real operator data ingested. LM-03 (23andMe): the DNA landing path exists (the upload route handles a 23andMe zip), but the LM-03 ACTION (parse SNPs → profile + `vault/dna/analysis.md`) is the separate sensitive analysis step — not built, no real DNA ingested → trigger NOT open. LM-02 (Apple Health wearable baseline): no real data → not open. LM-01 (First MD visit) **2026-07-13**: no trigger today (the 14-day scoped-drift-audit window opens **2026-06-29**, the 7-day MD-handoff window 2026-07-06). LM-04 (first HTML artifact): the renderers emit to the gitignored `vault/artifacts/generated/`; not triggered. No landmark ACTIONS due.
