...
## Scope Contract — Session 90 (2026-06-22)

Goal: Build **Wave A** of the conversational-intake build (the model boundary + the conversation path) via `/execute-plan`, drive it through the Wave-A checkpoint → Tier-2 → Tier-3 `/review-pr` → `/merge`, and close. (AUTONOMOUS per the operator's standing directive; self-confirmed.)

**Build-plan wave:** Wave A of `docs/build-plan/build-plan-conversational-intake.md` (5 tasks: `ADR-0015-T1`/`T2`/`T3`, `ADR-0017-T1`, `ADR-0016-T1`; serial spine `T1 → {T2 ∥ T3 ∥ 0017-T1} → 0016-T1`). **Prior-wave attestation:** Wave A is the entry wave (single entry point `ADR-0015-T1`); the upstream S88 foundation (ADRs 0015–0019, PR #216) + S89 build-planning (spec + build-plan + 8 recipes) are merged on `main` @ `0cd9a24`; the V1 baseline is green (1264 passed / 3 skipped) and `core-capability-audit` is wired.

Acceptance criteria:
- [ ] AC1 (plan-integrity grounding): `plan-integrity` grounds every Wave-A task's inputs against the LIVE tree (the seams `persist_capture`/`store.append`/`router.summarize`/`_author_callable`/`compute_plan`/`record_plan`/`_LOOPBACK` exist; the model-client-import baseline of 0 over `scripts/`) + confirms the dependency map is an acyclic DAG before the wave builds.
- [ ] AC2 (Wave A build via `/execute-plan`): the 5 Wave-A tasks built per their recipes (TDD, failing-capable), each an atomic commit — `scripts/model/{__init__,client,key_source}.py` + `keychain-setup.md`, `scripts/serve/chat.py` + `extract.py`, the `server.py` POST `/chat` route + the H-2 `identity_config` threading at BOTH call sites, the `generate_plan.py` `_author_callable` re-wire, + the test files.
- [ ] AC3 (Wave-A checkpoint, EXECUTED): the checkpoint Go/No-Go RAN green — single model boundary (0 imports outside `scripts/model/`), runtime key never tracked (0 key literal), fail-closed on all 3 consumers (client raise / `/chat` degraded turn / honest no-plan), `/chat` loopback + single-egress-class, extractor gate-at-write (0 `store.append` in `extract.py`), H-2 `identity_config` at BOTH call sites, `assemble`/clearance/translators/`record_plan` byte-unchanged, 0-shared-routine-edit numstat, `core-capability-audit.sh --self-test` green.
- [ ] AC4 (3-tier review → merge): Tier-1 SE self-check; Tier-2 (QA + Security + Architect + `plan-integrity` gate) over the whole-wave diff; Tier-3 `/review-pr` 6-agent with independence intact (profile-less blind-triage P3 + EXECUTED blind-verify P7); all LEGITIMATE findings fixed + blind-verified → `/merge`.
- [ ] AC5 (close): full automatic close on final post-merge `main` + cite the SHA (PF-S74-01).

Files I WILL touch: (Wave A) `scripts/model/*` (new); `scripts/serve/chat.py` + `extract.py` (new); `scripts/serve/server.py` (the `/chat` route + the H-2 threading); `scripts/plan/generate_plan.py` (the `_author_callable` re-wire ONLY — `assemble`/clearance/translators/`record_plan` byte-unchanged); `tests/model/*`, `tests/serve/test_question_strategy.py` + `test_chat.py` + `test_chat_no_egress.py` + `test_extract.py` + `test_capture.py`, `tests/plan/test_generate_plan.py`; the Wave-A recipes' `status` frontmatter; HANDOFF / PF-log / harvest / beads / session-note / overview at close.
Files I will NOT touch: `scripts/ingest/*` (`ingest.py`/`adapter.py`/`scheduler.py` — 0-shared-routine-edit, NFR-4); `scripts/plan/assemble.py` + the clearance gate + the per-domain translators + `plan_schema.record_plan` (byte-unchanged); `scripts/plan/router.py` + `scripts/serve/capture.py` token vocabulary + `vault/design/templates/intake.py` (those are WAVE B — ADR-0018/0019, next session); the dashboard (design-LOCKED); `vault/library/*`; the operator's stranded MAIN checkout (`a-plus-maxing` @ `dfd44e8`) + its parallel-wiki WIP (NEVER touch destructively); `main` directly; the macOS keychain (the agent never touches it — env-injection only).
NOT doing: Wave B (the demographic activation + the `SUMMARY_FIELD_SET` extension + the end-to-end usable-plan proof — ADR-0018/0019/0017-T2 — next session); the LIVE end-to-end run (operator-present; the operator injects the key — after Wave B); ANY live API call (Wave A is mock/fixture-tested, 0 API spend).
Invariants at risk: ADR-0001/ADR-0016 (the crown-jewel PII boundary — Wave A builds the egress seam; the live-conversation carve-out is mock-only this wave, nothing raw committed); INV-CORE-CAPABILITY (Wave A wires the programmatic author path through the one client — `core-capability-audit --self-test` must stay green); INV-BRANCH-NOT-MAIN; INV-ROLE-INLINING (every dispatch full profile); INV-CLOSE-AUDIT / INV-PF-ATTESTATION / INV-SKILL-TRACE / INV-HARVEST-CAPTURE at close. **The `AP-SELF-REVIEW-UNDER-PROBES-INTERACTION-SURFACE` class (PF-S75→S78, recurrence 5):** the Tier-1 self-check grounds every field the new code reads off an existing contract (value domain enumerated) + checks branching against this contract's exclusions BEFORE Tier-2/3; the Tier-2/Tier-3 briefs explicitly probe the new-code × existing-infra interaction surface (the model client × the 3 consumers; the extractor × `persist_capture` × `store`; the de-id derived path × the 8j6 gate). PF-S87-01: the build's headline acceptance is a usable plan (proven end-to-end in Wave B; Wave A proves the wired author path mock). PF-S71-01: the park maneuver for worktree commits when the main checkout sits on `main`. PF-S74-01: re-run the close gate on final post-merge `main` + cite the SHA.

**Core-capability-first gate (PF-S63-02):** does the core capability work yet? **NO — but Wave A is the direct path to it** (it builds the FIRST programmatic model client + wires the plan-author through it; `core-capability-audit --self-test` proves the wired author path mock). This is NOT secondary work; the gate is satisfied.

**S90 self-confirmation (autonomous):** contract self-confirmed per the operator's standing directive ("set up everything to run the strict autonomous loop and we'll do it in the next session" + "only stop if you absolutely need me to weigh in; choose the most robust / most rigorous path"). The build is mock/fixture-tested (0 API spend); the live run is the operator-present checkpoint after Wave B.

### S90 Scope Contract Evaluation (2026-06-22, volatile)

- **AC1 (plan-integrity grounding) — PASS.** `plan-integrity` ground every Wave-A task's inputs against the LIVE tree (the seams `persist_capture`/`store.append`/`router.summarize`/`_author_callable`/`compute_plan`/`record_plan`/`_LOOPBACK` exist; model-client-import baseline 0 over `scripts/`; `scripts/model/` greenfield) + confirmed the DAG acyclic with artifact-named edges — verdict **GO**. Two cosmetic narrative-drift notes (a stale branch-anchor + minor line-offsets); SE agents grounded against the live symbols.
- **AC2 (Wave A build via `/execute-plan`) — PASS.** The 5 Wave-A tasks built per their recipes (TDD, failing-capable + negative controls), each an atomic commit (`cb0dcc1`/`0ecdce6`/`ab0aad1`/`fb269a9`/`c3033ab`): `scripts/model/*`, `scripts/serve/{chat,extract}.py`, the `server.py` `/chat` route + H-2 threading at BOTH call sites, the `generate_plan.py` re-wire.
- **AC3 (Wave-A checkpoint, EXECUTED) — PASS.** The checkpoint Go/No-Go RAN green: single model boundary (0 imports outside `scripts/model/`), runtime key never tracked (0 real key literal), fail-closed on all 3 consumers, `/chat` loopback + single-egress-class, extractor gate-at-write (0 `store.append` in `extract.py`), H-2 `identity_config` at BOTH call sites, `assemble`/`router` byte-unchanged, 0-shared-routine-edit numstat empty, `core-capability-audit --self-test` green.
- **AC4 (3-tier review → merge) — PASS.** Tier-1 per-task TDD; Tier-2 (QA+Security+Architect+`plan-integrity`, EXECUTED — 4 findings fixed incl. the in-population Canadian-postal crown-jewel gap); Tier-3 `/review-pr` 6-agent with independence INTACT (10 Suggestion-tier findings; 6 LEGITIMATE + 1 adopted fixed → 7/7 blind-verified RESOLVED). Suite 1364 passed / 3 skipped → `/merge` (PR #220, REST, full-40-char SHA guard).
- **AC5 (close) — PASS** (this close; close-audit + harvest-gate green on final post-merge `main`, SHA cited per PF-S74-01).

### Drift checks (S90 close)

- **Task drift:** One JUSTIFIED scope expansion, flagged not silent — the Tier-2 Security HIGH-1 crown-jewel fix touched `scripts/guard/pii_scan.py` + `tests/guard/test_pii_scan.py`, OUTSIDE the original WILL-touch list. Justification: a REAL in-population (the operator's locale uses a non-US, Canadian-format postal) raw-PII leak on the crown-jewel value-PII boundary (NFR-1, the marquee Wave-A concern) that Wave B's real-client author leg will ACTIVATE; the build-plan's risk-ordering says build the fail-closed boundary BEFORE the egress that rests on it, so fixing the gate now is in-spirit. The fix is additive (one CA postal pattern, 0 false positives). No silent drift; all 5 ACs PASS.
- **Architecture drift:** The crown-jewel PII boundary (ADR-0001/ADR-0016) was STRESSED and HELD — the `/chat` single-egress-class is proven CLOSED by execution (the model payload carries only the de-identified `TurnIntent` structure), and the in-population value-PII gap was caught + fixed + blind-verified before merge. INV-CORE-CAPABILITY: the re-wire wires the programmatic author path through the one client; `core-capability-audit --self-test` green (the carried NOTE-T3-1 — the production conversational author leg gets a real client in Wave B — is explicit + beaded `j432`). `assemble`/`router`/shared-ingest byte-unchanged. Toward the vision (the conversation path is built).
- **Vision drift:** None. The system gains the first programmatic model client + the conversation path — the conversational-intake direction the S87 pivot set. `design/vision.md` first sentence unchanged.

### PF attestation

S90 close (2026-06-22): **One new PF promoted — PF-S90-01** (the crown-jewel value-PII gate's Tier-1 tests probed US-locale fixtures only; the operator's Canadian postal slipped into the plan-author egress — Tier-2-caught in-population, recurrence 6 of the interaction-surface class, the PII-scanner-coverage sub-class cf PF-S76-01; fixed + Tier-3 blind-verified, 0 reached the operator; structural fix beaded `a-plus-maxing-v70t`). The defining POSITIVE: the marquee crown-jewel interaction (the `/chat`-payload-vs-`summarize`-raw-passthrough leak) was SELF-CAUGHT at Tier-1 — the interaction-probe discipline's first clean self-catch on the highest-stakes surface. Full attestation + the per-PR skill-trace table (PR #220) + the disclosure ledger (11 caught, 0 to operator) in `memory/process-failures.md` Session 90. Prior lessons referenced DESCRIPTIVELY (the never-hand-roll-the-build discipline; the read-the-gated-skill-in-full discipline; the worktree-commit park workaround; close on final post-merge main + cite the SHA; the drive-through-autonomous-boundaries discipline), never by bare token.

**Historical (kept for reference):** `vault/sessions/session-90.md`.

## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-89 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; …; S87 archived S86; S88 archived S87; S89 archived S88; S90 archived S89) to keep this handoff lean. The current (S90) scope contract is above; the archive holds the prior-session archaeology.
...
1. **Conversational-intake WAVE A is BUILT + 3-tier reviewed + MERGED (PR #220 → `main`, 2026-06-22 S90 close).** The system's first programmatic model client + the conversation path are live: `scripts/model/` (swappable no-train client + runtime key source), `scripts/serve/chat.py` (de-identified question strategy + per-turn dispatch), `scripts/serve/extract.py` (gate-at-write extractor), the `/chat` egress seam, the plan-author re-wire through the one client. All mock/fixture-tested (0 live-API spend). **Do NOT re-propose Wave A.** The next surface is **S91 = Wave B** (ADR-0018 demographic activation + ADR-0019 `SUMMARY_FIELD_SET` extension + ADR-0017-T2 end-to-end), then the LIVE operator-present run (PF-S87-01: a usable plan, not just chat plumbing).
2. **The carried Wave-B obligations (must land in Wave B or a gap recurs).** **NOTE-T3-1 (Factory-to-Component, blocking at Wave-B close):** `ADR-0017-T2` MUST wire a REAL `ModelClient` into the production author leg + assert it non-tautologically — the Wave-A seam defaults to `_FixedEnvelopeClient` (captured-envelope backward-compat) and the conversational author path has no real-client consumer yet; else PF-S63-02 recurs. **NOTE-T2-2:** the `intake_complete` consumer must treat it as "stop asking", NOT "plan ready" — plannability is `domain_done` + acknowledged `record_only`. **The per-token coarseness proof** uses an INDEPENDENT output scan (the 8j6 gate does NOT run on the raw-backed-derived path). **PF-S90-01 (`v70t`):** seed the operator's actual CANADIAN locale into the crown-jewel value-PII gate tests (the US-only fixtures missed the in-population case). **The API key (RESOLVED):** at the live run the OPERATOR injects `export ANTHROPIC_API_KEY=$(security find-generic-password -s "quant-primary-api" -w)` — `key_source` now reads that env-var + keychain service (Tier-2 fix); the agent NEVER touches the keychain. NEVER store/print/commit the key value (PUBLIC repo).
3. **Standing mechanics (all HELD this session).** Build via **`/execute-plan`** (never hand-rolled, PF-S36-01); the 3-tier review (Tier-1 SE → Tier-2 QA+Architect+Security → Tier-3 `/review-pr` with the profile-less blind-triage + EXECUTED blind-verify INTACT) + **`plan-integrity`** grounding at wave-start/gating; store-surface tasks satisfy `docs/checklists/store-adversarial-tests.md`; full role profiles inlined VERBATIM (the hook denies abbreviation); `/review-pr` off the THREE-dot diff vs the wave base; `/merge` (REST fallback when GraphQL exhausted, full-40-char SHA guard); `git add` SEPARATE from `git commit`; **the `block-commit-main` worktree false-block → PARK the main checkout on a same-commit branch, commit in the worktree, un-park** (PF-S71-01; used once at S90 open, the operator's WIP verified byte-identical at park + un-park); re-run the close gate on final post-merge `main` + cite the SHA (PF-S74-01). **The operator's MAIN checkout was parked on `s90-park` during the build + UN-PARKED at this close back to `main` @ `dfd44e8` (as of 2026-06-22 S90 close); their parallel-wiki WIP (15 files) is preserved untouched.** Carry-over beads: `v70t` (PF-S90-01), `j432`/`pqb0`/`zwow` (Wave-B follow-ups) + the prior open set.

## Current State (volatile)

- **S90 (2026-06-22): built conversational-intake WAVE A — the model boundary + conversation path — MERGED via PR #220 → `main`.** 5 tasks via `/execute-plan` (the serial spine `T1 → {T2 ∥ T3 ∥ 0017-T1} → 0016-T1`): `scripts/model/{client,key_source}.py` + `keychain-setup.md` (single model boundary, fail-closed, runtime key never tracked); `scripts/serve/chat.py` (`plan_next_turn` de-identified store-grounded strategy + the per-turn `/chat` dispatch); `scripts/serve/extract.py` (model-proposes-gate-disposes extractor, 0 own `store.append`) + the H-2 `identity_config` wiring at BOTH call sites; the `/chat` egress seam (loopback, single-egress-class, fail-closed degraded turn); the `generate_plan.py` `_author_callable` re-wire (honest no-plan on failure; `assemble`/`router` byte-unchanged). All on synthetic fixtures + mock clients (0 live-API spend).
- **The crown jewel held under stress.** The `/chat` single-egress-class boundary is proven CLOSED (the model payload carries ONLY the de-identified `TurnIntent` structure, never `summarize`'s raw free-text values) — the SE SELF-CAUGHT a real leak at Tier-1 via its own negative control. Tier-2 Security caught the in-population Canadian-postal value-PII gate gap (PF-S90-01, fixed + blind-verified). `rx`/PII boundary, fail-closed on all 3 consumers, 0-shared-routine-edit, all executed-verified.
- **Reviews (all gates held):** Tier-1 per-task TDD (failing-capable + negative controls) → Tier-2 (QA+Security+Architect+`plan-integrity`, EXECUTED — 4 findings fixed) → Tier-3 `/review-pr` 6-agent with independence INTACT (10 Suggestion-tier findings; 6 LEGITIMATE + 1 adopted fixed → 7/7 blind-verified RESOLVED). `plan-integrity`: GO. Quality gate PASS (0 Critical, 0 Important).
- **pytest 1364 passed / 3 skipped** on the merged Wave-A tree; `core-capability-audit --self-test` green; the wired author path proven mock.
- **One new PF — PF-S90-01** (the crown-jewel value-PII gate's Tier-1 tests used US-locale fixtures only; the operator's Canadian postal slipped — Tier-2-caught in-population, fixed + verified; structural fix beaded `v70t`). The marquee crown-jewel interaction self-caught at Tier-1 (the discipline's first clean self-catch on the highest-stakes surface).
- **Beads:** epic `w3y8` advanced (Wave A built); new: `v70t` (PF-S90-01), `j432` (Wave-B author-contract alignment), `pqb0` (Wave-B `declined_domains` consumer), `zwow` (Wave-B `/chat` body ceiling).
- **POST-S90-CLOSE HOUSEKEEPING (2026-06-22→23): collapsed to a clean SINGLE TRUNK.** The parallel wiki-research lane finished + merged its bottom-13 peptide sweep (PRs #221–#235) on top of Wave A → `origin/main` @ `7f7fa6f` (Wave A `0e84cc2` is an ancestor; no collision). Synced the main checkout `dfd44e8 → 7f7fa6f`; **removed both extra worktrees** (the S90 build worktree + the redundant `feature/wiki-peptides`, which carried 0 unmerged peptide content — recoverable SHA `0404c24`); **cleared a 14-deep stash backlog** of prior-session daemon churn. Wave B now runs in the main checkout on a feature branch — the **PF-S71-01 park dance is retired**. New beads: `f22s` (wiki research-report enum-lint hardening), `oves` (semaglutide dead-link), `02m1` (the daemon-config decision below).
- **Daemon-churn finding (now disciplined):** the multi-session "benign daemon churn" is actually the basic-memory daemon rewriting `vault/**/*.md` frontmatter on sync — cosmetic on content files (permalink/title-fold/newline) but **destructive on template `{placeholder}` YAML** (→ `null` mappings). Never committed (stash-and-discard protected it). Discipline now in **CLAUDE.md → Vault / daemon hygiene** (never commit vault churn; the cross-worktree model). The complete fix (global `ensure_frontmatter_on_sync: false`) is **operator-owned** (affects all 24 bm projects) — beaded `02m1` with exact steps. **For S91: do NOT `git add vault/` blindly; the vault working tree is perpetually daemon-dirty — stage by explicit path.**

**Historical (kept for reference):** `vault/sessions/session-90.md`.

## What Is Next (volatile)

### S91 — `/execute-plan` Wave B (form activation + token vocabulary + the end-to-end usable-plan proof)

Wave A is merged. S91 builds **Wave B** of `docs/build-plan/build-plan-conversational-intake.md` (the serial spine `ADR-0018-T1 → ADR-0019-T1 → ADR-0017-T2`; recipes under `docs/task-plan/`). Read `/execute-plan` IN FULL first (PF-S17-01); `plan-integrity`-ground Wave B's inputs against the live tree before building (the four orphan tokens ∈ `SUMMARY_FIELD_SET`; the `postal-address`→`equipment-access-class` source collision; the `router.py` extension points). Build mock/fixture-tested first (0 live-API spend).

**Sequence:**
1. `plan-integrity`-ground Wave B → **`/execute-plan` Wave B**: `ADR-0018-T1` (demographic-form activation — the 4 orphan tokens get real inputs; reconcile the `equipment-access-class` double-source collision; form stays objective-only) → `ADR-0019-T1` (extend `SUMMARY_FIELD_SET` with de-identified chat-sourced tokens; **the per-token coarseness proof uses an INDEPENDENT output scan — the 8j6 gate does NOT run on the raw-backed-derived path**; disjointness tripwires green; the `pka` store-adversarial battery) → `ADR-0017-T2` (the PF-S87-01 HEADLINE end-to-end: a fixture conversation → `/chat` → `persist_capture` → `summarize` → `generate_plan` → a USABLE rendered plan; CONCERN-1 record-only loop-termination) → the Wave-B checkpoint Go/No-Go → Tier-2 (QA+Security+Architect+design-reviewer+`plan-integrity`) → Tier-3 `/review-pr` → `/merge` → close.
2. **The carried Wave-A obligations land HERE (blocking at Wave-B close):** `ADR-0017-T2` MUST wire a REAL `ModelClient` into the production author leg + assert it non-tautologically (NOTE-T3-1 — else PF-S63-02 recurs, the seam currently defaults to `_FixedEnvelopeClient`); the `intake_complete` consumer treats it as "stop asking", NOT "plan ready" (NOTE-T2-2 — plannability = `domain_done` + acknowledged `record_only`); seed the operator's actual CANADIAN locale into the crown-jewel value-PII gate tests (PF-S90-01 / `v70t`).
3. **THEN the LIVE end-to-end run** (the final, operator-present checkpoint, after Wave B's CI checkpoint is green): the OPERATOR injects the key in one command (the agent NEVER touches the keychain — the credential-exploration guard blocks it): `export ANTHROPIC_API_KEY=$(security find-generic-password -s "quant-primary-api" -w)`. `quant-primary-api` is the operator's Anthropic API key, no-train by default (= threat-model B; ~30-day retention, not ZDR). Drive a real conversation → a real plan. **Per PF-S87-01 the acceptance is a USABLE plan, not just working chat plumbing.** NEVER store/print/commit the key value (PUBLIC repo).

**Deferred / NOT blocking:** the Wave-B follow-up beads (`j432` author-contract alignment — do before the real-client wiring; `pqb0` `declined_domains` consumer; `zwow` `/chat` body ceiling — defensive, needs operator sign-off); the operator's `bpc-157` WIP reconcile (their call); the local-model migration (North Star, post-V1).

## Landmark window check (close step 8.7)

S90 close (2026-06-22): RE-OPENED `vault/meta/landmarks.md` at close step 8.7 (unchanged this session). No landmark edited — S90 built conversational-intake Wave A on PII-free SYNTHETIC fixtures + mock clients; no real operator data, no live API. LM-01 (First MD visit) **2026-07-13**: today (2026-06-22) is BEFORE the 14-day scoped-drift-audit window open (**2026-06-29**, 7 days out) → no trigger today; **NOTE for S91+: the first session on/after 2026-06-29 runs the scoped drift audit on the LM-01 relevant_scopes** (the 7-day MD-handoff window opens 2026-07-06). LM-02 (Apple Health) / LM-03 (23andMe): no real data ingested → not triggered. LM-04 (first HTML artifact): the renderers emit to the gitignored `vault/artifacts/generated/`; not triggered. No landmark ACTIONS due.
