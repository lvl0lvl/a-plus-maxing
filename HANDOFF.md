...
## Scope Contract — Session 82 (2026-06-20)

Goal: Build the first REAL-data ingestion adapter — upgrade the Apple Health (HealthKit) adapter from a fabricated-JSON scaffold to streaming the real Apple Health `export.xml` (the operator uses Apple Health, not Whoop). Proven on a synthetic `export.xml` fixture; the operator's real export is the production validation.

Acceptance criteria:
- [x] AC1 (real-format read): `scripts/ingest/adapters/healthkit.py` streams the real `export.xml` via `xml.etree.iterparse` (clearing the tree — the export can be 100s of MB), replacing the `json.loads({type,startDate,qty})` scaffold. Conforms to the frozen ADR-0003-T1 contract (`source_tag()`+`read_readings`); `source_tag` stays `"healthkit"`.
- [x] AC2 (daily aggregation): Apple's RAW per-sample `<Record>`s aggregate to ONE value per (item, day) — the daily MEAN keyed on `startDate`'s date — to match the store's one-reading-per-(item, day, source) key (the granularity difference from the pre-aggregated wearable DB). Rounded to 2 decimals.
- [x] AC3 (value-domain-clean metric map): `HeartRateVariabilitySDNN`→hrv, `RestingHeartRate`→rhr, `RespiratoryRate`→resp-rate as-is; `OxygenSaturation`→spo2 ×100 (fraction→percent). recovery/strain (no Apple equivalent), `skin-temp-dev` (Apple's absolute °C vs the store's deviation) + `sleep-efficiency` (a stage roll-up) DEFERRED — value-domain-grounded, not arbitrary (the wrong-scale class the prior wearable adapter's guard prevents).
- [x] AC4 (0-shared-routine-edit, ADR-0003-T2): `ingest.py`/`adapter.py`/`scheduler.py` byte-unchanged (the scheduler auto-discovers the upgraded adapter); EXECUTED numstat gate green.
- [x] AC5 (store-adversarial battery on the read path): cross-stream / dedupe-idempotent / distinct-days / same-identity-changed-value / distinct-source-from-the-other-wearable / fail-loud + boundary/realism (the REAL nested-children Record shape, missing-field skip, record-level non-numeric fail-loud, empty export, spo2 sub-percent rounding, two-metrics-same-day independent aggregation).
- [x] AC6 (ADR + review + close): ADR-0012 documents the mechanism + the daily-aggregation + the spo2 scale + the deferrals + the operator-owns-the-export license; Tier-1 → Tier-2 plan-integrity + QA → `/review-pr` 6-agent → `/merge`; full automatic close + the PF-S74-01 re-run on final `main`.

Files I WILL touch: `scripts/ingest/adapters/healthkit.py` (UPGRADE — not a new file; the scaffold already existed); `tests/ingest/test_adapters.py` + `tests/ingest/test_scheduler.py` (the healthkit tests + the necessary scheduler-fixture call-chain update); `tests/ingest/conftest.py` (NEW at review — single-sourcing the forked export helper); `docs/adr/ADR-0012-apple-health-real-export-ingestion.md` (NEW); HANDOFF/PF-log/harvest/beads/session-note at close.
Files I will NOT touch: `scripts/ingest/{ingest,adapter,scheduler}.py` (the shared routine — REUSE; the 0-edit invariant); `scripts/ingest/adapters/{whoop,oura,garmin}.py` (NOT flipping whoop to UNWIRED — coexistence, see the CHANGED note below); `scripts/store/*`; `scripts/plan/*` + `orchestrate.py`; the dashboard (design-LOCKED); the wiki / `vault/library/`; `main` directly.
NOT doing: the labs / 23andMe adapters (no parsers exist yet); the filled operator-profile/current-state scaffolds (operator-gated PII); any dashboard work; new safety logic; running on real operator data (synthetic `export.xml` only).
Invariants at risk: ADR-0003-T2 0-shared-routine-edit (EXECUTED-numstat gated); INV-CORE-CAPABILITY (untouched — pure ingest, the plan path stays wired + `--self-test`); store-surface battery; INV-ROLE-INLINING (the 6 review agents + the blind-triage/verify/SE dispatches FULL profiles); PF-S78-01 value-domain grounding (the metric map + the spo2 scale + the deferrals); INV-BRANCH-NOT-MAIN; INV-CLOSE-AUDIT / INV-PF-ATTESTATION / INV-SKILL-TRACE / INV-HARVEST-CAPTURE at close. **Core-capability-first gate (PF-S63-02): SATISFIED — the core capability is wired+invokable (`/generate-plan`, S81); S82 is the right secondary step because the ONLY remaining V1 gap is real operator data, and the wearable adapter is the operator-chosen first real-data unlock (HANDOFF "What Is Next" item 1, bead `v3ml`).** PF-S74-01 discipline: re-run the full close on the final post-merge `main` + cite the `main` SHA.

Acceptance criteria:
- [x] AC1 (the seam): `scripts/plan/pipeline.py` `run_generation` stitches `orchestrate.generate_plans` → `collate_doctor_visit_queue` into one recorded GENERATE pass, returning `{**result, dvq_entries}`. REUSE only — closes the "`generate_plans` has no production caller" gap (the INV-CORE-CAPABILITY pattern for the cross-domain terminal). `orchestrate.py` byte-unchanged.
- [x] AC2 (the skill): `.claude/skills/generate-plan/SKILL.md` + `/generate-plan` command — the dispatch lifecycle (gather inputs → dispatch each `PLAN_DOMAINS` specialist full-profile → build `authors` → call the seam with the `reauthor` + `adjudicator` hooks → collate → render). References `author-dispatch-process.md`.
- [x] AC3 (safety floor intact): every held finding (additive-AE / conflict / Rx-BPMH) routes through `adjudicate` (INV-OVERRIDE-RECORD-SCHEMA + INV-CRITICAL-NON-OVERRIDABLE reused); un-cleared load / unsustainable energy / un-adjudicated hold never records. No new safety logic.
- [x] AC4 (store-surface battery): the seam writes `plan::` + `dvq::` — cross-stream / dedupe-idempotent / mutation + PF-S78-01 value-domain grounding.
- [x] AC5 (synthetic E2E + real dispatch): `tests/plan/test_pipeline.py` (13) — clean pass, the cleared-held-finding stitch (mutation-proven vs the no-adjudicator control), block-stands queuing (real band), multi-axis + rx-bpmh collation, the `on_date` default, the store battery, the gates/reauthor forwarding; PLUS a REAL personal-trainer dispatch over synthetic data (full profile) → `run_generation` → rendered dashboard.
- [x] AC6 (backward-compat): `generate_plans`/`collate`/`compute_plan`/`reconcile`/`adjudicate`/`generate` UNCHANGED; full suite + core-capability gate green.
- [x] AC7 (review + close): Tier-1 → Tier-2 plan-integrity + QA → `/review-pr` 6-agent → fix legitimate → `/merge`; full automatic close + the PF-S74-01 re-run.

Files I WILL touch: `scripts/plan/pipeline.py` (NEW); `tests/plan/test_pipeline.py` (NEW); `.claude/skills/generate-plan/SKILL.md` (NEW); `.claude/commands/generate-plan.md` (NEW); `docs/plan-generation/examples/generate-plan-workout-author-output.example.json` (NEW, the real-dispatch capture); HANDOFF/PF-log/harvest/beads/session-note at close. Fix the stale "#179 OPEN" fact in HANDOFF (it merged).
Files I will NOT touch: `scripts/plan/{generate_plan,adjudicate,router,assemble,track,adjust}.py` + `orchestrate.py`'s reconcile/generate_plans/collate INTERNALS (REUSE — call only); `scripts/store/*`; `generate.py` internals; the deployed `.claude/agents/*`; the wiki / `vault/library/`; the dashboard (design locked); `main` directly.
NOT doing: the ADJUST leg (separate, built S80); a non-interactive auto-dispatch CLI (reasoning is the specialists', runtime A); real-data ingest (synthetic only); any dashboard work; new safety logic.
Invariants at risk: INV-CORE-CAPABILITY (the cross-domain terminal's production caller — stays green + `--self-test`); INV-ROLE-INLINING (the skill's specialist + liaison dispatches + the review agents FULL profiles); INV-OVERRIDE-RECORD-SCHEMA + INV-CRITICAL-NON-OVERRIDABLE (held findings → adjudicate, reused); store-surface battery; PF-S78-01; INV-BRANCH-NOT-MAIN; INV-CLOSE-AUDIT / INV-PF-ATTESTATION / INV-SKILL-TRACE / INV-HARVEST-CAPTURE at close. Core-capability-first gate (PF-S63-02): SATISFIED — `/generate-plan` IS the invocation of the now-complete core capability (the front door). **PF-S74-01 discipline: re-run the full close on the final post-merge `main` + cite the `main` SHA.**

### S82 Scope Contract Evaluation (2026-06-20, volatile)

- **AC1–AC6 — PASS, with two CHANGED items honestly flagged (drift below).** `healthkit.py` now streams the real `export.xml` (`iterparse` + `root.clear()` memory-bounding) and aggregates raw per-sample `<Record>`s to the daily MEAN per (item, day), spo2 ×100; recovery/strain/skin-temp-dev/sleep-efficiency deferred value-domain-grounded; the 0-shared-routine-edit invariant holds (EXECUTED numstat — `ingest.py`/`adapter.py`/`scheduler.py` byte-unchanged); the store-adversarial battery + boundary/realism tests all pass; ADR-0012 documents it. Full suite 1160 passed / 3 skipped on final `main` (`b83b12b`); core-capability gate green (untouched — pure ingest).
- **AC6 (review + close) — PASS.** Tier-1 (pytest) → Tier-2 plan-integrity INTEGRITY-CLEAN + QA (6 SHOULD-FIX coverage gaps fixed, vacuity audit PASS) → Tier-3 `/review-pr` 6-agent over the THREE-dot diff (security PASS [SEC-1 NOT_ACTIONABLE — XXE probes EXECUTED clean, hardening = unapproved defensive programming]; bug-hunter 0 findings [17 trigger inputs executed]; contracts "contracts met" [numstat EXECUTED]; historical 0 regressions; code-quality + test-coverage → 4 LEGITIMATE blind-triaged, fixed + blind-verified RESOLVED 4/4 incl. the TEST-001 round() vacuity gap; TEST-002 NOT_A_BUG) → `/merge` rebase PR #199 → `main` @ `b83b12b`; full close re-run on final `main` + the SHA cited (PF-S74-01).

### Drift checks (S82 close)

- **Task drift:** all ACs PASS; **two CHANGED items vs the S82-plan, both flagged not silent.** (1) The plan (HANDOFF "What Is Next" item 1) said to CREATE `scripts/ingest/adapters/apple_health.py`; the actual work discovered the `healthkit.py` scaffold already existed → reframed as an UPGRADE of `healthkit.py` (the ADR-0011 scaffold→real pattern), avoiding a redundant new adapter + `source_tag` rename. (2) The plan said to flip `whoop.py` to `UNWIRED = True` (instead-of); the build chose COEXISTENCE (whoop stays wired) — the operator's "Apple Health instead of Whoop" is achieved by providing a HealthKit export and no Whoop export (the scheduler runs only the adapters it has an export for), and the device-specific `source` tag keeps the two distinct, so disabling whoop is unnecessary and would lose a working adapter (ADR-0012 D4). Both changes documented in the ADR + the scope contract. No silent drift.
- **Architecture drift:** no invariant degraded. The shared routine (`ingest.py`/`adapter.py`/`scheduler.py`) is byte-unchanged (REUSE only — contracts EXECUTED the numstat; the ADR-0003-T2 0-shared-routine-edit invariant strengthened by a passing gate). INV-CORE-CAPABILITY untouched (pure ingest — the plan path stays wired, `--self-test` green). INV-ROLE-INLINING held (the 6 review agents + the blind-triage/verify/SE dispatches full-profile; the `enforce-role-inlining` hook satisfied first-time). The value-domain-grounding discipline held (the spo2 ×100 + the deferred absolute-vs-deviation skin-temp + the unmapped composites — security/bug/contracts/historical confirmed no wrong-scale value lands on a stream). Toward the vision.
- **Vision drift:** none, toward-vision. The system now ingests the operator's REAL wearable source format (Apple Health `export.xml`) end-to-end — the first real-data adapter, the bridge from synthetic to real. `design/vision.md` first sentence unchanged. No drift.

### PF attestation

S82 close (2026-06-20): **No new PF-class entries this session** — every finding was caught by the layered review or a mechanical guard BEFORE merge (nothing escaped). The notable observed-but-not-promoted item is a vacuity near-miss: the Tier-2 spo2 rounding test was IEEE-754-coincidentally-vacuous w.r.t. `round()`, caught by the independent Tier-3 test-coverage agent EXECUTING the round()-removal mutation (suite stayed GREEN), fixed with a non-terminating-mean test proven RED — the no-tautological-test discipline working as designed, a recurrence-watch (not an escape). Full attestation + the per-PR skill-trace table (PR #199) + the disclosure ledger in `memory/process-failures.md` Session 82.

**Historical (kept for reference):** `vault/sessions/session-81.md` (the `/generate-plan` S81 close).

## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-81 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; …; S80 archived S79; S81 archived S80; S82 archived S81) to keep this handoff lean. The current (S82) scope contract is above; the archive holds the prior-session archaeology.
...
1. **First real-data adapter is BUILT — the remaining V1 gap is the operator's ACTUAL data + the rest of ingest (PF-S63-02 satisfied).** S82 upgraded the Apple Health adapter to read the real `export.xml` (synthetic-fixture-validated); with `/generate-plan` (S81) + the closed loop (S70-S80) the machinery is complete. **Do NOT propose new pipeline mechanism, a dashboard REDESIGN (design-LOCKED), `/generate-plan` (DONE S81), the Apple Health adapter (DONE S82), or whoop-disable (rejected — coexistence) as next work.** The genuine next surface is the REST of operator-data ingest (the labs + 23andMe adapters — no parsers exist yet; the filled operator-profile/current-state scaffolds; all operator-gated PII → a gitignored local store, ADR-0005) + LM-01 prep. Before any secondary scope, the PF-S63-02 gate answer is "YES — wired AND invokable end-to-end on synthetic"; state WHY the secondary work is right in the scope contract.
2. **An adapter that translates a NEW source format must value-domain-ground its metric map AND prove every documented decision goes RED if removed (the S82 watch).** S82 mapped Apple's HK types to store items with the spo2 ×100 fraction→percent scale and DEFERRED skin-temp-dev (absolute-vs-deviation) — the wrong-scale class the prior wearable adapter's strain-guard exists to prevent. The Tier-2 vacuity audit proved its HEADLINE tests RED but a rounding test added for a documented sub-clause (`round(…,2)`) was IEEE-coincidentally-vacuous; the independent Tier-3 test-coverage agent caught it by EXECUTING the round()-removal mutation. **For a source-format adapter: every metric maps to a value-pinned test that REDs a transposed map; every documented numeric decision (scale, rounding, aggregation) gets a test proven RED by removing it — not just the headline capability; aggregation/granularity choices get a per-(key) independence test.** Caught at Tier-3, nothing escaped.
3. **The disciplines that HELD + standing mechanics.** (a) **Value-domain-ground every field new code emits** (PF-S78-01): the spo2 ×100, the deferred absolute-vs-deviation skin-temp, the unmapped composites — a wrong-scale value on a stream is a correctness defect. (b) **Store-surface battery** on every store read/write surface (here a READ adapter: cross-stream / dedupe-idempotent / distinct-days / same-identity-changed-value / distinct-source / fail-loud); **REUSE-only** for the shared routine (the ADR-0003-T2 0-edit invariant, EXECUTED-numstat gated). (c) **Paste FULL role profiles VERBATIM** — the 6 review agents + blind-triage/verify/SE dispatches; the `enforce-role-inlining` hook is satisfied first-time when the whole `~/Documents/Projects/skills_library/roles/<role>/agent.md` is pasted. (d) **Close on FINAL post-merge `main`** (`cvf1`): re-run pytest + `close-audit --session N` + cite the SHA. Standing mechanics: **bd is FIXED** (the S81-prep `bd init` resolved the repo-ID mismatch; bd works on a feature branch in the trunk; the worktree+park dance is the FALLBACK); **bd still auto-sweeps a cosmetic `issues.jsonl` re-encode into commits → restore to main's version + re-commit `--no-verify`**; review/merge off the **THREE-dot diff** (`main...branch`) not two-dot — a branch behind main shows PHANTOM wiki deletions (HIST-1); GraphQL EXHAUSTED → REST for PR-create/merge (`gh api`, full-40-char SHA guard); `git add` SEPARATE from `git commit`; the compound `git push && gh api` trips the block-dangerous parser → push as its own command; **two parallel wiki sessions push `origin/main` AND leave uncommitted `vault/` changes in the trunk — never touch `vault/library/` or their uncommitted `vault/` files (S82 ran the close in a clean worktree off post-merge `main` to avoid them)**; **close-step-8 freshness un-gated until `dfg1` — RUN step 8 each close**. Carry-overs: `6aen` / `cjgg`/`uerm`/`pwpr`/`sip9` / `cvf1` / `dfg1`/`df1x` / `aem1` / `axm7` / `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`/`5fw1`.

## Current State (volatile)

- **S82 (2026-06-20) upgraded the Apple Health (HealthKit) ingestion adapter to read the real `export.xml` (PR #199 → `main` @ `b83b12b`, rebase).** The first REAL-data adapter — the bridge from synthetic to the operator's actual wearable source format.
- **`scripts/ingest/adapters/healthkit.py` (UPGRADED):** streams the real Apple Health `export.xml` via `xml.etree.iterparse` (clearing the tree — the export can be 100s of MB), maps the value-domain-clean HK types to store items (`HeartRateVariabilitySDNN`→hrv, `RestingHeartRate`→rhr, `RespiratoryRate`→resp-rate as-is; `OxygenSaturation`→spo2 ×100 fraction→percent), and AGGREGATES Apple's raw per-sample `<Record>`s to the daily MEAN per (item, day) keyed on `startDate`'s date (rounded to 2 decimals — the granularity difference from the pre-aggregated wearable DB). recovery/strain (no Apple equivalent) + `skin-temp-dev` (Apple's absolute °C vs the store's deviation) + `sleep-efficiency` (a stage roll-up) are DEFERRED, value-domain-grounded. `source_tag` stays `"healthkit"`. Replaced the fabricated `json.loads({type,startDate,qty})` scaffold.
- **Coexistence, not replacement (CHANGED from the S82-plan):** the build did NOT flip whoop to `UNWIRED` — "Apple Health instead of Whoop" is achieved by providing a HealthKit export and no Whoop export (the scheduler runs only the adapters it has an export for); the device-specific `source` tag keeps the two distinct. The shared routine (`ingest.py`/`adapter.py`/`scheduler.py`) is BYTE-UNCHANGED (the ADR-0003-T2 0-shared-routine-edit invariant, EXECUTED-numstat gated).
- **ADR-0012** documents the streamed read + the daily-mean aggregation + the spo2 scale + the value-domain-grounded deferrals + the operator-owns-the-export license path. `tests/ingest/conftest.py` (NEW at review) single-sources the export test helper.
- **Review (PR #199):** Tier-2 plan-integrity INTEGRITY-CLEAN + QA (6 SHOULD-FIX coverage gaps fixed, vacuity audit PASS) → Tier-3 `/review-pr` 6-agent over the THREE-dot diff (security PASS [SEC-1 NOT_ACTIONABLE — XXE probes EXECUTED clean]; bug-hunter 0 findings [17 trigger inputs executed]; contracts "contracts met" [numstat EXECUTED]; historical 0 regressions; code-quality + test-coverage 4 LEGITIMATE fixed + blind-verified RESOLVED 4/4 incl. the TEST-001 round() vacuity gap; TEST-002 NOT_A_BUG) → `/merge` rebase.
- **No new PF this session** — every finding caught by the layered review or a mechanical guard before merge. Watch: a source-format adapter's metric map gets value-pinned tests AND every documented numeric decision gets a RED-if-removed test (the Tier-2 rounding test was IEEE-coincidentally-vacuous; Tier-3 caught it by mutation execution).
- **pytest 1160 passed / 3 skipped on final post-merge `main` (`b83b12b`, 2026-06-20 S82 close; the 3rd skip vs the trunk is the PII-config env gate, not a regression).** Core-capability gate green (untouched — pure ingest).
- **Beads:** `v3ml` (the Apple Health adapter) CLOSED. New `dfbi` (follow-on: map the deferred `skin-temp-dev` + `sleep-efficiency` + register the wearable-only store items in `biomarker_meta`). Carry-overs `cjgg`/`uerm`/`pwpr`/`sip9`/`cvf1`/`dfg1`/`df1x`/`aem1`/`axm7`/`6aen`/`0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`/`5fw1`.
- **Active landmarks:** LM-01 (First MD visit) **2026-07-13** — the 14-day scoped-drift-audit window opens **2026-06-29** (no trigger today). LM-02 (wearable baseline) is now Apple Health per ADR-0012 (no real data ingested → no trigger). LM-03 23andMe date-TBD.

**Historical (kept for reference):** `vault/sessions/session-82.md`.

## What Is Next (volatile)

### S83 — the rest of operator-data ingest (the Apple Health adapter is DONE) [operator-gated]

**The V1 machinery is COMPLETE** (`/generate-plan` + the plan→act→measure→adjust loop + the SBAR handout + face sheet) **and the first real-data adapter (Apple Health) is BUILT (S82).** What's DONE (do not re-propose): the closed loop, `/generate-plan` (S81), the Apple Health adapter (S82), the design PR #179 (merged), the dashboard (design-LOCKED). The remaining V1 surface is operator-gated:

1. **The rest of operator-data ingest.** The labs + 23andMe adapters (no parsers exist yet — only the ADR-0003 framework + now the Apple Health adapter; same scaffold→real pattern), the filled `operator-profile.md`/`current-state.md` scaffolds, the MD-visit outcome — all operator-gated (the PII boundary + ADR-0005 → a gitignored local store, never committed). Each new adapter: ADR-0003-T1 contract, 0 shared-routine edits (ADR-0003-T2), the store-adversarial battery, value-domain-grounded metric mapping + a RED-if-removed test per documented numeric decision (the S82 watch).
2. **Close the S82 follow-on** (the deferred `skin-temp-dev` absolute→deviation derivation + `sleep-efficiency` stage roll-up + registering the wearable-only store items in `biomarker_meta`) — bead created this close. Needs a per-operator temperature baseline (the Health app's job) for skin-temp-dev; not blocking.
3. **LM-01 (July 13 MD visit) prep** — the 14-day scoped-drift-audit window opens **2026-06-29**; the 7-day MD-handoff-GENERATION window opens **2026-07-06**. The SBAR render + face sheet + `/generate-plan` + now the wearable adapter are all BUILT — the remaining LM-01 gap is REAL operator data (an actual Apple Health export) + an MD-handoff artifact GENERATED from it.

**Discipline:** scope contract at open; bd is FIXED so a `feature/*`/`fix/*` branch in the main trunk commits cleanly (the worktree is the FALLBACK — S82 used it because two parallel wiki sessions held uncommitted `vault/` changes in the trunk); for any store surface, the store-surface battery; **at Tier-1, value-domain-ground every field new code emits (PF-S78-01) + every documented numeric decision (scale/round/aggregate) gets a RED-if-removed test (the S82 watch)**; for specialist-reasoning, a REAL specialist dispatch (full profile, paste it VERBATIM); for a visual artifact, Pencil + the design agents + operator sign-off FIRST (never solo); `/review-pr` (off the THREE-dot diff) → `/merge`; **re-run the close gate on the final post-merge `main` + cite the SHA (PF-S74-01)**.

**Deferred (NOT blocking):** `6aen` (worktree-path edit guard — PF-S79-01); `cjgg`/`uerm`/`pwpr`/`sip9` (value-domain + interaction grounding); `dfg1` (the doc-freshness gate — PF-S73-01 structural fix) + `df1x`; `cvf1` (the mechanical post-merge-`main` verify); `aem1` (the parenthetical-name interaction-match precision gap); `axm7` (extend the core-capability gate to the orchestrator); `5fw1` (the harvest-gate heading-regex fragility); the `llna` hook fix; carry-overs `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`; operator-gated July-visit prep (LM-01) — first session on/after **2026-06-29** runs the LM-01 scoped drift audit.

## Landmark window check (close step 8.7)

S82 close (2026-06-20): RE-OPENED `vault/meta/landmarks.md` at close step 8.7. **LM-02 edited** — its wearable source clarified to Apple Health per ADR-0012 (the operator uses Apple Health, not Whoop; the adapter is now built). No baseline data ingested → the "baseline-start + 30 days" trigger has NOT opened; the adapter is built on a SYNTHETIC `export.xml` fixture (no real operator data, no PII). LM-01 (First MD visit) **2026-07-13**: no trigger window open today (the 14-day scoped-drift-audit window opens **2026-06-29**, the 7-day MD-handoff window 2026-07-06). The remaining LM-01 gap is REAL operator data + an MD-handoff artifact GENERATED from it. LM-04 (first HTML artifact) NOT triggered — the renderers emit to the gitignored `vault/artifacts/generated/`. LM-03 (23andMe) date-TBD. No landmark ACTIONS due (only the LM-02 source clarification).
