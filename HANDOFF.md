...
### S75 Scope Contract Evaluation (2026-06-19, volatile)

- **AC1 (reconcile holds the declaring domain) — PASS.** `reconcile` holds a conflict-declaring domain via an INDEPENDENT `conflict_held` set (refined from a single-reason `holds` entry after the Tier-3 SEC-1/BUG-1 findings — a domain can carry both a `holds` reason AND a conflict, each cleared on its own). Detect+report preserved. Mutation-proven RED.
- **AC2 (generate_plans routes each via the gate) — PASS.** `generate_plans` adjudicates each `conflict_held` domain through the REUSED `adjudicate` gate (a `_conflict_safety_finding` → the S74 override-record schema + critical-non-overridable gate, `adjudicate.py` UNCHANGED); a content-valid override clears it; a domain records only when in NEITHER `holds` NOR `conflict_held`. No new store stream. Mutation-proven RED.
- **AC3 (real liaison E2E) — PASS.** A real deployed `medical-liaison` dispatch adjudicated a genuine author-declared cross-domain conflict (MEDIUM/H3, content-valid override) → the held supplement cleared + recorded; captured `liaison-conflict-cleared.example.json` (+ a `liaison-conflict-blocked.example.json` companion). The gate generalized to the conflict axis unchanged (the audit passes on the conflict envelope).
- **AC4 (backward-compat + integration mandate) — PASS.** No-adjudicator → a conflict HOLDS (the safe default); the 2 pre-existing conflict tests were UPDATED to pin the new behavior. Full suite 996/3, floor 15/0, `--self-test` + core-capability green.
- **AC5 (review + close) — PASS.** plan-integrity (MF-1 design-doc freshness — fixed) + QA (1 MUST + 4 SHOULD precedence/parity tests — fixed) Tier-2 → `/review-pr` 6-agent Tier-3 (2 MUST-FIX SAFETY bugs SEC-1/BUG-1 + SHOULD-FIX → all fixed, mutation-proven, blind-verified 5/5 RESOLVED) → `/merge`; full close, close gate re-run on final `main` + the `main` SHA cited (PF-S74-01 discipline).

### Drift checks (S75 close)

- **Task drift:** all 5 ACs PASS. In-scope additions beyond the literal WILL-touch: `vault/design/plan-generation-pipeline-v1.md` (the design spec the S75 scope contract DROPPED from WILL-touch — Tier-2 plan-integrity flagged it MF-1; the freshness obligation was lost between sessions, gate-caught + fixed). The hold model was REFINED to independent `conflict_held` mid-session in response to Tier-3 (flagged, not silent). No silent drift.
- **Architecture drift:** no invariant degraded; the S74 gate + its 2 invariants REUSED unchanged (`adjudicate.py` not in the diff). INV-CORE-CAPABILITY green; INV-BRANCH-NOT-MAIN held (built in `../aplus-s75cfaj` off `main`, idle trunk parked `parking/s75-cfaj`); INV-ROLE-INLINING held (real liaison + the 6 review agents + plan-integrity + QA full-profile — the enforce-role-inlining hook caught + corrected one QA-profile abbreviation). Toward the design (the conflict axis closed).
- **Vision drift:** none, toward-vision. The conflict axis now holds-and-adjudicates via the same terminal gate; the multi-concern hold model is correct (each concern clears independently). `design/vision.md` first sentence unchanged.

### PF attestation

S75 close (2026-06-19): **One new PF promoted — PF-S75-01** (Tier-1/Tier-2 self-review under-probed the new cfaj safety mechanism's INTERACTION surface; Tier-3 caught 2 safety-design holes — recurrence 2; all gate-caught before merge, fixed + mutation-proven + blind-verified 5/5; structural Tier-1-interaction-probe discipline beaded `sip9`). The build's other disciplines held — both cfaj behaviors mutation-proven RED; a REAL medical-liaison conflict dispatch verified the gate generalizes unchanged; the integration mandate honored (the 2 pre-existing conflict tests updated). The PF-S74-01 discipline applied (the full close re-run on the final post-merge `main` + the `main` SHA cited). Full attestation + the per-PR skill-trace table (PR #173) + the disclosure ledger in `memory/process-failures.md` Session 75.


## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-73 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; …; S73 archived S72; S74 archived S73; S75 archived S74) to keep this handoff lean. The current (S75) scope contract is above; the archive holds the prior-session archaeology.
...
1. **Adversarially self-probe a composable safety mechanism's INTERACTION surface at Tier-1 (PF-S75-01, recurrence 2).** Twice now (S74 routing-keys, S75 cfaj multi-concern) Tier-3 caught a SAFETY-design hole my Tier-1/Tier-2 self-check missed — both cross-feature INTERACTIONS I never adversarially probed (a domain held by additive-AE AND a distinct conflict → clearing one released the un-adjudicated other; a conflict on a bounce-success path). Before Tier-2/3, for any safety hold/gate that COMPOSES with existing ones: enumerate + test its interaction with EVERY existing hold/gate, the >1-concurrent-concern-on-one-entity case, and clear-one-while-another-open. **Model composable safety holds as MULTI-CONCERN from the start (a domain can have N independent reasons to be held), never single-reason-per-entity** (bead `sip9`).
2. **Run the close gate on the FINAL post-merge `main`, not the pre-merge worktree (PF-S74-01).** Every close re-runs pytest + `close-audit --session N` on `main` AFTER the merge+pull and the attestation CITES the post-merge `main` SHA (until `cvf1` lands the mechanical post-merge-verify). Never claim "fully gated"/"synced" before re-verifying the actual merged trunk; log an operator-surfaced catch AS operator-surfaced.
3. **Standing mechanics + carry-overs.** **Worktree discipline (PF-S70-01/S71-01):** separate `git worktree`s; park the idle trunk OFF `main`; Read worktree files at the worktree-absolute path; re-verify `git branch --show-current` before every commit; restore `main` at session end; **provision the worktree `.venv`**; **commit fixes PROMPTLY (`git checkout -- <file>` reverts an uncommitted fix to HEAD).** **Paste FULL role profiles verbatim — abbreviating one is denied by `enforce-role-inlining` (caught S75).** GraphQL EXHAUSTED → REST for PR-create/merge (`gh api`, full-40-char SHA guard). `git add` separate from `git commit` (PII hook). bd repo-ID mismatch + `.beads/daemon-error` + vault re-mangle benign — no backfill `bd sync`; commit `issues.jsonl` in a worktree (bd hooks skip there). Two parallel wiki sessions push to `origin/main` — rebase-sync at merge, never touch `vault/library/`. **Close-step-8 freshness un-gated until `dfg1` — RUN step 8 explicitly each close.** Carry-overs: `sip9` (Tier-1 interaction-probe, PF-S75-01) / `cvf1` (close-gate-on-final-`main`, PF-S74-01) / `dfg1`/`df1x` (doc-freshness gate + backlog) / `aem1` / `axm7` / `rxbp` (Rx/BPMH Phase-4 axis) / `0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`. Pencil: ALWAYS pass `filePath`.

## Current State (volatile)

- **S75 (2026-06-19) wired author-conflict adjudication (`cfaj`) — the Phase-4 follow-on that extends the held-line gate to the conflict axis.** `scripts/plan/orchestrate.py`: an author-declared cross-domain conflict (`report["conflicts"]`, was detect-only) HOLDS the declaring (`from`) domain in an INDEPENDENT `conflict_held` set and routes it through the REUSED S74 `adjudicate` gate (`adjudicate.py` unchanged — the gate generalizes); a content-valid override releases it; a domain records only when in NEITHER `holds` NOR `conflict_held` (so each concern clears on its own). Verified E2E over a real `medical-liaison` conflict dispatch (`liaison-conflict-{cleared,blocked}.example.json`).
- **PR #173** — Tier-2 plan-integrity (MF-1 design-doc freshness fixed) + QA (1 MUST + 4 SHOULD precedence/parity tests) → `/review-pr` 6-agent Tier-3 (**2 MUST-FIX SAFETY bugs** — SEC-1 additive-AE-clear releases an un-adjudicated coincident conflict; BUG-1 bounce-success stale conflict hold — fixed via the independent `conflict_held` refactor, mutation-proven RED, **blind-verified 5/5 RESOLVED**) → `/merge`. Skill-trace table in `memory/process-failures.md` Session 75. Single-PR close.
- **PF-S75-01 promoted** (Tier-1/Tier-2 self-review under-probed the cfaj safety mechanism's INTERACTION surface; recurrence 2 of safety-design-holes-reaching-Tier-3; all gate-caught before merge; structural Tier-1-interaction-probe discipline beaded `sip9`).
- **pytest 996/3 (worktree, native `.venv`).** Core-capability gate green; negative-test floor 15/0.
- **Beads:** `71s4` (cfaj WIRED; `rxbp` + the doctor-visit-queue artifact + the measure/adjust legs remain); `cfaj` CLOSED; `sip9` (PF-S75-01) + `cvf1` (PF-S74-01) + `mdv` CLOSED; carry-overs `dfg1`/`df1x`/`aem1`/`axm7`/`rxbp`/`llna`/`0qf6`/`lczu`/`9etx`/`p5wx`/`7may`/`po4x`.
...
**Historical (kept for reference):** `vault/sessions/session-75.md`.

## What Is Next (volatile)

### S76 — the remaining Phase-4 surfaces + the measure/adjust legs

The additive-AE + conflict axes both close to operator-usable via the terminal gate. Next, REUSING the S74 gate (the override-record validator + the critical-non-overridable gate + the `safety_finding` envelope; the multi-concern hold model now established), per `vault/design/plan-generation-pipeline-v1.md` Phase 4 — in a dedicated `git worktree`, idle trunk off `main`, with a provisioned `.venv`:

1. **The supplement↔Rx BPMH axis** (`rxbp`) — the liaison's marquee watchlist check (warfarin/levothyroxine/CYP3A4/antithrombotic-stacking). Adds the operator medication-list read THROUGH the `router.summarize` PII de-identification boundary (ADR-0001) — a substrate surface that EARNS its own adversarial PII-safety + E2E verification. Reuses the gate; the hold composes with the existing holds (apply the PF-S75-01 interaction-probe).
2. **The doctor-visit-queue collation + SBAR handout artifact** — the medical-liaison's owned queue surface (a substrate write — satisfy `docs/checklists/store-adversarial-tests.md`).
3. Then the **measure** leg (a production caller for `record_plan_tracking` — currently schema-only) + the **adjust** leg (re-plan from tracking) — the rest of the closed loop.

**Discipline:** scope contract at open (LIST the design spec in WILL-touch — MF-1); **worktree + idle trunk off `main` + provision the `.venv`**; **adversarially self-probe any new composable safety hold's interaction surface at Tier-1 (PF-S75-01)**; **write close artifacts in-worktree BEFORE merge**; **commit fixes promptly**; **paste FULL role profiles verbatim**; **verify the production path E2E with a real `medical-liaison` dispatch**; `/review-pr` → `/merge`; **re-run the close gate on the final post-merge `main` + cite the `main` SHA (PF-S74-01)**.
