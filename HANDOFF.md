...
## Scope Contract — Session 89 (2026-06-22)

Goal: Set up everything the next session needs to run the conversational-intake BUILD as a strict autonomous `/execute-plan` loop — produce the build-planning pipeline (spec → build-plan → 8 task-plan recipes) grounded in the accepted ADR set (0015–0019) + the design, document the runtime keychain-key retrieval, and write the strict-loop directive — WITHOUT spending on the live API (mock/fixture-testable; the live end-to-end run is the next session's operator-present checkpoint). Then close. (AUTONOMOUS per the operator's standing directive; self-confirmed.)

Acceptance criteria:
- [ ] AC1 (spec): a build spec at `docs/spec/adr-0015-0019-conversational-intake-spec.md` — ADRs 0015–0019 + the design → buildable FRs/NFRs/binary ACs, grounded against the live tree, carrying the 6 deferred design-stage requirements (per-token coarseness proof / `identity_config` wiring / CONCERN-1/2 / M-3 / the crown-jewel PII NFR / the PF-S87-01 end-to-end bar).
- [ ] AC2 (build-plan): `docs/build-plan/build-plan-conversational-intake.md` — dependency-ordered waves + EXECUTABLE per-wave checkpoint Go/No-Go gates + 3-tier review assignments; the live-API run is the final operator-present checkpoint.
- [ ] AC3 (task-plan): the 8 per-task TDD recipes under `docs/task-plan/` (ADR-0015-T1/T2/T3, 0016-T1, 0017-T1/T2, 0018-T1, 0019-T1), failing-capable (not tautological), store-surface tasks carry the adversarial battery, model-client tasks test against a MOCK (no live API).
- [ ] AC4 (key access): the keychain key-retrieval documented for the build loop (runtime fetch, never tracked — PUBLIC repo); the exact keychain item NAME is the one operator-confirm item.
- [ ] AC5 (strict-loop directive): the HANDOFF "What Is Next" set to the S90 strict-autonomous `/execute-plan` build-loop directive.
- [ ] AC6 (close): full S89 close + close-audit on FINAL post-merge `main` + cite the SHA (PF-S74-01).

Files I WILL touch: `docs/spec/`, `docs/build-plan/`, `docs/task-plan/` (the planning artifacts); the close docs (HANDOFF / `memory/process-failures.md` / `vault/sessions/session-89.md` / `vault/meta/overview.md` / scope-contract-archive).
Files I will NOT touch: the operator's MAIN-checkout vault WIP (`stash@{0}` + the 3 `bpc-157` files — never destructively); ANY `scripts/` production code (the build is the next session's loop); `main` directly; the keychain (beyond documenting the retrieval — NEVER store/print/commit the key).
NOT doing: building the conversational intake agent (the S90 strict `/execute-plan` loop); any live API call (no spend this session); plan-integrity-grounding the recipes (the build loop runs `plan-integrity` at wave-start per V1 Build Execution).
Invariants at risk: INV-CORE-CAPABILITY (the planning advances toward the first model client; no `scripts/` change, the wired path stays green); INV-CLOSE-AUDIT / INV-PF-ATTESTATION / INV-SKILL-TRACE / INV-HARVEST-CAPTURE (close gates); INV-BRANCH-NOT-MAIN.

**Core-capability-first gate (PF-S63-02):** does the core capability work yet? **NO** (no model client; the build is S90). This session is NOT secondary work — it is the build-PLANNING for the first model integration (the direct path to the core). The gate is satisfied.

**S89 self-confirmation (autonomous):** contract self-confirmed per the operator's directive ("do everything we can before spending on the API; set up everything to run the strict autonomous loop; do it next session; then run the full close"). The planning is fresh-dispatched (quality), no API spend; the live build is the next session's loop.

### S89 Scope Contract Evaluation (2026-06-22, volatile)

- **AC1 (spec) — PASS.** `docs/spec/adr-0015-0019-conversational-intake-spec.md` — 8 tasks / 2 waves, FRs + 5 NFRs, all 6 deferred design-stage requirements as explicit ACs, grounded against the live tree; surfaced + ACd a real in-scope defect (the `equipment-access-class` double-source collision the demographic activation must reconcile).
- **AC2 (build-plan) — PASS.** `docs/build-plan/build-plan-conversational-intake.md` — Wave A (model boundary + conversation path, 5 tasks) → Wave B (form activation + token vocab + end-to-end, 3 tasks), executable checkpoint Go/No-Go gates, 3-tier review assignments; the live-API run is the final operator-present checkpoint.
- **AC3 (task-plan) — PASS.** 8 TDD recipes under `docs/task-plan/` (ADR-0015-T1/T2/T3, 0016-T1, 0017-T1/T2, 0018-T1, 0019-T1), failing-capable + negative-controlled, store-surface tasks carry the adversarial battery, model-client tasks test against a MOCK (no live API).
- **AC4 (key access) — PARTIAL at close, RESOLVED post-close.** The runtime-fetch discipline was documented (never tracked — PUBLIC repo); the exact keychain item NAME (`quant-primary-api`, the operator's Anthropic key — no-train by default = threat-model B) was confirmed by the operator in the follow-up immediately after the close. Design: the model client reads `ANTHROPIC_API_KEY` from ENV; the OPERATOR injects it at the live run (`export ANTHROPIC_API_KEY=$(security find-generic-password -s "quant-primary-api" -w)`) — the agent never touches the keychain (the credential guard blocks it).
- **AC5 (strict-loop directive) — PASS.** "What Is Next" set to the S90 strict-autonomous `/execute-plan` build-loop directive.
- **AC6 (close) — PASS** (this close; close-audit + harvest-gate green on final post-merge `main`, SHA cited).

### Drift checks (S89 close)

- **Task drift:** None. AC1–AC3 + AC5–AC6 PASS; AC4 PARTIAL (the keychain item name is the one operator-confirm item — the broad scan was guard-blocked, correctly; flagged, not silent). No silent drift.
- **Architecture drift:** None. Planning only — no `scripts/` code changed; the wired core-capability path stays green (core-capability-audit passes). The plan ADVANCES toward the first model client (the build is S90).
- **Vision drift:** None. The planning realizes the (already-updated) conversational vision; no new vision change.

### PF attestation

S89 close (2026-06-22): No new PF-class entries this session. The build-planning pipeline (spec → build-plan → 8 task-plan recipes) was produced by FRESH-context sub-agents (to keep quality high in a long conversation) and self-validated against the live tree; the recipes carry the 6 deferred design-stage requirements as failing-capable ACs (the per-token coarseness proof correctly uses an independent output scan, NOT the 8j6 gate). The keychain broad-scan was guard-blocked — the correct outcome (a credential-exploration guard firing as designed); I asked for the specific item name instead. Full attestation + skill-trace + disclosure ledger in `memory/process-failures.md` Session 89. Prior lessons referenced DESCRIPTIVELY (the verify-first discipline; the never-hand-roll-the-build via `/execute-plan` discipline; the secret-never-tracked posture on the public repo; close on final post-merge main + cite the SHA), never by bare token.

**Historical (kept for reference):** `vault/sessions/session-89.md`.

## Historical Scope Contracts (archived)

Scope contracts for Sessions 5-88 + their evaluations were moved to `vault/sessions/scope-contract-archive.md` (S32 archived the S5-31 set; …; S86 archived S85; S87 archived S86; S88 archived S87; S89 archived S88) to keep this handoff lean. The current (S89) scope contract is above; the archive holds the prior-session archaeology.
...
1. **The conversational-intake BUILD is fully PLANNED + ready; S90 runs the strict autonomous `/execute-plan` loop.** The spec (`docs/spec/adr-0015-0019-conversational-intake-spec.md`), build-plan (`docs/build-plan/build-plan-conversational-intake.md`), and 8 task-plan recipes (`docs/task-plan/ADR-001{5,6,7,8,9}-T*.md`) are on `main` (merged at the S89 close). S90 = `/execute-plan` Wave A → checkpoint → Wave B → checkpoint, then the LIVE end-to-end run (a real conversation → a usable plan, PF-S87-01) using the keychain key. **Build/mock-test everything first (no API spend); the live run is the final operator-present checkpoint** (the operator's directive: minimize spend).
2. **The API key (RESOLVED) + the surface-for-review items.** The build's model client reads the Anthropic key from the ENV (`ANTHROPIC_API_KEY`); at the live-run step the OPERATOR injects it from the keychain — `export ANTHROPIC_API_KEY=$(security find-generic-password -s "quant-primary-api" -w)` (the operator's action; the agent NEVER touches the keychain — the credential guard blocks it). `quant-primary-api` is the operator's Anthropic API key (no-train by default = threat-model B; ~30-day retention; not ZDR). NEVER store/print/commit the key value (PUBLIC repo). Surface for operator review before the live build: the autonomously-added **ADR-0019** + the **RT-01 fail-closed contract** + the **Claude no-train default** (ADR-0015). Per PF-S87-01, the build's headline acceptance is a USABLE plan from the conversation, not just chat plumbing.
3. **Standing mechanics + the operator's data state.** Build via **`/execute-plan`** (recipes ready — never hand-rolled, PF-S36-01); the build's 3-tier review (Tier-1 SE → Tier-2 QA+Architect+Security → Tier-3 `/review-pr`) + the **`plan-integrity`** role grounds the build-plan at wave-start; store-surface tasks satisfy `docs/checklists/store-adversarial-tests.md`; full role profiles inlined VERBATIM; `/merge` (REST fallback when GraphQL exhausted, full-40-char SHA guard); `git add` SEPARATE from `git commit`; **the `block-commit-main` worktree false-block → PARK the main checkout on a same-commit branch (WIP carries), commit in the worktree, un-park** (PF-S71-01; used 3× this conversation, WIP verified identical each time); re-run the close gate on final post-merge `main` + cite the SHA (PF-S74-01). **The operator's MAIN checkout is on `main` @ `dfd44e8` (2026-06-22 S89 close), behind origin — S90 syncs at start;** their parallel-wiki WIP is in `stash@{0}` + 3 `bpc-157` files (their call — never touch destructively). The basic-memory daemon `permalink:` churn is benign.

## Current State (volatile)

- **S89 (2026-06-22): the conversational-intake BUILD is fully PLANNED.** With the S88 foundation (ADRs 0015–0019 + design) merged, S89 produced the build-planning pipeline so S90 is a pure strict `/execute-plan` loop. No `scripts/` code changed; no API spend.
- **The planning artifacts (on `main`, S89 close):** the spec (`docs/spec/adr-0015-0019-conversational-intake-spec.md` — 8 tasks/2 waves, FRs+NFRs, the 6 deferred requirements as ACs); the build-plan (`docs/build-plan/build-plan-conversational-intake.md` — Wave A model-boundary+conversation, Wave B form-activation+tokens+end-to-end, executable checkpoints, the live-API run as the final operator-present checkpoint); the 8 TDD recipes (`docs/task-plan/`). All FRESH-dispatched (clean context → high quality in a long conversation) + self-validated against the live tree; failing-capable tests + negative controls; the per-token coarseness proof uses an independent output scan (NOT the 8j6 gate, which doesn't run on the derived path); model-client tasks mock-tested (no live API).
- **The API key (resolved post-close):** the operator's Anthropic key is the keychain item `quant-primary-api` (no-train by default = threat-model B). The build's model client reads `ANTHROPIC_API_KEY` from the ENV; at the live run the operator injects it (`export ANTHROPIC_API_KEY=$(security find-generic-password -s "quant-primary-api" -w)`) — the agent never touches the keychain (the credential guard blocks it). The key value is never tracked/printed (PUBLIC repo).
- **pytest 1264 passed / 3 skipped on `main` (2026-06-22 S89 close).** No code changed (planning only); the core-capability path stays wired.
- **No new PF.** The planning was fresh-dispatched + self-validated; the keychain credential-exploration guard fired as designed.
- **Beads:** epic `w3y8` advanced (the build is fully planned). `bd` operations still DEFERRED (the worktree-`bd` flush + the operator's WIP `.beads` — S90 creates build beads against the synced main trunk).

**Historical (kept for reference):** `vault/sessions/session-89.md`.

## What Is Next (volatile)

### S90 — the strict autonomous `/execute-plan` build loop

Everything is set up. S90 runs the conversational-intake BUILD as a strict autonomous loop. The planning is DONE + on `main`: the spec (`docs/spec/adr-0015-0019-conversational-intake-spec.md`), the build-plan (`docs/build-plan/build-plan-conversational-intake.md`), and the 8 task-plan recipes (`docs/task-plan/ADR-001{5,6,7,8,9}-T*.md`). Read `/execute-plan` IN FULL first (PF-S17-01).

**Sequence (the strict loop):**
1. Sync the trunk; `plan-integrity`-ground the build-plan against the live tree (per V1 Build Execution) before Wave A.
2. **`/execute-plan` Wave A** (the model client + question strategy + plan-author rewire + `/chat` seam + extractor) → the Wave-A checkpoint Go/No-Go (EXECUTED: fail-closed on all 3 consumers; loopback + single-egress-class; gate-at-write [extractor reaches the store only via `persist_capture`]; H-2 `identity_config` threaded at BOTH call sites; 0-shared-routine-edit numstat; `core-capability-audit --self-test` green on the mock author path) → Tier-2 wave review (QA + Architect + Security) → Tier-3 `/review-pr` → `/merge`.
3. **`/execute-plan` Wave B** (demographic activation [reconcile the `equipment-access-class` collision] + `SUMMARY_FIELD_SET` extension [per-token coarseness proof via an INDEPENDENT output scan — the 8j6 gate does NOT run on the derived path] + the end-to-end gate) → the Wave-B checkpoint → 3-tier review → `/merge`.
4. **The LIVE end-to-end run** (the final, operator-present checkpoint): the build's model client reads the key from the ENV (`ANTHROPIC_API_KEY`, the standard SDK pattern — never a tracked file). The OPERATOR injects it from the keychain in one command (the agent NEVER touches the keychain — the credential-exploration guard blocks agent keychain reads, confirmed 3× this conversation): `export ANTHROPIC_API_KEY=$(security find-generic-password -s "quant-primary-api" -w)`. `quant-primary-api` is the operator's Anthropic API key — **no-train by default** (= the threat-model-B posture the ADRs assume; bounded ~30-day retention, not ZDR; the operator can confirm in the Anthropic Console). Then drive a real conversation → a real plan. **Per PF-S87-01 the acceptance is a USABLE plan, not just working chat plumbing.** Everything before this is mock/fixture-tested (no API spend — the operator's directive). NEVER store/print/commit the key value (PUBLIC repo).
5. Close each wave (a wave = a session boundary).

**Surface for operator review before the live build:** the autonomously-added ADR-0019 + the RT-01 fail-closed contract + the Claude no-train default — the operator can course-correct first.

**Deferred / NOT blocking:** the operator's `bpc-157` WIP reconcile (their call); the local-model migration (North Star, post-V1); the S86 Wave-B follow-up beads (moot under the pivot).

## Landmark window check (close step 8.7)

S89 close (2026-06-22): RE-OPENED `vault/meta/landmarks.md` at close step 8.7 (unchanged this session). No landmark edited — S89 was PII-free planning work (spec/build-plan/recipes; no operator data; no live API). LM-01 (First MD visit) **2026-07-13**: the 14-day scoped-drift-audit window opens **2026-06-29** (7 days out), the 7-day MD-handoff window 2026-07-06. **NOTE for S90: if S90 runs on/after 2026-06-29, run the scoped drift audit on the LM-01 relevant_scopes.** LM-02 (Apple Health) / LM-03 (23andMe): no real data ingested → not triggered. LM-04 (first HTML artifact): not triggered. No landmark ACTIONS due.
