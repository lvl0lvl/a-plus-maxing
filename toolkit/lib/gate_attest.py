...
# Project-agnostic engine. The attestation MECHANICS (iteration freshness, the
# fenced-json scaffold, the attestation chain, verify-chain) are domain-neutral;
# each PROJECT supplies its own gate set — the schema directory, the attested-gate
# phase names, and the per-phase agent-source file — via config. Resolution order:
#   $GATE_ATTEST_CONFIG (a JSON path; keys: schema_dir, attested_gates, source_md)
#   then the built-in DEFAULTS below (a worked example — the a-plus-maxing
#   health-research gate set), with $GATE_ATTEST_SCHEMA_DIR honored for schema_dir.
# A new project drops its gate-*.schema.json files in a dir and points
# GATE_ATTEST_CONFIG at {schema_dir, attested_gates:[...], source_md:{phase:relpath}}.
_DEFAULT_ATTESTED_GATES = ("3.5", "4.25", "4.75", "6", "7.5", "8.5")
_DEFAULT_SOURCE_MD = {
...
def _load_gate_config():
    cfg = {}
    cfg_path = os.environ.get("GATE_ATTEST_CONFIG")
    if cfg_path and pathlib.Path(cfg_path).exists():
        cfg = json.loads(pathlib.Path(cfg_path).read_text())
    schema_dir = pathlib.Path(
        cfg.get("schema_dir")
        or os.environ.get("GATE_ATTEST_SCHEMA_DIR")
        or (pathlib.Path(__file__).resolve().parent.parent / "schemas")
    )
    attested = tuple(cfg.get("attested_gates", _DEFAULT_ATTESTED_GATES))
    source_md = cfg.get("source_md", _DEFAULT_SOURCE_MD)
    return schema_dir, attested, source_md


SCHEMA_DIR, ATTESTED_GATES, SOURCE_MD = _load_gate_config()
