...
## Pre-Flight Dependencies (run before Phase 1; HALT on any miss)

The skill is inert without its runtime. Verify these at session start — a fresh
clone or a different machine may lack them, and a silent miss fails mid-pipeline:

```bash
# (1) deep-research skill resolves (this skill WRAPS it — Hard Rule 8)
DR=~/.claude/skills/deep-research
[ -e "$DR/SKILL.md" ] || { echo "HALT deep-research-missing: install/repair ~/.claude/skills/deep-research before running"; exit 1; }
# (a broken symlink passes -e=false; -e on the *target* file catches that — AR-1)

# (2) gate_attest's python deps (the attestation chain is non-negotiable — Hard Rule 9)
python3 -c 'import jsonschema' 2>/dev/null || { echo "HALT jsonschema-missing: python3 -m pip install jsonschema"; exit 1; }
```

**MCP / tool substitution (AR-2).** This skill's `allowed-tools` names `mcp__tavily__*`
(web retrieval) and `mcp__basic-memory__*` (wiki write). If those MCP servers are
NOT connected in the running session, the skill is still runnable with substitutes:
`WebSearch` + `WebFetch` for retrieval, and direct `Write`/`Edit` to the vault path
for wiki notes. Substitutes preserve every gate (retrieval/judge/integrity logic is
tool-agnostic); only the transport changes. Note the substitution in the run log.
...
```json
{
  "phase": "2.75",
  "verdict": "PASS|HALT",
  "timestamp": "2026-06-18T00:00:00Z",       // REQUIRED — ISO-8601 date-time (schema: format date-time)
  "context_files": [{"path": "...", "sha256": "...", "loaded": true}, ...],
  "target": {"class": "...", "slug": "...", "type": "..."},
  "output_paths": {"research_report": "...", "compound_entry": "..."},
  "update_mode": false,
  // "update_reason_slug": "supersede-fabricated-cites"  // REQUIRED IFF update_mode=true; pattern ^[a-z0-9][a-z0-9-]{0,40}$
  "archive_paths": [],
  "halt_reasons": []
}
```
