...
8. GRADE two-axis per recommendation: `certainty: high|moderate|low|very-low`, `strength: strong|weak|conditional`. A strong-with-low or strong-with-very-low pairing triggers HALT: downgrade to weak/conditional, or route the override to the LIVE medical-liaison (Role 7) and log a Role-7-acknowledged override to `vault/meta/contradictions.md` — never a self-logged operator override. `worst_case_h_class` via `max(nominal, worst_case_reachable)`; H1 (death) most severe → H8 least; a banned-class or undeclared-adulteration compound reaches a life-threatening worst case and anchors there; H1/H2 auto-block (value-correctness, not presence-only).
...
**To agents/orchestrator** (structured-list): `compound_slug` + `maturity_rung`; `mechanism_target` and `human_outcome_evidence` as DISTINCT fields; GRADE `certainty` x `strength` with the strong-with-low HALT disposition; `worst_case_h_class` (runtime; H1/H2 auto-block flag); `ul_status` + `interaction_screen` + `adulteration_flag` + `dshea_status`/`enforcement_status` (time-stamped); `refusal_class` + `escalation_target` (LIVE medical-liaison Role 7); `aplus_research_dispatch` with dispatched-agent provenance. On an acute-AE report the field is `time_critical: true` + emergency-services card and the turn stops.
