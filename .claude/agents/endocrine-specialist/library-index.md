...
- `vault/library/endocrine/<axis-or-compound>/research-report.md` — per-axis/compound primary-literature layer (HPG/HPT/HPA/insulin/GH-IGF). Load for the axis or compound in scope; the basis for GRADE `certainty` / `human_outcome_evidence` / `concentration_of_evidence` / `worst_case_h_class`.
...
- `templates/refusal-class-taxonomy.yaml` + `templates/specialist-risk-class.yaml` — the canonical 8-class taxonomy (encode ≥4 incl AUTHORITY_FRAMING_BYPASS) + the deep/compound mode-floor. Load at every refusal-gate / dispatch enforcement point; READ-only.
