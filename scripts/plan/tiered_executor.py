...
Store surface: the executor's ONLY store write is the EXISTING ``plan_confirm.mark_pending`` on the
Tier-4 (and F2-default) path — it rides the frozen ``plan-confirm::`` stream under keying's unchanged
...
def _implicated_tiers(domain, entries, material):
    ...
    rule escalates ONLY on a material observation (an immaterial un-certified rule does nothing — no
    unsafe auto-apply) and, positively pinning no tier, contributes the Tier-4 fail-closed-UP DEFAULT.
    ...
    Args:
        ...
        material (bool): Whether the observation reads at/above the signal's materiality_threshold.
    ...
    for entry in entries:
        ...
        if not safety_gate and not cross_domain and material:
            # an in-domain un-certified rule (non-monotone / under-specified) on a material
            # observation: no tier positively implicated -> the Tier-4 fail-closed-UP DEFAULT (F2).
            tiers.add(TIER_4)
...
def execute_monitoring_day(config, observations, raw_intake, deid_client, dispatch, store_read, root,
    ...
    fired rules pin no tier (e.g. an immaterial un-certified rule) is inert (no routing). The
    executor's ONLY store write is the Tier-4 / F2-default ``mark_pending`` (the existing
    ``plan-confirm::`` sink; no new key/stream).
    ...
    for domain, domain_config in config.items():
        signals = {
            signal[monitoring_compiler.SIGNAL_NAME]: signal
        ...
        for signal_name, entries in entries_by_signal.items():
            ...
            tiers, cross_targets = _implicated_tiers(domain, entries, material)
            if not tiers:
                continue  # nothing implicated (an immaterial un-certified rule does nothing)
