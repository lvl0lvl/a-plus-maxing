...
# domain -> translator(recommendations, gates) -> plan dict | None. A new plan-domain
# author wires in by adding its translator here (the process generalizes; workout is first).
_PLAN_TRANSLATORS = {
...
def generate_plan(domain, author_output, store_read, root, *, plan_date, gates=None):
    ...
    Args:
        ...
        gates (dict, optional): Per-domain safety gate inputs (e.g. `clearance_granted`
            for workout). Defaults to all-conservative.

    Returns:
        (dict) A result record: `domain`, `specialist`, `recorded` (bool), `plan`
        (dict | None), `section` (the assembled section), `reason` (str | None — the
        coverage-gap kind or `no-actionable-recommendation` when nothing was recorded).
