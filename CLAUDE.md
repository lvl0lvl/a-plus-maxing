...
6. **Run test baseline** -- `.venv/bin/python -m pytest -q` (the V1 Python suite, since S31/Wave 2; `.venv` is the per-instance Python 3.14 runtime, gitignored — its deps are tracked in `requirements.txt` (pytest + jsonschema): provision a fresh clone with `python3 -m venv .venv && .venv/bin/python -m pip install -r requirements.txt`). If tests fail before you changed anything, fix that first. Non-Python sessions: the shell audit suites under `scripts/tests/*.sh` are the baseline for governance/tooling work.
...
Before adopting any secondary-work scope, answer in writing: **does the core capability — generating a followable health plan, end-to-end — work yet?** (As of S95: the BUILD is COMPLETE + mock/fixture-tested — the live no-train de-id backend, the subscription specialist/lens dispatch, the composed fail-closed `gate_dispatch`, the shared no-fork `plan_driver.drive`, and the `/generate-plan` front door are all wired author→de-id→dispatch→assemble→gate→driver→render (live-wiring PRs #247/#248/#250 → `main`; bead `71s4` closed S80). The ONLY remaining step is the operator-present LIVE run — real key + real data + spend — which is operator-gated, not a build task.) Secondary work (substrate, polish, adapters, governance) does NOT proceed on the strength of "the components each pass their own tests" — that is the exact PF-S63-02 drift. If a session's goal is legitimately secondary, the scope contract states the core-capability answer explicitly and why the secondary work is the right next step anyway. The MECHANICAL form of this gate (`core-capability-audit.sh`, a script + an A′-inversion `--self-test` asserting the A′ spine stays wired non-tautologically) LANDED with the S95 live-wiring repoint (ADR-0026-T4). **Guard-loosening corollary (PF-S63-02):** treat any recommendation to LOOSEN a drift-guard as itself a red flag — name the failure class the guard prevents and obtain explicit informed sign-off before acting; never default-accept.
...
8.5. **Run the close gate** -- `scripts/close-audit.sh --session <N>` is the single mandatory close gate (Rigor Framework v1.0.0 Discipline 1 step 7 / F-008 fail-closed; adopted S64). It runs the negative-test FLOOR first — `toolkit/tests/run-all-tests.sh` + `scripts/tests/run-all-tests.sh`, proving each audit can still go RED on bad input (F-007) — then the per-close roster below, FATALing (exit 2) if any constituent could not RUN (a skipped check never reads as clean). It additionally runs a NON-GATING `falsification-scan` advisory (vendored `toolkit/scripts/falsification-scan.sh`, Wave B / S65) over THIS session's `## Session <N>` PF note — WARN-only, never affecting the gate verdict; review any surfaced anti-pattern (increment-by-default / framework-circularity / a-priori-by-construction / soft-confirmation-laundering) and confirm or dismiss it in the close attestation. On any non-zero exit: do NOT commit until fixed or explicit user-adjudicated path-extension granted. Roster (also run `branch-completeness-audit.sh` at session OPEN too):
   ...
   - `scripts/core-capability-audit.sh` — INV-CORE-CAPABILITY (PF-S63-02 mechanical guard: the plan-generation path stays wired author→`assemble`→`record_plan`→rendered plan; structural greps + the `--self-test` wired-path E2E under `.venv`)

   The gate carries NO default floor exclusion as of S65 (the former tracked red, `test_audit_research_provenance.sh`, was FIXED by bead `a-plus-maxing-d1kc`: `jsonschema` installed into `.venv` + the provenance audit repointed at the `.venv` python; the full floor is green). The `CLOSE_AUDIT_FLOOR_EXCLUDE` override remains for any FUTURE tracked pre-existing red, printed LOUDLY (a stale exclusion fails the run). Conditional: for any aplus-research dispatch this session run `scripts/audit-research-provenance.sh <design-work-dir> <slug>` (bda — asserts the mode-required attested gates are PRESENT, then runs `gate_attest.py verify-chain`; closes the vacuous-pass hole where verify-chain alone skips ABSENT gates). This is the INV-RESEARCH-PROVENANCE-DISJOINT enforcement and is mandatory before any specialist's research feeds a wiki write.
...
- **What a-plus adopted (S65, Waves B+C):** the `falsification-scan` advisory (non-gating, close step 8.5), the approaches ledger (`vault/approaches/`, F-014) + the formalized disclosure ledger + the core-capability-first forcing function, the `plan-integrity` role wired into V1 Build Execution, and the `INV-CLOSE-AUDIT` + `INV-HARVEST-CAPTURE` register rows (change-discipline ritual). `d1kc` fixed; `po4x` a-plus-side template landed.
- **What a-plus adopted (S118, rigor `1.20.0` pull — operator-directed):** the versioned toolkit re-copy (10→26 vendored scripts; pin `1.0.0`→`1.20.0`; `docs/rigor-adoption-log.md` logs the run) + 3 consumer wirings. **roster-select:** `toolkit/scripts/roster-select.sh` is the canonical owner of the review-roster rule (1.18.0); classify EVERY review with it (`--files …` or `--base/--head`), paste its one-line `roster=…` output into the review header, and WIDEN-never-narrow below its verdict (on exit 2, widen to full-6 or halt). a-plus has no roster carve-outs → no `.rigor/roster-patterns.local`. **resume-claims-audit:** run `toolkit/scripts/resume-claims-audit.sh HANDOFF.md` at session open + before shipping a handoff; the HANDOFF `## Resume — S{N} (VOLATILE)` region carries one machine-checkable claim per sentence (backticked `project-id`+status; `main @ <sha>`; `landed in #N`; the `Stamped` line). ADVISORY until one full session cycle passes with zero FAIL, then gate per upstream bead `bud`. **render-before-claiming:** the global `/execute-plan` DESIGN GATE now render-attests changed UI screens under `toolkit/scripts/run-attest.sh` — NO project edit; applies the next time a UI recipe runs. Tracked follow-ons: `ffit` (the library-tree-scoped `test-roster-select.sh` floor red — the upstream layout-agnostic fix), `23q5` (the durable a-plus-side close-audit toolkit-floor exclusion for that red — the vendored runner ignores `RUN_ALL_TESTS_EXCLUDE`), `ijd0` (the vendored-but-unwired toolkit scripts to assess per a-plus conventions).
- **Deferred (beaded follow-ons):** the EXTERNAL/coupled tail only — global `~/.claude/skills`+`commands` sync (Wave D, `ckl1`) + wiring `enforce-heartbeat-clause` (`0qf6`, couples with Wave D); the core-capability MECHANICAL gate (with `71s4`); the upstream toolkit `po4x` fix; `p5wx` (init_instance auto-provision) / `7may` (provenance-audit interpolation hardening).
