...
- `rigor_version: 1.0.0` — pinned S64. The Rigor Framework + its runnable `toolkit/` is a versioned dependency (`~/Documents/Projects/skills_library/frameworks/rigor/`), not a copied snapshot. The toolkit is **vendored** at `toolkit/` (copied, not symlinked — it is the runtime enforcement floor and must survive clone/move). Do NOT edit the vendored scripts: they ship with their negative tests, and edits fork the pull line.
- **Pull cadence (Discipline 11 / Loop B):** at the periodic drift-audit boundary, diff `rigor_version` against `frameworks/rigor/VERSION`, read the `CHANGELOG.md` delta, and pull — re-running `toolkit/tests/run-all-tests.sh` (a new audit adopts only if its negative test passes, F-007) before adopting.
