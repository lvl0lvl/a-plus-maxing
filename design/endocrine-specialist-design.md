...
|---|---|---|---|---|
| Specialist-profile audit | full profile shape: ≤40-word Identity, ≥4 refusal-class tokens incl AUTHORITY_FRAMING_BYPASS, mode-floor (deep/compound), voice-register, negative-examples, PF-resolution, section count, ≤200 lines. NOTE: `--check refusal-classes` COUNTS class-shaped tokens and resolves taxonomy MEMBERSHIP only when the taxonomy file is present at audit time; the AUTHORITY_FRAMING_BYPASS-mandatory + AAS-clause membership assertions remain PROPOSED (§18). | `scripts/audit-specialist-profile.sh` (path resolves; incl `--check refusal-classes`, `--check mode-floor-correctness`, `--check aplus-mode-floor`) | LIVE | BLOCK |
...
4. **EC-OPTIMIZATION-WITHOUT-INDICATION (PROPOSED).** No check verifies the body refuses optimization-without-indication + targets mid-range IGF-1 (F17). Positioned to answer: Role 2. Informs §11.2 #3 (and §15.2 #1's all-8/optimization coverage); note §15.2 #3 is the LIVE aplus-research check, not this PROPOSED one.
...
| F-006 | Role 3 | PROPOSED §13 checks gate nothing | §13/§15.2/§18 | MODERATE | the 4 PROPOSED checks map to §15.2 #2 (AAS), #8 (composed-harm), #9 (assay-screen) + the optimization criterion (§11.2 #3 / §15.2 #1 coverage) — all prose-only/not-LIVE. (§15.2 #7 H1/H2 auto-block has no PROPOSED row; not in this set.) | LEGITIMATE (as-disclosed) | Already disclosed §18 + false-zero attestation; carry as deploy-gate + integrator beads. No change beyond existing disclosure. |
