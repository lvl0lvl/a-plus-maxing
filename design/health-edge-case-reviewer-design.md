...
| 11 | **Adjudication path named** [R5; Finding 8; arch R3.10] | Output references `adjudicator: medical-liaison` (Role 7) OR — pre-Role-7 — `adjudicator: operator (with override-acknowledgment per Role 1 §13 row 6 + log to vault/meta/contradictions.md)` | `scripts/audit-reviewer-output.sh --check adjudicator-named` (PROPOSED) | PROPOSED | BLOCK |
