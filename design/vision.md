...
**a-plus-maxing is a source-grounded health-intelligence system that helps an operator build physician-ready health plans — through a conversational intake agent reasoning over vetted research and their own private data — and track the progress and outcomes of those plans over time, while never using that private data to train a model and never committing it to version control.**
...
2. **PII trust boundary.** The operator's private data may be *transiently processed* by the model but is **never used to train a model** and **never committed to git** (the trunk and history stay PII-free, ADR-0005, threat-model B). There are two model-touching surfaces: **plan reasoning** runs over **de-identified summaries only** (ADR-0001 — `summarize` derives de-identified bands/classes before any plan-reasoning model sees the operator's state); and the **conversational intake** (ADR-0015/0016, S88) sends the *live conversation* raw to a no-train, bounded-retention commercial API so the intake agent can hold a real dialogue — a scoped, operator-signed-off relaxation of the zero-egress boundary, with the facts the planner reads kept de-identified by the extraction contract (ADR-0017) and a local model as the North Star that restores full on-device privacy. V1 achieves this with an individual commercial-API (no-train, 30-day) profile for PII-bearing dispatch and the subscription for PII-free library work. The GP product raises this to HIPAA + a Business Associate Agreement + per-tenant isolation — a documented upgrade, not a V1 burden.
...
V1 = the single-operator engine + a **conversational intake agent** (the codebase's first model client; ADR-0015–0019, S88) + outcome tracking + a simple HTML report/plan surface + the PII controls. The product phase adds multi-tenancy, hosting, accounts, and the regulated-PHI trust model. The PRD (S24) defines V1 concretely; subsequent ADRs / specs / build-plans detail it. The intake pivoted (S87, operator-confirmed) from a rigid form to a conversational agent: the form keeps only the objective layer (demographics + content uploads), and the rich sections (goals/training/nutrition/supplements/peptides) move to dialogue that writes de-identified facts the planner reads (ADR-0018) — formalized in ADR-0015–0019.
...
- `docs/adr/ADR-0015`–`ADR-0019` (S88) — the **conversational-intake pivot**: the first model client (ADR-0015), the scoped ADR-0001 egress relaxation (ADR-0016), the conversation→de-identified-store extraction contract (ADR-0017), the form/chat split (ADR-0018), and the `SUMMARY_FIELD_SET` extension so chat-sourced nutrition/supplement/peptide/training facts reach the planner (ADR-0019). These **supersede the rigid-form intake direction** (ADR-0014's rich-section capture, partially — its PII-routing contract survives) and formalize the model-egress relaxation. The `bd show hil` PII-boundary decision is now landed in ADR-0001 (+ its ADR-0016 relaxation).
