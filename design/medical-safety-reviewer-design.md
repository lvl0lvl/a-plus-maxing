...
| 2 | Three-axis severity composite (`safety_finding` YAML block) | Pass-3 specialists; orchestrator | Canonical schema per substrate Finding 5 (L185–L195). Fields: `finding_id`; `threat_model_cell: {adversary ∈ A1..A5, surface ∈ S1..S7, pattern ∈ P1..P10, harm_class ∈ H1..H8}` (per-cell input classification); `harm_class` (top-level = the composed `final_harm_class` = `max(Role3.nominal, Role4.worst_case_reachable)`; this is the field the §13 row 5 H1/H2 auto-block keys on, distinct from `threat_model_cell.harm_class`; every emitted finding MUST populate it — XR-S15-01 / bead dcy); `exploitability: {vector, complexity, privileges, user_interaction}`; `composite_band`; `decision_rule_applied`; `evidence`; `deploy_verdict`; `severity_proposed: true` and `severity_final: null` until adjudicator sets (substrate R10). |
...
safety_finding:
  ...
  harm_class: H2   # top-level composed final_harm_class = max(Role3.nominal, Role4.worst_case_reachable); the field the §13 row 5 H1/H2 auto-block keys on (XR-S15-01 / bead dcy)
...
| 5 | H1/H2 auto-block conditional-field audit (incl. worst_case_reachable) | Any finding with top-level `harm_class ∈ {H1, H2}` (the composed final; nominal OR worst_case_reachable) has `composite_band: CRITICAL` AND `deploy_verdict: BLOCK` AND (`override_path: null` OR `override_path` field absent — both satisfy the non-overridable property). The check keys on the top-level `harm_class` field (§4.4 row 2), which every finding MUST populate — not `threat_model_cell.harm_class` alone (XR-S15-01 / bead dcy) | `scripts/audit-safety-reviewer-output.sh --check h1h2-autoblock` | PROPOSED | BLOCK |
...
|---|---|---|
| BC-1 | Medical-liaison Role 7 deployed | **[TRIGGERED + ACTIONED 2026-05-29 — bead C.]** Role 7 (`medical-liaison`) is DEPLOYED on `main` (PR #8). `medical-liaison` is now the live `BLOCK_WITH_OVERRIDE_PATH` adjudicator default; the deployed Role 4 `agent.md` is flipped to match. **All pre-Role-7 `operator-with-warning` / `fallback_warning_prose` fallback language in THIS doc (§4.4 row 4, §6 step 5, EC-4, Risk-2) is SUPERSEDED** — kept for provenance, but the live path routes HIGH/MEDIUM to `medical-liaison`, not operator self-override. Detection (now met): `.claude/agents/medical-liaison/agent.md` exists and is non-empty. |
