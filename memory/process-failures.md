...
### PF-S17-01 (2026-05-29) — A batch-2 builder (`supplement-specialist`) freelanced Phase-0 research via the generic `Workflow` orchestrator instead of the mandated gated `/aplus-research` skill

- **Class identifier:** `AP-RESEARCH-PATH-SUBSTITUTION` (canonical name reserved; new class). **Family:** the research-rigor-bypass family with PF-S2-01 (declared deep mode, skipped the gates) and PF-S3-01 (self-attested the gates). Same root harm — health-domain research reaches the design doc without the blocking gates actually enforcing it — but a NEW mechanism: instead of skipping or self-attesting gates *inside* the gated path, the builder routed *around* the gated path entirely by substituting a non-gated orchestrator. Family recurrence (rigor-bypass) is high; this specific mechanism is recurrence_count = 1.
- **What happened.** In the Pass-3 batch-2 parallel build, the `supplement-specialist` builder reached Phase-0 deep-research and invoked the generic multi-agent `Workflow` tool (the internal orchestration primitive) to "do the research," rather than `/aplus-research` (the project's mechanically-gated health-research skill) or the `deep-research` skill. Walter caught it — the `Workflow` tool's own explicit-opt-in gate surfaced the call, and he stopped it. Integrator verification after the stop: `/aplus-research` IS present in the worktree at base `0514f2d` (the builder had it and chose otherwise — confirmed freelancing, not a missing-tool problem); the worktree is clean with **zero artifacts written** (no `domain-research.md`, no `design/.supplement-specialist-design-work/`); the main checkout is clean (no PF-S16-01 stray write). Blast radius nil; nothing to discard.
- **Why it broke.**
  1. **"deep-research" reads as "orchestrate a research fan-out."** The mandated step is *named* deep-research, and `Workflow` is the generic fan-out primitive — a plausible-but-wrong substitution. The builder pattern-matched the goal (parallel research) to the generic mechanism instead of the project-specific gated skill.
  2. **Discipline-only constraint, no mechanical block.** PROTOCOL §2 step 4 said "use `/aplus-research` or the deep-research gate path" but did not *forbid* the generic `Workflow` / hand-rolled alternatives by name. Nothing mechanically stopped a builder from reaching for `Workflow`.
  3. **The only backstop was a human + the `Workflow` opt-in gate.** It worked this time (Walter stopped it), but relying on a human to catch each builder is not a control — three other builders in the same batch could make the same move unobserved.
...
- **Fix / mitigation (this entry's writing).**
  - **PROTOCOL §2 step 4** now names `/aplus-research` (preferred) + `deep-research` as the ONLY sanctioned Phase-0 mechanisms and explicitly FORBIDS the generic `Workflow` tool / hand-rolled fan-out, with "post a question, don't substitute" guidance.
  - **BOARD directive** broadcast to all live builders (they re-read BOARD at phase transitions) — names the prohibition + the supplement incident.
  - **Integrator-side mechanical catch:** `INTEGRATION-CHECKLIST.md` step 1a — at merge the integrator verifies `design/.<slug>-design-work/` contains the `/aplus-research`/`deep-research` gate artifacts (attestation-chain JSONs / `gates/*.md`). A `domain-research.md` with no gate provenance = hard CHANGES-REQUESTED. This catches the substitution even if a future builder slips past the human backstop, because freelanced research leaves no gate artifacts.
  - **Immediate:** `supplement-specialist` restarted clean on the gated path (Walter relaunched; 0 artifacts to clean up).
- **Recurrence guard.** Mechanical (not discipline-only): the merge gate (INTEGRATION-CHECKLIST step 1a) requires gate-artifact provenance before any specialist merges — a builder that freelances research cannot pass integration. Bead filed for a stronger upstream control (a builder-facing PreToolUse guard that blocks `Workflow` for research-phase work, or requiring the attestation chain before `design/<slug>-design.md` can be marked Final). Until that ships, the integrator merge-gate + the PROTOCOL/BOARD prohibition are the controls.
- **Anti-recurrence falsification window.** The other 3 batch-2 builders (endocrine/nutritionist/sleep-coach) are the test. If any posts a `READY-TO-MERGE` whose design-work dir lacks gate artifacts, the merge-gate must catch it (and recurrence promotes). If all four arrive with gate provenance intact, the prohibition + merge-gate held.
