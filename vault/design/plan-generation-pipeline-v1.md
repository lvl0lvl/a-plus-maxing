...
**What this is:** the approved design spec for the plan-generation orchestration layer (bead `71s4`) — the target topology, with the **Build status** section below the live tracker of what is wired. As of S77 the layer is substantially built: the four plan-domain authors (S70–S71), the step-4 orchestrator reconciler (S72), the supplement↔peptide additive-AE screen (S73), the medical-liaison terminal adjudication gate for the held additive-AE finding (S74), the author-conflict adjudication (`cfaj`, S75), the supplement↔Rx BPMH axis (`rxbp`, S76), and the doctor-visit-queue DATA LAYER (S77 — the `dvq::` store stream + the collation of the adjudicated safety findings) all run through the production path (`scripts/plan/{generate_plan,orchestrate,adjudicate,router}.py`, `scripts/store/queue_schema.py`), mechanically guarded by `scripts/core-capability-audit.sh` (INV-CORE-CAPABILITY). What remains is runtime A's interactive-dispatch convenience surface (`/generate-plan`), the rendered SBAR handout artifact (a design-led visual surface — Pencil + operator sign-off + the design agents, never originated solo), and the measure/adjust legs. The earlier "unbuilt / no production caller" framing held only through S68 (PF-S63-02 — the drift this build closed).
