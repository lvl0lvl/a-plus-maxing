...
**What this is:** the approved design spec for the plan-generation orchestration layer (bead `71s4`) — the target topology, with the **Build status** section below the live tracker of what is wired. As of S74 the layer is substantially built: the four plan-domain authors (S70–S71), the step-4 orchestrator reconciler (S72), the supplement↔peptide additive-AE screen (S73), and the medical-liaison terminal adjudication gate for the held additive-AE finding (S74) all run through the production path (`scripts/plan/{generate_plan,orchestrate,adjudicate}.py`), mechanically guarded by `scripts/core-capability-audit.sh` (INV-CORE-CAPABILITY). What remains is runtime A's interactive-dispatch convenience surface (`/generate-plan`), the Phase-4 follow-on axes (`rxbp`/`cfaj` + the doctor-visit-queue artifact), and the measure/adjust legs. The earlier "unbuilt / no production caller" framing held only through S68 (PF-S63-02 — the drift this build closed).
