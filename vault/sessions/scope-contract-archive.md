...
Acceptance criteria:
- [ ] AC1: `scripts/ingest/adapters/whoop.py` rewritten to read noop's `whoop.sqlite` read-only (`sqlite3`, `mode=ro`), mapping `dailyMetric` rows → store readings `{item, timepoint=day, source="whoop", value}` for recovery / strain / hrv (`avgHrv`) / rhr (`restingHr`) / sleep-efficiency (`efficiency`) / spo2 (`spo2Pct`) / resp-rate (`respRateBpm`) / skin-temp-dev (`skinTempDevC`); NULL columns skipped; the fabricated `{metric_name,cycle_start,score}` JSON path removed.
- [ ] AC2: strain stored on the **0–21 scale** unchanged (a fixture strain=14.5 lands as 14.5, not rescaled); a deliberate rescale mutation goes RED.
- [ ] AC3: the adapter is **wired** (`UNWIRED` removed; the scheduler's data-driven discovery picks it up); `scripts/ingest/ingest.py` + `adapter.py` + `scheduler.py` at **0 edits** (the AC-3/AC-6 numstat gates stay green); `scheduler.py` still carries 0 `whoop` token references.
- [ ] AC4: store-adversarial battery on the whoop path (all four categories): cross-stream (recovery≠strain never cross-read), same-key dedupe (re-run appends 0 dupes; legit distinct rows both persist), dedupe-key boundary (each field's contribution), ≥1 mutation-style test that REDs when the keying/value mapping is deliberately broken.
- [ ] AC5: full suite green (≥ baseline 823 + new tests); the now-wrong scaffold/unwired tests in `tests/ingest/test_adapters.py` + `tests/ingest/test_scheduler.py` rewritten to the wired sqlite adapter.
- [ ] AC6: `docs/model-eval/local-model-evaluation-plan.md` updated with noop's AI Coach as prior art for the local-inference integration seam (BYO-provider protocol + OpenAI-compatible local endpoint à la Ollama/LM-Studio + consent-gated compact-summary context), license-honest (reference, not vendor — PolyForm).
- [ ] AC7: ADR-0011 OQ-2/OQ-3 marked RESOLVED (mechanism = read-only sqlite3, rationale recorded; the ADR delegated this to the build session).
- [ ] AC8: full session close per protocol — 5 audits incl. `skill-trace-audit.sh --session 63`, 6-clause rotation (archive the S62 contract at its on-main close SHA), PF attestation with the per-PR table, DOCUMENT_RUBRIC + landmarks RE-OPENED at steps 8/8.7; `/review-pr` (6-agent) + `/merge` both fresh via the Skill tool.

Files I WILL touch: `scripts/ingest/adapters/whoop.py`; `tests/ingest/test_adapters.py`; `tests/ingest/test_scheduler.py`; `scripts/store/biomarker_meta.py` (additive units/polarity entries for the new wearable markers — flagged expansion, no invented ranges); `docs/adr/ADR-0011-whoop-ingestion-via-noop-local-adapter.md` (OQ-2/OQ-3 resolution only); `docs/model-eval/local-model-evaluation-plan.md`; HANDOFF/vault/`.beads` at close.
Files I will NOT touch: `scripts/ingest/ingest.py`, `scripts/ingest/adapter.py`, `scripts/ingest/scheduler.py` (ADR-0003 0-edit shared routine + scheduler — wiring is via removing `UNWIRED` from whoop.py, never editing scheduler.py); `scripts/store/keying.py`, `scripts/store/store.py` (the store key/append path — the adapter writes through it unchanged); the other adapters (oura/garmin/healthkit); `PALETTE`/`SERIES`/`ACCENTS` (ADR-0009); `SUMMARY_FIELD_SET`/`EXCLUDED_RAW_PII` (PII boundary); `.claude/hooks/*`; `main` directly; **noop's repo (no vendoring — PolyForm Noncommercial)**.
NOT doing: the MCP-subprocess mechanism (sqlite path chosen; MCP deferred unless sqlite proves insufficient); **end-to-end validation against the operator's REAL `whoop.sqlite`** (gated on the operator installing noop + bonding the strap + offloading data — this session builds + tests against documented-schema fixtures; real-data validation stays the open tail of `mdzq`); imported-vs-`APPROXIMATE` confidence provenance (no provenance column in `dailyMetric`; no confidence field in the 4-field reading shape — deferred to the biomarker-page layer + needs a real sample); biomarker WIKI page creation (separate INV-WIKI-INGESTION-GATED concern); inventing reference ranges for recovery/strain (person/goal-dependent — honest absence); vendoring/linking noop code.
Invariants at risk: ADR-0003 0-edit-shared-routine (the AC-3/AC-6 numstat gates enforce it — wiring must keep ingest.py/adapter.py/scheduler.py at 0 edits); the store-adversarial battery mandate (`docs/checklists/store-adversarial-tests.md` — Tier-1 self-check AND Tier-2 QA); INV-SKILL-TRACE (the PR gets `/review-pr` + `/merge` fresh; per-PR table at close). INV-WIKI-INGESTION-GATED N/A (no vault entity page committed; `biomarker_meta.py` is code, not a wiki page).

### S63 Scope Contract Evaluation (2026-06-14, volatile)

- **AC1 (adapter rewritten — sqlite, dailyMetric→readings) — PASS.** `scripts/ingest/adapters/whoop.py` reads noop's `whoop.sqlite` read-only (`sqlite3` `mode=ro` via `Path.as_uri()` after the review SEC-1 fix), mapping `dailyMetric` → `{item, timepoint=day, source="whoop", value}` for recovery/strain/hrv(avgHrv)/rhr(restingHr)/sleep-efficiency(efficiency)/spo2(spo2Pct)/resp-rate(respRateBpm)/skin-temp-dev(skinTempDevC); NULLs skipped; the fabricated `{metric_name,cycle_start,score}` JSON path removed.
- **AC2 (strain 0-21, mutation-proven) — PASS.** strain carried unscaled (14.5→14.5); the rescale mutation (×100/21→69.05) proven RED.
- **AC3 (wired, 0-edit, scheduler 0 whoop tokens) — PASS.** `UNWIRED` removed → `scheduler._wired_adapters()` on merged main returns `['garmin','healthkit','oura','whoop']`; `git diff --stat origin/main…HEAD` for `ingest.py`/`adapter.py`/`scheduler.py` empty (the ADR-0003 0-edit invariant); `test_scheduler_carries_no_whoop_reference` green (scheduler.py names no source).
- **AC4 (store-adversarial battery — 4 categories + mutation) — PASS, strengthened by review.** cross-stream (recovery≠strain), dedupe (idempotent re-run + distinct-day persistence), dedupe-key boundary (value + source), mutation (constant-timepoint→RED). The #136 review HARDENED it: TEST-1 (value-pin each of the 8 column→item mappings — 6 had been presence-only; transposed-mapping→RED) + TEST-2 (honest-zero retention — `not value`→RED), both mutation-proven.
- **AC5 (suite green; scaffold/unwired tests rewritten) — PASS.** 833 passed / 2 skipped (823 baseline + net new whoop tests); the former unwired-scaffold + `test_whoop_not_invoked` + `test_whoop_carries_unwired_attribute` rewritten to the wired sqlite adapter.
- **AC6 (model-eval noop AI Coach prior art) — PASS.** `docs/model-eval/local-model-evaluation-plan.md` §3.5 + §4b/§8 pointers: noop's BYO-provider protocol + OpenAI-compatible local endpoint (Ollama) + consent-gated compact-summary context as integration prior art; license-honest (reference-not-vendor, PolyForm).
- **AC7 (ADR OQ-2/OQ-3 resolved) — PASS.** OQ-2/OQ-3 marked RESOLVED (read-only sqlite) + an S63 build amendment; the review (API-1/API-2) further aligned the Confirmation criterion (`source: whoop` + `(item,timepoint,source)`) + recorded the D4 `confidence`-carry deferral.
- **AC8 (full session close) — PASS** (this close; 5 audits incl. `skill-trace-audit.sh --session 63`, 6-clause rotation, PF attestation with the per-PR table, DOCUMENT_RUBRIC + landmarks RE-OPENED at 8/8.7; `/review-pr` + `/merge` both fresh via the Skill tool).

**CHANGED / flagged (none silent):** the WILL-touch list included `scripts/store/biomarker_meta.py` (additive units/polarity for the new wearable markers — flagged optional). It was **added then REVERTED**: the registry rows broke 5 dashboard/trend tests that deliberately use spo2/recovery/strain as *unregistered* exemplars → dropped (the data renders trend-only via honest-absence) + beaded as a deliberate follow-up. A scope REDUCTION (fewer files touched than planned), flagged. The review also beaded HIST-1/HIST-2 (`crgz` — spec + build-plan stale whoop-unwired gates, OUT_OF_SCOPE) + BUG-1 (`ienx` — multi-device fail-loud), 0 suppressed; `1uav` closed overtaken-by-events (its `test_whoop_not_invoked` target was replaced by `test_wired_set_invoked_includes_whoop`, which directly asserts wired-set membership).

### Drift checks (S63 close)

- **Task drift:** all 8 ACs PASS. One flagged scope REDUCTION (biomarker_meta reverted + beaded). The #136 review surfaced 7 LEGITIMATE (fixed+blind-verified 7/7) + 2 OUT_OF_SCOPE (beaded) + BUG-1 (beaded); no silent drift.
- **Architecture drift:** none degraded; the ADR-0003 source-extensible seam STRENGTHENED — a 4th wired adapter (whoop) added with **0 edits to the shared routine/scheduler** (the numstat gates green), the FIRST live-source (sqlite-reading) adapter, proving the seam extends to non-JSON sources. `keying.py`/`store.py`/`PALETTE`/`SERIES`/`ACCENTS`/`SUMMARY_FIELD_SET`/`EXCLUDED_RAW_PII` byte-identical (untouched). The store-adversarial battery mandate satisfied + hardened by the review. INV-SKILL-TRACE bound #136 (YES/YES) + the close PR; INV-TRUNK-COMPLETENESS green open + close; INV-WIKI-INGESTION-GATED N/A (no entity page).
- **Vision drift:** none. After S63 the same local-first health tracking + planning system, V1 complete; the wearable-ingestion path now has a real, WIRED WHOOP/noop adapter (read-only sqlite) replacing the fabricated scaffold — the first concrete step from "Whoop owned" toward live wearable data. The real-data E2E validation is gated on the operator. Matches `design/vision.md`.

### PF attestation

S63 close (2026-06-14): **No new PF-class entries this session.** Rationale: every candidate event was a falsification window HOLDING or a mechanism working as designed. **PF-S39-01 (gated-skill discipline) HELD** — PR #136 got BOTH `/review-pr` (6-agent) AND `/merge` invoked fresh via the Skill tool (`merge-methodology.md` read fresh; REST rebase under GraphQL throttle, full-40-char-SHA guard); the close PR's lifecycle likewise. Count stays 2. **PF-S40-01** (profile-less blind triage + blind verification, never self-triage) HELD + LOAD-BEARING — the blind triage ran repros confirming SEC-1 (a `#` in the path opens the WRONG file), TEST-1/TEST-2 (mutations survive green), classified 7 LEGITIMATE + 2 OUT_OF_SCOPE→bead, and correctly rejected BUG-1 as V1-bounded + BUG-3 as speculative + HIST-3 as unapproved-defensive-code; blind verify 7/7 RESOLVED. **PF-S26-01** (every legitimate fixed, 0 suppressed) HELD — 7 fixed+verified, 3 beaded, none dropped. **PF-S6-01 (verify-first) HELD — notably so given S62's disclosed README-authoring miss:** this session read noop's ACTUAL source (`docs/DATA_MODEL.md` schema, `Strand/AI/AICoach.swift`, `AIProvider.swift`) + the adapter architecture + the store contract + the biomarker registry BEFORE building/writing; the build was grounded in primary source, directly applying S62's PF-S6-01-family lesson. **PF-S51-01** (review/triage/verify agents read-only) HELD; tree pristine. **PF-S13-01/S37-01** (session-open from files; DOCUMENT_RUBRIC + landmarks RE-OPENED at 8/8.7) HELD. **PF-S49-01** N/A (no visual). Off-main-archive-SHA HELD — the S62 contract archived citing its on-main #133-close commit `ea9adcb` (`git merge-base --is-ancestor`-verified), count stays 2. **Observed, NOT promoted:** (a) I added `biomarker_meta` registry rows that broke 5 dashboard tests, caught immediately by running the suite → reverted + beaded; the build-then-verify mechanism working (caught by my own test run, nothing escaped) + a judgment lesson (a flagged-optional registry expansion had an unanticipated dashboard-test blast radius; drop+bead was the right call). (b) the #136 review caught real defects two/three levels into my own work (the URI read-only bypass especially) — the layered review working as designed, not an orchestrator PF. Full skill-trace table + observations in `memory/process-failures.md` (Session 63 section).
...
### S64 Scope Contract Evaluation (2026-06-15)
...
## Scope Contract — Session 71 (2026-06-18) [archived S72]

Goal: Wire the three remaining plan-domain authors (nutrition, supplements, peptides) end-to-end through the per-author dispatch process — each a per-domain translator + its domain safety gate + a test battery + a real specialist dispatch (full profile inlined) + a verified dashboard E2E — so the core capability covers all four plan domains. 7 ACs.

### S71 Scope Contract Evaluation + Drift + PF attestation

ACs (all 7 PASS): AC1 `_to_nutrition_plan` (aggregates N recs) / `_to_supplements_plan` (1:1) / `_to_peptides_plan` (single compound) in `_PLAN_TRANSLATORS`; AC2 `_nutrition_safety_gate` RED-S/LEA veto in `_DOMAIN_GATES`, mutation-proven RED; AC3 nutritionist/supplement/peptide dispatched for real (full profiles inlined), CLAIM-PHRASING RULE honored, peptide anchored BPC-157 at H2 + a clearance-gated decision-support draft; AC4 all four domains E2E-rendered; AC5 20 new tests incl. the store-adversarial four, suite 867/3; AC6 core-capability gate green + process doc updated; AC7 `/review-pr` (6-agent) PR #147 → fixed+blind-verified → `/merge` to `main`. Drift: all 7 ACs PASS, the cross-domain edges DEFERRED per the operator-signed-off held line (not silent); no invariant degraded (INV-CORE-CAPABILITY now spans 4 domains; INV-BRANCH-NOT-MAIN held); toward-vision. PF attestation: One PF promoted — PF-S71-01 (`block-commit-main` false-blocks a worktree commit when the main trunk sits on `main`; self-caught from the deny, parked the idle main trunk, no loss), 3-layer (bead `llna`). Full S71 detail in `memory/process-failures.md` Session 71.

## Scope Contract — Session 72 (2026-06-18) [archived S73]

Goal: Build the cross-domain reconciler — the orchestrator terminal function (design `vault/design/plan-generation-pipeline-v1.md` decision 4, part 1): a `/generate-plan` orchestrator that computes all four candidate plans in one pass, runs the nutrition→workout energy bounce + cross-domain overlap detection, and records the reconciled set. 7 ACs.

### S72 Scope Contract Evaluation + Drift + PF attestation

ACs (all 7 PASS): AC1 `compute_plan` split from `generate_plan` (compute the candidate, no record; single-domain contract unchanged, 45 tests + `--self-test` green); AC2 `scripts/plan/orchestrate.py` `generate_plans` computes every domain's candidate → `reconcile` → records survivors; AC3 the nutrition→workout energy bounce (`sustains:false` → re-author once under the ceiling → record reduced, else HOLD), each path mutation-proven RED; AC4 cross-domain overlap (distinct-domain dedup) + author-declared conflict detection (non-spoofable `from`) + the RED-S/LEA cross-domain short-circuit (Phase 0.5), mutation-proven RED; AC5 real personal-trainer + nutritionist (full profiles inlined) verified BOTH paths E2E (no-bounce + a real `sustains:false` ceiling-300 bounce → re-author to 260), envelopes under `docs/plan-generation/examples/cross-domain-*`; AC6 no new store stream (records via the existing `record_plan`), suite 899/3, core-capability gate green; AC7 plan-integrity + QA Tier-2 (7 fixed) → `/review-pr` 6-agent Tier-3 (10 LEGITIMATE fixed + blind-verified 10/10; 1 OOS → bead `axm7`) → `/merge` PR #149 to `main`. Drift: all 7 ACs PASS, one review-driven design-doc freshness fix (flagged, not silent); no invariant degraded (INV-CORE-CAPABILITY green, INV-BRANCH-NOT-MAIN held on the `../aplus-s72-reconciler` worktree); toward-vision. PF attestation: No new PF-class entries (the worktree path-read friction recurred + was harness-caught each time; no loss). Full S72 detail in `memory/process-failures.md` Session 72.
...
## Scope Contract — Session 75 (2026-06-19) [archived S76]

Goal: Wire the author-conflict adjudication (`cfaj`) — route author-declared cross-domain conflicts (`reconcile`'s `report["conflicts"]`, was detect-only) through the SAME S74 medical-liaison terminal gate. ACs: AC1 reconcile HOLDS the declaring (`from`) domain via an INDEPENDENT `conflict_held` set, mutation-proven RED (PASS); AC2 generate_plans routes each via the reused `adjudicate` gate, records only when in NEITHER `holds` NOR `conflict_held`, no new store stream (PASS); AC3 real medical-liaison conflict E2E — content-valid override clears+records, refused holds (PASS); AC4 backward-compat + the 2 pre-existing conflict tests updated, suite 996/3 (PASS); AC5 plan-integrity (MF-1 design freshness) + QA → /review-pr 6-agent (Tier-3 caught 2 MUST-FIX SAFETY bugs SEC-1/BUG-1 → the independent `conflict_held` refactor, mutation-proven, blind-verified 5/5 RESOLVED) → /merge, full close re-run on final `main` (PF-S74-01). All 5 PASS. PR #173 → main @ ad388a0. PF-S75-01 promoted (Tier-1/Tier-2 self-review under-probed the cfaj interaction surface; recurrence 2; bead `sip9`). Full eval in vault/sessions/session-75.md.

## Scope Contract — Session 76 (2026-06-19) [archived S77]

Goal: Wire the supplement↔Rx BPMH axis (`rxbp`) — read the operator medication surface through the `router.summarize` PII boundary (ADR-0006-T0) as de-identified Rx-interaction-class tokens (raw drug names never cross; no pharmacology DB in `scripts/`), hold a class-stacking compound, route it through the SAME S74 `adjudicate` gate via the multi-concern hold model. ACs: AC1 the PII boundary extends WITHOUT leaking raw names (`rx-interaction-classes` field + `medication-list` excluded; mutation-proven) — refined post-Tier-3 to a PER-TOKEN 8j6 scan (PASS); AC2 the independent `rx_bpmh_held` set + the reused gate, records only when in NONE of the three sets (PASS); AC3 the PF-S75-01 interaction-probe incl. the triple-concern case (PASS); AC4 two real medical-liaison BPMH dispatches DISCRIMINATING on the medication — aspirin→HIGH/H3 cleared, warfarin→CRITICAL/H2 auto-block, `liaison-rxbp-{cleared,blocked}.example.json` (PASS); AC5 backward-compat, suite 1030/3 (PASS); AC6 plan-integrity (2 freshness) + QA → /review-pr 6-agent (Tier-3 caught SEC-1, a REAL PII leak — the whole-value 8j6 scan defeated by `scan_text`'s 4096-cap — fixed per-token, blind-verified 3/3 RESOLVED) → /merge, full close on final `main` (PF-S74-01). All 6 PASS. PR #174 → main @ 365f5f8. PF-S76-01 promoted (Tier-1/Tier-2 under-probed the new PII-backstop's boundary-size interaction with the existing scan cap; recurrence 3; bead `pwpr`). Full eval in vault/sessions/session-76.md.

## Scope Contract — Session 77 (2026-06-19) [archived S78]

Goal: Wire the doctor-visit-queue DATA LAYER — a `dvq::queue` store stream + `collate_doctor_visit_queue`, recording the pipeline's adjudicated safety findings (additive-AE / conflict / rx-bpmh; cleared-via-override OR block-stands) as severity-ranked queue entries (design §9.4); the visual SBAR render deferred (design-led). ACs: AC1 the `dvq::queue` stream (cumulative, deduped by finding_id, severity-ranked), mutation-proven RED (PASS); AC2 the collation rebuilt via the same safety_finding builders, generate_plans unchanged (PASS); AC3 the store-adversarial battery 4/4 (PASS); AC4 a real medical-liaison collation E2E agreeing on the safety-tier lead, `doctor-visit-queue-collated.example.json` (PASS); AC5 backward-compat, suite 1063/3 (PASS); AC6 plan-integrity (MF-1 docstring) + QA (store-battery + the safety-ordering inversion) → /review-pr 6-agent (Tier-3 caught BUG-1/BUG-2 — the queue read composite_band off the adjudicate outcome, which doesn't carry it for block-stands → fixed via _adjudicate_with_band threading the real band, blind-verified RESOLVED) → /merge, full close on final `main` (PF-S74-01). All 6 PASS. PR #175 → main @ c68bc36. PF-S77-01 promoted (Tier-1 under-probed the new code's read of an existing contract; recurrence 4; bead `uerm`). Full eval in vault/sessions/session-77.md.

> Note: the S78 scope contract's full block was not appended here at the S79 close (a prior-close archive-append miss; the pointer chain says "S79 archived S78"). The S78 contract + evaluation live in `vault/sessions/session-78.md`.

## Scope Contract — Session 79 (2026-06-20) [archived S80]

Goal: Build the deferred doctor-visit SBAR handout renderer — render the plan pipeline's adjudicated safety findings (`dvq::queue`, the S77 data layer) as the SBAR one-page artifact via `generate.run('handout')`. The first half produced the DESIGN-LED mockups (prep + physician-copy face sheet, Clinical Light; design-critic APPROVED; PR #179 OPEN); the operator delegated the CODE autonomously. Scope finding: `report.py` already existed — the gap was the never-rendered `dvq::queue`. ACs: AC1 `handout.py` `render(store_read, _today)` SBAR one-pager, single-file (ADR-0004), no clinical verdict (PASS, mutation-proven); AC2 the dvq flagged-interactions section over `resolve_doctor_visit_queue` IN `store_read` (the PURE PII-boundary path), severity-ranked + band chip + source, grade/watchlist-tolerant, empty→honest absence (PASS, mutation-proven RED); AC3 honest `cs.awaiting` for the gated sections, initials-only, print-safe (PASS); AC4 `generate.run('handout')` wired + the `_today` seam (PASS); AC5 the store-surface battery + the idempotent same-date no-op (PASS); AC6 `read_profile`/`long_date`/source-tier promoted to `component_set`, `report.py` delegates (40 tests green) (PASS); AC7 backward-compat, suite 1115/2 (PASS); AC8 Tier-1 → Tier-2 plan-integrity INTEGRITY-CLEAN + QA (4 value-domain fixes) → /review-pr 6-agent (9 LEGITIMATE fixed + blind-verified, 5 NOT_A_BUG) → /merge, close on final `main` (PF-S74-01) (PASS). All 8 PASS. PR #182 → main @ fe267f0. PF-S79-01 promoted (the worktree-path EDIT slip; self-caught before commit, zero escaped impact; bead `6aen`). Full eval in vault/sessions/session-79.md.

## Scope Contract — Session 80 (2026-06-20) [archived S81]

Goal: Wire the adjust-leg PROGRESSION ALGORITHM — the closed loop's ADJUST leg (plan → act → measure → ADJUST), the LAST core pipeline piece. `scripts/plan/adjust.py` `adjust_plan` reads the plan-vs-actual progress (`track.resolve_plan_progress`), gates the honest "nothing to progress from" boundary, then records the domain SPECIALIST'S adjusted output via the REUSED `generate_plan` as a new dated plan — the de-load/advance reasoning is the specialist's, never invented in code. ACs: AC1 `adjust_plan` records the specialist's adjusted output via reused `generate_plan`, computes no progression rule (PASS); AC2 the full has_plan × has_tracking value domain decided + a fail-loud forward-date gate guarding the same-date store-identity dedupe-drop, validated via the store's `_check_date` (PASS); AC3 the safety floor (assemble's filters + clearance gate + RED-S/LEA veto) applies to the re-plan, no direct record path (PASS); AC4 a REAL personal-trainer dispatch authored a genuine adjustment (squat HELD @2/3 sets, hinge + plank advanced, no un-cleared load) (PASS); AC5 17 adjust tests incl. closed-loop E2E + boundary states + the safety floor on both domains + the forward-date gate + the store battery (PASS); AC6 the dispatch doc + the pipeline-doc Build-status WIRED (PASS); AC7 REUSE-only, suite 1132/2 (PASS); AC8 Tier-1 → Tier-2 plan-integrity INTEGRITY-CLEAN + QA (2 MUST FIX + 4 SHOULD FIX fixed — the forward-date gate fixed a real same-date dedupe-drop misreport) → /review-pr 6-agent (6 LEGITIMATE fixed + blind-verified, 2 NOT_A_BUG) → /merge, close on final `main` (PF-S74-01) (PASS). All 8 PASS. PR #187 → main @ 41e2c83. No new PF (every finding caught by the layered review before merge). Full eval in vault/sessions/session-80.md.


## Scope Contract — Session 83 (2026-06-21) [archived S84]

Goal: Build the operator-requested ingestion on-ramp — make loading real operator data a first-class, testable path: a `python -m scripts.ingest` CLI, DNA landing, the 6-step intake screen (faithful to the `2vFFC` mock), a tester README; proven on synthetic fixtures. ACs: AC1 the CLI auto-detects the source (Apple Health `export.xml` → store via the UNCHANGED `ingest.run`; 23andMe `.zip`/`.txt` → the gitignored `vault/dna/raw/` dropzone via `dna.land`) + `--source` override (PASS); AC2 `dna.py` validates the genotype shape, lands from a zip, surfaces variant-COUNT-only, landed-not-analyzed (PASS); AC3 `intake.py` + `generate.run('intake')` the 6-step wizard self-contained HTML (ADR-0004), live Step-1, steps 3-5 first-pass (PASS); AC4 `README.md` tester onboarding + the ADR-0001/0005 privacy guarantee (PASS); AC5 0-shared-routine-edit (numstat EXECUTED) + model/network-free ingest+render (security EXECUTED the egress check) (PASS); AC6 Tier-1 → Tier-3 `/review-pr` 6-agent → fix legitimate / bead the rest → `/merge` → close (PASS, with a DISCLOSED review-shape deviation: the Tier-3 triage/fix/verify sub-steps run directly + `/merge` as methodology in an over-long single conversation — recorded in the skill-trace + disclosure ledger; remediated S84 via the independent blind-triage + executed-verify, both confirming the merge SOUND). All 6 PASS. PR #205 → main @ `7376a53`; suite 1182/3. No new PF. Full eval in `vault/sessions/session-83.md`.


## Scope Contract — Session 84 (2026-06-21) [archived S85]

Goal: Execute the operator's standing P1 directive (bead `9qx5`) — run the independent PR-#205 review phases collapsed in S83 (a profile-less BLIND TRIAGE [review Phase 3] + EXECUTED BLIND VERIFY [Phase 7] against merged `main`), in a FRESH session, then fix-forward/bead. ACs: AC1 a profile-less agent independently classifies each PR-#205 finding cluster vs merged `main`, no access to the S83 verdicts (PASS); AC2 a profile-less agent RUNS each claimed fix, reports RESOLVED/STILL_PRESENT (PASS); AC2b the model/network-free ingest+render path re-confirmed sockets-blocked (PASS); AC3 fix-forward/bead any divergence (PASS — none; the merge was SOUND); AC4 full close + cite the SHA (PASS). All PASS. Two profile-less agents (opus, isolated worktrees off `8b52019`) confirmed the S83 ingestion-on-ramp merge SOUND: the 5 must-fix findings genuinely RESOLVED by execution; HIST-2 + BUG-205-01 reviewer-misreads; `07f6`/`gzf7`/`dw8u` correctly deferred. `9qx5` closed; `dw8u` sharpened. PR #207 → `main`; a ledger-accuracy correction (Caught 0→1) PR #208. No new PF (the S83 review-shape deviation was remediated). Full eval in `vault/sessions/session-84.md`.


## Scope Contract — Session 87 (2026-06-22) [archived S88]

Goal: Enable + evaluate the operator's real use of the intake website (sync the stranded trunk so they can run it), audit the intake's end-to-end wiring, resolve the intake's product direction, and set up the next session to build that direction autonomously. (EMERGENT session — retroactive contract.) ACs: AC1 trunk synced to `origin/main` @ `dfd44e8` WIP-preserving (PASS); AC2 server smoke-tested (PASS); AC3 intake-wiring audit — only 6 signals reach the planner, dead Step-1, 4 orphan tokens (PASS); AC4 operator-confirmed conversational-intake VISION PIVOT recorded (user memory `project_vision_pivot_conversational_intake` + epic `w3y8`) (PASS); AC5 S88 autonomous directive set in "What Is Next" (PASS); AC6 full close on final `main` + cite SHA (PASS). All PASS. One new PF: PF-S87-01 (plumbing-reviewed-not-end-to-end-capability). Architecture + vision drift YES, deliberately (the operator-signed-off ADR-0001 relaxation + the pivot). PR #214 → `main` @ `7061f60`. Full eval in `vault/sessions/session-87.md`.


## Scope Contract — Session 88 (2026-06-22) [archived S89]

Goal: Lay the architectural FOUNDATION for the conversational intake agent — author the ADR set via `/create-adr` (first model client + scoped ADR-0001 relaxation + extraction contract + form/chat split + plan-generation wiring), update the vision, produce a grounded design pass — then review, merge, close. (AUTONOMOUS; self-confirmed.) ACs: AC1 ADR set 0015–0019 via the full create-adr pipeline (PASS — ADR-0019 added during discovery, flagged); AC2 vision.md updated to the conversational direction (PASS); AC3 design pass via the design/architecture agents (PASS — + adversarial critique caught the 8j6-on-derived-token inaccuracy); AC4 crown-jewel boundary stated per-path (PASS); AC5 PR #216 /review-pr (3-agent docs subset, 4 Suggestions fixed) → rebase-merge → main @ 6d1d85f (PASS); AC6 full close on final main + PF-S74-01 (PASS). All PASS. No new PF (the layered reviews each caught what the prior missed, all before merge). Architecture drift YES, deliberately (the operator-signed-off ADR-0001 relaxation + the first model client). PR #216 → main @ 6d1d85f; close PR #217 → main @ 71eb853. Full eval in `vault/sessions/session-88.md`.

## Scope Contract — Session 89 (2026-06-22) [archived S90]

Goal: Set up everything S90 needed to run the conversational-intake BUILD as a strict autonomous `/execute-plan` loop — produce the build-planning pipeline (spec → build-plan → 8 task-plan recipes) grounded in the accepted ADR set (0015–0019) + the design, document the runtime keychain-key retrieval, write the strict-loop directive — WITHOUT live-API spend. Then close. (AUTONOMOUS; self-confirmed.) ACs: AC1 spec `docs/spec/adr-0015-0019-conversational-intake-spec.md` (PASS — 8 tasks/2 waves, FRs+5 NFRs, 6 deferred reqs as ACs, surfaced the `equipment-access-class` double-source collision); AC2 build-plan `docs/build-plan/build-plan-conversational-intake.md` (PASS — Wave A + Wave B, executable checkpoints); AC3 8 TDD recipes under `docs/task-plan/` (PASS — failing-capable, store battery, mock-tested model tasks); AC4 key access (PARTIAL at close, RESOLVED post-close — keychain item `quant-primary-api`, env `ANTHROPIC_API_KEY`, runtime-fetch never tracked); AC5 strict-loop directive (PASS); AC6 close + PF-S74-01 (PASS). No new PF (fresh-dispatched + self-validated planning; the credential-exploration guard fired as designed). No `scripts/` change; core-capability path stays green. Full eval in `vault/sessions/session-89.md`.

## Scope Contract — Session 90 (2026-06-22) [archived S91]

Goal: Build Wave A of the conversational-intake build (the model boundary + the conversation path) via `/execute-plan`, through the Wave-A checkpoint → Tier-2 → Tier-3 `/review-pr` → `/merge` → close. (AUTONOMOUS; self-confirmed.) ACs: AC1 plan-integrity grounding (PASS — GO; corrected line-citation drift); AC2 Wave A build via `/execute-plan` (PASS — 5 tasks, serial spine T1→{T2∥T3∥0017-T1}→0016-T1; commits cb0dcc1/0ecdce6/ab0aad1/fb269a9/c3033ab); AC3 Wave-A checkpoint EXECUTED green (single model boundary; runtime key never tracked; fail-closed on all 3 consumers; /chat loopback + single-egress-class; gate-at-write extractor; H-2 identity_config both call sites; assemble/router byte-unchanged; core-capability-audit --self-test green); AC4 3-tier review → merge (PASS — Tier-1 TDD; Tier-2 QA+Security+Architect+plan-integrity EXECUTED, 4 findings fixed incl. the in-population Canadian-postal value-PII gap → PF-S90-01; Tier-3 /review-pr 6-agent independence INTACT, 10 Suggestion-tier, 6+1 fixed → 7/7 blind-verified; PR #220 → main); AC5 close (PASS, PF-S74-01). One new PF — PF-S90-01 (crown-jewel value-PII gate tests used US-locale fixtures only; the operator's Canadian postal slipped — Tier-2-caught in-population, fixed + blind-verified; beaded v70t); the marquee /chat-payload-vs-summarize leak SELF-CAUGHT at Tier-1. Crown-jewel boundary held. pytest 1364/3. Full eval in `vault/sessions/session-90.md`.

## Scope Contract — Session 91 (2026-06-23) [archived S92]

Goal: Build Wave B of the conversational-intake build (demographic activation + the `SUMMARY_FIELD_SET` token vocabulary + the end-to-end usable-plan proof) via `/execute-plan`, through checkpoint → 3-tier review → merge → close. (AUTONOMOUS; self-confirmed.) RE-SCOPED mid-session by operator direction: ADR-0017-T2 superseded — the real plan-generation engine is a richer multi-agent pipeline that gets its own design pass (S92). ACs: AC1 plan-integrity grounding (PASS — CONDITIONAL GO; caught the NOTE-T3-1 production-wiring trap + the ADR-0018-T1 21-coupled-test manifest gap); AC2 Wave B build (PASS for the foundation — ADR-0018-T1 demographic activation `d2f338b` [SE correctly HALTED on the 21-test coupling, orchestrator adjudicated extend-manifest, tests UPDATED-not-weakened] + ADR-0019-T1 4 kind-2 chat-sourced tokens `90d064d`; ADR-0017-T2 CHANGED/re-scoped); AC3 Wave-B checkpoint CHANGED (foundation gates PASS — disjointness tripwires + WIRED_TOKENS⊆SUMMARY_FIELD_SET green, per-token coarseness output scans clean, store-adversarial battery non-tautological, byte-unchanged invariants; the end-to-end gate moot under the re-scope); AC4 3-tier review → merge (PASS — Tier-3 /review-pr 7-agent independence INTACT, 12 findings → 9 LEGITIMATE fixed + 9/9 blind-verified, 2 OUT_OF_SCOPE → bead f0gh; the always-set tokens got a `not-discussed` honesty sentinel; PR #237 → main); AC5 close (PASS, PF-S74-01). No new PF (the layered pipeline caught every gap before merge; the architecture clarification engaged honestly). Disclosure: the uncommitted S91 scope contract was reverted by a sub-agent git op (re-authored at close). pytest 1472/2. Full eval in `vault/sessions/session-91.md`.
