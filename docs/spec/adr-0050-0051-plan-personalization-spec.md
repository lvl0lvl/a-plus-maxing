...
| `scripts/plan/context_assembler.py` | Modify | (ADR-0051-T1) Add a separate `dna-report`-source read path that carries the FULL raw genotype panel (all stored SNPs incl. APOE/DRD2/BDNF), running the SAME per-value guard sequence as the allowlist path — `_is_flat_scalar` (`:165`, reject nested/non-scalar fail-closed, so a nested-dict third-party MRN/provider-name cannot smuggle past both the allowlist and the operator-value gate) AND `pii_scan.scan_operator_value` (`:188`), MINUS only the scoped `_RAW_GENOTYPE_PATTERNS` genotype-rejection exemption (`:178-187`, AC-3) — NOT `scan_operator_value` alone; the derived `genetic-trait-classes` token rides alongside unchanged |
...
**Acceptance Criteria:**
1. For EACH of the 4 renderable domains (workout→§1 personal-trainer, nutrition→§2 nutritionist, peptides→§3 peptide-specialist, supplements→§4 supplement-specialist), `_author_system_prompt(domain)` contains THAT specialist's own section-unique `specialist-plan-contracts.md` contract substring — a test asserts a section-unique Inputs- or Analysis→plan-phrase for ALL 4 domains (verifying the full domain→§ map, not just ≥1), so a mis-map on any one of the 4 domains REDs.
...
**Files to create/modify:**
- `scripts/plan/context_assembler.py` -- add a separate `dna-report`-source read path (reads `source == "dna-report"` items, keyed `<GENE> <rsID>` with `(X;Y)` values) that carries the FULL raw panel through the SAME per-value guard sequence as the allowlist path — `_is_flat_scalar` ([:165](../../scripts/plan/context_assembler.py), reject nested/non-scalar fail-closed) AND the reused `pii_scan.scan_operator_value` identity strip ([:188](../../scripts/plan/context_assembler.py)) — MINUS only the scoped `_RAW_GENOTYPE_PATTERNS` rejection ([:178-187](../../scripts/plan/context_assembler.py)) which is exempted ONLY on the genetics path; NOT via `_HEALTH_SUBSTANCE_ALLOWLIST` (keeps the partition assert at [:80-84](../../scripts/plan/context_assembler.py) satisfied and `router.py` frozen)
...
2. IDENTITY-leak probe (fail-closed RAISE, full ADR-0051 Validation threshold — name/DOB/contact/MRN/insurance-id/provider-name): two sub-cases, both fail-closed. (a) OPERATOR identity: seed a synthetic operator legal name (or DOB/contact) INTO a `dna-report` genetics field and assert the assembler RAISES via `pii_scan.scan_operator_value` ([context_assembler.py:188](../../scripts/plan/context_assembler.py)). (b) THIRD-PARTY identifier: seed a nested-dict / non-scalar `dna-report` value carrying a third-party MRN or provider-name (NOT the operator's own identity) and assert the assembler RAISES via `_is_flat_scalar` ([context_assembler.py:165](../../scripts/plan/context_assembler.py), the flat-scalar guard that fail-closes a nested value which `scan_operator_value` alone would not catch). So a seeded identity token never crosses on the genetics path. RED-capable BOTH ways: dropping `scan_operator_value` on the genetics path lets the operator name cross WITHOUT raising (RED); dropping `_is_flat_scalar` lets the nested third-party PII cross WITHOUT raising (RED); the test is invalid unless each mutation REDs its sub-case.
...
6. END-TO-END composition (integration, Wave-2 gate — placement, not existence): seed the store with synthetic `dna-report` genotypes, drive `_do_generate_plan` with a spy `self.client`, and assert the captured author payload for a genetics-consuming domain contains the seeded `<GENE> <rsID>=(X;Y)` genotype values — the raw panel reaches the author CALL, not just `assemble_context`'s return. RED-capable: a filtered author-input view that drops the genotype keys (or a `json.dumps` mishandling at [client.py:773](../../scripts/model/client.py)) makes the assertion FAIL; the test is invalid unless the genotypes being absent from the captured payload REDs it. Requires BOTH ADR-0050-T1 (author wiring) and ADR-0051-T1 (genetics carry), so it is a Wave-2 integration gate (closes the transitivity gap where ADR-0050-T1 AC-1 seeds only free-text and ADR-0051-T1 AC-1 checks only `assemble_context`'s return — neither asserts genotypes reach the front-door author payload).
7. `$0`: fixture/synthetic genotypes only (no live spend); `pytest tests/plan/test_context_assembler.py -q` passes and `.venv/bin/python -m pytest -q` stays green (0 NEW failures).

**Risk Mitigations:** ADR-0051 Consequence-Negative-3 "the `pii_scan` identity strip must be VERIFIED on a raw genetics payload" (AC-2, the identity-leak probe — operator identity via `scan_operator_value` AND nested third-party MRN/provider-name via `_is_flat_scalar`, to the full ADR-0051 threshold); Negative-5 "the assembler genotype-rejection must be SCOPED, not blanket-removed" (AC-3); Negative-6 / OQ-1 "mechanism constraint to keep `router.py` frozen" (AC-4, the frozen-surface probe + the separate-read-path mechanism); ADR-0051 Neutral "the derived token persists as the summary layer" (AC-5); CLAUDE.md Integration-Verification mandate "E2E asserts placement, not existence" — the end-to-end genotype→author-payload composition (AC-6) guards the crown-jewel raw-genome egress against a filtered-view / serialization drop the isolated per-unit tests miss; ADR-0051 Negative-1 (re-identification exposure) and Negative-2 (un-vetted read) are non-falsifiable / prompt-side residuals — documented assumptions per the Unresolved Concerns table, not build ACs (Negative-2 is bounded by ADR-0050-T2 AC-3)
...
|-----------|--------|-------------|--------------|
| Frozen ADR-0032 six numstat == 0 vs `3ab1c3ab` (`store.py`, `keying.py`, `pipeline.py`, `adjudicate.py`, `adjust.py`, `router.py`) | ADR-0050 Related Decisions + ADR-0051 Validation (frozen-surface probe) | ADR-0050-T1, ADR-0050-T2, ADR-0051-T1 | the `git diff --numstat 3ab1c3ab -- <six>` prints-nothing AC in each (ADR-0050-T1 AC-5, ADR-0050-T2 AC-7, ADR-0051-T1 AC-4) |
...
| `$0` build until the operator-present run (synthetic/mock fixtures, PUBLIC repo) | ADR-0050 OQ-3 + ADR-0051 OQ-4 (LIVE run operator-gated, out of build scope) | ADR-0050-T1, ADR-0050-T2, ADR-0051-T1 | ADR-0050-T1 AC-6 (spy client, no spend); ADR-0050-T2 AC-7; ADR-0051-T1 AC-7 (synthetic genotypes) |
...
### Integration Tests
- **Scope:** the front-door `_do_generate_plan` authoring path (`router.summarize` selection + `assemble_context` author input + the `generate_plans`→`compute_plan` replay) — ADR-0050-T1; the assembler's identity-leak probe across a full specialist payload — ADR-0051-T1; the END-TO-END genotype→author-payload composition (genotypes seeded in the store reach the captured front-door author call, not just `assemble_context`'s return) — ADR-0051-T1 AC-6, the Wave-2 integration gate spanning ADR-0050-T1 + ADR-0051-T1.
- **Approach:** in-process `_do_generate_plan` driven with a spy/fixture `self.client` (captures each `author(domain, summary)` call's args + call count) over a seeded store on the single-operator instance root; the assembler run end-to-end over a store seeding identity strings into a `dna-report` field; for the composition gate, a store seeded with synthetic `dna-report` genotypes with the captured author payload asserted to carry the `<GENE> <rsID>=(X;Y)` values (placement, not existence).
- **Criteria covered:** ADR-0050-T1 AC-1, AC-3, AC-4; ADR-0051-T1 AC-2, AC-6.

### Risk-Specific Tests
- **Scope:** the wiring-regression guard (ADR-0050-T1 AC-2, the band-reversion RED), the prompt-strip guards (ADR-0050-T2 AC-5), the single-source-invariant guard (ADR-0050-T2 AC-6, RED against the Alternative-C embedded-copy fork), the identity-leak probe (ADR-0051-T1 AC-2 — operator identity via `scan_operator_value` AND nested third-party MRN/provider-name via `_is_flat_scalar`), the genotype-rejection-scoped guard (ADR-0051-T1 AC-3), the end-to-end genotype→author-payload composition guard (ADR-0051-T1 AC-6, RED against a filtered-view / serialization drop), the frozen-six / frozen-surface numstat probes (all three tasks), and the no-new-model-call cost guard (ADR-0050-T1 AC-4).
- **Approach:** each ruling is mutation-gated — the test asserts the CLEAN direction truthy first, then a planted violation (reverting the author input to `router.summarize`; stripping the contract/directive from the prompt; embedding a static copy of a contract section in place of the runtime source; smuggling a genotype into a non-genetics carry; smuggling operator identity OR a nested third-party MRN/provider-name into a `dna-report` field; filtering the genotype keys out of the front-door author payload) must flip it RED, so the test cannot be built green-always.
- **Criteria covered:** ADR-0050-T1 AC-2, AC-4, AC-5; ADR-0050-T2 AC-5, AC-6; ADR-0051-T1 AC-2, AC-3, AC-4, AC-6.

**Self-verifying ACs (covered by execution, not a separate test asset):** each task's terminal `pytest …` AC is the run itself — ADR-0050-T1 AC-6, ADR-0050-T2 AC-7, ADR-0051-T1 AC-7. With these, every acceptance criterion maps to a test or a verification command.
...
- [x] Every task has at least one acceptance criterion (ADR-0050-T1: 6, ADR-0050-T2: 7, ADR-0051-T1: 7)
