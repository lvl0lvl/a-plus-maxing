...
This spec delivers the interactive intake website: an operator-started, ephemeral, loopback-only HTTP server (`python -m scripts.serve`) that serves the existing intake wizard and receives browser uploads, routing each into the UNCHANGED ingestion seam (ADR-0013), plus the web-form capture path that persists the wizard's Step-2..6 input by data class — de-identified `SUMMARY_FIELD_SET`-token fields to the local store, raw/rich context to the gitignored `vault/scaffold/filled/` (ADR-0014). The server also accepts the Apple Health export **zip** and extracts its `export.xml` member itself, mirroring `dna.land`'s zip-member extraction (the ADR-0012 2026-06-21 amendment). The deliverable is a new `scripts/serve/` package plus the form-handler wiring — a stdlib `http.server` bound to `127.0.0.1` only, a `multipart` reader that streams each upload to a server-chosen temp path, an Apple-Health-zip→`export.xml` extraction step, a route that dispatches the upload into `ingest.run` / `dna.land`, and a re-render via `generate.run('intake')`. There is no third-party web framework, no database engine, and no always-on daemon: the process is operator-started and exits when stopped.

The server is **glue, not a new ingestion path**. It reuses, byte-unchanged, the seams the data-in / data-out waves already built: `ingest.run(adapter, export_file, root)` ([scripts/ingest/ingest.py:39](../../scripts/ingest/ingest.py#L39) [VERIFIED]), `dna.land(source_file, dna_root)` ([scripts/ingest/dna.py:71](../../scripts/ingest/dna.py#L71) [VERIFIED]), the CLI's source-detection map `_EXT_SOURCE` + `_detect_source` ([scripts/ingest/__main__.py:39,42](../../scripts/ingest/__main__.py#L39) [VERIFIED]), `status.resolve` / `status.wearable_status` ([scripts/ingest/status.py:51,17](../../scripts/ingest/status.py#L17) [VERIFIED]), `intake.render(store_read, *, status, _today)` ([vault/design/templates/intake.py:263](../../vault/design/templates/intake.py#L263) [VERIFIED]) re-rendered through `generate.run('intake')` ([scripts/generate/generate.py:34](../../scripts/generate/generate.py#L34) [VERIFIED]), and `store.append(item, reading, root)` for the form-capture persistence ([scripts/store/store.py:134](../../scripts/store/store.py#L134) [VERIFIED]). The capture path routes only through `router.summarize`'s closed `SUMMARY_FIELD_SET` de-identification gate ([scripts/plan/router.py:18-36,352](../../scripts/plan/router.py#L18) [VERIFIED]) — a captured de-identified field is written to a store item named for its token, tagged `source: "intake"`, and reaches a specialist only via that gate. Every NEW network surface is wrapped in the already-audited OS-level egress guard `egress_guard.run(operation)` ([scripts/guard/egress_guard.py:82](../../scripts/guard/egress_guard.py#L82) [VERIFIED]), extended to wrap the request-handler dispatch.
...
| Step 5 — Supplements & peptides | **rx-interaction-classes** (de-identified class tokens) | curated | store item `rx-interaction-classes`, `source:"intake"` | `summarize` → `assemble` (BPMH axis) |
| Step 6 — Review & generate | (handoff only) "Save & open plan generation" → `/generate-plan` | n/a | n/a (NOT in-app generate) | `/generate-plan` |

`SUMMARY_FIELD_SET` membership of every "wired de-identified" token above is verified at [scripts/plan/router.py:18-36](../../scripts/plan/router.py#L18) [VERIFIED] (`goal-domains`, `goal-targets`, `goal-priority-order`, `hard-limits`, `recovery-status-band`, `rx-interaction-classes` all present). The raw Step-5 stack names are named-excluded raw PII ([scripts/plan/router.py:41-62,104-176](../../scripts/plan/router.py#L41) [VERIFIED]); the curation surface for `rx-interaction-classes` is operator/liaison-emitted (ADR-0014 OQ-2), not a code lookup of raw names.
...
| ADR-0012 amendment (2026-06-21) | Decision (input format) | The Apple Health upload accepts the export zip and extracts `export.xml` itself, mirroring `dna.land` | In scope (Wave A) | Built in `ADR-0013-T3`: the upload route extracts `*/export.xml` from an Apple Health zip (the `zipfile.is_zipfile` branch + member-locate + streamed copy, mirroring `_genotype_member` / `land` at [scripts/ingest/dna.py:49,92](../../scripts/ingest/dna.py#L49) [VERIFIED]), then routes the extracted xml into `ingest.run` via the unchanged `healthkit` adapter. The decompression-size ceiling (bead `07f6`) is operator-owned (local self-DoS, no egress) and tracked, not resolved here — the spec records a streamed-copy-with-a-byte-ceiling as the mitigation surface (`ADR-0013-T2` temp-staging). |
| Egress guard wrap | Constraint (ADR-0013 / ADR-0001) | The egress guard wraps the request-handler dispatch (sockets blocked) | In scope (Wave A) | `ADR-0013-T5` wraps a real upload→ingest→re-render request dispatch in `egress_guard.run` and asserts truthy (0 outbound calls); the loopback bind is a static assertion over the server's bind call ([scripts/guard/egress_guard.py:82](../../scripts/guard/egress_guard.py#L82) [VERIFIED]). The guard is OS-level and already audited at `ADR-0001-T1`; this build CONSUMES it over the new request path, adding no new egress control. |
...
| `scripts/serve/server.py` | Create | The stdlib `http.server` handler: GET `/` serves the wizard (re-rendered via `generate.run('intake')`); POST `/upload` reads the multipart body, stages to temp, extracts an Apple-Health zip, dispatches into `ingest.run` / `dna.land`, re-renders. Loopback-origin rejection. |
...
| `scripts/serve/apple_zip.py` | Create | Apple-Health-zip→`export.xml` extraction: `zipfile.is_zipfile` branch, locate the single `*/export.xml` member, stream it out (mirrors `dna.land`'s `_genotype_member`/zip-copy). Non-zip / already-`export.xml` passes through unchanged. |
| `scripts/serve/route.py` | Create | The upload-routing seam: map a staged upload to a source (reusing the CLI's `_EXT_SOURCE` / `_detect_source` logic) and dispatch into `ingest.run` (wearable) or `dna.land` (DNA); never edits the shared ingest/adapter/scheduler routines. |
...
| `tests/serve/test_apple_zip.py` | Create | Apple-Health zip → `export.xml` extraction; the `*/export.xml` member located; a non-zip / pre-extracted xml passes through; a zip with no `export.xml` fails loud. |
| `tests/serve/test_route.py` | Create | A staged `export.xml` routes into `ingest.run` (healthkit); a staged DNA `.zip` routes into `dna.land`; the shared `ingest.py`/`adapter.py`/`scheduler.py` are byte-unchanged (`git diff --numstat` = 0). |
| `tests/serve/test_egress.py` | Create | The egress guard over a real upload→ingest→re-render dispatch observes 0 outbound calls; injecting a synthetic outbound call drives the guard to FAIL (failing-capable). |
...
### ADR-0013-T3: Apple-Health-Zip → `export.xml` Extraction (Mirrors `dna.land`)

**Status:** TODO
**ADR Source:** ADR-0012, Amendment 2026-06-21 (the Apple Health ingest path accepts the export zip and extracts `export.xml` itself, locating the single `*/export.xml` entry, mirroring `dna.land`'s zip-member extraction; the streamed `iterparse` read is unchanged); ADR-0013, Related Decisions (ADR-0012 `complements`: the browser upload delivers the Apple Health zip this amended path accepts)
**Files to create/modify:**
- `scripts/serve/apple_zip.py` -- `zipfile.is_zipfile` branch; locate the single `*/export.xml` member; stream it out to a staged path (mirrors `_genotype_member` + the `land` zip-copy)
- `tests/serve/test_apple_zip.py` -- zip → `export.xml` extraction; member located; non-zip / pre-extracted xml passes through; a zip with no `export.xml` fails loud

**Acceptance Criteria:**
1. Given an Apple Health export `.zip` containing a single `*/export.xml` member, the extractor locates that member and streams it to a staged `export.xml` path the downstream read can open, verified by extracting a synthetic Apple-Health zip and asserting the staged file is the inner `export.xml` content (the same `zipfile.is_zipfile` → member-locate → `zf.open` stream-copy shape `dna.land` uses at [scripts/ingest/dna.py:92-99](../../scripts/ingest/dna.py#L92) [VERIFIED]).
2. A staged file that is ALREADY a raw `export.xml` (not a zip) passes through unchanged — the extractor returns it as-is (the `dna.land` raw-`.txt` passthrough analog), verified by passing a raw xml and asserting the returned path is the input path / content unchanged.
3. A zip with NO `*/export.xml` member fails loud (raises a stated error naming the missing member), never silently returning nothing — verified by extracting a zip without `export.xml` and asserting the raise (the `_genotype_member` fail-loud analog at [scripts/ingest/dna.py:67](../../scripts/ingest/dna.py#L67) [VERIFIED]).
4. The extraction is a streamed copy with the `ADR-0013-T2` byte ceiling applied (the extracted xml can be hundreds of MB; the copy is chunked, not a single-shot read), verified by `rg` finding the chunked stream-copy (0 single-shot full-member reads).
5. The healthkit adapter / `ingest.run` / `dna.py` are byte-unchanged — the extraction lives ONLY in `scripts/serve/apple_zip.py`, verified by `git diff --numstat <pre-task> -- scripts/ingest/adapters/healthkit.py scripts/ingest/ingest.py scripts/ingest/dna.py` reporting 0 changed lines (ADR-0013: the server adds no extraction logic to the ingest layer; ADR-0012: the streamed read is unchanged).
6. `pytest tests/serve/test_apple_zip.py` passes.

**Risk Mitigations:** ADR-0012 amendment (a browser-uploaded Apple Health zip cannot reach `ingest.run` without the extraction) — criteria 1-3 build the zip-awareness mirroring `dna.land`. ADR-0012 `07f6` (the hundreds-of-MB extracted xml) — criterion 4 streams the copy under the byte ceiling. ADR-0013 0-extraction-in-ingest invariant — criterion 5 keeps the ingest layer byte-unchanged.
**Dependencies:** ADR-0013-T2 (the temp-staging + byte ceiling the extraction reuses). Blocks: ADR-0013-T4.
...
**Files to create/modify:**
- `scripts/serve/route.py` -- map a staged upload to a source (reuse `_EXT_SOURCE` / `_detect_source`) and dispatch into `ingest.run` (wearable) or `dna.land` (DNA); the Apple-zip case extracts via `ADR-0013-T3` then routes the xml into `ingest.run` (healthkit)
- `scripts/serve/server.py` -- wire the POST `/upload` handler: stage (T2) → extract Apple-zip if needed (T3) → route (this task) → re-render via `generate.run('intake')`
...
3. A POST `/upload` of an Apple Health `.zip` extracts `export.xml` (via `ADR-0013-T3`) then routes the extracted xml into `ingest.run` (healthkit) — verified by `store.read` returning the healthkit-mapped readings from the zip's inner xml.
4. The source-detection logic REUSES the CLI's `_EXT_SOURCE` / `_detect_source` mapping ([scripts/ingest/__main__.py:39,42](../../scripts/ingest/__main__.py#L39) [VERIFIED]) — `route.py` references that map / function rather than re-deriving the extension→source rules, verified by `rg` finding the import / call (0 second extension-map definitions in `scripts/serve/`). NOTE: the Apple-Health zip routes to `healthkit` (the T3 extraction precedes routing), distinct from the CLI's `.zip → dna` default — `route.py` branches a zip on its content (Apple-Health-export shape → healthkit; 23andMe shape → dna), the one extension the website disambiguates by content.
...
6. The shared ingest routines are byte-unchanged: `git diff --numstat <pre-task> -- scripts/ingest/ingest.py scripts/ingest/adapter.py scripts/ingest/scheduler.py scripts/ingest/dna.py` reports 0 changed lines (the server is a new front door, adds 0 adapter / 0 store-write / 0 extraction-in-ingest logic — ADR-0013, ADR-0003 invariant).
...
### ADR-0013-T5: Egress Guard Over the Request-Handler Dispatch (0 Outbound, Loopback-Proven)

**Status:** TODO
**ADR Source:** ADR-0013, Decision (zero outbound network calls; the egress guard wraps the request-handler dispatch); ADR-0013, Validation Approach (Confirmation 2: the upload→ingest→re-render path makes 0 outbound calls under the egress guard with sockets blocked; Falsification 2: ≥1 outbound connection halts release); ADR-0001 "0 outbound calls carrying store content"
**Files to create/modify:**
- `scripts/serve/server.py` -- wrap the request-handler dispatch in `egress_guard.run(operation)` ([scripts/guard/egress_guard.py:82](../../scripts/guard/egress_guard.py#L82) [VERIFIED]) so any outbound call during request handling fails the OS-level no-network sandbox
- `tests/serve/test_egress.py` -- the guard over a real upload→ingest→re-render dispatch observes 0 outbound calls; an injected outbound call drives the guard to FAIL (failing-capable)

**Acceptance Criteria:**
1. A real upload→ingest→re-render request dispatch (a POST `/upload` of a synthetic `export.xml`) wrapped in ONE zero-arg closure passed to `egress_guard.run` returns truthy (0 outbound network calls observed across the whole dispatch) (ADR-0013 Confirmation 2) — the closure wraps the production request path, not a no-op.
2. Injecting a synthetic outbound call into the request-handler dispatch drives `egress_guard.run` to return falsy (FAIL) — the guard is failing-capable over the new request code path (subprocess/async-spawned calls included, since the guard is OS-level) (ADR-0013 Falsification 2) — verified by wrapping a dispatch that makes one outbound call and asserting the falsy result.
3. The request-handler dispatch in `scripts/serve/server.py` is wrapped by `egress_guard.run` (not merely tested under it) — `rg` finds the `egress_guard.run` wrap in `server.py` so the production server runs the request path under the sandbox, verified by the import + call presence.
4. `pytest tests/serve/test_egress.py` passes.

**Risk Mitigations:** ADR-0013 Negative-1 (the new listening socket is a leak surface; must be proven egress-free by test) — criteria 1-2 are the failing-capable 0-egress proof over the real request path. ADR-0001 "0 outbound calls carrying store content" — criterion 1 realizes that boundary on the server surface.
**Dependencies:** ADR-0013-T4 (the request path the guard wraps). Blocks: (Wave B consumes the guard-wrapped server).
...
**Dependencies:** ADR-0013-T4 (the server's POST handler the form submits to), ADR-0013-T5 (the guard-wrapped request path the capture write runs under). Store substrate (`store.append`, `keying.py`, `router.summarize`, `pii_scan.scan`) is a prior-wave prerequisite (verified present).
...
ADR-0013-T4 --> ADR-0013-T5   (the request path the egress guard wraps)
ADR-0013-T4 --> ADR-0014-T1   (the POST handler the capture form submits to)
ADR-0013-T5 --> ADR-0014-T1   (the guard-wrapped request path the capture write runs under)
```
...
**Wave grouping for the build plan:** Wave A = {ADR-0013-T1..T5} (the upload server, the more-foundational transport + the Apple-zip extraction + the egress proof). Wave B = {ADR-0014-T1} (the interactive wizard forms + the capture persistence + the store-adversarial battery, depending on Wave A's POST handler + guard-wrapped path).

## Test Strategy

### Unit Tests
- **Scope:** `scripts/serve/__main__.py`, `scripts/serve/server.py`, `scripts/serve/multipart.py`, `scripts/serve/apple_zip.py`, `scripts/serve/route.py`, `scripts/serve/capture.py`.
...
- **Criteria covered:** ADR-0013-T1 1-5; ADR-0013-T2 1-5; ADR-0013-T3 1-6; ADR-0013-T4 1-7; ADR-0013-T5 1-4; ADR-0014-T1 1-8.

### Integration Tests
- **Scope:** the cross-task paths — (a) the full upload→stage→extract→ingest→re-render request path; (b) the egress guard wrapped around the real request dispatch; (c) the capture round-trip (form → store item → `summarize`); (d) the fresh-clone tracked-file PII scan after a capture session; (e) the `git diff --numstat` 0-shared-routine-edit proofs (ADR-0013-T3 crit 5, ADR-0013-T4 crit 6).
...
- **ADR-0003 0-shared-routine-edit:** `git diff --numstat` = 0 on `ingest.py`/`adapter.py`/`scheduler.py`/`dna.py`/`healthkit.py` (ADR-0013-T3 crit 5, ADR-0013-T4 crit 6).
...
- [x] Loopback-only (ADR-0013 → ADR-0013-T1 crit 1); 0-egress under the guard (ADR-0001/0013 → ADR-0013-T5 crit 1-2); uploads land only in gitignored paths (ADR-0013/0014 → ADR-0013-T2 crit 2, ADR-0014-T1 crit 2-3); 0-shared-routine-edit (ADR-0003 → ADR-0013-T3 crit 5, ADR-0013-T4 crit 6); intake-only / no artifact-serving (ADR-0004/NG-4 → ADR-0013-T4 crit 5); the de-identification gate (ADR-0001/0014 → ADR-0014-T1 crit 1, 4).
