...
| ADR-0014 OQ-2 | Open Question | The curation surface for `rx-interaction-classes` (how the operator/liaison emits de-identified class tokens, never raw drug names, and where that step lives) | Proceed (record-only this build) | The Step-5 stack capture lands raw names ONLY in the gitignored scaffold; the `rx-interaction-classes` store item is written from the operator/liaison-curated class tokens, not derived in code from raw names ([scripts/plan/router.py:137-176](../../scripts/plan/router.py#L137) [VERIFIED]). This build captures the raw Step-5 stack record-only (Wave-B FIX-A) and the capture seam NEVER writes the model-bound `rx-interaction-classes` store item — that item is fed only by the deferred OQ-2 curation surface, not this build's form; no code lookup of raw names is built. |
...
6. The raw Step-5 `rx-interaction-classes` form field is NOT written to the model-bound `rx-interaction-classes` store item — it routes RECORD-ONLY to the gitignored scaffold (Wave-B FIX-A), because the form cannot be trusted to carry only de-identified class tokens and `pii_scan` does not detect drug names — verified by `rg` finding 0 raw-name→class lookup in `scripts/serve/` AND capturing a drug name into the rx field then asserting it is ABSENT from `store.read("rx-interaction-classes")` and present in the scaffold. The curated wired item is deferred to the OQ-2 curation surface (beaded; [scripts/plan/router.py:137-176](../../scripts/plan/router.py#L137) [VERIFIED]).
