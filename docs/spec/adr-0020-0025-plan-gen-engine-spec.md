...
How it fits the architecture: the dependency tiers (DAG §4) are Tier 1 = the PII envelope (ADR-0020 de-id IN ∥ ADR-0021 de-id OUT), Tier 2 = the runtime control surface (ADR-0022 orchestrator, depends on 0020's de-identified-summary input contract), Tier 3 = the review+output layers (ADR-0023 ∥ ADR-0024 ∥ ADR-0025, all depending only on Tiers 1-2). The RUNTIME stage order DIFFERS from the dependency tiers and is the sequence the orchestrator executes one plan through (DAG §"Runtime Stage Order"): **(1) de-id IN (0020) → (2) orchestrate/assemble (0022 + inner engine) → (3) {quality judge (0023) ∥ safety review (0024)} → (4) bounded revise loop → (5) de-id OUT re-insertion (0021, at render only) → (6) maintained-format render (0025)**. The load-bearing divergence: ADR-0021 (de-id OUT) is Tier-1 by dependency but executes LAST-but-one at render time.
...
| `scripts/plan/deid_in.py` | Create | The model-backed de-id-IN boundary: ingests raw plan-intake, returns the de-identified summary via the `ModelClient` no-train lane, fail-closed-halt on failure/outage. |
...
| `scripts/plan/plan_orchestrator.py` | Create | The programmatic subscription orchestrator: drives the de-id-IN → orchestrate/assemble → {judge ∥ safety} → revise → de-id-OUT → render runtime over the inner engine, inlining specialist role profiles, fail-closed on the inner safety gate. |
...
**Files to create/modify:**
- `scripts/plan/deid_in.py` -- the de-id-IN boundary: takes raw plan-intake + the wired `ModelClient`, returns the de-identified summary over the no-train lane; reuses the existing `ModelClient` seam (does NOT re-introduce the client); `router.summarize` survives unchanged as the persisted-side de-id
- `tests/plan/test_deid_in.py` -- unit tests with a mock/`_FixedEnvelopeClient`-style client + synthetic raw-PII fixtures

**Acceptance Criteria:**
1. `deid_in(raw_intake, client)` returns a de-identified summary object the orchestrator consumes; the call routes only through the `ModelClient` no-train lane (asserted by inspecting the lane on every outbound call in the test — count of de-id-IN calls on any training-eligible lane = 0).
...
4. `pytest tests/plan/test_deid_in.py` passes including the outage cases, against mock clients (0 live calls).

**Risk Mitigations:** ADR-0020 Consequence-Negative-4 / OQ-4 (whole-run outage unspecified would let the engine silently degrade de-id fidelity) — mitigated by the fail-closed halt (criteria 1-2). ADR-0025 Falsification-2 (no partial/stale re-emit) — criterion 3.
...
5. `pytest tests/plan/test_plan_orchestrator.py` passes including the three revise-loop probes (0 live calls).

**Risk Mitigations:** ADR-0023 Consequence-Negative-3 / ADR-0024 Consequence-Negative-2 (unbounded loop / multi-lens over-block) — mitigated by the N=3 cap (criterion 3) + safety-block terminal (criterion 2). ADR-0024 Consequence-Negative-3 (the safety tier is load-bearing) — criterion 2.
...
6. `pytest tests/generate/test_maintained.py` passes against synthetic fixtures (0 live calls, 0 real operator PII in the test tree).

**Risk Mitigations:** ADR-0025 Consequence-Negative-1 (maintained-artifact state/staleness/divergence) — mitigated by atomic re-emit (criterion 4). ADR-0025 Consequence-Negative-2 (widened local-PII surface) + Negative-3 (the name scan-scope hole) — mitigated by the gitignored-only output + the T-SCANSCOPE coverage (criterion 5). Constraint ADR-0004 (single-file/<500KB/WCAG-AA/0-external budget) — criterion 2. Constraint ADR-0005 (no committed PII) — criterion 5.
...
- **Criteria covered:** ADR-0021-T0-SCANSCOPE 1-6, ADR-0020-T1 1-5, ADR-0020-T2 1-4, ADR-0021-T1 1-5, ADR-0022-T1 1-6, ADR-0022-T2 1-5, ADR-0022-T3 1-5, ADR-0023-T1 1-5, ADR-0024-T1 1-5, ADR-0025-T1 1-6.

### Integration Tests
- **Scope:** the end-to-end runtime-stage-order path — de-id IN (0020) → orchestrate/assemble (0022 + inner engine) → {judge ∥ safety} → revise loop → de-id OUT (0021) → maintained render (0025) — over a synthetic fixture with mock clients.
...
### Risk-Specific Tests
- **Scope:** the crown-jewel leak probes (ADR-0020/0021/0025 raw-PII / re-inserted-PII thresholds), the de-id outage-halt (ADR-0020 OQ-4), the inner-safety-gate-bypass + standing-safety-block-terminal (ADR-0022/0024), the bounded-revise-cap (ADR-0022/0023), and the dispatch-cap (ADR-0022-T3).
- **Approach:** seed each adversarial condition (a synthetic raw-PII token; a whole-run outage; an unadjudicated hold; a standing safety block; a non-converging plan; a name-bearing committed artifact; an over-budget dispatch tally) and assert the 0-threshold / fail-closed outcome.
- **Criteria covered:** ADR-0021-T0-SCANSCOPE 2-4, ADR-0020-T1 2+5, ADR-0020-T2 1-3, ADR-0021-T1 2+4, ADR-0022-T1 4, ADR-0022-T2 2-3, ADR-0022-T3 2, ADR-0023-T1 3, ADR-0024-T1 2-3, ADR-0025-T1 4-5.
...
| ADR-0020-T1 | pass — `scripts/plan/deid_in.py` absent (Create); `tests/plan/test_deid_in.py` absent (Create) | pass — `ModelClient.author` seam present (`client.py:60`), `_author_callable`/`AUTHOR_CALL_FAILED` present (`generate_plan.py:90,68`); `router.summarize` persisted-side de-id present | pass — `ModelClient` + `_FixedEnvelopeClient` exist and are the declared mock seam (`generate_plan.py:371`) | pass — no existing de-id-IN module/test (grep over `tests/` found none) | Grounded |
