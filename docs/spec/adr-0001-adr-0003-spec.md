...
**Consequence-label convention:** throughout this spec, `ADR-XXXX Nn` denotes the n-th bullet of that ADR's Consequences → Negative list. The source ADRs render those bullets unlabeled, so `Nn` is a positional reference into the bullet list, not an ADR-defined register ID; the short paraphrase beside each label is the source of truth and will visibly mismatch if an ADR ever reorders its Negative bullets.
