...
| `vault/design/templates/app_view.html` | Modify | **[AMENDED 2026-07-17 S143 — PF-S142-01 retarget]** The credential UI markup + JS: extend the SERVED SPA `screen-wizard` step 9 ("API key") into the "Connect your data & keys" credential step (Surface A) + extend `screen-profile` with the "Connections & Keys" panel (Surface B); the per-source connection status is delivered CLIENT-SIDE via a `fetch('/settings/trackers')` (the already-routed keychain-derived endpoint, mirroring the `/settings/key` pill) — no server render-state seam. (The prior manifest named `intake.py` — an UNSERVED CLI-only 6-step artifact; `server.py` serves only `generate.run('app')`, whose intake IS this SPA `screen-wizard`, already a 9-step flow with step 9 = "API key".) **[+AMENDED S144]** ALSO Modified by ADR-0049-T2 for the **"Plan updates" section** of `screen-profile` (the Update-now trigger POSTing the existing CSRF-gated `/plan-loop` + the Off/Weekly/Daily schedule radio + its client-side schedule JS) — a distinct section following T4's Connections & Keys panel, dependency-ordered. |
| ~~`vault/design/templates/app_shell.py`~~ | — | **[AMENDED S143 (T4) + S144 (T2) — REMOVED from BOTH]** Neither T4 nor T2 touches `app_shell.py`. `screen-profile`'s markup lives in `app_view.html`; `app_shell.py` holds only the `_PLATFORM_SCREENS` id-tuple and fills `<!--MARKER-->` placeholders. T4's connection status and T2's schedule state are both delivered CLIENT-SIDE (mirroring the `/settings/key` pill), not via a server render-state seam — the prior `connections=` inject was a dead extension point (`_tracker_status` is a JSON HTTP handler, not a render source; QA-M1/Arch-F1), and the same reasoning applies to the schedule setting. |
...
- **[AMENDED S143 (T4) + S144 (T2) — PF-S142-01 class]** `vault/design/templates/app_view.html` is Modified by BOTH ADR-0048-T4 (the credential step 9 + the Connections & Keys panel + the client-side `/settings/trackers` status JS) AND ADR-0049-T2 (the "Plan updates" section + the client-side schedule JS) — **distinct sections of `screen-profile`**, dependency-ordered (T2 depends on T4, which builds the panel T2's section follows). `vault/design/templates/app_shell.py` is Modified by NEITHER (the markup lives in `app_view.html`; both surfaces' live state is client-side). `scripts/serve/server.py` is Modified by ADR-0048-T3 (per-source writes), ADR-0048-T2 (the connect-start route), and ADR-0049-T2 (the schedule-setting route only — the trigger reuses the existing `/plan-loop`) — distinct handler regions, cross-wave.
...
**Files to create/modify:**
- `vault/design/templates/app_view.html` -- **[AMENDED 2026-07-17 S144 — PF-S142-01 class, same as the T4 retarget]** the My Info "Plan updates" section in `screen-profile`: the Update-now button (POSTing the already-routed CSRF-gated `/plan-loop`, `server.py:235`→`_do_plan_loop`) + the Off/Weekly/Daily schedule radio + the client-side schedule JS (mirroring the `/settings/key` pill pattern). (The prior manifest named `app_shell.py` — but `screen-profile`'s MARKUP lives in `app_view.html` [11 refs]; `app_shell.py` holds only the `_PLATFORM_SCREENS` id-tuple [1 ref] and fills `<!--MARKER-->` placeholders. `app_shell.py` is NOT touched by T2 — the schedule state is client-side, not a server render-state seam, consistent with the S143 T4 decision.)
- `scripts/serve/server.py` -- **[AMENDED S144]** the schedule-SETTING persistence route only (no existing cadence-setting mechanism; mirrors the `/settings/key` route shape + its `application/json` CSRF gate). The manual TRIGGER needs no new route — it reuses the already-routed CSRF-gated `/plan-loop` (`:235`→`_do_plan_loop`@`:865`, whose `application/json` 415 gate `:895` precedes the degrade early-return `:904` which precedes `regenerate` `:911`), which fires `run_orchestrated`→`drive`; note `loop_dispatch = None` (`server.py:175`) so the loop is honestly degraded until bead `23xr` arms the metered dispatch — the trigger issues 0 metered spend today.
- `tests/serve/test_plan_update_trigger.py` -- one-re-gen-per-request, schedule-setting persistence, Daily inert
- `tests/serve/test_app_shell.py` -- **[AMENDED S144 — Arch-F8/judge-D5]** admit the two NEW same-origin fetch literals (`/plan-loop`, `/settings/schedule`) to the fetch allowlist (`_KNOWN_LOOPBACK` + the same-origin-loopback assertion set) — the baseline REDs without it; exact literals, no prefix-loosening

**Acceptance Criteria:**
1. **[AMENDED 2026-07-17 S144 — Arch-F3: the metered `loop_dispatch` is `None` until bead `23xr`]** The "Update my plan now" trigger fires exactly ONE loop tick per explicit request through the existing CSRF-gated `/plan-loop` → `_do_plan_loop` → `run_orchestrated`→`drive` seam, and 0 ticks with no request and no armed schedule. Today `loop_dispatch = None` (`server.py:175`), so the tick reaches the honest-degraded response (`{degraded: true, reason: "loop-dispatch-unavailable"}`) at **0 metered spend / 0 dispatches** — the loop is honestly degraded until `23xr` arms the metered dispatch. AC-1 asserts the loop entry-point is invoked EXACTLY ONCE per request (a spy, `$0`), NOT metered spend (which is 0 by construction until `23xr`).
...
3. **[AMENDED 2026-07-17 S144 — Arch-F5: no scheduler consumes the intent until `23xr`/glzi]** The schedule SETTING persists the selected cadence (Off default); selecting Weekly records the weekly cadence intent, **read back by the Profile schedule UI on load** (the current-cadence round-trip that survives reload). NO scheduler consumes the intent today — `activate.status()` reads the REAL launchd registration, not this store reading, and arming the weekly runner is the operator-owned `activate.enable()` (out of scope). The recorded `plan-update-cadence` intent stream is an explicit handoff to `23xr`/glzi (when the daily/auto-run enable path lands, it reads this intent — flagged in `23xr`).
...
5. **[AMENDED S144 — anchors corrected]** The "Update my plan now" trigger reuses the CSRF-gated `_do_plan_loop` handler (`server.py:865`; the SEC `application/json` 415 gate at `server.py:895`; the `/plan-loop` route at `server.py:235`), so a cross-site CORS-simple `text/plain` POST is refused 415 BEFORE any dispatch/loop work; the schedule-setting route carries the identical `application/json` gate. Mutation-proven RED-capable: for `/plan-loop` the STATUS (415, which flips to a 200 degraded body if the gate is dropped — the 0-invocations form is vacuous under `loop_dispatch=None`); for the new `/settings/schedule` the "415 with 0 `store.append`" (no early-return between its gate and the append).
6. `git diff --numstat 3ab1c3ab -- scripts/store/store.py scripts/store/keying.py scripts/plan/pipeline.py scripts/plan/adjudicate.py scripts/plan/adjust.py scripts/plan/router.py` prints nothing.
...
| ADR-0049-T2 | **[AMENDED S144 — Arch-F4]** pass (`app_view.html` [the "Plan updates" markup in `screen-profile` — NOT `app_shell.py`, which holds only the `_PLATFORM_SCREENS` id-tuple], `server.py` [the `/settings/schedule` route only — the trigger reuses `/plan-loop`], `test_plan_update_trigger.py` [Create], + `tests/serve/test_app_shell.py` [the fetch-allowlist admission]) | pass (`/plan-loop`@235/`_do_plan_loop`@865 loop path — response `{results, degraded, reason}`, NO `ok` field, always degraded until `23xr`; `DAILY_MONITOR_LABEL`@activate:52 label-only, no daily enable) | pass (`_do_plan_loop`→`plan_loop.regenerate`→`run_orchestrated`; the mockup "Plan updates" section; the cadence intent recorded via `store.append`, read back by the Profile UI, unconsumed by any scheduler until `23xr`) | pass | **Grounded (S144 re-reconciled)** |
