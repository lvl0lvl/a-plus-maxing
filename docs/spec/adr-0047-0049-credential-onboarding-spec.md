...
| `vault/design/templates/intake.py` | Modify | Add the "Connect your data & keys" credential step to the intake stepper (mockup Surface A) |
| `vault/design/templates/app_shell.py` | Modify | Add the My Info "Connections & Keys" panel (T4-0048) AND the "Plan updates" section (T2-0049) — two distinct sections of `screen-profile` |
...
**Files to create/modify:**
- `vault/design/templates/intake.py` -- add the "Connect your data & keys" credential step to the intake stepper (mockup Surface A)
- `vault/design/templates/app_shell.py` -- add the My Info "Connections & Keys" panel (mockup Surface B), reachable from `screen-profile`
...
**Acceptance Criteria:**
1. The intake stepper renders a "Connect your data & keys" step with one-click connect cards (Whoop, Oura, Garmin, Google), an Oura PAT paste fast-path, an Apple Health "Get the Shortcut" affordance, and the D5 model-key choice (shared alpha default / bring-your-own) — matching the enumerated elements of `prototype/credential-onboarding-mockup.html` Surface A. (The step is appended to the LIVE intake stepper; see the Repo-Grounding Ledger reconciliation of the mockup's "Step 9 of 9" framing.)
