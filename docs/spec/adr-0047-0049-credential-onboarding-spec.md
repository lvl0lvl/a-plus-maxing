...
| `vault/design/templates/app_view.html` | Modify | **[AMENDED 2026-07-17 S143 — PF-S142-01 retarget]** The credential UI markup: extend the SERVED SPA `screen-wizard` step 9 ("API key") into the "Connect your data & keys" credential step (Surface A) + extend `screen-profile` with the "Connections & Keys" panel (Surface B). (The prior manifest named `intake.py` — an UNSERVED CLI-only 6-step artifact; `server.py` serves only `generate.run('app')`, whose intake IS this SPA `screen-wizard`, already a 9-step flow with step 9 = "API key".) |
| `vault/design/templates/app_shell.py` | Modify | **[AMENDED 2026-07-17 S143]** The render-state for the Connections & Keys panel (T4-0048): the keychain-derived per-source connection map (via `_tracker_status`/`oauth_pull._read_oauth_credential`) filled into the `screen-profile` markup; AND the "Plan updates" section render-state (T2-0049) — distinct sections. (`app_shell.render` reads `app_view.html` + fills `<!--MARKER-->` placeholders.) |
...
**Files to create/modify:**
- `vault/design/templates/app_view.html` -- **[AMENDED 2026-07-17 S143 — PF-S142-01]** extend the served SPA `screen-wizard` step 9 ("API key") into the "Connect your data & keys" credential step (Surface A) + extend `screen-profile` with the "Connections & Keys" panel (Surface B). (Retargeted from `intake.py`, which is unserved — see the amended File Manifest + Repo-Grounding Ledger.)
- `vault/design/templates/app_shell.py` -- **[AMENDED 2026-07-17 S143]** the render-state for Surface B's connection panel: the keychain-derived per-source connection map (`_tracker_status`) filled into the `screen-profile` markup via `app_shell.render`'s marker substitution
