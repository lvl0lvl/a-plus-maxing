...
| `scripts/plan/orchestrate.py` | Modify | **SUPERSEDES** `reconcile`'s five hard-coded pair-specific holds (337-456) with one uniform `cross_domain_seams` pass; the safety-critical subset (RED-S/LEA, additive-AE, Rx-BPMH) is retained as always-on floors (re-based, not deleted). ADR-0043-T2. *[AMENDED 2026-07-12 — Architect, Wave-4 recipe review: also modified by ADR-0043-T3 — `cross_domain_seams` seam-key single-source (pule) + conflict-finding (ncsy)]* |
...
| `scripts/plan/horizons.py` | Modify | Re-bases the ADR-0038 horizon date-range composition: `window_block` (132) reads the comprehensive model's first-class periodization, not the flat `plan::<domain>` history. ADR-0044-T3. *[AMENDED 2026-07-12 — Architect, Wave-4 recipe review: also a D1 `PLAN_DOMAINS`-iterator consumer ADR-0043-T3 coordinates with on the roster growth]* |
...
| `scripts/plan/tailoring.py` | Modify (coordinate) | *[AMENDED 2026-07-12 — Architect, Wave-4 recipe review]* D1 `PLAN_DOMAINS`-iterator consumer — coordinated with ADR-0043-T3's roster growth per the recipe. ADR-0043-T3. |
| `scripts/plan/plan_orchestrator.py` | Modify (coordinate) | *[AMENDED 2026-07-12 — Architect, Wave-4 recipe review]* D1 `PLAN_DOMAINS`-iterator consumer (`_dispatch_domains`) — coordinated with ADR-0043-T3's roster growth per the recipe. ADR-0043-T3. |
| `scripts/serve/confirm.py` | Modify (coordinate) | *[AMENDED 2026-07-12 — Architect, Wave-4 recipe review]* D1 `PLAN_DOMAINS`-iterator consumer — coordinated with ADR-0043-T3's roster growth per the recipe. ADR-0043-T3. |
| `tests/serve/test_server.py` | Modify | *[AMENDED 2026-07-12 — Architect, Wave-4 recipe review]* Behavioral-test rewrite (QA-identified) — the `set(...) == set(PLAN_DOMAINS)` assertion the roster growth forces. ADR-0043-T3. |
| `tests/serve/test_plan_loop.py` | Modify | *[AMENDED 2026-07-12 — Architect, Wave-4 recipe review]* Behavioral-test rewrite (QA-identified) — the `set(...) == set(PLAN_DOMAINS)` assertion the roster growth forces. ADR-0043-T3. |
| `tests/serve/test_plan_loop_hold.py` | Modify | *[AMENDED 2026-07-12 — Architect, Wave-4 recipe review]* Behavioral-test rewrite (QA-identified) + the `plan_driver.py` frozen-guard carve (Wave-4 reconciliation). ADR-0043-T3. |
...
- **[AMENDED 2026-07-12 — Architect, Wave-4 recipe review]** additional co-landing source surfaces (the recipe expands the manifest from 6 to 9 source files, grounding the Wave-3 review beads that post-date the spec's original 0043-T3 manifest):
  - `scripts/plan/orchestrate.py` (Modify) — the `cross_domain_seams` seam-key single-source (pule) + the conflict-finding (ncsy), co-landing with the synthesize path's reconcile consumption. Disjoint from ADR-0043-T2's reconcile-generalization region (Wave 3).
  - `scripts/plan/domain_program.py` (Modify) — the `cross_domain_seams` seam-key constants (pule) + the `prescription` top-level-identity pin (ubsp). Disjoint from ADR-0041-T1's create.
  - `scripts/store/plan_model.py` (Modify) — the `read_standing_plan` renderable-shape reconciliation (qrg4). Disjoint from ADR-0044-T4's `read_plan_version` confirm-hold region — the Wave-4 intra-wave pair, resolved by non-overlap verification + build order 0044-T4→0043-T3 (see the build plan's Cross-Spec Coordination table; NOT a dependency edge).
- **[AMENDED 2026-07-12 — Architect, Wave-4 recipe review]** behavioral-test rewrites (QA-identified — the `set(...) == set(PLAN_DOMAINS)` assertions the roster growth forces): `tests/serve/test_server.py` (Modify), `tests/serve/test_plan_loop.py` (Modify), `tests/serve/test_plan_loop_hold.py` (Modify — also the `plan_driver.py` frozen-guard carve).
- **[AMENDED 2026-07-12 — Architect, Wave-4 recipe review]** D1 `PLAN_DOMAINS`-iterator consumers coordinated with the roster growth (per the recipe): `scripts/plan/horizons.py` (0044-T3 — coordinate), `scripts/plan/tailoring.py`, `scripts/plan/plan_orchestrator.py`, `scripts/serve/confirm.py`.
...
- `scripts/store/plan_schema.py` (Modify) — supersede the four thin validators, `record_plan`, `resolve_plan`, and `PLAN_DOMAINS`'s stored-plan-domain-key role — routed to `plan_model`. `PLAN_DOMAINS`'s dispatch-roster-registry role survives (RT-009; grown by ADR-0043-T3 **[AMENDED 2026-07-12 — Architect, 0046-T1 re-sequence (Option C): the dispatch-roster-registry growth is ADR-0043-T3 (Wave 4), NOT ADR-0046-T1; see ruling-0046-t1-sequencing.md]**). Disjoint from the tuple-growth region.
...
**Risk Mitigations:** ADR-0046 Consequences-Negative-1 (dispatch fan-out cost scales with active count) — AC-5 progressive activation caps at the active set. ADR-0046 Consequences-Negative-2 (activation mis-fire either direction) — AC-3 false-positive + AC-4 false-negative probes. ADR-0046 Consequences-Negative-4 (roster >4 trips `LARGE_CHANGE_THRESHOLD_DOMAINS`, disposition #36) — the explicit threshold re-base is ADR-0046-T2 (a hard sequencing precondition before the scaled loop runs live). Constraint-propagation (ADR-0041 — each dispatched specialist emits the uniform shape) — **[AMENDED 2026-07-12 — Architect, 0046-T1 re-sequence (Option C)]** 0046-T1's 58z0 projection CONSUMES the ADR-0041-T2 migrated uniform shape (the grown `_PLAN_TRANSLATORS` that EMIT the migrated shape are ADR-0043-T3's roster growth, Wave 4; see ruling-0046-t1-sequencing.md).
**Dependencies:** ADR-0041-T1 (each dispatched specialist emits the uniform DOMAIN PROGRAM), ADR-0041-T2 (**[AMENDED 2026-07-12 — Architect, 0046-T1 re-sequence (Option C)]** the migrated `_PLAN_TRANSLATORS` uniform shape that 0046-T1's 58z0 projection CONSUMES; the shape migration lands first — the roster growth that rebases on it is now ADR-0043-T3's, Wave 4; see ruling-0046-t1-sequencing.md) *[AMENDED 2026-07-12 — Architect, Wave-4 recipe review: the ADR-0044-T1 dependency is REMOVED — post-Option-C 0046-T1 ships only `activation.py` + the 58z0 `generate_plan.py` projection; `activation.py` imports neither `plan_model` nor `plan_schema`, and `generate_plan.py` imports `plan_schema` but not `plan_model`, so 0046-T1 depends on ADR-0044-T1 for nothing; surviving predecessors are ADR-0041-T1 + ADR-0041-T2]*
...
ADR-0041-T2 --> ADR-0046-T1      (shared generate_plan.py _PLAN_TRANSLATORS; shape migration before the 58z0 projection)
...
*[AMENDED 2026-07-12 — Architect, Wave-4 recipe review]* The `ADR-0044-T1 --> ADR-0046-T1` edge is REMOVED: post-Option-C 0046-T1 no longer writes `plan_schema.py` (the roster growth moved to ADR-0043-T3), so the shared-`plan_schema.py` justification for that edge is void — `activation.py` imports neither `plan_model` nor `plan_schema`, and 0046-T1's `generate_plan.py` 58z0 projection imports `plan_schema` but not `plan_model`, so 0046-T1 depends on 0044-T1 for nothing. 0046-T1's surviving predecessors are 0041-T1 + 0041-T2. The `ADR-0041-T2 --> ADR-0046-T1` edge STAYS (both write `generate_plan.py`). The DAG stays acyclic and the six-wave structure is unchanged (removing an edge cannot create a cycle or reorder waves).
...
3. **ADR-0044-T2** (←0044-T1), **ADR-0046-T1** (←0041-T1, 0041-T2) *[AMENDED 2026-07-12 — Architect, Wave-4 recipe review: 0044-T1 predecessor dropped]*, **ADR-0045-T1** (←0041-T1, 0044-T1), **ADR-0043-T2** (←0041-T1, 0043-T1)
...
**Tier consistency:** every dependency edge points down-tier or within-tier — Tier-1 tasks (0041/0042) are in groups 1-2; Tier-2 tasks (0043/0044/0046) span groups 2-5; Tier-3 tasks (0045) span groups 3-6, each depending on its Tier-1/Tier-2 predecessors (0045-T1 ← 0041-T1 + 0044-T1; 0045-T2 ← 0043-T2 + 0044-T4). No Tier-3 task precedes a Tier-2 dependency. **Intra-group file-writer check:** no parallel group holds two writers of one mutable file *[AMENDED 2026-07-12 — Architect, Wave-4 recipe review: EXCEPT the one named group-4 `plan_model.py` pair — 0044-T4 `read_plan_version` ∥ 0043-T3 `read_standing_plan`, disjoint function regions resolved by non-overlap verification + build order 0044-T4→0043-T3; the build's first intra-group shared-file pair, NOT a dependency edge (an edge would push 0043-T3 to group 5)]* — group 3's `plan_schema.py` writer (0046-T1) is ordered after group 2's `plan_schema.py` writer (0044-T1) **[AMENDED 2026-07-12 — Architect, 0046-T1 re-sequence (Option C): 0046-T1 no longer writes `plan_schema.py` — the `plan_schema.py` writer ordered after 0044-T1 is now ADR-0043-T3 (Wave 4); 0046-T1 remains only a `generate_plan.py` writer (58z0 projection); see ruling-0046-t1-sequencing.md §B5]**; group 3's `care_chat.py` writer (0043-T2) is ordered after group 2's (0043-T1); the `plan_model.py` writers 0044-T1/T2/T4 are chained across groups 2→3→4, plus *[AMENDED 2026-07-12 — Architect, Wave-4 recipe review]* the fourth writer 0043-T3 (group 4, `read_standing_plan`) forming the named group-4 intra-group pair with 0044-T4 (`read_plan_version`) — disjoint regions, non-overlap verification + build order, above; the two `plan_loop.py` writers (0043-T3 group 4, 0046-T2 group 5) are ordered; the three `generate_plan.py` writers (0042-T1 group 2, 0041-T2 group 2, 0046-T1 group 3) touch disjoint regions and are serialized 0042-T1 → 0041-T2 → 0046-T1.
...
- **Per-ADR-scoped freeze-break numstat probes (each scoped to its OWN commit, excluding sibling-superseded files):** 0041-T2 AC-4 (generate_plan.py+assemble.py; frozen four + siblings EMPTY); 0042-T1 (router.py EMPTY, the plan-path USE superseded not router.py); 0043-T2 AC-5 (orchestrate.py; frozen four + siblings EMPTY); 0043-T3 AC-5 (frozen four EMPTY); 0044-T1 AC-4 (pipeline.py+adjudicate.py+store.py+keying.py == 0; four named plan_schema.py surfaces superseded; siblings excluded); 0046-T1 **[AMENDED 2026-07-12 — Architect, 0046-T1 re-sequence (Option C)]** (net-new activation.py + the 58z0 generate_plan.py projection; frozen four EMPTY; NO plan_schema.py growth — the `grows plan_schema.py`/`plan_driver.py` dispatch-registry freeze-break probe is ADR-0043-T3's, Wave 4; see ruling-0046-t1-sequencing.md); 0045-T1/T2/T3 (net-new, frozen four EMPTY — no freeze break of its own). The probes are scoped per-commit so they do not forbid each other's intended supersession.
...
- [x] No parallel group holds two writers of one mutable file EXCEPT the one named group-4 `plan_model.py` pair (0044-T4 `read_plan_version` ∥ 0043-T3 `read_standing_plan`, disjoint function regions resolved by non-overlap verification + build order 0044-T4→0043-T3, NOT a dependency edge — *[AMENDED 2026-07-12 — Architect, Wave-4 recipe review]*); all other shared-file writers — `generate_plan.py`, `care_chat.py`, `plan_model.py`, `plan_schema.py`, `plan_loop.py` — are serialized across groups via ordering edges; disjoint regions documented

### Constraint Propagation
- [x] Constrained ADR tasks reflect upstream constraints in their ACs (ADR-0041 uniform shape → 0043-T2 AC-1 reconciles it / 0044-T1 AC-1 stores it / 0045-T1 AC-1 compiles fields 3-4 / 0046-T1 AC via activation + the 58z0 projection over the ADR-0041-T2 migrated shape **[AMENDED 2026-07-12 — Architect, 0046-T1 re-sequence (Option C): the grown `_PLAN_TRANSLATORS` EMITTING the migrated shape is ADR-0043-T3's roster growth (Wave 4); 0046-T1's constraint contribution is the activation gate + the 58z0 projection that CONSUMES the migrated shape; see ruling-0046-t1-sequencing.md]**; ADR-0043 front-door → 0043-T3 AC-3; ADR-0044 config-home → 0045-T1 AC-4)
