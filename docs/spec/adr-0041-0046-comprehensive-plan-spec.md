...
| `scripts/plan/generate_plan.py` | Modify | **SUPERSEDES** the four thin translators + `_PLAN_TRANSLATORS` (123-291) via the transitional additive adapter (ADR-0041-T2); the plan-path `router.summarize` call at :356 swaps to the Context Assembler (ADR-0042-T1); `_PLAN_TRANSLATORS` grows 4→15+ (ADR-0046-T1). Three disjoint regions, serialized. |
...
| `scripts/plan/plan_driver.py` | Modify | Grows `_ROLE_OF_DOMAIN` (117-122) from the four hard-wired domains to the 15+-specialist roster (each maps to its inlined role profile). ADR-0046-T1. |
...
### ADR-0046-T1: Progressive-Activation Gate at Dispatch + Grow the Dispatch Roster 4→15+
**Status:** TODO
**ADR Source:** ADR-0046, Decision (scale dispatch from the four hard-wired domains to the 15+-specialist roster gated by PROGRESSIVE ACTIVATION; the active-domain set is computed from the operator's surface at plan time, not the closed 4-tuple; GROWS `PLAN_DOMAINS`'s dispatch-roster role `plan_schema.py:49`, `_ROLE_OF_DOMAIN` `plan_driver.py:117-122`, `_PLAN_TRANSLATORS` `generate_plan.py:286-291`; RT-009 — the dispatch-registry role, distinct from ADR-0044's stored-key role); OQ-1 (activation signal — resolved in-spec, disposition #23); OQ-2 (gate at DISPATCH, disposition #24); OQ-3 (§1-13 card-emitting vs §14-genetics/§15-labs partition, disposition #25); Validation confirmation #1/#2 + falsification (all three)
**Files to create/modify:**
...
- `scripts/store/plan_schema.py` (Modify) — `PLAN_DOMAINS`'s dispatch-roster-registry role grows 4→15+ (keeps tuple shape; RT-009 — the stored-key role is retired by ADR-0044-T1). Disjoint region from ADR-0044-T1's validator/resolver supersession.
- `scripts/plan/plan_driver.py` (Modify) — `_ROLE_OF_DOMAIN` (117-122) grows to the 15+-specialist roster (each domain → its inlined role-profile slug).
- `scripts/plan/generate_plan.py` (Modify) — `_PLAN_TRANSLATORS` (286-291) grows 4→15+ (disjoint from ADR-0041-T2's translator-shape migration + ADR-0042-T1's :356 swap).
...
2. Active domain dispatched + rendered (confirmation #1): seed a goal/data in a domain D beyond the original four; **dispatch-count-for-D == 1 AND card-for-D present == True**.
