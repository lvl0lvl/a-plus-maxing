...
2. A content scan of all tracked files for operator-PII tokens returns hit count 0 on a fresh clone (`block-pii-commit.sh` run in scan mode invokes the cross-spec `pii_scan.scan(tracked_files)` over `git ls-files` and reports 0 hits — the hook reuses the data-in token set, not an independent bash reimplementation).
...
5. `block-pii-commit.sh` blocks (non-zero exit) a commit when a PII token is planted in a tracked file — the hook routes the staged/tracked file set through the cross-spec `pii_scan.scan` and blocks on its ≥1 hit count — and passes (exit 0) on a staged tree with 0 PII tokens.
...
**Dependencies:** Cross-spec: `ADR-0001-T1` (which builds `scripts/guard/pii_scan.py` exposing `pii_scan.scan(tracked_files)` — the single tracked-file PII-token-set definition `block-pii-commit.sh` INVOKES over the staged/tracked file set rather than reimplementing the token scan in bash; reuse, one shared token set with the data-in PII guard) and `ADR-0002-T1` (which added the `vault/store/` gitignore entry this task extends) from `docs/spec/adr-0001-adr-0003-spec.md`. No in-spec predecessor (entry point).
...
8. Each domain section reflects at least one operator-specific input drawn from the `ADR-0006-T1` router summary (a stated goal, the January-2026 issue, or a hard limit): the test plants a distinguishing operator input in the summary and asserts the corresponding section surfaces it; a section that reflects 0 operator inputs fails the test.
9. No recommendation in the assembled plan contradicts a stated operator hard limit: the test plants a hard limit in the summary plus an otherwise-routable recommendation that violates it and asserts the assembled plan omits or flags that recommendation (HALT honored); a plan that emits the contradicting recommendation unmarked fails the test.
10. `pytest tests/plan/test_assemble.py` passes.

> Sizing note: this task carries 10 criteria (over the 3-7 target). Criteria 1-6 are the distinct FR-1 / NFR-5 / NFR-6 / FR-5 / OQ-1 obligations the ADR Decision and Validation Approach make load-bearing and mutually non-overlapping (attribution, sourcing, population-mismatch, thin-library gap, no-specialist gap, composition integrity); criteria 8-9 are the FR-2 per-section personalization + HALT/hard-limit obligation (each section reflects an operator-specific input; no recommendation contradicts a stated hard limit); criterion 7 is the cross-task PII boundary and criterion 10 the test gate. The set is held at 10 rather than split because criteria 1-9 are properties of one composition pass over a single file (`assemble.py`) — the same pass that routes, composes, and attributes also personalizes each section and applies the HALT filter — so splitting them would fragment one vertical slice; the file count (2) stays within the right-sized band.

**Risk Mitigations:** ADR-0006 Negative (composition/attribution adds an orchestration layer; a composition bug can drop a section's attribution or fabricate a cross-domain claim no single specialist sourced) — criteria 1, 6 assert per-section attribution and 0 fabricated cross-domain claims (the composition-attribution integrity check). ADR-0006 Negative (routing needs a matching specialist; a no-specialist domain has no routing target) — criterion 5 (coverage-gap-by-absence). ADR-0006 Negative (plan richness bounded by thin library) — accepted, documented limitation (Component Overview); criterion 4 enforces the honest coverage-gap that is the accepted contract. ADR-0006 Decision (per-section personalization from the operator's profile/goals/current state) — criterion 8 asserts each section surfaces an operator-specific input. ADR-0006 Positive (HALT rules: no recommendation contradicts a stated hard limit — PRD FR-2 / US-1) — criterion 9 asserts the assembled plan omits or flags a recommendation that violates a stated hard limit. Constraint D1→ADR-0006 (no-train summaries) — criterion 7.
...
- **Criteria covered:** ADR-0004-T0 criteria 1-7 (spike report content checks); ADR-0006-T0 criteria 1-6 (spike report content checks); ADR-0004-T1 criteria 1-4, 6, 7; ADR-0004-T2 criteria 1-4, 6; ADR-0004-T3 criteria 1-3, 5, 6; ADR-0005-T1 criteria 1, 3-7; ADR-0005-T2 criteria 1, 4, 6; ADR-0006-T1 criteria 1-2, 4-6; ADR-0006-T2 criteria 1-6, 8-10; ADR-0007-T1 criteria 1-5, 7; ADR-0007-T2 criteria 1-4, 6, 7.
...
- **HALT / hard-limit safety (ADR-0006 Positive — honoring HALT rules so no recommendation contradicts a stated hard limit; PRD FR-2 / US-1):** `ADR-0006-T2` criterion 9 plants a hard limit plus a recommendation that violates it and asserts the assembled plan omits or flags it; criterion 8 asserts each section surfaces an operator-specific input from the router summary (per-section personalization).
...
- [x] Every task has at least one acceptance criterion — each has 6-10 (ADR-0006-T2 carries 10: criteria 1-6 the FR-1/NFR-5/NFR-6/FR-5/OQ-1 obligations, 8-9 the FR-2 personalization + HALT obligation, 7 the PII boundary, 10 the test gate).
