...
- **Risk N4 — new raw-input surface / committed-PII (ADR-0027 Consequence-Negative-4 / Constraint
  ...
  during the call for BOTH the synthetic raw-PII token AND the synthetic KEY token → count of files
  carrying EITHER token = 0 (Cycle 3, the raw-intake-in-memory-only tmp-tree scan; the KEY-token leg
  closes the gitignored-key-residue gap the diff-only `key-never-committed` grep misses). A MUTATION
  step (a deliberately-injected `path.write_text(raw_intake)` inside the call boundary) makes the same
  scan find ≥1 file and go RED — proving the scan is non-vacuous.
...
- AC-7 (raw-intake-in-memory-only, LOAD-BEARING for ADR-0005): run the live-wired backend over a
  ...
  over the whole tmp tree and grep EVERY file written during the de-id call for BOTH the synthetic
  raw-PII token AND the synthetic KEY token (patch `key_source.resolve` to return a synthetic key
  token, so the call resolves it; the same rglob asserts that token never lands on disk) → count of
  files carrying EITHER token = 0. The KEY-token leg (SEC-2) closes the gitignored-key-residue gap
  the diff-only `key-never-committed` grep misses — a key written to a gitignored cache/tmp file
  would pass the diff grep but fail this on-disk scan. The scan is a tmp-tree filesystem rglob
...
Run: `.venv/bin/python -m pytest tests/model/test_client.py -q`
Expected: this is a PIN-EXISTING-BEHAVIOR cycle — the Cycle-1 method already writes nothing, so the
clean scan is GREEN-on-the-oracle the moment the scan code is correct (do NOT chase a spurious "why
isn't AC-7 failing" loop). The RED obligation for this cycle is satisfied by the MUTATION VARIANT, not
the clean case:
- `test_deidentify_raw_intake_never_written_to_disk` (clean) — Expected: GREEN-on-oracle (0 files
  under the tmp tree carry the raw-PII token; the live method holds the raw intake in-memory only).
- `test_deidentify_in_memory_scan_red_on_injected_write` (MUTATION variant) — Expected: FAIL — the
  injected `write_text(raw_intake)` makes the SAME rglob find ≥1 file; this RED is the scan's
  RED-capability demonstration (the non-vacuous lock). The variant's assertion is that the scan
  DETECTS the leak, so the test that EXPECTS the leak-detection is the RED-capable proof.
(If the clean case does NOT pass once the scan is correct, investigate — the scan may not rglob the
right tree. If the mutation case does NOT go RED, investigate — the mutation may not actually write.)
...
Run: `.venv/bin/python -m pytest tests/model/test_client.py -q`
Expected: this is a PIN-EXISTING-BEHAVIOR cycle — the Cycle-1 method returns only the parsed
patched summary, so the clean probe is GREEN-on-the-oracle the moment the probe is wired (do NOT
chase a spurious "why isn't AC-8 failing" loop). The RED obligation for this cycle is satisfied by
the LEAK VARIANT, not the clean case:
- `test_deidentify_no_raw_pii_token_past_boundary` (clean) — Expected: GREEN-on-oracle (the live
  method returns only the patched `SUMMARY_FIELD_SET`-shaped summary; 0 raw tokens in the returned +
  `deid_in`-surfaced summary — the seed reaching the assertion is what proves the probe is wired).
- the LEAK-VARIANT probe (a patched SDK that echoes the raw name into a summary value) — Expected:
  FAIL — the probe finds the raw token past the boundary and goes RED. This RED is the probe's
  RED-capability demonstration; a probe that cannot go RED is worthless.
...
- [ ] AC-7 (raw-intake-in-memory-only, tmp-tree scan WITH MUTATION) — verified by
      ...
      carrying EITHER the raw-PII token OR the synthetic KEY token [SEC-2: the key-residue leg closes
      the gap the diff-only `key-never-committed` grep misses]) +
      `test_deidentify_in_memory_scan_red_on_injected_write` (the mutation: an injected
      `write_text(raw_intake)` makes the scan find ≥1 file and go RED — the non-vacuous lock).
