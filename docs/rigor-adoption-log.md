...
| Date | Pinned | Library `VERSION` | Origin delta | Action | Outcome |
|---|---|---|---|---|---|
| 2026-07-07 (S118; operator-directed) | 1.0.0 | 1.20.0 | versioned **+20 minor** (1.1.0→1.20.0) | **PULLED** — `rsync --delete` mirror re-copied the pristine `toolkit/` (**10→26 scripts**; +16 new, 25 updated, 12 stale runtime-generated `harvest-gate` fixtures removed); bumped pin `1.0.0`→`1.20.0`; wired 3a/3b/3c (below) | **First real versioned pull** (the S67 run held on unversioned drift). Toolkit floor `34 passed, 1 failed`; the 1 fail is library-tree-scoped (below) — reportable, did NOT block the bump. |

**Delta that applies to a-plus** (scripts it runs, updated by the re-copy): `harvest-gate.sh` (1.1.0 killed the vacuous-pass false-green + 1.5.0 the word-boundary prior-ref fix — the `Recurrence:`/#8 false-FAIL class a-plus hit), `gate_attest.py` (1.2.0 AR-6/AR-7). **The 3 consumer wirings:** **3a roster-select.sh** (1.18.0 — a-plus owns no review-roster prose to re-point + has no carve-outs, so no `.rigor/roster-patterns.local` needed; wiring = classify each review with `bash toolkit/scripts/roster-select.sh` and paste its one-line output into the review header, widen-never-narrow), **3b resume-claims-audit.sh** (1.19.0 — adopted by writing a machine-checkable `VOLATILE` resume region in HANDOFF; stays ADVISORY until one full session cycle with zero FAIL, then gate per upstream bead `bud`), **3c render-before-claiming** (1.20.0 — NO project edit; follow the updated `/execute-plan` DESIGN GATE next time a UI recipe runs; INAPPLICABLE this pass). **Vendored-but-NOT-wired** (per the operator's don't-wire-beyond-3; bead `ijd0`): `/update-rigor` (1.12.0 — future pulls become one gated command), `enforce-merge-readiness` (1.11.0), `version-bump-audit` (1.10.0 — catches the `lczu` unversioned-drift class), `run-attest` (1.3.0), `watchdog` (1.4.0), the design/wiring/seam/res3 gates (1.6.0–1.9.0/1.15.0–1.17.0, node/Playwright).

**Finding — the reference lesson (bead `ffit`): `test-roster-select.sh` is library-tree-scoped.** Its line-141 git-range smoke does `cd $HERE/../../../..` (up 4 from `toolkit/tests/`) expecting the git-repo root — valid ONLY for the library's `skills_library/frameworks/rigor/toolkit/` nesting. a-plus's toolkit is at the repo root (`a-plus-maxing/toolkit/`, up 3 to root), so `../../../..` overshoots into non-git `~/Documents` → `git rev-parse HEAD~1` fails → roster-select.sh correctly FATALs `fatal-unreadable` → the 42nd assertion FAILs (41 pass). **roster-select.sh itself works in a-plus** (`--files` + real-range classify correctly; the temp-repo range-mode cases pass). Not fixable here (never edit the vendored test). **Upstream fix:** resolve the repo root via `git rev-parse --show-toplevel` (layout-agnostic), not a hardcoded `../` depth — fixes it for EVERY consumer whose toolkit isn't at the library's exact nesting. **A-plus consequence (beaded `23q5`, NOT an operator fork):** this red sits in a-plus's close-audit toolkit-floor (`toolkit/tests/run-all-tests.sh`, run all-or-nothing); the vendored runner has NO exclusion logic (only `CLOSE_AUDIT_SKIP_SUITE`, verified — it ignores `RUN_ALL_TESTS_EXCLUDE`), so `CLOSE_AUDIT_FLOOR_EXCLUDE` reaches only the a-plus floor (`scripts/tests/run-all-tests.sh`). Per the operator's don't-wire-beyond-3 directive ("bead it if that is the pattern here"), the durable a-plus-side fix — a small `CLOSE_AUDIT_FLOORS` wrapper that runs the toolkit test scripts minus a documented-exclusion list — is beaded `23q5`, NOT built this pass. **Interim** (this close + future closes until `ffit`'s upstream fix + re-pull, or `23q5`, lands): run the close with `CLOSE_AUDIT_FLOORS` set to the a-plus floor and record the manual toolkit-floor result (34 passed, 1 tracked-red = `ffit`) in the attestation. **No guard-loosening (PF-S63-02):** roster-select.sh is verified working in a-plus, and the toolkit negative-tests are verified 34/34-non-red this session, so the floor's F-007 protection is independently satisfied; the durable fix is the upstream layout-agnostic `git rev-parse --show-toplevel`. This is the close-audit's already-documented "documented pre-existing red" path, not a new exception.

**Extrapolation for other adopters:** the versioned pull worked cleanly (mirror re-copy + F-007 floor re-run + pin bump), but a vendored TEST can carry a layout assumption that only holds in the library's own tree. When a consumer nests the toolkit at a different depth than `frameworks/rigor/toolkit/`, any test that walks a hardcoded `../` count to find the repo root will mis-resolve. The durable fix is upstream (layout-agnostic root discovery); the consumer-side symptom is a floor red that its floor-exclude mechanism may not reach.
