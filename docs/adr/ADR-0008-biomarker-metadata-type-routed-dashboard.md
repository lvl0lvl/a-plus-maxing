...
- `trend(item, prev, latest)` — polarity-aware: `up`/`down` polarity maps the numeric direction to `improving`/`flat`/`regressing`; `in-range` polarity judges by movement relative to the registered range (distance-to-range shrinking → improving); no registered polarity → `None`.
