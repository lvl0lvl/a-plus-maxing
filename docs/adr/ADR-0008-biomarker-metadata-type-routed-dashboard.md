...
**D4 — Polarity-aware `_trend_token`.** `router._trend_token` consults the registry via the readings' `item`: a directional change on a registered-polarity marker resolves to `improving`/`regressing`; an UNREGISTERED marker keeps the documented fail-closed raise (never fabricate a value judgment). The generic `raw-lab-values` stream stays unregistered — per-marker lab trends for the summary are the Track-2 residual (`juc` keeps that scope). [AMENDED 2026-06-13 (juc router build, bead `juc`)]: DELIVERED — the summary's `recent-trend-direction` is now registry-driven worst-wins over the `biomarker::` polarity feed (`router._recent_trend_direction`), not the generic `raw-lab-values` stream; `_trend_token` is reused per-stream with its fail-closed raises retained as safety nets. See `vault/decisions/2026-06-12-juc-trend-polarity-design.md`.
...
- The router's `recent-trend-direction` still raises for the generic `raw-lab-values` stream (unregistered by design); the production summary path is unchanged until Track-2 lands per-marker lab streams (`juc` residual). [AMENDED 2026-06-13 (juc router build, bead `juc`)]: SUPERSEDED — `recent-trend-direction` is now derived registry-driven worst-wins over the `biomarker::` polarity feed; `raw-lab-values` is de-plumbed from the summary path (removed from `_RAW_TO_FIELD`, kept in `EXCLUDED_RAW_PII` as the boundary promise), so the generic-stream raise no longer participates in the production summary. `SUMMARY_FIELD_SET` unchanged.
