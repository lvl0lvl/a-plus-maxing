...
- The design note's `i2yw` projection-readout-on-chips clause is DEFERRED: trend chips carry the trend word/arrow only, and the projection block remains a `render_views`-only surface until a later slice.
