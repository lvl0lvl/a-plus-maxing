...
[AMENDED 2026-06-27, ADR-0030 §amends]: The "import/ingestion routine… model-independently, sending zero operator PII to any model" clause above is SCOPED to admit ONE authorized exception — the operator-confirmed no-train-lane extraction call ADR-0030 introduces for universal any-format upload ingestion (a raw uploaded file → the no-train model → operator-confirmed `(item, timepoint, source, value)` readings → the unchanged `store.append` sink). Only that one extraction front door uses the lane; the deterministic per-source adapters and the store/generation paths stay model-independent, the persisted store stays de-identified, and the off-machine no-egress posture for every OTHER path is unchanged. The extraction egress is bounded to the same no-train lane (0 file bytes to any non-no-train endpoint), operator-confirm-before-land, and retractable to a local model at the ADR-0015 seam. (See Related Decisions; ADR-0030.)
...
| [ADR-0027 (Live No-Train De-Id Backend)](ADR-0027-live-no-train-deid-backend.md) | relates | ADR-0027 is the LIVE form of the crown-jewel de-id-IN relaxation ADR-0020 introduced; this ADR's summaries-not-raw + persisted-side-de-identified invariant is honored (the live de-id RELOCATES de-id to a model boundary, the persisted store stays de-identified — `router.summarize` survives as the persisted-side gate). The 0020↔0001 tension stays anchored on ADR-0020 (above); 0027 `relates` as its live implementation. Inverse of ADR-0027 → 0001 `relates` ([.pipeline/live-wiring/dag.md §3](.pipeline/live-wiring/dag.md) [VERIFIED]). |
| [ADR-0030 (Model-Based Universal Ingestion)](ADR-0030-model-based-universal-ingestion.md) | amended-by | ADR-0030 scopes this ADR's ingestion clause (Decision: "the import/ingestion routine… model-independently, sending zero operator PII to any model") to authorize ONE extraction-egress class on the no-train lane — the operator-confirmed any-format upload extraction call — the FIRST extension of the no-egress boundary onto the ingestion axis. The persisted-side-de-identified invariant and the off-machine no-egress posture for every other path survive unchanged; only the extraction front door uses the lane. Tension bounded by no-train-lane-only (0 file bytes to any non-no-train endpoint) + operator-confirm-before-land + ADR-0015-retractable-to-local + gitignored-landing (see the [AMENDED 2026-06-27, ADR-0030 §amends] Decision note). |
| [ADR-0031 (Local-Extraction-First PDF Ingestion)](ADR-0031-local-extraction-first-pdf-ingestion.md) | constrains | This ADR's crown-jewel no-egress boundary constrains ADR-0031 — and ADR-0031 is a FURTHER move TOWARD it, not a new relaxation: it amends ADR-0030's mechanism so the raw PDF binary never egresses (text-extracted on-device first) and only scopeable extracted TEXT crosses the authorized no-train lane, REDUCING file-content egress within the class ADR-0030 already authorized (no new amendment to this ADR). Bounded by no-train-lane-only (0 raw-binary bytes to any endpoint; 0 text bytes to any non-no-train endpoint) + operator-confirm-before-land + ADR-0015-retractable-to-local. Inverse of ADR-0031 → 0001 `constrains` (inbound; [.pipeline/dag.md](.pipeline/dag.md)). |
| [ADR-0032 (DNA-Aware Planning via De-Associated Current-Science Variant Research)](ADR-0032-dna-aware-planning.md) | amended-by | ADR-0032 adds a NEW named egress class — **generic, operator-de-associated genetic-variant literature queries** — extending this ADR's egress boundary (the egress-extension convention this ADR already records for ADR-0016 / ADR-0030). What egresses is a generic gene/variant literature question anyone could ask; the operator's identity, the raw genotypes (incl. the specific allele call), AND the genotype↔operator link stay 100% local, so the persisted-side-de-identified + raw-containment invariants hold. The residual is a WEAK correlation: the variant SELECTION (which generic questions are asked) traces to the operator's genome — operator-de-associated, never operator-identifying, and signed off as a named, bounded class. Inverse of the ADR-0032 → 0001 amends/extends edge ([.pipeline/dag.md](.pipeline/dag.md)). |
| [ADR-0035 (Care-Agent Review-on-Intake Trigger + Meds Curation)](ADR-0035-care-agent-intake-review.md) | amended-by | ADR-0035 adds a FURTHER named egress class — **de-associated raw medication names → the no-train model for interaction-class curation** — extending this ADR's egress boundary, a sibling of the ADR-0032 de-associated-variant-query extension on the SAME de-association principle. What egresses is the raw drug NAMES only; the operator's identity and the name↔med (identity↔fact) linkage stay 100% local, and only the de-identified `rx-interaction-classes` class token persists as a planner input, so the persisted-side-de-identified + identity-containment invariants hold. Operator-signed-off (PF-S63-02, 2026-06-30). Inverse of the ADR-0035 → 0001 amends/extends edge ([.pipeline/dag.md](.pipeline/dag.md)). |
...
| 2026-06-24 | v1.5 — live-wiring set backfill: added inverse `relates` edge to ADR-0027 (the LIVE form of the 0020↔0001 de-id-IN relaxation; the tension stays anchored on ADR-0020). | Walter McGivney |
| 2026-06-27 | v1.6 — ADR-0030 §amends backfill: inverse `amended-by` ADR-0030 edge + Decision scoping note (the ingestion-model-independence clause scoped to admit the one operator-confirmed no-train-lane extraction call; the persisted-side-de-identified + off-machine no-egress posture for every other path unchanged). | Walter McGivney |
| 2026-06-28 | v1.7 — ADR-0031 backfill: inverse `constrains` ADR-0031 edge (ADR-0031 is a FURTHER move toward the crown-jewel ideal — raw binary stays on-device, only extracted text crosses the no-train lane, reducing egress within ADR-0030's already-authorized class; no new amendment to this ADR). | Walter McGivney |
| 2026-06-29 | v1.8 — ADR-0032 backfill: inverse `amended-by` ADR-0032 edge (DNA-aware planning adds a new named egress class — generic, operator-de-associated genetic-variant literature queries; the raw genotypes + identity + the genotype↔operator link stay 100% local, the residual a weak variant-selection correlation, signed off and bounded). | Walter McGivney |
