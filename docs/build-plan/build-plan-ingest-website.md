...
This plan executes one spec (`docs/spec/adr-0013-adr-0014-spec.md`, tier 6, ADR-0013 + ADR-0014 + the ADR-0012 amendment) as two dependency-ordered waves: **Wave A — the upload server** (the loopback `scripts/serve/` HTTP server, the multipart reader, the zip-aware `healthkit` adapter that accepts an Apple Health zip and extracts `export.xml` itself, the upload route into the UNCHANGED ingest seam, and the failing-capable egress proof) and **Wave B — the interactive wizard** (the Step-2..6 forms, the by-data-class capture persistence per ADR-0014, the Step-6 `/generate-plan` handoff, the design-critic pass on Steps 3-5, and the store-adversarial battery). 6 tasks total (5 in Wave A, 1 in Wave B), all software — a new `scripts/serve/` package over the existing `vault/store/` + dropzones + the markdown vault, plus the one zip-aware extension of `scripts/ingest/adapters/healthkit.py`. The substrate is local Python 3.12+ + the stdlib `http.server` / `multipart` / `zipfile` (NO third-party web framework, NO database engine, NO always-on daemon); tooling is `pytest`, `rg`, `git`. The server is GLUE: it reuses `ingest.run` / `dna.land` / `status.resolve` / `intake.render` / `generate.run('intake')` / `store.append` byte-unchanged (the Apple-Health-zip extraction lives in the `healthkit` adapter, NOT the server — per ADR-0013's `complements ADR-0012` edge), and is egress-free by construction (proven by the `egress_guard.run`-as-test-harness egress proof, NOT a production wrap — the guard forks a child whose response cannot return to the parent handler); every script is built by SE, with QA + Security + Architect review on every wave (the design-reviewer added on Wave B), and `plan-integrity` gating the checkpoints.
...
| `rg` (ripgrep) on PATH | Reuse / no-artifact-route / streamed-copy / egress-free-by-construction / 0-model-call scans (ADR-0013-T3, ADR-0013-T4, ADR-0013-T5, ADR-0014-T1) | `rg --version` |
...
| The egress guard `scripts/guard/egress_guard.py` present + failing-capable | Wave A's 0-egress proof (ADR-0013-T5) CONSUMES the already-audited OS-level guard AS A TEST HARNESS (it forks a child returning only a bool — it cannot wrap the production request; the server is egress-free by construction) | `python3 -c "from scripts.guard.egress_guard import run; assert run(lambda: open('/dev/null').read())"` |
...
| ADR-0013-T3 | Zip-Aware Healthkit Adapter — `read_readings` Accepts an Apple Health Zip (Mirrors `dna.land`) | SE | 1 day |
| ADR-0013-T4 | Upload Route → Ingest Dispatch + Intake Re-Render (Intake-Only) | SE + Security review | 1-2 days |
| ADR-0013-T5 | Egress TEST Over the Request-Handler Dispatch (0 Outbound, Egress-Free by Construction) | SE + Security review | 0.5-1 day |
...
- `pytest tests/serve/test_server.py tests/serve/test_bind.py tests/serve/test_multipart.py tests/ingest/test_adapters.py tests/serve/test_route.py tests/serve/test_serve_no_egress.py` passes (ADR-0013-T1 crit 5, T2 crit 5, T3 crit 7, T4 crit 7, T5 crit 4).
...
- **Zip-aware healthkit adapter (ADR-0013-T3 crit 1-3):** `read_readings` on a synthetic Apple-Health zip yields the SAME readings as on the extracted `export.xml`; a non-zip raw xml reads as today; a zip with no `export.xml` (and a non-zip non-xml) fail loud; the inner-xml copy is streamed (crit 4, `rg` finds `shutil.copyfileobj`); the zip ingests via the CLI too (crit 6, dual-entry-point).
- **Route + intake-only (ADR-0013-T4 crit 1-5):** a POST of a synthetic `export.xml` lands readings via the unchanged `ingest.run` and re-renders the wizard; a synthetic DNA `.zip` lands via the unchanged `dna.land`; an Apple-Health `.zip` routes to healthkit (the zip-aware adapter extracts internally; the route does NOT extract); the source-detection REUSES `_EXT_SOURCE`/`_detect_source` (0 second extension-map in `scripts/serve/`); NO dashboard/report artifact route exists (a GET for an artifact path 404s; `rg` finds 0 artifact-serving route).
- **0-shared-routine-edit (ADR-0013-T3 crit 5; ADR-0013-T4 crit 6):** `git diff --numstat <pre-wave> -- scripts/ingest/ingest.py scripts/ingest/adapter.py scripts/ingest/scheduler.py` reports 0 changed lines — the server/route add 0 adapter / 0 store-write / 0 extraction logic to the SHARED routine. `dna.py` is also unedited. NOTE: `scripts/ingest/adapters/healthkit.py` is the ADAPTER that gains zip-awareness in `ADR-0013-T3` — it is EXPECTED to change and is deliberately NOT in the numstat=0 set (ADR-0003-T2 protects the shared routine, not the adapters).
- **Failing-capable egress proof (ADR-0013-T5 crit 1-3) — the load-bearing Wave-A safety gate:** the egress TEST runs a REAL upload→ingest→re-render dispatch inside `egress_guard.run` and asserts truthy (0 outbound calls; store-write side-effects persist on disk), AND an INJECTED outbound call into the dispatch drives the guard to FAIL (falsy). A guard that does NOT flip to FAIL on the injected call is a no-go (the guard must intercept the request code path — subprocess/async included, since the guard is OS-level). The PRODUCTION server is egress-free BY CONSTRUCTION — `rg` over `scripts/serve/` finds 0 outbound HTTP clients; the guard is the TEST harness, NOT a production wrap (it forks a child whose re-rendered response never returns to the parent handler, so it cannot wrap the production request).
...
- **Per-wired-field round-trip (ADR-0014-T1 crit 1):** each wired de-identified field (Step-2 `goal-domains`/`goal-targets`/`goal-priority-order`/`hard-limits`; Step-3 `recovery-status-band`) → `store.read(token)` returns the reading tagged `source:"intake"` → `summarize` returns it under the token; every token asserted ∈ `SUMMARY_FIELD_SET`; 0 wired fields missing from the summary. (`rx-interaction-classes` is NOT in the wired round-trip set — Wave-B FIX-A routes the untrusted form field record-only, pending the ADR-0014 OQ-2 curation surface.)
...
- **Record-only `rx-interaction-classes` form field (ADR-0014-T1 crit 6, Wave-B FIX-A):** the untrusted form field routes RECORD-ONLY — the capture seam NEVER writes the model-bound `rx-interaction-classes` store item; `rg` finds 0 raw-name→class lookup in `scripts/serve/` (raw drug names never cross the gate). The model-bound item is fed only by the curation surface deferred to ADR-0014 OQ-2.
...
| ADR-0013-T3 | SE | The zip-aware `healthkit` adapter mirrors the already-audited `dna.land` zip-member pattern; it adds no new egress/PII control of its own (a local file extraction, model-free, network-free) — no separate Security review (SEC-02-style de-scoping, recorded below). |
...
| ADR-0013-T5 | SE + Security review | The egress proof over the request path — the PII no-egress boundary on the new surface; Security audits the failing-capable egress TEST + the egress-free-by-construction grep (the guard is a test harness, not a production wrap). |
...
**SEC-02-style Security de-scoping for `ADR-0013-T3` (documented, not silent).** `ADR-0013-T3` (the zip-aware `healthkit` adapter) carries a 0-egress / decompression-size concern, which by the rule above would read as "SE + Security review." It is deliberately NOT assigned a separate Security review: the zip-member extraction mirrors the already-audited `dna.land` zip-member pattern ([scripts/ingest/dna.py:49,92-99](../../scripts/ingest/dna.py#L49)) — a local, model-free, network-free file extraction that implements no new egress/PII control. The residual (the guard's coverage of the extraction code path) is closed by the failing-capable egress proof at the Wave-A checkpoint (`ADR-0013-T5`), whose test dispatch runs the WHOLE request path including an Apple-Health-zip ingest (which exercises the adapter's extraction), not by a per-task sign-off. The decompression-size ceiling (bead `07f6`) extends to this path and is tracked, not resolved here. Recorded so the absence is explicit.
...
- **Tests:** `pytest tests/serve/test_server.py tests/serve/test_bind.py tests/serve/test_multipart.py tests/ingest/test_adapters.py tests/serve/test_route.py tests/serve/test_serve_no_egress.py`; `git diff --numstat <pre-wave-A> -- scripts/ingest/ingest.py scripts/ingest/adapter.py scripts/ingest/scheduler.py scripts/ingest/dna.py` (expect 0 — the SHARED routine + DNA path; `healthkit.py` is the adapter that gains zip-awareness and is EXPECTED to change, deliberately NOT in this set); `rg "dashboard|report" scripts/serve/` (expect 0 artifact-serving route); `rg "socket.create_connection|urllib|http.client|requests|httpx" scripts/serve/` (expect 0 — egress-free by construction).
- **Acceptance Criteria Verified:** ADR-0013-T1 1-5; ADR-0013-T2 1-5; ADR-0013-T3 1-7; ADR-0013-T4 1-7; ADR-0013-T5 1-4.
- **Artifacts Present:** `scripts/serve/__init__.py`, `scripts/serve/__main__.py`, `scripts/serve/server.py`, `scripts/serve/multipart.py`, `scripts/serve/route.py`, the zip-aware `scripts/ingest/adapters/healthkit.py`, and their test files (incl. `tests/ingest/test_adapters.py`, `tests/serve/test_serve_no_egress.py`).
- **Cross-task integration check (egress test green with sockets blocked):** the egress TEST runs a REAL upload→ingest→re-render dispatch inside `egress_guard.run` and returns truthy (0 outbound; store-write side-effects persist on disk), AND an injected outbound call drives it to FAIL — the guard is failing-capable over the request path before any operator PII is captured through it (Wave B). The PRODUCTION server is egress-free by construction (the grep finds 0 outbound clients). The upload→ingest→re-render E2E is exercised on a synthetic `export.xml`, a synthetic Apple-Health zip (extracted inside the zip-aware healthkit adapter), AND a synthetic DNA zip (all three land via the seam).
- **Go/No-Go:** All tests pass; the bind is loopback-only; a `../../x` filename cannot escape the temp dir; the zip-aware healthkit adapter yields the same readings as the extracted xml and fails loud on a missing member; the route dispatches into the UNCHANGED `ingest.run`/`dna.land` (0 shared-routine edits on `ingest.py`/`adapter.py`/`scheduler.py`; `healthkit.py` changed as expected); NO artifact-serving route exists; the egress test is failing-capable and the production server is egress-free by construction (0 outbound clients). Any non-loopback bind, any temp escape, any shared-routine edit (on the three protected files), any artifact-serving route, any outbound client in `scripts/serve/`, or a guard that does not flip to FAIL is a no-go.
...
- **The traversal-safe + oversize-bounded staging (`ADR-0013-T2`) precedes the route (`ADR-0013-T4`)** — a malicious filename or an oversize body is contained at the multipart-staging boundary before the staged file reaches the ingest seam. (The zip-aware healthkit adapter, `ADR-0013-T3`, does its own tempfile-backed inner-xml staging, mirroring `dna.land`; the decompression-size ceiling, bead `07f6`, extends to it and is tracked.)
...
**SEC-02-style de-scoping — `ADR-0013-T3` carries a decompression-size / 0-egress concern but gets NO separate Security review (documented).** The zip-aware `healthkit` adapter mirrors the already-audited `dna.land` zip-member pattern (a local, model-free, network-free file extraction); the residual (the guard's coverage of the extraction code path) is closed by the failing-capable egress proof at `ADR-0013-T5`, whose test dispatch runs an Apple-Health-zip ingest through the whole request path (exercising the adapter's extraction). The decompression-size ceiling (bead `07f6`) extends to this path and is tracked. Recorded in the Agent Assignment Matrix preamble and here so the absence is explicit, not an oversight.
