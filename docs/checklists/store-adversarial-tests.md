...
3. **Dedupe-key boundary cases.** The dedupe identity is `(item, timepoint, source)` — `value` is EXCLUDED (`scripts/store/keying.py`, ADR-0002's D3 follow-up via ADR-0003 OQ-2). Test each field's contribution: same key + different value collides on normal ingest (second write dropped, by design); any single field differing does not collide. A test asserting only "no duplicates after re-run" does not cover this.
   **Correction path:** corrections via `store.correct` / `scripts/ingest/correction.py:manual_correction` append-and-supersede instead (ADR-0002 v1.4). Adversarial tests must cover: a superseded line never renders as current; re-running the same correction appends 0 lines; a correction never surfaces in another stream; a never-stored identity fails loud. Landed pins: `tests/store/test_correction.py` (`test_correct_unstored_identity_raises_and_writes_nothing`, `test_append_after_correction_still_drops_same_identity`, `test_correction_never_surfaces_in_another_stream`, `test_correct_leaves_same_timepoint_other_source_identity_intact`).
