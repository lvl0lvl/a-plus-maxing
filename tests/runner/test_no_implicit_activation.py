...
no build / provision / `init_instance` step may load a runner entry. It has three arms:
  ...
  - EXECUTED mutation-RED: injecting a REALISTIC `activate.enable()` — in BOTH call-forms — into a
    SCRATCH provision path, forced down the CRON branch, drives the REAL `crontab -l` counter to >=1
    and reddens the guard; then it is reverted + the crontab restored in an unconditional teardown. A
    guard that stays green under this injection — or that reddens only via a fixture-HOME counter — is
    tautological. This proves (i) the counter reads REAL launchd/cron state and (ii) the grep matches
...
SCRATCH copy only. The real-state arms FAIL LOUD (never `pytest.skip`) if no scheduler is available.
"""
...
# The real-state test-hygiene helpers (one home, in the Cycle-1 file): the forced-cron unique-label
# context manager with a bullet-proof disable()+crontab-restore teardown, and the direct real-crontab read.
from tests.runner.test_activation_surface import _crontab_lines_for, forced_cron_label
...
def test_behavioral_arm_init_instance_arms_nothing(tmp_path):
    ...
    # after run(). Forced cron + unconditional teardown; the real init_instance.py is NEVER edited.
...
def test_mutation_red_guard_catches_injected_activation(tmp_path):
    ...
    # path, force it down the CRON branch (forced_cron_label monkeypatches the launchd probe False), and
    # EXECUTE it — confirming (a) the grep arm matches BOTH forms, (b) the behavioral counter reports
    # >=1 FROM the real `crontab -l` (NOT a fixture-HOME), and (c) the guard rests RED. Then the injected
    # label is disabled + the crontab restored in the forced_cron_label finally (a crash between the
    # injected enable() and teardown must not leave a REAL armed entry). FAILS LOUD if no scheduler.
    with forced_cron_label() as label:
        ...
        # (b) EXECUTE the injected enable() -> a REAL crontab line for the unique label (cron forced).
        spec = importlib.util.spec_from_file_location("aplus_scratch_provision_mutated", scratch)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        mod.run()

        # the behavioral counter reads >=1 FROM real `crontab -l` (the scratch-HOME tautology killer).
        ...
        assert _crontab_lines_for(label), "the injected entry is not in the REAL crontab (fixture-HOME read?)"
    ...
    # after the unconditional teardown, NO real entry for the injected label survives.
    assert _crontab_lines_for(label) == [], "a REAL armed crontab entry survived teardown (implicit activation)"
