...
_FROZEN_ENGINE_PATHS = (
    ...
    *sorted(
        ...
        if p.name not in ("router.py", "horizons.py", "tailoring.py",
                          # [comprehensive-plan carve-out — Architect ruling docs/adr/.pipeline/frozen-guard-reconciliation-ruling.md §2: generate_plan.py = ADR-0042/0041/0046/0043 operator-signed-off (HARD, ADR Phase-1 gate) superseded plan front door, guarded behaviorally by tests/plan/test_generate_plan.py + core-capability-audit.sh + the per-ADR numstat probes; domain_program.py = ADR-0041 new DOMAIN PROGRAM schema, guarded by tests/plan/test_domain_program.py; context_assembler.py = ADR-0042 new crown-jewel de-id boundary, guarded by tests/plan/test_context_assembler.py; feature/comprehensive-plan-adr]
                          "generate_plan.py", "domain_program.py", "context_assembler.py",
                          # [Wave-3 comprehensive-plan carve-out — F-011 frozen-guard reconciliation, Architect Option-A: orchestrate.py = ADR-0043-T2 (reconcile via cross_domain_seams) superseded, guarantor tests/serve/test_orchestrator_reconcile.py; track.py = ADR-0044-T2 (mixed-history reader re-point of resolve_plan_progress) superseded, guarantor tests/store/test_plan_model_reader.py + tests/plan/test_track.py; activation.py = ADR-0046-T1 NEW active-domains gate, guarantor tests/plan/test_activation.py; monitoring_compiler.py = ADR-0045-T1 NEW monitoring rule compiler, guarantor tests/plan/test_monitoring_compiler.py]
                          "orchestrate.py", "track.py", "activation.py", "monitoring_compiler.py",
                          # assemble.py CARVED OUT — ADR-0041-T2 (uniform-program migration) superseded the assemble composer; behavioral guarantor tests/plan/test_assemble.py + tests/plan/test_generate_plan_uniform.py. Wave-2 frozen-guard reconciliation, Architect Option-A ruling.
                          "assemble.py",
                          # [Wave-4 comprehensive-plan carve-out — F-011 frozen-guard reconciliation, Architect Option-A: plan_driver.py = ADR-0043-T3 dispatch-registry _ROLE_OF_DOMAIN growth 4→13 superseded the plan driver's role map, guarantor tests/plan/test_activation.py::test_registries_coherent_over_grown_roster]
                          "plan_driver.py",
                          # [Wave-5 comprehensive-plan carve-out — F-011 frozen-guard reconciliation: tiered_executor.py = ADR-0045-T2 NEW four-tier deterministic executor (Create-only module, an EXTEND not a REBUILD; its fail-closed tier-routing is guarded behaviorally by tests/plan/test_tiered_executor.py + the 7-row mutation battery), feature/comprehensive-plan-wave-5]
                          "tiered_executor.py")  # router.py (de-id summary spine) guarded by test_router_additive_only_from_fork; horizons.py is a NEW post-ADR-0032 read-layer feature module (ADR-0038-T1..T3), NOT crown-jewel spine — its invariants (single progress site, no new store stream, peptides-untracked, no fabricated deadline) are guarded by tests/plan/test_horizons.py, which catches the insertion-shaped regressions a numstat additive-guard would miss. Architect ruling, feature/dyn-loop-w2. tailoring.py (ADR-0037 care-lane tailoring) is a NEW post-ADR-0032 crown-jewel module (raw _care_profile egress + T2 safety gates), extended T1/T2/T3 so byte-freeze is impossible; it carries a DUAL risk shape with BOTH guarantors — its INSERTION-shape egress risk is guarded behaviorally by tests/plan/test_tailoring.py (T1 AC-4 static-scan; T3 AC-2/AC-3 wire-scan/artifact-only), its DELETION-shape safety-gate-removal risk by test_tailoring_additive_only_from_fork (PROSPECTIVE — vacuous until tailoring.py is on main, since a new-since-fork file reports 0 deletions vs the fork). Architect ruling, feature/dyn-loop-w5, 3rd carve-out.
    ),
)
