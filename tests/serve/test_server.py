...
def test_care_review_adds_no_route_no_new_client(tmp_path):
    ...
    # ADR-0048-T2 authorization-code connect-start route — a later phase legitimately adds it),
    # /settings/schedule (the ADR-0049-T2 plan-update cadence-intent write route — a later phase
    # legitimately adds it) = 11.
    ...
    assert server_src.count('self.path == "/') == 11, "the POST route-table branch count changed unexpectedly"
